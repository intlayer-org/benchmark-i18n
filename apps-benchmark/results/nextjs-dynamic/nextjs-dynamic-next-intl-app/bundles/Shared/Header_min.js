...
var g = (e) => /^https?:\/\//.test(e ?? ""), _ = ({ href: e, children: t, ...n }) => {
	let r = l().locale ?? "en", i = g(e.toString());
	return p(f, {
...
function re(e, t) {
	return x(e, this, y, t.cache.create(), t.serializer);
}
var ie = function() {
...
var D = /^\.(?:(0+)(\*)?|(#+)|(0+)(#+))$/g, me = /^(@+)?(\+|#+)?[rs]?$/g, he = /(\*)(0+)|(#+)(0+)|(0+)/g, ge = /^(0+)$/;
function _e(e) {
	let t = {};
...
function xe(e) {
	...
	for (let n of e) {
		...
		if (D.test(n.stem)) {
			...
			n.stem.replace(D, function(e, n, r, i, a, o) {
...
var je = RegExp(`^${F.source}*`), Me = RegExp(`${F.source}*$`);
function L(e, t) {
	return {
		start: e,
		end: t
...
var Ne = !!Object.fromEntries, Pe = !!String.prototype.trimStart, Fe = !!String.prototype.trimEnd, R = Ne ? Object.fromEntries : function(e) {
	let t = {};
...
var ze = class {
	...
	parseTag(e, t) {
		...
		if (this.bumpIf(">")) {
			...
			if (this.bumpIf("</")) {
				if (this.isEOF() || !B(this.char())) return this.error(P.INVALID_TAG, L(o, this.clonePosition()));
				let e = this.clonePosition();
				return r === this.parseTagName() ? (this.bumpSpace(), this.bumpIf(">") ? {
					...
					err: null
				} : this.error(P.INVALID_TAG, L(o, this.clonePosition()))) : this.error(P.UNMATCHED_CLOSING_TAG, L(e, this.clonePosition()));
			} else return this.error(P.UNCLOSED_TAG, L(n, this.clonePosition()));
		} else return this.error(P.INVALID_TAG, L(n, this.clonePosition()));
	}
	parseTagName() {
		let e = this.offset();
	...
	parseArgument(e, t) {
		let n = this.clonePosition();
		if (this.bump(), this.bumpSpace(), this.isEOF()) return this.error(P.EXPECT_ARGUMENT_CLOSING_BRACE, L(n, this.clonePosition()));
		if (this.char() === 125) return this.bump(), this.error(P.EMPTY_ARGUMENT, L(n, this.clonePosition()));
		let r = this.parseIdentifierIfPossible().value;
		if (!r) return this.error(P.MALFORMED_ARGUMENT, L(n, this.clonePosition()));
		if (this.bumpSpace(), this.isEOF()) return this.error(P.EXPECT_ARGUMENT_CLOSING_BRACE, L(n, this.clonePosition()));
		switch (this.char()) {
			...
			case 44: return this.bump(), this.bumpSpace(), this.isEOF() ? this.error(P.EXPECT_ARGUMENT_CLOSING_BRACE, L(n, this.clonePosition())) : this.parseArgumentOptions(e, t, r, n);
			default: return this.error(P.MALFORMED_ARGUMENT, L(n, this.clonePosition()));
		}
	}
	...
	parseArgumentOptions(e, t, n, r) {
		...
		switch (a) {
			case "": return this.error(P.EXPECT_ARGUMENT_TYPE, L(i, o));
			case "number":
			case "date":
			case "time": {
				this.bumpSpace();
				...
				if (this.bumpIf(",")) {
					...
					if (n.err) return n;
					let r = Le(n.val);
					if (r.length === 0) return this.error(P.EXPECT_ARGUMENT_STYLE, L(this.clonePosition(), this.clonePosition()));
					e = {
						style: r,
				...
				if (e && e.style.startsWith("::")) {
					...
					if (a === "number") {
						...
						if (t.length === 0) return this.error(P.EXPECT_DATE_TIME_SKELETON, i);
						let r = t;
			...
			case "select": {
				...
				if (this.bumpSpace(), !this.bumpIf(",")) return this.error(P.EXPECT_SELECT_ARGUMENT_OPTIONS, L(i, { ...i }));
				this.bumpSpace();
				let o = this.parseIdentifierIfPossible(), s = 0;
				if (a !== "select" && o.value === "offset") {
					if (!this.bumpIf(":")) return this.error(P.EXPECT_PLURAL_ARGUMENT_OFFSET_VALUE, L(this.clonePosition(), this.clonePosition()));
					this.bumpSpace();
					let e = this.tryParseDecimalInteger(P.EXPECT_PLURAL_ARGUMENT_OFFSET_VALUE, P.INVALID_PLURAL_ARGUMENT_OFFSET_VALUE);
					if (e.err) return e;
			...
			default: return this.error(P.INVALID_ARGUMENT_TYPE, L(i, o));
		}
	}
	tryParseArgumentClose(e) {
		return this.isEOF() || this.char() !== 125 ? this.error(P.EXPECT_ARGUMENT_CLOSING_BRACE, L(e, this.clonePosition())) : (this.bump(), {
			val: !0,
			err: null
		});
	}
	parseSimpleArgStyleIfPossible() {
		...
		for (; !this.isEOF();) switch (this.char()) {
			case 39: {
				this.bump();
				let e = this.clonePosition();
				if (!this.bumpUntil("'")) return this.error(P.UNCLOSED_QUOTE_IN_ARGUMENT_STYLE, L(e, this.clonePosition()));
				this.bump();
				break;
			}
			case 123:
	...
	tryParsePluralOrSelectOptions(e, t, n, r) {
		...
		for (;;) {
			if (s.length === 0) {
				...
				if (t !== "select" && this.bumpIf("=")) {
					let t = this.tryParseDecimalInteger(P.EXPECT_PLURAL_ARGUMENT_SELECTOR, P.INVALID_PLURAL_ARGUMENT_SELECTOR);
					if (t.err) return t;
					c = L(e, this.clonePosition()), s = this.message.slice(e.offset, this.offset());
				} else break;
			}
			if (o.has(s)) return this.error(t === "select" ? P.DUPLICATE_SELECT_ARGUMENT_SELECTOR : P.DUPLICATE_PLURAL_ARGUMENT_SELECTOR, c);
			s === "other" && (i = !0), this.bumpSpace();
			let r = this.clonePosition();
			if (!this.bumpIf("{")) return this.error(t === "select" ? P.EXPECT_SELECT_ARGUMENT_SELECTOR_FRAGMENT : P.EXPECT_PLURAL_ARGUMENT_SELECTOR_FRAGMENT, L(this.clonePosition(), this.clonePosition()));
		...
		return a.length === 0 ? this.error(t === "select" ? P.EXPECT_SELECT_ARGUMENT_SELECTOR : P.EXPECT_PLURAL_ARGUMENT_SELECTOR, L(this.clonePosition(), this.clonePosition())) : this.requiresOtherClause && !i ? this.error(P.MISSING_OTHER_CLAUSE, L(this.clonePosition(), this.clonePosition())) : {
			val: a,
			err: null
		};
	}
...
function V(e) {
	e.forEach((e) => {
		if (delete e.location, we(e) || Te(e)) for (let t in e.options) delete e.options[t].location, V(e.options[t].value);
		else j(e) && Oe(e.style) || (M(e) || Ce(e)) && N(e.style) ? delete e.style.location : De(e) && V(e.children);
	});
}
function Ue(e, t = {}) {
	t = {
	...
	if (n.err) {
		let e = SyntaxError(P[n.err.kind]);
...
function K(e, t, n, r, i, a, o) {
	...
	for (let c of e) {
		...
		if (j(c)) {
			let e = typeof c.style == "string" ? r.number[c.style] : Oe(c.style) ? c.style.parsedOptions : void 0;
			if (e && e.scale) {
				let t = e.scale || 1;
				if (typeof l == "bigint") {
...
function Ze(...[e, t, n, i]) {
	if (Array.isArray(t)) throw new C(w.INVALID_MESSAGE, void 0);
	if (typeof t == "object") throw new C(w.INSUFFICIENT_PATH, void 0);
	if (typeof t == "string") {
		let e = function(e, t) {
	...
	try {
		...
		throw new C(w.INVALID_MESSAGE, void 0);
	}
	let f = d.format(n);
...
function $e(e) {
	...
	return function({ cache: e, formats: n, formatters: i, getMessageFallback: a = X, locale: o, messagesOrError: s, namespace: c, onError: l, timeZone: u }) {
		let d = s instanceof C;
		function f(e, t, n, r) {
			let i = new C(t, n);
			return l(i), r ?? a({
				error: i,
		...
		function p(p, m, h, g) {
			...
			if (d) {
				...
				try {
					...
					if (!_) return f(p, w.MISSING_MESSAGE, e.message, _);
					v = _;
				}
			}
			try {
				return Ze(Y(c, p), v, m && function(e) {
					let n = {};
				...
				let t, n;
				return e instanceof C ? (t = e.code, n = e.originalMessage) : (t = w.FORMATTING_ERROR, n = e.message), f(p, t, n, _);
			}
		}
		function m(e, t, n, r) {
			let i = p(e, t, n, r);
			return typeof i == "string" ? i : f(e, w.INVALID_MESSAGE, void 0);
		}
		return m.rich = p, m.markup = (e, t, n, r) => p(e, t, n, r), m.raw = (e) => {
			...
			try {
				...
				return f(e, w.MISSING_MESSAGE, t.message);
			}
		}, m.has = (e) => {
			if (d) return !1;
			try {
...
function nt({ formats: e, getMessageFallback: t, messages: n, onError: r, ...i }) {
	return {
		...
		messages: n || void 0,
		onError: r || Qe,
		getMessageFallback: t || X
	};
}
//#endregion
//#region ../../../node_modules/.bun/use-intl@4.9.1+b1ab299f0a400331/node_modules/use-intl/dist/esm/production/react.js
var Q = n(void 0);
function rt({ children: e, formats: t, getMessageFallback: n, locale: r, messages: a, now: o, onError: c, timeZone: l }) {
	let u = i(Q), d = s((() => u?.cache || se()), [r, u?.cache]), f = s((() => u?.formatters || ce(d)), [d, u?.formatters]), m = s((() => ({
		...nt({
			locale: r,
...
function st(e) {
	return function(e, t, n) {
		let { cache: r, formats: i, formatters: a, getMessageFallback: o, locale: c, onError: l, timeZone: u } = it(), d = e[n], f = et(t, n);
		return u || at || !ot || (at = !0, l(new C(w.ENVIRONMENT_FALLBACK, void 0))), s((() => $e({
			cache: r,
			formatters: a,
...
function bt({ children: t, locale: n, messages: r }) {
	return a(() => {
