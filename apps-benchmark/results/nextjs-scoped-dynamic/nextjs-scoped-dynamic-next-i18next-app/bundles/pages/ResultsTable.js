import { Profiler, createContext, createElement, useCallback, useContext, useEffect, useMemo, useRef, useState } from "react";
...
var localeNamespaces = /* @__PURE__ */ new Set();
async function loadNamespaceMessages(locale, namespace) {
...
async function preloadNamespaces(locale, requestedNamespaces = namespaces) {
	const safeLocale = locale || "en";
	for (const namespace of requestedNamespaces) {
		const cacheKey = `${safeLocale}:${namespace}`;
		if (localeNamespaces.has(cacheKey)) continue;
		const messages = await loadNamespaceMessages(safeLocale, namespace);
		instance.addResourceBundle(safeLocale, namespace, messages, true, true);
...
preloadNamespaces("en", namespaces);
...
function AppProviders({ children }) {
	...
	useEffect(() => {
		...
		(async () => {
			...
			if (!cancelled && i18n_default.language !== locale) await i18n_default.changeLanguage(locale);
		})();
		return () => {
			cancelled = true;
		};
