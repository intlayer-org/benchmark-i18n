...
function we(e) {
	...
	for (s = 0; s < e.length; s++) switch (i = e[s], t) {
		...
		case G:
			...
			break;
		case K:
			i === "}" ? (d(), t = U) : be(i) ? r !== "" && (d(), t = q) : (u(), t = K);
			break;
		case q: i == "}" ? t = U : be(i) ? t = q : c(H);
	}
...
var X = null;
function Re(e) {
	return X ||= Le(`${Ie}/@tolgee/web@${e}/dist/${Ne}`).then(() => window[Pe][Fe]), X;
}
...
function yt() {
	return Ke().use(De()).use(qe()).updateDefaults({
