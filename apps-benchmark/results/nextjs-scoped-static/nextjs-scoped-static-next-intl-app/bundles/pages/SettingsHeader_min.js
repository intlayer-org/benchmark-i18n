...
function yn(e, t) {
	...
	for (let r = 0; r < e.length; r++) {
		...
		if (i === "j") {
			...
			let o = 1 + (a & 1), s = a < 2 ? 1 : 3 + (a >> 1), c = bn(t);
...
var xn = RegExp(`^${vn.source}*`), Sn = RegExp(`${vn.source}*$`);
function B(e, t) {
	return {
		start: e,
		end: t
...
var Cn = !!Object.fromEntries, wn = !!String.prototype.trimStart, Tn = !!String.prototype.trimEnd, En = Cn ? Object.fromEntries : function(e) {
	let t = {};
	...
	return e.replace(Sn, "");
}, kn = /* @__PURE__ */ RegExp("([^\\p{White_Space}\\p{Pattern_Syntax}]*)", "yu");
function An(e, t) {
	return kn.lastIndex = t, kn.exec(e)[1] ?? "";
}
var jn = class {
	message;
	position;
			...
			case "time": {
				...
				if (e && e.style.startsWith("::")) {
					...
					if (a === "number") {
						...
						let r = t;
						this.locale && (r = yn(t, this.locale));
						let o = {
							type: F.dateTime,
							pattern: r,
...
function H(e) {
	e.forEach((e) => {
		if (delete e.location, pn(e) || mn(e)) for (let t in e.options) delete e.options[t].location, H(e.options[t].value);
		else un(e) && _n(e.style) || (dn(e) || fn(e)) && L(e.style) ? delete e.style.location : gn(e) && H(e.children);
	});
}
function Fn(e, t = {}) {
	t = {
...
function K(e, t, n, r, i, a, o) {
	...
	for (let c of e) {
		...
		if (un(c)) {
			let e = typeof c.style == "string" ? r.number[c.style] : _n(c.style) ? c.style.parsedOptions : void 0;
			if (e && e.scale) {
				let t = e.scale || 1;
				if (typeof l == "bigint") {
...
function qn(e) {
	...
	return function({ cache: e, formats: n, formatters: i, getMessageFallback: a = Gn, locale: o, messagesOrError: s, namespace: c, onError: l, timeZone: u }) {
		...
		function p(p, m, h, g) {
			...
			try {
				return Wn(Y(c, p), v, m && function(e) {
					let n = {};
...
function Xn({ formats: e, getMessageFallback: t, messages: n, onError: r, ...i }) {
	return {
		...
		messages: n || void 0,
		onError: r || Kn,
		getMessageFallback: t || Gn
	};
}
var Z = n(void 0);
function Zn({ children: e, formats: t, getMessageFallback: n, locale: r, messages: a, now: s, onError: l, timeZone: u }) {
	let d = i(Z), f = o((() => d?.cache || Qt()), [r, d?.cache]), p = o((() => d?.formatters || $t(f)), [f, d?.formatters]), m = o((() => ({
		...Xn({
			locale: r,
