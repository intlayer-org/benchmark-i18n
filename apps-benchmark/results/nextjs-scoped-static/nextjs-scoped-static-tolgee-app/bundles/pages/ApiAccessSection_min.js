...
function al() {
	return Xs().use(ks()).use(Zs()).updateDefaults({
		...
		staticData: Object.fromEntries(il.map((e) => [e, () => Promise.resolve(rl(e))]))
	});
}
//#endregion
//#region tolgee/client.tsx
var ol = al().init(), sl = ({ language: e, staticData: t, children: n }) => {
	let r = f();
	return a(() => {
		let { unsubscribe: e } = ol.on("permanentChange", () => {
			r.refresh();
		});
		return () => e();
	}, [r]), u(rc, {
...
function ul({ children: e, locale: n, staticData: r }) {
	return a(() => {
