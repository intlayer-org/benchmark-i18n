...
function WhatWeMeasure() {
	return jsxs("section", {
		...
		children: [jsx("h2", {
			...
			children: jsx(import_index_client.T, { children: "What We Measure" })
		}), jsx("ul", {
			className: "space-y-4",
			children: [
				{
					metric: jsx(import_index_client.T, { children: "Bundle size impact" }),
					desc: jsx(import_index_client.T, { children: "The additional JavaScript bytes sent to users when the i18n library and its translation files are included. This directly affects download time on slow networks." })
				},
				{
					metric: jsx(import_index_client.T, { children: "Rendering overhead" }),
					desc: jsx(import_index_client.T, { children: "How much extra time the library adds to React's render cycle. Libraries that inject translations via a single context provider can cause unnecessary re-renders across the component tree." })
				},
				{
					metric: jsx(import_index_client.T, { children: "Hydration cost" }),
					desc: jsx(import_index_client.T, { children: "During SSR, translation data is serialized into HTML. Large dictionaries increase the HTML payload and slow down hydration — the moment the page becomes interactive." })
				},
				{
					metric: jsx(import_index_client.T, { children: "Lazy loading effectiveness" }),
					desc: jsx(import_index_client.T, { children: "Whether splitting translations by route or namespace actually reduces the initial load, and what trade-offs it introduces (waterfall requests, FOUC, cache complexity)." })
				},
				{
					metric: jsx(import_index_client.T, { children: "Locale switch speed" }),
					desc: jsx(import_index_client.T, { children: "How fast the app can switch from one language to another at runtime — including fetching new translations, re-rendering components, and updating the DOM." })
				}
			].map((m) => jsxs("li", {
