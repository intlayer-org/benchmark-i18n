...
function m() {
	return r("section", {
		...
		children: [n("h2", {
			...
			children: n(p.T, { children: "What We Measure" })
		}), n("ul", {
			className: "space-y-4",
			children: [
				{
					metric: n(p.T, { children: "Bundle size impact" }),
					desc: n(p.T, { children: "The additional JavaScript bytes sent to users when the i18n library and its translation files are included. This directly affects download time on slow networks." })
				},
				{
					metric: n(p.T, { children: "Rendering overhead" }),
					desc: n(p.T, { children: "How much extra time the library adds to React's render cycle. Libraries that inject translations via a single context provider can cause unnecessary re-renders across the component tree." })
				},
				{
					metric: n(p.T, { children: "Hydration cost" }),
					desc: n(p.T, { children: "During SSR, translation data is serialized into HTML. Large dictionaries increase the HTML payload and slow down hydration — the moment the page becomes interactive." })
				},
				{
					metric: n(p.T, { children: "Lazy loading effectiveness" }),
					desc: n(p.T, { children: "Whether splitting translations by route or namespace actually reduces the initial load, and what trade-offs it introduces (waterfall requests, FOUC, cache complexity)." })
				},
				{
					metric: n(p.T, { children: "Locale switch speed" }),
					desc: n(p.T, { children: "How fast the app can switch from one language to another at runtime — including fetching new translations, re-rendering components, and updating the DOM." })
				}
			].map((e) => r("li", {
