...
var De = RegExp(`^${A.source}*`), Oe = RegExp(`${A.source}*$`);
function M(e, t) {
	return {
		start: e,
		end: t
...
var ke = !!Object.fromEntries, Ae = !!String.prototype.trimStart, je = !!String.prototype.trimEnd, N = ke ? Object.fromEntries : function(e) {
	let t = {};
	...
	return e.replace(Oe, "");
}, P = /* @__PURE__ */ RegExp("([^\\p{White_Space}\\p{Pattern_Syntax}]*)", "yu");
function Pe(e, t) {
	return P.lastIndex = t, P.exec(e)[1] ?? "";
}
var Fe = class {
	message;
	position;
	...
	parseMessage(e, t, n) {
		...
		for (; !this.isEOF();) {
			...
			if (i === 123) {
				...
				if (n) break;
				return this.error(k.UNMATCHED_CLOSING_TAG, M(this.clonePosition(), this.clonePosition()));
	...
	parseTag(e, t) {
		...
		if (this.bumpIf(">")) {
			...
			if (this.bumpIf("</")) {
				if (this.isEOF() || !F(this.char())) return this.error(k.INVALID_TAG, M(o, this.clonePosition()));
				let e = this.clonePosition();
				return r === this.parseTagName() ? (this.bumpSpace(), this.bumpIf(">") ? {
					...
					err: null
				} : this.error(k.INVALID_TAG, M(o, this.clonePosition()))) : this.error(k.UNMATCHED_CLOSING_TAG, M(e, this.clonePosition()));
			} else return this.error(k.UNCLOSED_TAG, M(n, this.clonePosition()));
		} else return this.error(k.INVALID_TAG, M(n, this.clonePosition()));
	}
	parseTagName() {
		let e = this.offset();
	...
	parseArgument(e, t) {
		let n = this.clonePosition();
		if (this.bump(), this.bumpSpace(), this.isEOF()) return this.error(k.EXPECT_ARGUMENT_CLOSING_BRACE, M(n, this.clonePosition()));
		if (this.char() === 125) return this.bump(), this.error(k.EMPTY_ARGUMENT, M(n, this.clonePosition()));
		let r = this.parseIdentifierIfPossible().value;
		if (!r) return this.error(k.MALFORMED_ARGUMENT, M(n, this.clonePosition()));
		if (this.bumpSpace(), this.isEOF()) return this.error(k.EXPECT_ARGUMENT_CLOSING_BRACE, M(n, this.clonePosition()));
		switch (this.char()) {
			...
			case 44: return this.bump(), this.bumpSpace(), this.isEOF() ? this.error(k.EXPECT_ARGUMENT_CLOSING_BRACE, M(n, this.clonePosition())) : this.parseArgumentOptions(e, t, r, n);
			default: return this.error(k.MALFORMED_ARGUMENT, M(n, this.clonePosition()));
		}
	}
	...
	parseArgumentOptions(e, t, n, r) {
		...
		switch (a) {
			case "": return this.error(k.EXPECT_ARGUMENT_TYPE, M(i, o));
			case "number":
			case "date":
			case "time": {
				this.bumpSpace();
				...
				if (this.bumpIf(",")) {
					...
					if (n.err) return n;
					let r = Ne(n.val);
					if (r.length === 0) return this.error(k.EXPECT_ARGUMENT_STYLE, M(this.clonePosition(), this.clonePosition()));
					e = {
						style: r,
						styleLocation: M(t, this.clonePosition())
					};
				}
				let t = this.tryParseArgumentClose(r);
				if (t.err) return t;
				let i = M(r, this.clonePosition());
			...
			case "select": {
				...
				if (this.bumpSpace(), !this.bumpIf(",")) return this.error(k.EXPECT_SELECT_ARGUMENT_OPTIONS, M(i, { ...i }));
				this.bumpSpace();
				let o = this.parseIdentifierIfPossible(), s = 0;
				if (a !== "select" && o.value === "offset") {
					if (!this.bumpIf(":")) return this.error(k.EXPECT_PLURAL_ARGUMENT_OFFSET_VALUE, M(this.clonePosition(), this.clonePosition()));
					this.bumpSpace();
			...
			default: return this.error(k.INVALID_ARGUMENT_TYPE, M(i, o));
		}
	}
	tryParseArgumentClose(e) {
		return this.isEOF() || this.char() !== 125 ? this.error(k.EXPECT_ARGUMENT_CLOSING_BRACE, M(e, this.clonePosition())) : (this.bump(), {
			val: !0,
			err: null
		});
	}
	parseSimpleArgStyleIfPossible() {
		...
		for (; !this.isEOF();) switch (this.char()) {
			case 39: {
				this.bump();
				let e = this.clonePosition();
				if (!this.bumpUntil("'")) return this.error(k.UNCLOSED_QUOTE_IN_ARGUMENT_STYLE, M(e, this.clonePosition()));
				this.bump();
				break;
			}
			case 123:
	...
	tryParsePluralOrSelectOptions(e, t, n, r) {
		...
		for (;;) {
			if (s.length === 0) {
				...
				if (t !== "select" && this.bumpIf("=")) {
					...
					if (t.err) return t;
					c = M(e, this.clonePosition()), s = this.message.slice(e.offset, this.offset());
				} else break;
			}
			...
			if (!this.bumpIf("{")) return this.error(t === "select" ? k.EXPECT_SELECT_ARGUMENT_SELECTOR_FRAGMENT : k.EXPECT_PLURAL_ARGUMENT_SELECTOR_FRAGMENT, M(this.clonePosition(), this.clonePosition()));
		...
		return a.length === 0 ? this.error(t === "select" ? k.EXPECT_SELECT_ARGUMENT_SELECTOR : k.EXPECT_PLURAL_ARGUMENT_SELECTOR, M(this.clonePosition(), this.clonePosition())) : this.requiresOtherClause && !i ? this.error(k.MISSING_OTHER_CLAUSE, M(this.clonePosition(), this.clonePosition())) : {
			val: a,
			err: null
		};
	}
...
function I(e) {
	e.forEach((e) => {
		if (delete e.location, w(e) || T(e)) for (let t in e.options) delete e.options[t].location, I(e.options[t].value);
		else x(e) && D(e.style) || (S(e) || C(e)) && O(e.style) ? delete e.style.location : E(e) && I(e.children);
	});
}
function ze(e, t = {}) {
	t = {
	...
	return t?.captureLocation || I(n.val), n.val;
}
//#endregion
...
var L = function(e) {
	...
	constructor(e, t, n, r) {
		super(`Invalid values for "${e}": "${t}". Options are "${Object.keys(n).join("\", \"")}"`, L.INVALID_VALUE, r);
	}
}, Ve = class extends R {
	constructor(e, t, n) {
		super(`Value for "${e}" must be of type ${t}`, L.INVALID_VALUE, n);
	}
}, He = class extends R {
	constructor(e, t) {
		super(`The intl string context variable "${e}" was not provided to the string "${t}"`, L.MISSING_VALUE, t);
	}
}, z = function(e) {
...
function V(e, t, n, r, i, a, o) {
	...
	for (let c of e) {
		...
		if (S(c)) {
			let e = typeof c.style == "string" ? r.date[c.style] : O(c.style) ? c.style.parsedOptions : void 0;
			s.push({
				type: z.literal,
		...
		if (C(c)) {
			let e = typeof c.style == "string" ? r.time[c.style] : O(c.style) ? c.style.parsedOptions : r.time.medium;
			s.push({
				type: z.literal,
				value: n.getDateTimeFormat(t, e).format(l)
			});
		...
		if (x(c)) {
			let e = typeof c.style == "string" ? r.number[c.style] : D(c.style) ? c.style.parsedOptions : void 0;
			if (e && e.scale) {
				let t = e.scale || 1;
				if (typeof l == "bigint") {
		...
		if (E(c)) {
			...
			if (!B(u)) throw new Ve(l, "function", o);
...
function tt(e = et()) {
	...
	return {
		getDateTimeFormat: i,
		getNumberFormat: a,
		getMessageFormat: h((e, t, n, r) => new qe(e, t, n, {
			formatters: {
				getNumberFormat: a,
				getDateTimeFormat: i,
				getPluralRules: o
...
var Z = ({ locale: e, formats: t, messages: n, defaultLocale: r, defaultFormats: i, fallbackOnEmptyString: a, onError: o, timeZone: s, defaultRichTextElements: c }, l, u = { id: "" }, d, f) => {
	...
	Qe(!!p, "[@formatjs/intl] An `id` must be provided to format a message. You can either:\n1. Configure your build toolchain with [babel-plugin-formatjs](https://formatjs.github.io/docs/tooling/babel-plugin)\nor [@formatjs/ts-transformer](https://formatjs.github.io/docs/tooling/ts-transformer) OR\n2. Configure your `eslint` config to include [eslint-plugin-formatjs](https://formatjs.github.io/docs/tooling/linter#enforce-id)\nto autofix this issue");
	...
	if (d = {
		...
		if (a === !1 && g === "") return g;
		if ((!m || e && e.toLowerCase() !== r.toLowerCase()) && o(new Ze(u, e)), m) try {
...
function dt({ locale: e, onError: t }, n, r, i) {
	Intl.DisplayNames || t(new R("Intl.DisplayNames is not available in this environment.\nTry polyfilling it using \"@formatjs/intl-displaynames\"\n", L.MISSING_INTL_API));
	let a = q(i, ut);
	try {
		return n(e, a).of(r);
	} catch (n) {
...
function gt({ locale: e, onError: t }, n, r, i = {}) {
	Intl.ListFormat || t(new R("Intl.ListFormat is not available in this environment.\nTry polyfilling it using \"@formatjs/intl-listformat\"\n", L.MISSING_INTL_API));
	let a = q(i, ft);
	try {
...
function vt({ locale: e, onError: t }, n, r, i = {}) {
	Intl.PluralRules || t(new R("Intl.PluralRules is not available in this environment.\nTry polyfilling it using \"@formatjs/intl-pluralrules\"\n", L.MISSING_INTL_API));
	let a = q(i, _t);
	try {
...
function bt({ locale: e, formats: t, onError: n }, r, i = {}) {
	let { format: a } = i;
	return r(e, q(i, yt, !!a && Y(t, "relative", a, n) || {}));
}
function xt(e, t, n, r, i = {}) {
	r ||= "second", Intl.RelativeTimeFormat || e.onError(new R("Intl.RelativeTimeFormat is not available in this environment.\nTry polyfilling it using \"@formatjs/intl-relativetimeformat\"\n", L.MISSING_INTL_API));
	try {
...
function Ct({ locale: e, formats: t, onError: n }, r, i = {}) {
	let { format: a } = i;
	return r(e, q(i, St, a && Y(t, "number", a, n) || {}));
}
function wt(e, t, n, r = {}) {
	try {
...
function Ot(e, t) {
	...
	return i ? !Intl.NumberFormat.supportedLocalesOf(i).length && o ? o(new Xe(`Missing locale data for locale: "${i}" in Intl.NumberFormat. Using default locale: "${a}" as fallback. See https://formatjs.github.io/docs/react-intl#runtime-requirements for more details`)) : !Intl.DateTimeFormat.supportedLocalesOf(i).length && o && o(new Xe(`Missing locale data for locale: "${i}" in Intl.DateTimeFormat. Using default locale: "${a}" as fallback. See https://formatjs.github.io/docs/react-intl#runtime-requirements for more details`)) : (o && o(new Ye(`"locale" was not configured, using "${a}" as fallback. See https://formatjs.github.io/docs/react-intl/api#intlshape for more details`)), r.locale = r.defaultLocale || "en"), Dt(r), {
		...r,
		formatters: n,
...
}, Mt = (t) => e.Children.toArray(t).map((t, n) => e.isValidElement(t) ? a(e.Fragment, { children: t }, n) : t);
function Nt(e) {
	return function(t) {
		return e(Mt(t));
	};
...
function Rt(e) {
	return e && Object.keys(e).reduce((t, n) => {
		let r = e[n];
		return t[n] = B(r) ? Nt(r) : r, t;
	}, {});
}
var zt = (e, t, n, r, ...i) => {
	let a = Z(e, t, n, Rt(r), ...i);
...
function Ht(t) {
	let n = e.useRef(et()), r = e.useRef(void 0), i = e.useRef(void 0), o = {};
	for (let e in t) t[e] !== void 0 && (o[e] = t[e]);
	let s = Vt({
		...jt,
		...o
	});
	return (!r.current || !Pt(r.current, s)) && (r.current = s, i.current = Bt(s, n.current)), At(i.current), a(It, {
		value: i.current,
		children: t.children
	});
}
Ht.displayName = "IntlProvider";
var Ut = Ht;
//#endregion
...
function Gt() {
	...
	let o = Lt(), s = e === "auto" ? o.formatMessage({ id: "theme-toggle.themeModeAutoSystemClick" }) : e === "light" ? o.formatMessage({ id: "theme-toggle.themeModeLightClick" }) : o.formatMessage({ id: "theme-toggle.themeModeDarkClick" });
	return a("button", {
		type: "button",
		onClick: n,
		"aria-label": s,
