...
var Le = class {
	...
	parseTag(e, t) {
		...
		if (this.bumpIf(">")) {
			...
			if (this.bumpIf("</")) {
				if (this.isEOF() || !P(this.char())) return this.error(j.INVALID_TAG, N(o, this.clonePosition()));
				let e = this.clonePosition();
				return r === this.parseTagName() ? (this.bumpSpace(), this.bumpIf(">") ? {
					...
					err: null
				} : this.error(j.INVALID_TAG, N(o, this.clonePosition()))) : this.error(j.UNMATCHED_CLOSING_TAG, N(e, this.clonePosition()));
			} else return this.error(j.UNCLOSED_TAG, N(n, this.clonePosition()));
		} else return this.error(j.INVALID_TAG, N(n, this.clonePosition()));
	}
	parseTagName() {
		let e = this.offset();
		...
		switch (a) {
			...
			case "time": {
				...
				if (this.bumpIf(",")) {
					...
					if (n.err) return n;
					let r = Pe(n.val);
					if (r.length === 0) return this.error(j.EXPECT_ARGUMENT_STYLE, N(this.clonePosition(), this.clonePosition()));
					e = {
						style: r,
			...
			case "select": {
				...
				if (this.bumpSpace(), !this.bumpIf(",")) return this.error(j.EXPECT_SELECT_ARGUMENT_OPTIONS, N(i, { ...i }));
				this.bumpSpace();
	...
	parseSimpleArgStyleIfPossible() {
		...
		for (; !this.isEOF();) switch (this.char()) {
			case 39: {
				this.bump();
				let e = this.clonePosition();
				if (!this.bumpUntil("'")) return this.error(j.UNCLOSED_QUOTE_IN_ARGUMENT_STYLE, N(e, this.clonePosition()));
				this.bump();
				break;
			}
			case 123:
	...
	tryParsePluralOrSelectOptions(e, t, n, r) {
		...
		return a.length === 0 ? this.error(t === "select" ? j.EXPECT_SELECT_ARGUMENT_SELECTOR : j.EXPECT_PLURAL_ARGUMENT_SELECTOR, N(this.clonePosition(), this.clonePosition())) : this.requiresOtherClause && !i ? this.error(j.MISSING_OTHER_CLAUSE, N(this.clonePosition(), this.clonePosition())) : {
			val: a,
			err: null
		};
	}
...
function F(e) {
	e.forEach((e) => {
		...
		else _e(e) && Ce(e.style) || (ve(e) || ye(e)) && A(e.style) ? delete e.style.location : k(e) && F(e.children);
	});
}
function Ve(e, t = {}) {
	t = {
...
function V(e, t, n, r, i, a, o) {
	...
	for (let c of e) {
		...
		if (ve(c)) {
			let e = typeof c.style == "string" ? r.date[c.style] : A(c.style) ? c.style.parsedOptions : void 0;
			s.push({
				type: z.literal,
		...
		if (ye(c)) {
			let e = typeof c.style == "string" ? r.time[c.style] : A(c.style) ? c.style.parsedOptions : r.time.medium;
			s.push({
				type: z.literal,
				value: n.getDateTimeFormat(t, e).format(l)
			});
		...
		if (_e(c)) {
			let e = typeof c.style == "string" ? r.number[c.style] : Ce(c.style) ? c.style.parsedOptions : void 0;
			if (e && e.scale) {
				let t = e.scale || 1;
				if (typeof l == "bigint") {
		...
		if (k(c)) {
			...
			if (!B(u)) throw new He(l, "function", o);
...
function tt(e = et()) {
	...
	return {
		getDateTimeFormat: i,
		getNumberFormat: a,
		getMessageFormat: g((e, t, n, r) => new U(e, t, n, {
			formatters: {
				getNumberFormat: a,
				getDateTimeFormat: i,
				getPluralRules: o
...
function rt(e, t) {
	...
	return {
		...n,
		...e,
		date: nt(Z(n.date, t), Z(e.date || {}, t)),
		time: nt(Z(n.time, t), Z(e.time || {}, t))
	};
}
var Q = ({ locale: e, formats: t, messages: n, defaultLocale: r, defaultFormats: i, fallbackOnEmptyString: a, onError: o, timeZone: s, defaultRichTextElements: c }, l, u = { id: "" }, d, f) => {
	...
	if (Array.isArray(g) && g.length === 1 && g[0].type === D.literal) return g[0].value;
	if (d = {
		...c,
		...d
	}, t = rt(t, s), i = rt(i, s), !g) {
...
function bt({ locale: e, formats: t, onError: n }, r, i = {}) {
	let { format: a } = i;
	return r(e, J(i, yt, !!a && X(t, "relative", a, n) || {}));
}
...
function Ct({ locale: e, formats: t, onError: n }, r, i = {}) {
	let { format: a } = i;
	return r(e, J(i, St, a && X(t, "number", a, n) || {}));
}
function wt(e, t, n, r = {}) {
	try {
...
function Gt() {
	let e = Lt();
	return Wt("AboutHeader"), o(i, { children: [a("h1", {
