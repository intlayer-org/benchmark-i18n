...
var b = /^\.(?:(0+)(\*)?|(#+)|(0+)(#+))$/g, x = /^(@+)?(\+|#+)?[rs]?$/g, fe = /(\*)(0+)|(#+)(0+)|(0+)/g, pe = /^(0+)$/;
function me(e) {
	let t = {};
	return e[e.length - 1] === "r" ? t.roundingPriority = "morePrecision" : e[e.length - 1] === "s" && (t.roundingPriority = "lessPrecision"), e.replace(x, function(e, n, r) {
...
var je = !!Object.fromEntries, Me = !!String.prototype.trimStart, Ne = !!String.prototype.trimEnd, Pe = je ? Object.fromEntries : function(e) {
	let t = {};
	...
	return e.replace(Ae, "");
}, Le = /* @__PURE__ */ RegExp("([^\\p{White_Space}\\p{Pattern_Syntax}]*)", "yu");
function Re(e, t) {
...
var ze = class {
	...
	parseTag(e, t) {
		...
		if (this.bumpIf(">")) {
			...
			if (this.bumpIf("</")) {
				if (this.isEOF() || !j(this.char())) return this.error(O.INVALID_TAG, A(o, this.clonePosition()));
				let e = this.clonePosition();
				return r === this.parseTagName() ? (this.bumpSpace(), this.bumpIf(">") ? {
					...
					err: null
				} : this.error(O.INVALID_TAG, A(o, this.clonePosition()))) : this.error(O.UNMATCHED_CLOSING_TAG, A(e, this.clonePosition()));
			} else return this.error(O.UNCLOSED_TAG, A(n, this.clonePosition()));
		} else return this.error(O.INVALID_TAG, A(n, this.clonePosition()));
	}
	parseTagName() {
		let e = this.offset();
	...
	parseArgument(e, t) {
		let n = this.clonePosition();
		if (this.bump(), this.bumpSpace(), this.isEOF()) return this.error(O.EXPECT_ARGUMENT_CLOSING_BRACE, A(n, this.clonePosition()));
		if (this.char() === 125) return this.bump(), this.error(O.EMPTY_ARGUMENT, A(n, this.clonePosition()));
		let r = this.parseIdentifierIfPossible().value;
		if (!r) return this.error(O.MALFORMED_ARGUMENT, A(n, this.clonePosition()));
		if (this.bumpSpace(), this.isEOF()) return this.error(O.EXPECT_ARGUMENT_CLOSING_BRACE, A(n, this.clonePosition()));
		switch (this.char()) {
			...
			case 44: return this.bump(), this.bumpSpace(), this.isEOF() ? this.error(O.EXPECT_ARGUMENT_CLOSING_BRACE, A(n, this.clonePosition())) : this.parseArgumentOptions(e, t, r, n);
			default: return this.error(O.MALFORMED_ARGUMENT, A(n, this.clonePosition()));
		}
	}
	...
	parseArgumentOptions(e, t, n, r) {
		...
		switch (a) {
			case "": return this.error(O.EXPECT_ARGUMENT_TYPE, A(i, o));
			case "number":
			case "date":
			case "time": {
				this.bumpSpace();
				...
				if (this.bumpIf(",")) {
					...
					if (n.err) return n;
					let r = Ie(n.val);
					if (r.length === 0) return this.error(O.EXPECT_ARGUMENT_STYLE, A(this.clonePosition(), this.clonePosition()));
					e = {
						style: r,
			...
			case "select": {
				...
				if (this.bumpSpace(), !this.bumpIf(",")) return this.error(O.EXPECT_SELECT_ARGUMENT_OPTIONS, A(i, { ...i }));
				this.bumpSpace();
				let o = this.parseIdentifierIfPossible(), s = 0;
				if (a !== "select" && o.value === "offset") {
					if (!this.bumpIf(":")) return this.error(O.EXPECT_PLURAL_ARGUMENT_OFFSET_VALUE, A(this.clonePosition(), this.clonePosition()));
					this.bumpSpace();
			...
			default: return this.error(O.INVALID_ARGUMENT_TYPE, A(i, o));
		}
	}
	tryParseArgumentClose(e) {
		return this.isEOF() || this.char() !== 125 ? this.error(O.EXPECT_ARGUMENT_CLOSING_BRACE, A(e, this.clonePosition())) : (this.bump(), {
			val: !0,
			err: null
		});
	}
	parseSimpleArgStyleIfPossible() {
		...
		for (; !this.isEOF();) switch (this.char()) {
			case 39: {
				this.bump();
				let e = this.clonePosition();
				if (!this.bumpUntil("'")) return this.error(O.UNCLOSED_QUOTE_IN_ARGUMENT_STYLE, A(e, this.clonePosition()));
				this.bump();
				break;
			}
			case 123:
	...
	tryParsePluralOrSelectOptions(e, t, n, r) {
		...
		for (;;) {
			...
			if (!this.bumpIf("{")) return this.error(t === "select" ? O.EXPECT_SELECT_ARGUMENT_SELECTOR_FRAGMENT : O.EXPECT_PLURAL_ARGUMENT_SELECTOR_FRAGMENT, A(this.clonePosition(), this.clonePosition()));
		...
		return a.length === 0 ? this.error(t === "select" ? O.EXPECT_SELECT_ARGUMENT_SELECTOR : O.EXPECT_PLURAL_ARGUMENT_SELECTOR, A(this.clonePosition(), this.clonePosition())) : this.requiresOtherClause && !i ? this.error(O.MISSING_OTHER_CLAUSE, A(this.clonePosition(), this.clonePosition())) : {
			val: a,
			err: null
		};
	}
...
function M(e) {
	e.forEach((e) => {
		if (delete e.location, xe(e) || Se(e)) for (let t in e.options) delete e.options[t].location, M(e.options[t].value);
		else ve(e) && Te(e.style) || (ye(e) || be(e)) && D(e.style) ? delete e.style.location : we(e) && M(e.children);
	});
}
function Ue(e, t = {}) {
	t = {
...
function R(e, t, n, r, i, a, o) {
	...
	for (let c of e) {
		...
		if (we(c)) {
			...
			if (!L(u)) throw new We(l, "function", o);
...
function Je(e, t) {
	return t ? Object.keys(e).reduce((n, r) => (n[r] = qe(e[r], t[r]), n), { ...e }) : e;
}
function z(e) {
	return { create() {
		return {
...
var B = class e {
	...
	constructor(t, n = e.defaultLocale, r, i) {
		...
		this.formats = Je(e.formats, r), this.formatters = i && i.formatters || Ye(this.formatterCache);
	}
	format = (e) => {
...
function rt(e = nt()) {
	...
	return {
		getDateTimeFormat: i,
		getNumberFormat: a,
		getMessageFormat: h((e, t, n, r) => new B(e, t, n, {
			formatters: {
				getNumberFormat: a,
				getDateTimeFormat: i,
				getPluralRules: o
...
function at(e, t) {
	...
	return {
		...n,
		...e,
		date: it(J(n.date, t), J(e.date || {}, t)),
		time: it(J(n.time, t), J(e.time || {}, t))
	};
}
var Y = ({ locale: e, formats: t, messages: n, defaultLocale: r, defaultFormats: i, fallbackOnEmptyString: a, onError: o, timeZone: s, defaultRichTextElements: c }, l, u = { id: "" }, d, f) => {
	...
	et(!!p, "[@formatjs/intl] An `id` must be provided to format a message. You can either:\n1. Configure your build toolchain with [babel-plugin-formatjs](https://formatjs.github.io/docs/tooling/babel-plugin)\nor [@formatjs/ts-transformer](https://formatjs.github.io/docs/tooling/ts-transformer) OR\n2. Configure your `eslint` config to include [eslint-plugin-formatjs](https://formatjs.github.io/docs/tooling/linter#enforce-id)\nto autofix this issue");
...
var mt = ["type", "style"], ht = Date.now();
function gt(e) {
	return `${ht}_${e}_${ht}`;
}
function _t(e, t, n, r = {}) {
	let i = vt(e, t, n, r).reduce((e, t) => {
		let n = t.value;
		return typeof n == "string" && typeof e[e.length - 1] == "string" ? e[e.length - 1] += n : e.push(n), e;
	}, []);
	return i.length === 1 ? i[0] : i.length === 0 ? "" : i;
}
function vt({ locale: e, onError: t }, n, r, i = {}) {
...
var xt = ["numeric", "style"];
function St({ locale: e, formats: t, onError: n }, r, i = {}) {
	let { format: a } = i;
	return r(e, G(i, xt, !!a && q(t, "relative", a, n) || {}));
}
...
function Tt({ locale: e, formats: t, onError: n }, r, i = {}) {
	let { format: a } = i;
	return r(e, G(i, wt, a && q(t, "number", a, n) || {}));
}
function Et(e, t, n, r = {}) {
	try {
		return Tt(e, t, r).format(n);
	} catch (t) {
...
function Dt(e, t, n, r = {}) {
	try {
		return Tt(e, t, r).formatToParts(n);
	} catch (t) {
		e.onError(new U("Error formatting number.", e.locale, t));
	}
	return [];
...
function kt(e) {
	e.onWarn && e.defaultRichTextElements && Ot(e.messages || {}) && e.onWarn("[@formatjs/intl] \"defaultRichTextElements\" was specified but \"message\" was not pre-compiled. \nPlease consider using \"@formatjs/cli\" to pre-compile your messages for performance.\nFor more details see https://formatjs.github.io/docs/getting-started/message-distribution");
}
function At(e, t) {
	let n = rt(t), r = {
		...tt,
	...
	return i ? !Intl.NumberFormat.supportedLocalesOf(i).length && o ? o(new Qe(`Missing locale data for locale: "${i}" in Intl.NumberFormat. Using default locale: "${a}" as fallback. See https://formatjs.github.io/docs/react-intl#runtime-requirements for more details`)) : !Intl.DateTimeFormat.supportedLocalesOf(i).length && o && o(new Qe(`Missing locale data for locale: "${i}" in Intl.DateTimeFormat. Using default locale: "${a}" as fallback. See https://formatjs.github.io/docs/react-intl#runtime-requirements for more details`)) : (o && o(new Ze(`"locale" was not configured, using "${a}" as fallback. See https://formatjs.github.io/docs/react-intl/api#intlshape for more details`)), r.locale = r.defaultLocale || "en"), kt(r), {
		...
		$t: Y.bind(null, r, n),
		formatList: _t.bind(null, r, n.getListFormat),
		formatListToParts: vt.bind(null, r, n.getListFormat),
		formatDisplayName: pt.bind(null, r, n.getDisplayNames)
	};
}
//#endregion
//#region ../../../node_modules/.bun/react-intl@10.1.1+b2e33729a97476bf/node_modules/react-intl/src/utils.js
function jt(e, t, n = Error) {
	if (!e) throw new n(t);
}
function Z(e) {
	jt(e, "[React Intl] Could not find required `intl` object. <IntlProvider> needs to exist in the component ancestry.");
}
var Mt = {
	...tt,
	textComponent: e.Fragment
}, Nt = (t) => e.Children.toArray(t).map((t, n) => e.isValidElement(t) ? i(e.Fragment, { children: t }, n) : t);
function Pt(e) {
	return function(t) {
		return e(Nt(t));
	};
...
function Rt(e) {
	return e && Object.keys(e).reduce((t, n) => {
		let r = e[n];
		return t[n] = L(r) ? Pt(r) : r, t;
	}, {});
}
var zt = (e, t, n, r, ...i) => {
	let a = Y(e, t, n, Rt(r), ...i);
...
function Ht(t) {
	let { formatMessage: n, textComponent: a = e.Fragment } = Lt(), { id: o, description: s, defaultMessage: c, values: l, children: u, tagName: d = a, ignoreTag: f } = t, p = n({
		id: o,
		description: s,
		defaultMessage: c
	}, l, { ignoreTag: f });
...
function Wt(t) {
	...
	return (!r.current || !Q(r.current, s)) && (r.current = s, a.current = Bt(s, n.current)), Z(a.current), i(It, {
		value: a.current,
		children: t.children
	});
}
Wt.displayName = "IntlProvider";
var Gt = Wt;
//#endregion
