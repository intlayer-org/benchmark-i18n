...
export default function ResultsTable() {
  ...
  return (
    <section>
      <h2 className="mb-6 text-2xl font-bold text-foreground">{content.sampleResults}</h2>
      <div className="overflow-x-auto rounded-lg border border-border">
        <table className="w-full text-sm">
          <thead className="bg-muted">
            <tr>
              <th className="px-4 py-3 text-left font-medium text-muted-foreground">{content.library}</th>
              <th className="px-4 py-3 text-left font-medium text-muted-foreground">{content.bundleSize}</th>
              <th className="px-4 py-3 text-left font-medium text-muted-foreground">{content.lookupTime}</th>
              <th className="px-4 py-3 text-left font-medium text-muted-foreground">{content.lazyLoading}</th>
            </tr>
          </thead>
          <tbody>
            {results.map((r) => (
