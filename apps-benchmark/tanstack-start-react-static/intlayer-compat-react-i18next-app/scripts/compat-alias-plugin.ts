 ...
 *
 * `@intlayer/react-i18next` and `@intlayer/i18next` ship ESM only (their
 * `exports` map has no `require` condition), so `require.resolve` throws on
 * them. `import.meta.resolve` honours the `import` condition; `require.resolve`
...
/**
 * Mirrors the `react-i18next` + `i18next` → `@intlayer/*` aliasing that the
 * compat Vite plugin applies to the app build, so the component-size and
 * lib-size measurements bundle the compat adapter instead of the real
 * react-i18next / i18next packages that the components still import by name.
 */
...
export const reactI18nextCompatAlias = () => ({
  name: "react-i18next-compat-alias",
