...
function ChevronDown(props: { class?: string }) {
  return (
    <svg
      width="14"
      height="14"
...
export default function Header() {
  ...
  const mockPages = () => [
    { to: `/${currentLocale()}/products`, label: m.header_products() },
    { to: `/${currentLocale()}/pricing`, label: m.header_pricing() },
    { to: `/${currentLocale()}/team`, label: m.header_team() },
    { to: `/${currentLocale()}/blog`, label: m.header_blog() },
    { to: `/${currentLocale()}/careers`, label: m.header_careers() },
    { to: `/${currentLocale()}/faq`, label: m.header_faq() },
    { to: `/${currentLocale()}/contact`, label: m.header_contact() },
    { to: `/${currentLocale()}/settings`, label: m.header_settings() },
  ];
    ...
    <header class="sticky top-0 z-50 border-b border-border bg-card/80 backdrop-blur-lg">
      <nav class="container flex h-16 items-center justify-between">
        <div class="flex items-center gap-8">
          ...
          <div class="hidden items-center gap-6 text-sm font-medium md:flex">
            ...
            <A
              ...
              inactiveClass=""
            >
              {m.header_methodology()}
            </A>

            <div class="relative">
              <button
                type="button"
                class="flex cursor-pointer items-center gap-1 border-none bg-transparent nav-link"
              ...
              {isMockPagesOpen() && (
                <div
                  ...
                  onMouseLeave={() => setIsMockPagesOpen(false)}
                >
                  <div class="overflow-hidden rounded-md border border-border bg-card py-1 shadow-lg">
                    <For each={mockPages()}>
                      {(page) => (
                        <A
                          href={page.to}
        ...
        <div class="flex items-center gap-4">
          ...
          >
            <span class="sr-only">{m.shared_goToGithub()}</span>
