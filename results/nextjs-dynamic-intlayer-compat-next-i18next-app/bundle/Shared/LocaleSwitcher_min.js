...
var Se = "translation", Ce = "enumeration", E = "plural", we = "insertion", Te = "object", Ee = "array", D = "html", De = "gender", Oe = "select", O = (e, t, n) => ({
	...
	let r = Ct()[e];
	return r ? Bt(r, t, n) : (Et.has(e) || (w({ log: v })(typeof window > "u" ? `Dictionary ${xe(e)} was not found. Using fallback proxy.` : `Dictionary ${e} was not found. Using fallback proxy.`, { level: "warn" }), Et.add(e)), Tt(e));
}, Dt = (e) => {
...
}, jt = q, Mt = q, Nt = q, Pt = q, Ft = q, It = (e) => q, Lt = q, Rt = (e, t = !0) => [
	...
	return o < Ht && g.includes("$t(") && (g = g.replace(/\$t\(\s*([^),]+?)\s*(?:,[^)]*)?\)/g, (n, l) => {
		let u = Y({
			locale: e,
			namespace: c,
			key: l.trim(),
...
var qt = ({ locale: e, setLocale: t, availableLocales: n, namespace: r, keyPrefix: i, dictionaryContent: a }) => {
	...
	let { locales: l, defaultLocale: u } = g ?? {}, [d, p] = c(e ?? Jt ?? t ?? u);
	i(() => {
		e && e !== d && p(e);
	}, [e]), i(() => {
		en();
...
}, sn = ({ children: e, i18n: t }) => (t !== void 0 && w({ log: v })(`${T("I18nextProvider", b)}: the \`i18n\` prop has no effect with intlayer. Intlayer manages its own i18n instance — you can safely remove the prop.`), f(nn, { children: e }));
...
function pn({ children: e }) {
	...
	return a(() => {
		...
		i18n: dn,
		children: [f(fn, { locale: t }), e]
	});
}
function mn({ children: e }) {
	return f(pn, { children: e });
}
function hn() {
	return f(mn, { children: f(ee, {}) });
}
