...
import { createContext as ne, useCallback as re, useContext as x, useEffect as S, useLayoutEffect as C, useMemo as ie, useRef as w, useState as T } from "react";
import { jsx as E, jsxs as ae } from "react/jsx-runtime";
import { jsxDEV as D } from "react/jsx-dev-runtime";
import { useParams as oe } from "next/navigation";
var se = (e, t = o?.locales, n = o?.defaultLocale) => {
...
}, ue = process.env.INTLAYER_ROUTING_STORAGE_COOKIES === "false", de = process.env.INTLAYER_ROUTING_STORAGE_LOCALSTORAGE === "false", fe = process.env.INTLAYER_ROUTING_STORAGE_SESSIONSTORAGE === "false";
...
var O = {
	...
	if (!ue) for (let n = 0; n < (e.storage.cookies ?? []).length; n++) try {
		let i = t?.getCookie?.(e.storage.cookies[n].name);
		if (r(i)) return i;
	} catch {}
	if (!de) for (let n = 0; n < (e.storage.localStorage ?? []).length; n++) try {
		let i = t?.getLocaleStorage?.(e.storage.localStorage[n].name);
		if (r(i)) return i;
	} catch {}
	if (!fe && e.storage.sessionStorage) for (let n = 0; n < e.storage.sessionStorage.length; n++) try {
		let i = t?.getSessionStorage?.(e.storage.sessionStorage[n].name);
		if (r(i)) return i;
	} catch {}
}, me = (t, n) => {
	if (n?.isCookieEnabled !== !1) {
		if (!ue && e.storage.cookies) for (let r = 0; r < e.storage.cookies.length; r++) {
			let { name: i, attributes: a } = e.storage.cookies[r];
			try {
				n?.setCookieStore && n.setCookieStore(i, t, {
					...a,
					expires: ce(a.expires)
				});
			} catch {
				try {
					n?.setCookieString && n.setCookieString(i, le(i, t, a));
				} catch {}
			}
		}
		if (!de && e.storage.localStorage && n?.setLocaleStorage) for (let r = 0; r < e.storage.localStorage.length; r++) {
		...
		if (!fe && e.storage.sessionStorage && n?.setSessionStorage) for (let r = 0; r < e.storage.sessionStorage.length; r++) {
	...
	try {
		...
		let n = t.maximize();
		return he.includes(n.script ?? "") ? "rtl" : "ltr";
	} catch {
		return "ltr";
	}
}, _e = 50, k = /* @__PURE__ */ new Map(), A = /* @__PURE__ */ new Set(), ve = (e) => {
...
function j(e, t, n) {
	...
	if (!c) {
		...
		s.size > _e && s.clear(), c = new t(r, n), s.set(i, c);
	}
	return c;
}
var M = (e) => y(v, e), N = (e) => y(d, e), xe = (e) => {
	let t = {};
...
})() }), R = (e) => y(l, e), z = (e, t) => y(b, e, { variable: t }), Te = (e) => {
	let t = 0, n = () => {
		let n = [], i = "";
		for (; t < e.length;) {
			let a = e[t];
...
}), Le = (e, t, n) => e[j("PluralRules", n).select(t)] ?? e.other, Re = [
	"__intlayer_icu_var",
...
var Je = ({ locale: e, setLocale: t, availableLocales: n, namespace: r, keyPrefix: i, dictionaryContent: a }) => {
...
}, Ye = process.env.INTLAYER_EDITOR_ENABLED !== "false" && i?.enabled && typeof window < "u" && window.self !== window.top, Xe = pe(O), Ze = (e, t) => me(e, {
	...O,
	isCookieEnabled: t
}), Qe = () => {
	...
	let { locales: c, defaultLocale: l } = o ?? {}, [u, d] = T(e ?? Xe ?? t ?? l);
	S(() => {
		e && e !== u && d(e);
	}, [e]), S(() => {
		rt();
	...
	let n = Array.isArray(e) ? e[0] ?? "translation" : e ?? "translation", { locale: r, setLocale: i, availableLocales: a } = ot(), o = t?.keyPrefix, { translate: s, i18n: c } = ie(() => Je({
		locale: r,
		setLocale: i,
		availableLocales: a ?? [],
		namespace: n,
...
var Q = "/Users/aymericpineau/Documents/benchmark-bloom/apps-benchmark/nextjs-dynamic/intlayer-compat-next-i18next-app/components/pages/home/Hero.tsx";
function ut() {
	let { t: e } = st();
	return lt("Hero"), D("section", {
...
function ht({ children: e }) {
	let t = oe().locale ?? "en", [n] = T(() => typeof performance < "u" ? performance.now() : 0);
	return C(() => {
		pt("AppRoot", n);
	}, [n]), S(() => {
...
var $ = "/Users/aymericpineau/Documents/benchmark-bloom/apps-benchmark/nextjs-dynamic/intlayer-compat-next-i18next-app/components/pages/home/Hero.wrapper.tsx";
function vt() {
