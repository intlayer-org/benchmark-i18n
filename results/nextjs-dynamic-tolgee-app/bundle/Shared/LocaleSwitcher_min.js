...
function Se({ options: e }) {
	let t = de(), n = L(!1, () => o.isFetching(), t.onFetchingChange.emit), r = L(!1, () => S.isLoading(), t.onLoadingChange.emit), i = be(t.onLanguageChange, t.onPendingLanguageChange, t.onRunningChange), a = ye(i.getLanguage, i.getInitialOptions, i.getAvailableLanguages, u, _, v, f, t), o = he(t, a.getBackendRecord, a.getBackendDevRecord, i.withDefaultNs, i.isInitialLoading, n, r);
	e && p(e);
	let s;
	t.onUpdate.listen(() => {
...
var we = () => {
	...
	return t;
}, Te = 0, z = 1, Ee = 2, De = class extends Error {
	constructor(e, t, n) {
		let r, i = "";
		e === Te ? r = "Empty parameter" : e === z ? (r = "Unexpected character", i = "Did you forget to use FormatIcu to render ICU message syntax?") : r = "Unexpected end", super(`Tolgee parser: ${r} at ${t} in "${n}"` + (i ? "\n" + i : "")), this.code = e, this.index = t;
	}
};
function Oe(e) {
	return /\s/.test(e);
}
var B = 0, V = 1, H = 2, U = 3, W = 4, ke = /* @__PURE__ */ new Set([
	H,
	V,
	B
]), G = "'", Ae = /* @__PURE__ */ new Set([
	"{",
	"}",
	G
...
function Me(e) {
	...
	for (s = 0; s < e.length; s++) switch (i = e[s], t) {
		case B:
			...
			break;
		case V:
			Ae.has(i) ? (n = n.slice(0, -1) + i, t = H) : (n += i, t = B);
			break;
		case H:
			i === G ? t = B : (n += i, t = H);
			break;
		case U:
			i === "}" ? (d(), t = B) : Oe(i) ? r !== "" && (d(), t = W) : (u(), t = U);
			break;
		case W: i == "}" ? t = B : Oe(i) ? t = W : c(z);
	}
...
(S = console.assert) == null || S.call(console, ze), RegExp(`([${["‌", "‍"].join("")}]{9})+`, "g");
function Be(e, t) {
	return t ? `${t}: ${e}` : e;
}
...
}, Ye = 3e3, Xe = 200, Ze = 0, J = /* @__PURE__ */ new Map(), Qe = !1, Y, X;
...
var et = (e) => new q("unavailable", `the Tolgee browser extension did not answer ${e} in time`), tt = () => `${Date.now()}-${Ze++}-${Math.random()}`;
function nt() {
...
function rt(e) {
	return Y ||= new Promise((t, n) => {
		let r = () => window.postMessage({ type: Ke }, window.origin), i = () => {
			clearInterval(a), it(), X = void 0, n(new q("unavailable", "the Tolgee browser extension did not answer"));
		}, a = setInterval(() => {
			if (Date.now() > e) {
				i();
				return;
...
var Z = "__tolgee_", xt = `${Z}apiKey`, St = `${Z}apiUrl`, Ct = `${Z}branch`, wt = `${Z}projectId`, Tt = `${Z}session`, Et = "tolgee-in-context-tools.umd.min.js", Dt = "@tolgee/in-context-tools", Ot = "InContextTools", kt = "https://cdn.jsdelivr.net/npm", At = null;
function jt(e) {
...
function Lt() {
	...
	if (i && r) return x(b({}, a), { transport: at() });
}
function Rt() {
	let e = [];
...
function en() {
	let e = Zt(["language"]), t = e.getLanguage() || "en", n = l(), r = c(), i = (t) => {
		e.changeLanguage(t);
		let i = r.split("/");
		i[1] = t;
		let a = i.join("/") || `/${t}`;
...
var tn = (e, t, n) => {
	...
	try {
		return (await tn(Object.assign({
			"./locales/de.json": () => import("../i18n/locales/de.json"),
			"./locales/en.json": () => Promise.resolve().then(() => dn),
		...
		return (await Promise.resolve().then(() => dn)).default;
	}
}, rn = [
	"en",
	"fr",
...
function an() {
	return Ht().use(Fe()).updateDefaults({ staticData: Object.fromEntries(rn.map((e) => [e, () => nn(e)])) });
}
var on = an().init(), sn = ({ language: e, staticData: t, children: n }) => {
	let r = l();
	return i(() => {
		let { unsubscribe: e } = on.on("permanentChange", () => {
			r.refresh();
		});
		return () => {
			e();
...
function un({ children: e, locale: t, staticData: n }) {
...
}, An = "Cancel", jn = "Copy", Mn = { mockBanner: "⚠️ This page contains mock data for benchmarking purposes only. It is not related to any real business or service." }, Nn = "Read More", Pn = "Save Changes", Fn = {
