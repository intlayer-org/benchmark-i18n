...
var Te = () => {
	...
	return t;
}, Ee = 0, z = 1, De = 2, Oe = class extends Error {
	constructor(e, t, n) {
		let r, i = "";
		e === Ee ? r = "Empty parameter" : e === z ? (r = "Unexpected character", i = "Did you forget to use FormatIcu to render ICU message syntax?") : r = "Unexpected end", super(`Tolgee parser: ${r} at ${t} in "${n}"` + (i ? "\n" + i : "")), this.code = e, this.index = t;
	}
};
function ke(e) {
	return /\s/.test(e);
}
var B = 0, V = 1, H = 2, U = 3, W = 4, Ae = /* @__PURE__ */ new Set([
	H,
	V,
	B
]), G = "'", je = /* @__PURE__ */ new Set([
	"{",
	"}",
	G
...
function Ne(e) {
	...
	for (s = 0; s < e.length; s++) switch (i = e[s], t) {
		case B:
			...
			break;
		case V:
			je.has(i) ? (n = n.slice(0, -1) + i, t = H) : (n += i, t = B);
			break;
		case H:
			i === G ? t = B : (n += i, t = H);
			break;
		case U:
			i === "}" ? (d(), t = B) : ke(i) ? r !== "" && (d(), t = W) : (u(), t = U);
			break;
		case W: i == "}" ? t = B : ke(i) ? t = W : c(z);
	}
...
(C = console.assert) == null || C.call(console, Be), RegExp(`([${["‌", "‍"].join("")}]{9})+`, "g");
function Ve(e, t) {
	return t ? `${t}: ${e}` : e;
}
...
}, Xe = 3e3, Ze = 200, Qe = 0, J = /* @__PURE__ */ new Map(), $e = !1, Y, X;
...
var tt = (e) => new q("unavailable", `the Tolgee browser extension did not answer ${e} in time`), nt = () => `${Date.now()}-${Qe++}-${Math.random()}`;
function rt() {
...
function it(e) {
	return Y ||= new Promise((t, n) => {
		let r = () => window.postMessage({ type: qe }, window.origin), i = () => {
			clearInterval(a), at(), X = void 0, n(new q("unavailable", "the Tolgee browser extension did not answer"));
		}, a = setInterval(() => {
			if (Date.now() > e) {
				i();
				return;
...
var Z = "__tolgee_", St = `${Z}apiKey`, Ct = `${Z}apiUrl`, wt = `${Z}branch`, Tt = `${Z}projectId`, Et = `${Z}session`, Dt = "tolgee-in-context-tools.umd.min.js", Ot = "@tolgee/in-context-tools", kt = "InContextTools", At = "https://cdn.jsdelivr.net/npm", jt = null;
function Mt(e) {
...
function Rt() {
	...
	if (i && r) return S(x({}, a), { transport: ot() });
}
function zt() {
	let e = [];
...
var Kt = { useSuspense: !1 }, qt, Jt = () => (qt ||= e.createContext(void 0), qt), Q = void 0, Yt = ({ tolgee: n, options: r, children: a, fallback: o, ssr: s }) => {
	i(() => {
	...
	let { tolgee: r, options: a } = Xt(), o = te(e), c = k(o).join(":"), l = Object.assign(Object.assign({}, a), t), { rerender: u, instance: d } = Zt(), f = s([]);
	f.current = [];
	let p = r.isLoaded(o);
	i(() => {
		let e = r.on("update", u);
...
var nn = (e, t, n) => {
	...
	try {
		return (await nn(Object.assign({
			...
			"./locales/en.json": () => Promise.resolve().then(() => fn),
		...
		return (await Promise.resolve().then(() => fn)).default;
	}
}, an = [
	"en",
	"fr",
...
function on() {
	return Ut().use(Ie()).updateDefaults({ staticData: Object.fromEntries(an.map((e) => [e, () => rn(e)])) });
}
var sn = on().init(), cn = ({ language: e, staticData: t, children: n }) => {
	let r = d();
	return i(() => {
		let { unsubscribe: e } = sn.on("permanentChange", () => {
			r.refresh();
		});
		return () => {
			e();
...
function dn({ children: e, locale: t, staticData: n }) {
...
}, jn = "Cancel", Mn = "Copy", Nn = { mockBanner: "⚠️ This page contains mock data for benchmarking purposes only. It is not related to any real business or service." }, Pn = "Read More", Fn = "Save Changes", In = {
