...
var De = () => {
	...
	return t;
}, Oe = 0, L = 1, ke = 2, Ae = class extends Error {
	constructor(e, t, n) {
		let r, i = "";
		e === Oe ? r = "Empty parameter" : e === L ? (r = "Unexpected character", i = "Did you forget to use FormatIcu to render ICU message syntax?") : r = "Unexpected end", super(`Tolgee parser: ${r} at ${t} in "${n}"` + (i ? "\n" + i : "")), this.code = e, this.index = t;
	}
};
function je(e) {
	return /\s/.test(e);
}
var R = 0, z = 1, B = 2, V = 3, H = 4, Me = /* @__PURE__ */ new Set([
	B,
	z,
	R
]), U = "'", Ne = /* @__PURE__ */ new Set([
	"{",
	"}",
	U
...
function Fe(e) {
	...
	for (s = 0; s < e.length; s++) switch (i = e[s], t) {
		case R:
			...
			break;
		case z:
			Ne.has(i) ? (n = n.slice(0, -1) + i, t = B) : (n += i, t = R);
			break;
		case B:
			i === U ? t = R : (n += i, t = B);
			break;
		case V:
			i === "}" ? (d(), t = R) : je(i) ? r !== "" && (d(), t = H) : (u(), t = V);
			break;
		case H: i == "}" ? t = R : je(i) ? t = H : c(L);
	}
...
(C = console.assert) == null || C.call(console, He), RegExp(`([${["‌", "‍"].join("")}]{9})+`, "g");
function Ue(e, t) {
	return t ? `${t}: ${e}` : e;
}
...
}, Qe = 3e3, $e = 200, et = 0, K = /* @__PURE__ */ new Map(), tt = !1, q, J;
...
var rt = (e) => new G("unavailable", `the Tolgee browser extension did not answer ${e} in time`), it = () => `${Date.now()}-${et++}-${Math.random()}`;
function at() {
...
function ot(e) {
	return q ||= new Promise((t, n) => {
		let r = () => window.postMessage({ type: Ye }, window.origin), i = () => {
			clearInterval(a), st(), J = void 0, n(new G("unavailable", "the Tolgee browser extension did not answer"));
		}, a = setInterval(() => {
			if (Date.now() > e) {
				i();
				return;
...
var Y = "__tolgee_", wt = `${Y}apiKey`, Tt = `${Y}apiUrl`, Et = `${Y}branch`, Dt = `${Y}projectId`, Ot = `${Y}session`, kt = "tolgee-in-context-tools.umd.min.js", At = "@tolgee/in-context-tools", jt = "InContextTools", Mt = "https://cdn.jsdelivr.net/npm", Nt = null;
function Pt(e) {
...
function Bt() {
	...
	if (i && r) return S(x({}, a), { transport: ct() });
}
function Vt() {
	let e = [];
...
var Jt = { useSuspense: !1 }, Yt, Xt = () => (Yt ||= e.createContext(void 0), Yt), X = void 0, Zt = ({ tolgee: n, options: r, children: a, fallback: o, ssr: s }) => {
	i(() => {
	...
	let { tolgee: r, options: a } = Qt(), o = te(e), c = k(o).join(":"), l = Object.assign(Object.assign({}, a), t), { rerender: u, instance: d } = $t(), f = s([]);
	f.current = [];
	let p = r.isLoaded(o);
	i(() => {
		let e = r.on("update", u);
...
var cn = (e, t, n) => {
	...
	try {
		return (await cn(Object.assign({
			...
			"./locales/en.json": () => Promise.resolve().then(() => _n),
		...
		return (await Promise.resolve().then(() => _n)).default;
	}
}, un = [
	"en",
	"fr",
...
function dn() {
	return Gt().use(Re()).updateDefaults({ staticData: Object.fromEntries(un.map((e) => [e, () => ln(e)])) });
}
var fn = dn().init(), pn = ({ language: e, staticData: t, children: n }) => {
	let r = d();
	return i(() => {
		let { unsubscribe: e } = fn.on("permanentChange", () => {
			r.refresh();
		});
		return () => {
			e();
...
function gn({ children: e, locale: t, staticData: n }) {
...
}, In = "Cancel", Ln = "Copy", Rn = { mockBanner: "⚠️ This page contains mock data for benchmarking purposes only. It is not related to any real business or service." }, zn = "Read More", Bn = "Save Changes", Vn = {
