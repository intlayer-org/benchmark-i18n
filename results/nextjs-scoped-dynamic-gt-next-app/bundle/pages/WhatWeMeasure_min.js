...
function _s() {
	return _("section", {
		...
		children: [g("h2", {
			...
			children: g(X, { children: "What We Measure" })
		}), g("ul", {
			className: "space-y-4",
			children: [
				{
					metric: g(X, { children: "Bundle size impact" }),
					desc: g(X, { children: "The additional JavaScript bytes sent to users when the i18n library and its translation files are included. This directly affects download time on slow networks." })
				},
				{
					metric: g(X, { children: "Rendering overhead" }),
					desc: g(X, { children: "How much extra time the library adds to React's render cycle. Libraries that inject translations via a single context provider can cause unnecessary re-renders across the component tree." })
				},
				{
					metric: g(X, { children: "Hydration cost" }),
					desc: g(X, { children: "During SSR, translation data is serialized into HTML. Large dictionaries increase the HTML payload and slow down hydration — the moment the page becomes interactive." })
				},
				{
					metric: g(X, { children: "Lazy loading effectiveness" }),
					desc: g(X, { children: "Whether splitting translations by route or namespace actually reduces the initial load, and what trade-offs it introduces (waterfall requests, FOUC, cache complexity)." })
				},
				{
					metric: g(X, { children: "Locale switch speed" }),
					desc: g(X, { children: "How fast the app can switch from one language to another at runtime — including fetching new translations, re-rendering components, and updating the DOM." })
				}
			].map((e) => _("li", {
