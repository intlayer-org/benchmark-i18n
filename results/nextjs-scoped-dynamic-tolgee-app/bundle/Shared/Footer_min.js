...
}, _ = Object.defineProperty, v = Object.defineProperties, y = Object.getOwnPropertyDescriptors, b = Object.getOwnPropertySymbols, x = Object.prototype.hasOwnProperty, S = Object.prototype.propertyIsEnumerable, C = (e, t, n) => t in e ? _(e, t, {
	enumerable: !0,
	configurable: !0,
	writable: !0,
	value: n
...
(te = console.assert) == null || te.call(console, Ue), RegExp(`([${["‌", "‍"].join("")}]{9})+`, "g");
function We(e, t) {
	return t ? `${t}: ${e}` : e;
}
...
}, $e = 3e3, et = 200, tt = 0, J = /* @__PURE__ */ new Map(), nt = !1, Y, rt;
async function it({ type: e, replyType: t, payload: n, timeoutMs: r = qe, progressType: i, onProgress: a }) {
	st();
	let o = Date.now() + r;
	await ct(Math.min(o, Date.now() + $e));
	let s = o - Date.now();
	if (s <= 0) throw at(e);
	let c = ot();
	return new Promise((r, o) => {
		let l = setTimeout(() => {
			J.delete(c), lt(), o(at(e));
		}, s);
		J.set(c, {
			replyType: t,
			progressType: i,
...
var at = (e) => new q("unavailable", `the Tolgee browser extension did not answer ${e} in time`), ot = () => `${Date.now()}-${tt++}-${Math.random()}`;
function st() {
...
function ct(e) {
	return Y ||= new Promise((t, n) => {
		let r = () => window.postMessage({ type: Xe }, window.origin), i = () => {
			clearInterval(a), lt(), rt = void 0, n(new q("unavailable", "the Tolgee browser extension did not answer"));
		}, a = setInterval(() => {
			if (Date.now() > e) {
				i();
				return;
...
var ht = "ABCDEFGHIJKLMNOPQRSTUVWXYZ234567";
function gt(e) {
	let t = ht.indexOf(e);
...
function vt(e) {
	...
	for (let o = 0; o < t; o++) r = r << 5 | gt(e[o]), n += 5, n >= 8 && (a[i++] = r >>> n - 8 & 255, n -= 8);
	return _t(a.buffer);
}
function yt(e) {
	if (e) try {
...
function xt(e) {
	...
	return r ? {
		...
		hasCredential: !!t,
		projectId: yt(t) ?? n,
		requiresExplicitProject: bt(t) === "tgpat"
	};
}
function St(e) {
...
function wt({ message: e, recievingMessage: t, data: n, attempts: r = 1, timeout: i = 300 }) {
...
var X = "__tolgee_", Et = `${X}apiKey`, Dt = `${X}apiUrl`, Ot = `${X}branch`, kt = `${X}projectId`, At = `${X}session`, jt = "tolgee-in-context-tools.umd.min.js", Mt = "@tolgee/in-context-tools", Nt = "InContextTools", Pt = "https://cdn.jsdelivr.net/npm", Ft = null;
function It(e) {
	return Ft ||= Vt(Lt(e)).then(() => window[Mt][Nt]), Ft;
}
function Lt(e) {
	return Rt() || `${Pt}/@tolgee/web@${e}/dist/${jt}`;
}
function Rt() {
	if (Be()) return;
	let e = window.__TOLGEE_IN_CONTEXT_URL__;
	return Bt(e, window.location) ? e : void 0;
}
var zt = (e) => e === "localhost" || e === "127.0.0.1" || e === "::1" || e === "[::1]";
function Bt(e, t) {
	if (!e || !zt(t.hostname)) return !1;
	try {
		let n = new URL(e, t.href);
		return n.origin === t.origin || zt(n.hostname);
	} catch {
		return !1;
	}
}
...
function Ht() {
	let e = sessionStorage.getItem(Et) || void 0, t = sessionStorage.getItem(Dt) || void 0, n = sessionStorage.getItem(Ot) || void 0, r = sessionStorage.getItem(kt) || void 0, i = Qe(sessionStorage.getItem(At));
	if (!t) return;
	let a = w(w({ apiUrl: t }, r === void 0 ? {} : { projectId: r }), n === void 0 ? {} : { branch: n });
	if (e) return ee(w({}, a), { apiKey: e });
	if (i && r) return ee(w({}, a), { transport: ut() });
}
function Ut() {
	let e = [];
...
function Wt(e) {
	if (!e.isDev()) return;
	let { requiresExplicitProject: t, projectId: n } = xt(e.getInitialOptions());
...
(() => {
	...
	return Wt(e), e.on("running", ({ value: e }) => {
		e && Gt(() => {
			t.update(n()).catch(Ut);
		});
	}), Ht() && (async () => {
		let e = await It("prerelease");
		return (t) => {
			let n = Ht();
...
var Xt = { useSuspense: !1 }, Zt, Qt = () => (Zt ||= e.createContext(void 0), Zt), Z = void 0, $t = ({ tolgee: n, options: r, children: a, fallback: o, ssr: s }) => {
	i(() => {
	...
	l = Yt(n, u, d, !!s);
	let [f, p] = c(!l.isLoaded()), m = Object.assign(Object.assign({}, Xt), r), h = Qt();
	...
	let { tolgee: r, options: a } = en(), o = re(e), c = A(o).join(":"), l = Object.assign(Object.assign({}, a), t), { rerender: u, instance: d } = tn(), f = s([]);
	f.current = [];
	let p = r.isLoaded(o);
	i(() => {
		let e = r.on("update", u);
...
var on = (t) => {
	...
	return Object.entries(t || {}).forEach(([t, r]) => {
		if (typeof r == "function") n[t] = (e) => r(cn(e));
		else if (e.isValidElement(r)) {
			let i = r;
			n[t] = (t) => i.props.children === void 0 && t?.length ? e.cloneElement(i, {}, cn(t)) : e.cloneElement(i);
		} else n[t] = r;
	}), n;
};
function sn(e) {
...
var cn = (t) => {
	let n = an(t);
	return Array.isArray(n) ? n.map((t, n) => e.createElement(e.Fragment, { key: n }, sn(t))) : sn(n);
}, ln = (t) => {
	let n = t.keyName || t.children;
	n === void 0 && console.error("T component: keyName not defined");
	let r = t.defaultValue || (t.keyName ? t.children : void 0), i = cn(t.t({
		key: n,
		params: on(t.params),
		defaultValue: r,
		noWrap: t.noWrap,
	...
	let { t: n } = nn();
	return e.createElement(ln, Object.assign({ t: n }, t));
};
function dn() {
	let { t: e, ...t } = rn();
	return {
			...
			children: [p("div", {
				...
				children: [
					p("div", { children: [f("h3", {
						...
						children: f(Q, { keyName: "footer.anOpenSourceTestApplication" })
					})] }),
					p("div", { children: [f("h3", {
						className: "mb-2 text-sm font-semibold text-foreground",
						children: f(Q, { keyName: "footer.resources" })
					}), f("ul", {
						className: "space-y-1",
						children: t.map((e) => f("li", { children: e.isInternal ? f(g, {
							href: e.href,
					...
					p("div", { children: [f("h3", {
						className: "mb-2 text-sm font-semibold text-foreground",
						children: f(Q, { keyName: "footer.contact" })
					}), f("p", {
						className: "text-sm text-muted-foreground",
				...
				children: f(Q, { keyName: "footer.builtWith" })
			})]
		})
	});
}
...
var hn = pn().init(), gn = ({ language: e, staticData: t, children: n }) => {
	let r = d(), a = o(() => ({ [e]: mn(e, t[e] ?? {}) }), [e, t]);
	return i(() => {
		let { unsubscribe: e } = hn.on("permanentChange", () => {
			r.refresh();
		});
		return () => {
			e();
...
function yn({ children: e, locale: t, staticData: n }) {
...
async function Sn(e, t = ["common"]) {
	let n = e || "en", r = await Promise.all(t.map(async (e) => [e, await xn(n, e)]));
	return Object.fromEntries(r);
}
var Cn = "en";
async function wn({ children: e }) {
	let t = await Sn(Cn);
	return f(yn, {
		locale: Cn,
		staticData: t,
