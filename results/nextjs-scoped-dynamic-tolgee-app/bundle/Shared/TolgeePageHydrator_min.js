...
function ye({ options: e }) {
	let t = oe(), n = ge(!1, () => o.isFetching(), t.onFetchingChange.emit), r = ge(!1, () => E.isLoading(), t.onLoadingChange.emit), i = _e(t.onLanguageChange, t.onPendingLanguageChange, t.onRunningChange), a = he(i.getLanguage, i.getInitialOptions, i.getAvailableLanguages, u, _, v, f, t), o = ue(t, a.getBackendRecord, a.getBackendDevRecord, i.withDefaultNs, i.isInitialLoading, n, r);
	e && p(e);
	let s;
	t.onUpdate.listen(() => {
...
var xe = () => {
	...
	return t;
}, Se = 0, L = 1, Ce = 2, we = class extends Error {
	constructor(e, t, n) {
		let r, i = "";
		e === Se ? r = "Empty parameter" : e === L ? (r = "Unexpected character", i = "Did you forget to use FormatIcu to render ICU message syntax?") : r = "Unexpected end", super(`Tolgee parser: ${r} at ${t} in "${n}"` + (i ? "\n" + i : "")), this.code = e, this.index = t;
	}
};
function Te(e) {
	return /\s/.test(e);
}
var R = 0, z = 1, B = 2, V = 3, H = 4, Ee = /* @__PURE__ */ new Set([
	B,
	z,
	R
]), U = "'", De = /* @__PURE__ */ new Set([
	"{",
	"}",
	U
...
function ke(e) {
	...
	for (s = 0; s < e.length; s++) switch (i = e[s], t) {
		case R:
			...
			break;
		case z:
			De.has(i) ? (n = n.slice(0, -1) + i, t = B) : (n += i, t = R);
			break;
		case B:
			i === U ? t = R : (n += i, t = B);
			break;
		case V:
			i === "}" ? (d(), t = R) : Te(i) ? r !== "" && (d(), t = H) : (u(), t = V);
			break;
		case H: i == "}" ? t = R : Te(i) ? t = H : c(L);
	}
...
(_ = console.assert) == null || _.call(console, Ie), RegExp(`([${["‌", "‍"].join("")}]{9})+`, "g");
function Le(e, t) {
	return t ? `${t}: ${e}` : e;
}
...
}, Ke = 3e3, qe = 200, Je = 0, K = /* @__PURE__ */ new Map(), Ye = !1, q, J;
...
var Ze = (e) => new G("unavailable", `the Tolgee browser extension did not answer ${e} in time`), Qe = () => `${Date.now()}-${Je++}-${Math.random()}`;
function $e() {
...
function et(e) {
	return q ||= new Promise((t, n) => {
		let r = () => window.postMessage({ type: Ue }, window.origin), i = () => {
			clearInterval(a), tt(), J = void 0, n(new G("unavailable", "the Tolgee browser extension did not answer"));
		}, a = setInterval(() => {
			if (Date.now() > e) {
				i();
				return;
...
var Y = "__tolgee_", vt = `${Y}apiKey`, yt = `${Y}apiUrl`, bt = `${Y}branch`, xt = `${Y}projectId`, St = `${Y}session`, Ct = "tolgee-in-context-tools.umd.min.js", wt = "@tolgee/in-context-tools", Tt = "InContextTools", Et = "https://cdn.jsdelivr.net/npm", Dt = null;
function Ot(e) {
...
function Pt() {
	...
	if (i && r) return g(h({}, a), { transport: nt() });
}
function Ft() {
	let e = [];
...
var Ht = { useSuspense: !1 }, Ut, Wt = () => (Ut ||= e.createContext(void 0), Ut), X = void 0, Gt = ({ tolgee: r, options: i, children: o, fallback: s, ssr: c }) => {
	n(() => {
		X?.run !== r.run && (X && X.stop(), X = r, r.run().catch((e) => {
			console.error(e);
		}).finally(() => {
			p(!1);
		}));
...
var $ = Kt().init(), qt = ({ language: e, staticData: t, children: r }) => {
	let a = o(), c = i(() => ({ [e]: Q(e, t[e] ?? {}) }), [e, t]);
	return n(() => {
...
function Jt({ locale: e, messages: t, children: n }) {
	return i(() => {
		$.setEmitterActive(!1), $.addStaticData({ [e]: Q(e, t) }), $.setEmitterActive(!0);
	}, [e, t]), n;
}
function Yt() {
	if (!(typeof window > "u")) {
...
function Zt({ children: e, locale: t, staticData: i }) {
...
async function en(e, t = ["common"]) {
	let n = e || "en", r = await Promise.all(t.map(async (e) => [e, await $t(n, e)]));
	return Object.fromEntries(r);
}
var tn = "en";
async function nn({ children: e }) {
	let t = await en(tn);
	return s(Zt, {
		locale: tn,
		staticData: t,
