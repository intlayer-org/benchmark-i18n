...
function At(e, t, n) {
	...
	for (let r of e) switch (r.type) {
		...
		case w.number: {
			let e = T(t.variables, r.value), { scale: n, ...a } = typeof r.style == "string" ? Tt[r.style] ?? {} : r.style?.type === Je.number ? r.style.parsedOptions : {}, o = jt(e, n);
			i(Mt(t, a).format(o));
			break;
		}
		case w.date:
		case w.time: {
			let e = T(t.variables, r.value), n = r.type === w.date ? Et : Ot;
			i(Nt(t, typeof r.style == "string" ? n[r.style] : r.style?.type === Je.dateTime ? r.style.parsedOptions : r.type === w.time ? Ot.medium : void 0).format(e));
			break;
		}
		case w.select: {
			let e = String(T(t.variables, r.value)), n = Rt(r.options, e) ?? r.options.other;
			if (!n) throw zt(r.value, e, r.options);
			At(n.value, t).forEach(i);
			break;
		}
		case w.plural: {
			let e = T(t.variables, r.value), n = `=${String(e)}`, a = Rt(r.options, n), o = typeof e == "bigint" ? e : Number(e), s = typeof o == "bigint" ? o - BigInt(r.offset) : o - r.offset;
			if (!a && Lt(r.options)) {
...
var Ht = RegExp(`^${E}$`);
function Ut(e) {
	return e.type === w.select && Ht.test(e.value) && !!e.options.other && (e.options.other.value.length === 0 || e.options.other.value.length > 0 && e.options.other.value[0]?.type === w.literal);
}
function Wt(e) {
	let t = e;
...
var Sn = (e, t, n) => {
	...
	"Toto",
	"Thai"
]), wn = (e) => e >= "qaa" && e <= "qtz", A = (e, t) => {
	e = k(t, e) || e;
	try {
		let { language: t, region: n, script: r } = D.get("Locale", e), i = 1 + Number(!!n) + Number(!!r);
		return !(e.split("-").length !== i || D.get("DisplayNames", ["en"], { type: "language" }).of(t) === t && !wn(t) || n && D.get("DisplayNames", ["en"], { type: "region" }).of(n) === n || r && D.get("DisplayNames", ["en"], { type: "script" }).of(r) === r && !Cn.has(r));
	} catch {
		return !1;
	}
}, j = (e) => {
...
function Dn(e, t, n, r) {
	if (n && !n.allValid || !A(e, r) || !A(t, r) || En(e, t)) return !1;
	if (!n) return !0;
	let i = yn(t);
...
function Un(e, t = "en", n) {
	...
	try {
		let i = j(e), a = D.get("Locale", e), o = a.language, s = Hn([
			...
			t,
			"en"
		], g = D.get("DisplayNames", m, { type: "language" }), _ = D.get("DisplayNames", h, { type: "language" }), v = s?.name, y = s?.nativeName || s?.name, ee = v || g.of(e) || e, b = y || _.of(e) || e, te = s?.maximizedName || v || g.of(u) || e, x = s?.nativeMaximizedName || y || _.of(u) || e, S = s?.minimizedName || v || g.of(p) || e, ne = s?.nativeMinimizedName || y || _.of(p) || e, re = s?.languageName || v || g.of(o) || e, C = s?.nativeLanguageName || y || _.of(o) || e, ie = s?.nameWithRegionCode || (c ? `${re} (${c})` : ee), ae = s?.nativeNameWithRegionCode || (c ? `${C} (${c})` : b) || ie, oe = D.get("DisplayNames", m, { type: "region" }), se = D.get("DisplayNames", h, { type: "region" }), ce = s?.regionName || (d ? oe.of(d) : "") || "", le = s?.nativeRegionName || (d ? se.of(d) : "") || "", ue = D.get("DisplayNames", m, { type: "script" }), de = D.get("DisplayNames", h, { type: "script" });
		return {
			code: i,
			name: ee,
			nativeName: b,
		...
		o = s?.scriptCode || o;
		let ee = s?.scriptName || "", b = s?.nativeScriptName || "", te = s?.nameWithRegionCode || (v ? `${g} (${v})` : c), x = s?.nativeNameWithRegionCode || (y ? `${_} (${y})` : l), S = s?.emoji || "🌍";
		return {
			code: t,
			name: c,
			nativeName: l,
...
var vr = class {
	...
	3204031479,
	3329325298
]), L = /* @__PURE__ */ new Uint32Array(64), br = class extends vr {
	constructor(e) {
		super(64, e, 8, !1);
	}
	get() {
	...
	process(e, t) {
		...
		for (let e = 0; e < 64; e++) {
			let t = F(o, 6) ^ F(o, 11) ^ F(o, 25), u = l + t + gr(o, s, c) + yr[e] + L[e] | 0, d = (F(n, 2) ^ F(n, 13) ^ F(n, 22)) + _r(n, r, i) | 0;
...
var Jr = class {
	...
	setConfig({ apiKey: e, devApiKey: t, sourceLocale: n, targetLocale: r, locales: i, projectId: a, customMapping: o, baseUrl: s }) {
		if (e && (this.apiKey = e), t && (this.devApiKey = t), a && (this.projectId = a), n && (this.sourceLocale = er(n), !P(this.sourceLocale, o))) throw Error(Pr(this.sourceLocale));
		if (r && (this.targetLocale = er(r), !P(this.targetLocale, o))) throw Error(Pr(this.targetLocale));
		if (i) {
			let e = [], t = [];
			if (i.forEach((n) => {
				let r = er(n);
...
function li() {
	...
	let e = ui(() => "production");
	return e ? e === "development" ? "development" : "production" : ui(() => !1) === !0 ? "development" : "production";
}
function ui(e) {
	try {
		return e();
...
var { getConditionStore: Ci, setConditionStore: wi } = Si(C({
	source: "gt-i18n",
	severity: "Error",
...
function Vi({ type: e, remoteTranslationLoaderParams: t, loadTranslations: n }) {
	...
	switch (e) {
		...
		case "disabled": return r === null ? async () => ({}) : Hi(C({
...
async function Xi({ key: e, sourceValue: t, targetValue: n, translateEntry: r }) {
	...
	let a = U(n) ? n : {}, o = /* @__PURE__ */ new Set([...Object.keys(t), ...Object.keys(a)]), s = await Promise.all(Array.from(o).map(async (n) => {
		...
		Oi(n, i);
		let o = t[n];
		return o === void 0 ? [n, W(a[n])] : [n, await Xi({
			key: i,
			sourceValue: o,
			targetValue: a[n],
			translateEntry: r
...
var ea = 12e3, ta = class {
	constructor(e) {
		...
		this.createTranslateMany = Ri(H().getGTClass(), this.config.runtimeTranslation?.timeout ?? ea, {
...
var { getConditionStore: ga, setConditionStore: _a, isConditionStoreInitialized: va } = Si(C({
	source: "@generaltranslation/react-core",
	severity: "Error",
	...
	function i({ source: i, target: a, locales: o = ["en"], enableI18n: s }) {
		...
		if (Array.isArray(a) && !Array.isArray(i) && i && (i = [i]), Array.isArray(i) && Array.isArray(a)) {
			...
			return a.map((i, a) => {
				...
				if (Wt(i)) return _(e.Fragment, { children: t({
					variableType: i.v || "v",
					variableValue: n[i.k],
...
function Ga(e, { locale: t, id: n }) {
	...
	for (let e of n.split(".")) {
		if (!Ka(e) || !U(i) || !Object.prototype.hasOwnProperty.call(i, e)) return;
		i = i[e];
	}
	return i;
}
...
var Ja = class {
	...
	getTranslateSnapshot = (e, t = {}) => Ha(t, e) ?? q().lookupTranslation(e.locale, e.message, e.options);
	getDictionaryEntrySnapshot = (e, t = {}) => Ua(t, e) ?? q().lookupDictionary(e.locale, e.id);
	getDictionaryObjectSnapshot = (e, t = {}) => Wa(t, e) ?? q().lookupDictionaryObj(e.locale, e.id);
...
function Ya({ i18nConfigParams: e, nextI18nCacheParams: t } = cn()) {
	La({
		...e,
		...t,
...
function uo(e) {
	let t = process.env.NODE_ENV !== "production" && H().isDevHotReloadEnabled(), n = ro(), r = /* @__PURE__ */ new Map();
	return l(() => {
...
function Do(e) {
	let t = ro(), n = io(), r = lo(), i = h((n) => t.subscribeToTranslate(e, n), () => t.getTranslateSnapshot(e, n), () => t.getTranslateSnapshot(e, n));
	...
	let r = n, i = (e) => {
		...
		if (a) {
			...
			if ((e[1] === "automatic" || e[2] === "automatic") && (i.injectionType = "automatic"), e[0] === "translate" && (e[0] = "fragment"), e[0] === "variable" && (i.variableType = e?.[1] || "variable"), e[0] === "plural") {
				let e = Object.entries(n).reduce((e, [t, n]) => (Qt(t) && (e[t] = Mo(n, r)), e), {});
...
function Po(e, t) {
	...
	if (a) {
		...
		if (n === "plural") return r(e, { ...Object.entries(i).reduce((e, [n, r]) => (e[n] = Qt(n) || n === "children" ? Fo(r, t) : r, e), {}) });
...
var Bo = (e) => {
	...
	if (t["data-_gt"]) {
		...
		if (r === "variable") {
			let n = e.variableType || "variable", r = xa(t, n), i = tn(n);
			return {
				i: e.id,
				k: r,
				v: i
...
function Qo({ children: e, _locale: t, _enableI18n: n, _renderPreparedT: r = jo, ...i }) {
	...
	if (process.env.NODE_ENV !== "production" && H().isDevHotReloadEnabled() && f == null && m.current != null && d) return m.current;
	let h = r({
...
function rs(e) {
	let t = J(), n = ts(t.getLocaleCookieName());
...
function vs() {
	...
	return v("section", {
		...
		children: [_("h2", {
			...
			children: _(Zo, { children: "Profile" })
		}), v("div", {
			className: "space-y-4",
			children: [v("div", { children: [_("label", {
				htmlFor: e,
				className: "mb-1 block text-sm font-medium text-foreground",
				children: _(Zo, { children: "Display Name" })
			}), _("input", {
				id: e,
				defaultValue: "John Developer",
				className: "w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-ring"
			})] }), v("div", { children: [_("label", {
				htmlFor: t,
				className: "mb-1 block text-sm font-medium text-foreground",
				children: _(Zo, { children: "Email" })
			}), _("input", {
				id: t,
