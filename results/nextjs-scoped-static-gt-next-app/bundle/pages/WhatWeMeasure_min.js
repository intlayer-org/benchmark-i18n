...
function g() {
	return a("section", {
		...
		children: [i("h2", {
			...
			children: i(h.T, { children: "What We Measure" })
		}), i("ul", {
			className: "space-y-4",
			children: [
				{
					metric: i(h.T, { children: "Bundle size impact" }),
					desc: i(h.T, { children: "The additional JavaScript bytes sent to users when the i18n library and its translation files are included. This directly affects download time on slow networks." })
				},
				{
					metric: i(h.T, { children: "Rendering overhead" }),
					desc: i(h.T, { children: "How much extra time the library adds to React's render cycle. Libraries that inject translations via a single context provider can cause unnecessary re-renders across the component tree." })
				},
				{
					metric: i(h.T, { children: "Hydration cost" }),
					desc: i(h.T, { children: "During SSR, translation data is serialized into HTML. Large dictionaries increase the HTML payload and slow down hydration — the moment the page becomes interactive." })
				},
				{
					metric: i(h.T, { children: "Lazy loading effectiveness" }),
					desc: i(h.T, { children: "Whether splitting translations by route or namespace actually reduces the initial load, and what trade-offs it introduces (waterfall requests, FOUC, cache complexity)." })
				},
				{
					metric: i(h.T, { children: "Locale switch speed" }),
					desc: i(h.T, { children: "How fast the app can switch from one language to another at runtime — including fetching new translations, re-rendering components, and updating the DOM." })
				}
			].map((e) => a("li", {
