...
function ae(e, t) {
	return y(e, this, ne, t.cache.create(), t.serializer);
}
var oe = function() {
...
var T = /^\.(?:(0+)(\*)?|(#+)|(0+)(#+))$/g, E = /^(@+)?(\+|#+)?[rs]?$/g, ge = /(\*)(0+)|(#+)(0+)|(0+)/g, _e = /^(0+)$/;
function ve(e) {
	let t = {};
...
function Oe(e, t) {
	...
	for (let r = 0; r < e.length; r++) {
		...
		if (i === "j") {
			...
			let o = 1 + (a & 1), s = a < 2 ? 1 : 3 + (a >> 1), c = ke(t);
...
var Ae = RegExp(`^${I.source}*`), je = RegExp(`${I.source}*$`);
function R(e, t) {
	return {
		start: e,
		end: t
...
var Me = !!Object.fromEntries, Ne = !!String.prototype.trimStart, Pe = !!String.prototype.trimEnd, z = Me ? Object.fromEntries : function(e) {
	let t = {};
...
var Re = class {
	...
	parseTag(e, t) {
		...
		if (this.bumpIf(">")) {
			...
			if (this.bumpIf("</")) {
				if (this.isEOF() || !V(this.char())) return this.error(O.INVALID_TAG, R(o, this.clonePosition()));
				let e = this.clonePosition();
				return r === this.parseTagName() ? (this.bumpSpace(), this.bumpIf(">") ? {
					...
					err: null
				} : this.error(O.INVALID_TAG, R(o, this.clonePosition()))) : this.error(O.UNMATCHED_CLOSING_TAG, R(e, this.clonePosition()));
			}
	...
	parseArgument(e, t) {
		let n = this.clonePosition();
		if (this.bump(), this.bumpSpace(), this.isEOF()) return this.error(O.EXPECT_ARGUMENT_CLOSING_BRACE, R(n, this.clonePosition()));
		if (this.char() === 125) return this.bump(), this.error(O.EMPTY_ARGUMENT, R(n, this.clonePosition()));
		let r = this.parseIdentifierIfPossible().value;
		if (!r) return this.error(O.MALFORMED_ARGUMENT, R(n, this.clonePosition()));
		if (this.bumpSpace(), this.isEOF()) return this.error(O.EXPECT_ARGUMENT_CLOSING_BRACE, R(n, this.clonePosition()));
		switch (this.char()) {
			...
			case 44: return this.bump(), this.bumpSpace(), this.isEOF() ? this.error(O.EXPECT_ARGUMENT_CLOSING_BRACE, R(n, this.clonePosition())) : this.parseArgumentOptions(e, t, r, n);
			default: return this.error(O.MALFORMED_ARGUMENT, R(n, this.clonePosition()));
		}
	}
	...
	parseArgumentOptions(e, t, n, r) {
		...
		switch (a) {
			case "": return this.error(O.EXPECT_ARGUMENT_TYPE, R(i, o));
			case "number":
			case "date":
			case "time": {
				this.bumpSpace();
				...
				if (this.bumpIf(",")) {
					...
					if (n.err) return n;
					let r = Ie(n.val);
					if (r.length === 0) return this.error(O.EXPECT_ARGUMENT_STYLE, R(this.clonePosition(), this.clonePosition()));
					e = {
						style: r,
				...
				if (e && e.style.startsWith("::")) {
					...
					}
					{
						...
						let r = t;
						this.locale && (r = Oe(t, this.locale));
						let o = {
							type: A.dateTime,
							pattern: r,
			...
			case "select": {
				...
				if (this.bumpSpace(), !this.bumpIf(",")) return this.error(O.EXPECT_SELECT_ARGUMENT_OPTIONS, R(i, { ...i }));
				this.bumpSpace();
				let o = this.parseIdentifierIfPossible(), s = 0;
				if (a !== "select" && o.value === "offset") {
					if (!this.bumpIf(":")) return this.error(O.EXPECT_PLURAL_ARGUMENT_OFFSET_VALUE, R(this.clonePosition(), this.clonePosition()));
					this.bumpSpace();
			...
			default: return this.error(O.INVALID_ARGUMENT_TYPE, R(i, o));
		}
	}
	tryParseArgumentClose(e) {
		return this.isEOF() || this.char() !== 125 ? this.error(O.EXPECT_ARGUMENT_CLOSING_BRACE, R(e, this.clonePosition())) : (this.bump(), {
			val: !0,
			err: null
		});
	}
	parseSimpleArgStyleIfPossible() {
		...
		for (; !this.isEOF();) switch (this.char()) {
			case 39: {
				this.bump();
				let e = this.clonePosition();
				if (!this.bumpUntil("'")) return this.error(O.UNCLOSED_QUOTE_IN_ARGUMENT_STYLE, R(e, this.clonePosition()));
				this.bump();
				break;
			}
			case 123:
	...
	tryParsePluralOrSelectOptions(e, t, n, r) {
		...
		for (;;) {
			...
			if (!this.bumpIf("{")) return this.error(t === "select" ? O.EXPECT_SELECT_ARGUMENT_SELECTOR_FRAGMENT : O.EXPECT_PLURAL_ARGUMENT_SELECTOR_FRAGMENT, R(this.clonePosition(), this.clonePosition()));
		...
		return a.length === 0 ? this.error(t === "select" ? O.EXPECT_SELECT_ARGUMENT_SELECTOR : O.EXPECT_PLURAL_ARGUMENT_SELECTOR, R(this.clonePosition(), this.clonePosition())) : this.requiresOtherClause && !i ? this.error(O.MISSING_OTHER_CLAUSE, R(this.clonePosition(), this.clonePosition())) : {
			val: a,
			err: null
		};
	}
...
function H(e) {
	e.forEach((e) => {
		if (delete e.location, Ce(e) || we(e)) for (let t in e.options) delete e.options[t].location, H(e.options[t].value);
		else M(e) && De(e.style) || (N(e) || P(e)) && F(e.style) ? delete e.style.location : Ee(e) && H(e.children);
	});
}
function He(e, t = {}) {
	t = {
...
function q(e, t, n, r, i, a, o) {
	...
	for (let c of e) {
		...
		if (Ee(c)) {
			...
			if (!Ke(u)) throw new Ue(l, "function", o);
...
function $e(t) {
	...
	return function({ cache: t, formats: r, formatters: i, getMessageFallback: a = Ze, locale: o, messagesOrError: s, namespace: c, onError: l, timeZone: u }) {
		...
		function p(p, m, ee, h) {
			...
			if (d) {
				...
				try {
					...
					if (!g) return f(p, S.MISSING_MESSAGE, e.message, g);
					_ = g;
				}
			}
			try {
				return Xe(X(c, p), _, m && function(t) {
					let r = {};
				...
				let t, n;
				return e instanceof x ? (t = e.code, n = e.originalMessage) : (t = S.FORMATTING_ERROR, n = e.message), f(p, t, n, g);
			}
		}
		function m(e, t, n, r) {
			let i = p(e, t, n, r);
...
function nt({ formats: e, getMessageFallback: t, messages: n, onError: r, ...i }) {
	return {
		...
		messages: n || void 0,
		onError: r || Qe,
		getMessageFallback: t || Ze
	};
}
var Q = t(void 0);
function rt({ children: e, formats: t, getMessageFallback: n, locale: i, messages: a, now: s, onError: c, timeZone: l }) {
	let u = r(Q), d = o((() => u?.cache || le()), [i, u?.cache]), p = o((() => u?.formatters || ue(d)), [d, u?.formatters]), m = o((() => ({
		...nt({
			locale: i,
...
function st(e) {
	return function(e, t, n) {
		let { cache: r, formats: i, formatters: a, getMessageFallback: s, locale: c, onError: l, timeZone: u } = it(), d = e[n], f = et(t, n);
		return u || at || !ot || (at = !0, l(new x(S.ENVIRONMENT_FALLBACK, void 0))), o((() => $e({
			cache: r,
			formatters: a,
