...
var De = !!Object.fromEntries, Oe = !!String.prototype.trimStart, ke = !!String.prototype.trimEnd, Ae = De ? Object.fromEntries : function(e) {
	...
	return e.replace(Ee, "");
}, Ne = /* @__PURE__ */ RegExp("([^\\p{White_Space}\\p{Pattern_Syntax}]*)", "yu");
function Pe(e, t) {
	return Ne.lastIndex = t, Ne.exec(e)[1] ?? "";
}
var Fe = class {
	constructor(e, t = {}) {
	...
	parseMessage(e, t, n) {
		...
		for (; !this.isEOF();) {
			...
			if (i === 123) {
				...
				if (n) break;
				return this.error(T.UNMATCHED_CLOSING_TAG, L(this.clonePosition(), this.clonePosition()));
	...
	parseTag(e, t) {
		...
		if (this.bumpIf(">")) {
			...
			if (this.bumpIf("</")) {
				if (this.isEOF() || !R(this.char())) return this.error(T.INVALID_TAG, L(o, this.clonePosition()));
				let e = this.clonePosition();
				return r === this.parseTagName() ? (this.bumpSpace(), this.bumpIf(">") ? {
					...
					err: null
				} : this.error(T.INVALID_TAG, L(o, this.clonePosition()))) : this.error(T.UNMATCHED_CLOSING_TAG, L(e, this.clonePosition()));
			}
			return this.error(T.UNCLOSED_TAG, L(n, this.clonePosition()));
		}
		return this.error(T.INVALID_TAG, L(n, this.clonePosition()));
	}
	parseTagName() {
		let e = this.offset();
	...
	parseArgument(e, t) {
		let n = this.clonePosition();
		if (this.bump(), this.bumpSpace(), this.isEOF()) return this.error(T.EXPECT_ARGUMENT_CLOSING_BRACE, L(n, this.clonePosition()));
		if (this.char() === 125) return this.bump(), this.error(T.EMPTY_ARGUMENT, L(n, this.clonePosition()));
		let r = this.parseIdentifierIfPossible().value;
		if (!r) return this.error(T.MALFORMED_ARGUMENT, L(n, this.clonePosition()));
		if (this.bumpSpace(), this.isEOF()) return this.error(T.EXPECT_ARGUMENT_CLOSING_BRACE, L(n, this.clonePosition()));
		switch (this.char()) {
			...
			case 44: return this.bump(), this.bumpSpace(), this.isEOF() ? this.error(T.EXPECT_ARGUMENT_CLOSING_BRACE, L(n, this.clonePosition())) : this.parseArgumentOptions(e, t, r, n);
			default: return this.error(T.MALFORMED_ARGUMENT, L(n, this.clonePosition()));
		}
	}
	...
	parseArgumentOptions(e, t, n, r) {
		...
		switch (a) {
			case "": return this.error(T.EXPECT_ARGUMENT_TYPE, L(i, o));
			case "number":
			case "date":
			case "time": {
				this.bumpSpace();
				...
				if (this.bumpIf(",")) {
					...
					if (n.err) return n;
					let r = Me(n.val);
					if (r.length === 0) return this.error(T.EXPECT_ARGUMENT_STYLE, L(this.clonePosition(), this.clonePosition()));
					e = {
						style: r,
			...
			case "select": {
				...
				if (this.bumpSpace(), !this.bumpIf(",")) return this.error(T.EXPECT_SELECT_ARGUMENT_OPTIONS, L(i, { ...i }));
				this.bumpSpace();
				let o = this.parseIdentifierIfPossible(), s = 0;
				if (a !== "select" && o.value === "offset") {
					if (!this.bumpIf(":")) return this.error(T.EXPECT_PLURAL_ARGUMENT_OFFSET_VALUE, L(this.clonePosition(), this.clonePosition()));
					this.bumpSpace();
			...
			default: return this.error(T.INVALID_ARGUMENT_TYPE, L(i, o));
		}
	}
	tryParseArgumentClose(e) {
		return this.isEOF() || this.char() !== 125 ? this.error(T.EXPECT_ARGUMENT_CLOSING_BRACE, L(e, this.clonePosition())) : (this.bump(), {
			val: !0,
			err: null
		});
	}
	parseSimpleArgStyleIfPossible() {
		...
		for (; !this.isEOF();) switch (this.char()) {
			case 39: {
				this.bump();
				let e = this.clonePosition();
				if (!this.bumpUntil("'")) return this.error(T.UNCLOSED_QUOTE_IN_ARGUMENT_STYLE, L(e, this.clonePosition()));
				this.bump();
				break;
			}
			case 123:
	...
	tryParsePluralOrSelectOptions(e, t, n, r) {
		...
		for (;;) {
			...
			if (!this.bumpIf("{")) return this.error(t === "select" ? T.EXPECT_SELECT_ARGUMENT_SELECTOR_FRAGMENT : T.EXPECT_PLURAL_ARGUMENT_SELECTOR_FRAGMENT, L(this.clonePosition(), this.clonePosition()));
		...
		return a.length === 0 ? this.error(t === "select" ? T.EXPECT_SELECT_ARGUMENT_SELECTOR : T.EXPECT_PLURAL_ARGUMENT_SELECTOR, L(this.clonePosition(), this.clonePosition())) : this.requiresOtherClause && !i ? this.error(T.MISSING_OTHER_CLAUSE, L(this.clonePosition(), this.clonePosition())) : {
			val: a,
			err: null
		};
	}
...
function z(e) {
	e.forEach((e) => {
		if (delete e.location, xe(e) || j(e)) for (let t in e.options) delete e.options[t].location, z(e.options[t].value);
		else k(e) && N(e.style) || (A(e) || be(e)) && P(e.style) ? delete e.style.location : M(e) && z(e.children);
	});
}
function ze(e, t = {}) {
	t = {
...
var Z = t(void 0);
function ot({ children: e, formats: t, getMessageFallback: n, locale: i, messages: a, now: s, onError: l, timeZone: u }) {
	let d = r(Z), f = o(() => d?.cache || ae(), [i, d?.cache]), p = o(() => d?.formatters || se(f), [f, d?.formatters]), m = o(() => ({
		...at({
			locale: i,
...
var ct = !1, lt = typeof window > "u";
function ut(e, t, n) {
	let { cache: r, formats: i, formatters: a, getMessageFallback: s, locale: c, onError: l, timeZone: u } = st(), d = e[n], f = nt(t, n);
	return !u && !ct && lt && (ct = !0, l(new v(y.ENVIRONMENT_FALLBACK, "There is no `timeZone` configured, this can lead to markup mismatches caused by environment differences. Consider adding a global default: https://next-intl.dev/docs/configuration#time-zone"))), o(() => et({
		cache: r,
		formatters: a,
...
var mt = "/Users/aymericpineau/Documents/benchmark-bloom/apps-benchmark/nextjs-scoped-static/next-intl-app/components/MockBanner.tsx", ht = () => {
	let e = Q();
	return u("div", {
...
}, $ = "/Users/aymericpineau/Documents/benchmark-bloom/apps-benchmark/nextjs-scoped-static/next-intl-app/components/pages/blog/BlogHeader.tsx";
function gt() {
	let e = Q(), t = Q("blog-header");
	return u(l, { children: [
		u(ht, {}, void 0, !1, {
			fileName: $,
			lineNumber: 11,
...
var yt = "/Users/aymericpineau/Documents/benchmark-bloom/apps-benchmark/nextjs-scoped-static/next-intl-app/components/AppProviders.tsx";
function bt({ children: e, locale: t, messages: n }) {
...
function Lr(e, t) {
	...
	for (let [e, r] of Object.entries(t)) {
		let t = n[e];
		n[e] = typeof r == "object" && r && !Array.isArray(r) && typeof t == "object" && t && !Array.isArray(t) ? Lr(t, r) : r;
	}
	return n;
}
function Rr(e) {
	return e.reduce((e, t) => Lr(e, t), {});
}
function zr(e, t) {
	let n = Fr[e] ?? Fr.en, r = new Set(t);
	r.add("shared");
	let i = [];
	for (let e of r) {
		let t = n[e];
...
var Vr = Br("en"), Hr = "/Users/aymericpineau/Documents/benchmark-bloom/apps-benchmark/nextjs-scoped-static/next-intl-app/scripts/Wrapper.tsx", Ur = "en";
...
var Gr = "/Users/aymericpineau/Documents/benchmark-bloom/apps-benchmark/nextjs-scoped-static/next-intl-app/components/pages/blog/BlogHeader.wrapper.tsx";
function Kr() {
	return u(Wr, { children: u(gt, {}, void 0, !1, {
		fileName: Gr,
		lineNumber: 9,
		columnNumber: 11
	}, this) }, void 0, !1, {
