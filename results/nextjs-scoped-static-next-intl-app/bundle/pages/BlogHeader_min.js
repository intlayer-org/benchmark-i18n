...
function Sn(e, t) {
	...
	for (let r = 0; r < e.length; r++) {
		...
		if (i === "j") {
			...
			let o = 1 + (a & 1), s = a < 2 ? 1 : 3 + (a >> 1), c = Cn(t);
...
var wn = RegExp(`^${I.source}*`), Tn = RegExp(`${I.source}*$`);
function R(e, t) {
	return {
		start: e,
		end: t
...
var En = !!Object.fromEntries, Dn = !!String.prototype.trimStart, On = !!String.prototype.trimEnd, z = En ? Object.fromEntries : function(e) {
	let t = {};
...
var Mn = class {
	...
	parseTagName() {
		let e = this.offset();
		for (this.bump(); !this.isEOF() && Pn(this.char());) this.bump();
	...
	tryParseLeftAngleBracket() {
		return !this.isEOF() && this.char() === 60 && (this.ignoreTag || !Nn(this.peek() || 0)) ? (this.bump(), "<") : null;
	}
	tryParseQuote(e) {
	...
	parseIdentifierIfPossible() {
		let e = this.clonePosition(), t = this.offset(), n = jn(this.message, t), r = t + n.length;
			...
			case "time": {
				...
				if (e && e.style.startsWith("::")) {
					...
					if (a === "number") {
						...
						let r = t;
						this.locale && (r = Sn(t, this.locale));
						let o = {
							type: A.dateTime,
							pattern: r,
	...
	bumpSpace() {
		for (; !this.isEOF() && Fn(this.char());) this.bump();
	}
	peek() {
		if (this.isEOF()) return null;
...
function H(e) {
	e.forEach((e) => {
		if (delete e.location, _n(e) || vn(e)) for (let t in e.options) delete e.options[t].location, H(e.options[t].value);
		else M(e) && xn(e.style) || (N(e) || gn(e)) && P(e.style) ? delete e.style.location : bn(e) && H(e.children);
	});
}
function In(e, t = {}) {
	t = {
...
function K(e, t, n, r, i, a, o) {
	...
	for (let c of e) {
		...
		if (M(c)) {
			let e = typeof c.style == "string" ? r.number[c.style] : xn(c.style) ? c.style.parsedOptions : void 0;
			if (e && e.scale) {
				let t = e.scale || 1;
				if (typeof l == "bigint") {
		...
		if (bn(c)) {
			...
			if (!Vn(u)) throw new Rn(l, "function", o);
		...
		if (_n(c)) {
			...
			if (!a) throw new Ln(c.value, l, Object.keys(c.options), o);
...
function Un(e, t) {
	return t ? Object.keys(e).reduce((n, r) => (n[r] = Hn(e[r], t[r]), n), { ...e }) : e;
}
function q(e) {
	return { create() {
		return {
...
var J = class e {
	...
	constructor(t, n = e.defaultLocale, r, i) {
		...
		this.formats = Un(e.formats, r), this.formatters = i && i.formatters || Wn(this.formatterCache);
	}
	format = (e) => {
...
function qn(t) {
	...
	return function({ cache: t, formats: r, formatters: i, getMessageFallback: a = X, locale: o, messagesOrError: s, namespace: c, onError: l, timeZone: u }) {
		...
		function p(p, m, h, g) {
			...
			try {
				return Gn(Y(c, p), v, m && function(t) {
					let r = {};
...
var Yn = 86400;
7 * Yn, 365 * Yn;
function Xn({ formats: e, getMessageFallback: t, messages: n, onError: r, ...i }) {
	return {
		...i,
		formats: e || void 0,
		messages: n || void 0,
...
var Q = t(void 0);
function Zn({ children: e, formats: t, getMessageFallback: n, locale: i, messages: a, now: s, onError: c, timeZone: u }) {
	let d = r(Q), f = o((() => d?.cache || $t()), [i, d?.cache]), p = o((() => d?.formatters || en(f)), [f, d?.formatters]), m = o((() => ({
		...Xn({
			locale: i,
...
var $n = !1, er = typeof window > "u";
function tr(e) {
	return function(e, t, n) {
		let { cache: r, formats: i, formatters: a, getMessageFallback: s, locale: c, onError: l, timeZone: u } = Qn(), d = e[n], f = Jn(t, n);
		return u || $n || !er || ($n = !0, l(new w(T.ENVIRONMENT_FALLBACK, void 0))), o((() => qn({
			cache: r,
			formatters: a,
