...
function hn() {
	...
	i(() => {
		...
		if (t !== "auto") return;
		let e = window.matchMedia("(prefers-color-scheme: dark)"), n = () => mn("auto");
	...
	function r() {
		...
		n(e), mn(e), window.localStorage.setItem("theme", e);
	}
...
function pu() {
	return Yt().use(He()).updateDefaults({
		apiKey: process.env.NEXT_PUBLIC_TOLGEE_API_KEY,
		apiUrl: process.env.NEXT_PUBLIC_TOLGEE_API_URL,
		staticData: Object.fromEntries(fu.map((e) => [e, () => Promise.resolve(du(e))]))
	});
}
var mu = pu().init(), hu = ({ language: e, staticData: t, children: n }) => {
	let r = f();
	return i(() => {
		let { unsubscribe: e } = mu.on("permanentChange", () => {
			r.refresh();
		});
		return () => {
			e();
...
function vu({ children: e, locale: t, staticData: n }) {
