...
function Zl() {
	return Ut().use(Le()).updateDefaults({
		apiKey: process.env.NEXT_PUBLIC_TOLGEE_API_KEY,
		apiUrl: process.env.NEXT_PUBLIC_TOLGEE_API_URL,
		staticData: Object.fromEntries(Xl.map((e) => [e, () => Promise.resolve(Yl(e))]))
	});
}
var Ql = Zl().init(), $l = ({ language: e, staticData: t, children: n }) => {
	let r = d();
	return i(() => {
		let { unsubscribe: e } = Ql.on("permanentChange", () => {
			r.refresh();
		});
		return () => {
			e();
...
function nu({ children: e, locale: t, staticData: n }) {
