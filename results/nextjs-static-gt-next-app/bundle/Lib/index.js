...
function getDictionaryListenerKey({ locale, id }) {
	return `${locale}:${id}`;
}
function n$3() {
	return getI18nCache();
...
var l$3 = class extends I18nConfig {
	...
	constructor(r = {}, i = s$1) {
		super(r), p$2(i), Object.defineProperty(this, c$6, { value: !0 }), this.renderStrategy = i, this.localeCookieName = r.localeCookieName ?? "generaltranslation.locale", this.regionCookieName = r.regionCookieName ?? "generaltranslation.region", this.enableI18nCookieName = r.enableI18nCookieName ?? "generaltranslation.enable-i18n";
	}
	getRenderStrategy() {
...
var { getConditionStore: n$2, setConditionStore: r$7, isConditionStoreInitialized: i$6 } = createConditionStoreSingleton(createDiagnosticMessage$1({
	source: `@generaltranslation/react-core`,
	severity: `Error`,
...
function r$6(r) {
	f$3(r, `server-render`), r$8(new t$1(r));
}
...
function n$1(t, n) {
	return getDictionaryEntry(i$4(t, n));
}
function r$4(e, t) {
	return i$4(e, t);
}
function i$4(e, { locale: n, id: r }) {
	let i = e?.[n];
	if (!i) return;
	if (!r) return i;
	let o = i;
...
var l$2 = class {
	...
	updateTranslations = (t) => {
		n$3().updateTranslations(t);
	};
	updateDictionaries = (t) => {
		n$3().updateDictionaries(t);
	};
	translate = async (t) => n$3().lookupTranslationWithFallback(t.locale, t.message, t.options).then(() => {
		this.emitTranslateEvent(t);
	}).catch((e) => this.logRuntimeTranslationError(e));
	translateDictionaryEntry = (t) => {
		n$3().lookupDictionaryWithFallback(t.locale, t.id).then(() => {
			this.emitDictionaryEvent(t);
		}).catch((e) => this.logRuntimeTranslationError(e));
	};
	translateDictionaryObject = (t) => {
		n$3().lookupDictionaryObjWithFallback(t.locale, t.id).then(() => {
			this.emitDictionaryEvent(t);
		}).catch((e) => this.logRuntimeTranslationError(e));
	};
	...
	getTranslateSnapshot = (t$2, r = {}) => t(r, t$2) ?? n$3().lookupTranslation(t$2.locale, t$2.message, t$2.options);
	getDictionaryEntrySnapshot = (t, n = {}) => n$1(n, t) ?? n$3().lookupDictionary(t.locale, t.id);
	getDictionaryObjectSnapshot = (t, n = {}) => r$4(n, t) ?? n$3().lookupDictionaryObj(t.locale, t.id);
...
function l$1() {
	let e = r$3(), n = e.getLocale();
	return {
		locale: n,
		shouldTranslate: e.getEnableI18n() && getI18nConfig().requiresTranslation(n)
	};
}
function n() {
	return s()?.i18nStore || i$5();
}
function r$1() {
	return s()?.translationsSnapshot || {};
}
function i$2() {
	return s()?.dictionariesSnapshot || {};
}
var c$2 = () => {};
var l = () => {};
var u = () => {};
...
function p$1(t) {
	let n = s()?.onMissingTranslation, r = S(t);
...
function h() {
	let n = s()?.onMissingDictionaryEntry, i = S(c$3());
	return useCallback((e) => {
		n ? n(e) : i(getDictionaryListenerKey(e), {
...
function _() {
	let n = s()?.onMissingDictionaryObj, i = S(c$3());
	return useCallback((e) => {
		n ? n(e) : i(getDictionaryListenerKey(e), {
...
var y = process.env.NODE_ENV === `production` ? d$1 : p$1;
var b = process.env.NODE_ENV === `production` ? m$1 : h;
var x = process.env.NODE_ENV === `production` ? g : _;
function S(e) {
	let t = process.env.NODE_ENV !== `production` && getI18nConfig().isDevHotReloadEnabled(), r = n(), a = /* @__PURE__ */ new Map();
	return useEffect(() => {
...
function r(r, i, a) {
	let o = useRef(0), s = useCallback((e) => i((t) => {
		r.current.has(a(t)) && (o.current++, e());
	}), [
		i,
		a,
		r
	]), c = useCallback(() => o.current, []);
	useSyncExternalStore(s, c, c);
}
var i$1 = process.env.NODE_ENV === `production` ? () => {} : r;
function d(i = [], c, l) {
	let d = r$1(), p = n(), m = process.env.NODE_ENV !== `production` && getI18nConfig().isDevHotReloadEnabled(), h = y(l), g = useRef(null);
	return g.current ??= /* @__PURE__ */ new Set(), f$1(i, g, c, l), i$1(g, p.subscribeToTranslationEvents, getTranslateListenerKey), useCallback((e) => {
		let t = getTranslateListenerKey(e);
		m && g.current.add(t);
		let n = p.getTranslateSnapshot(e, d);
		return n == null && m && h(e), n;
	}, [
		p,
		d,
		h,
...
function f$1(n$4, r, s, u) {
	let d = n(), f = process.env.NODE_ENV !== `production` && getI18nConfig().isDevHotReloadEnabled(), p = r$1(), m = useMemo(() => n$4?.length > 0 && u && f, [
		n$4,
		u,
		f
	]), h = useMemo(() => m ? n$4.map(({ message: e, ...t }) => {
		let n = t.$locale ?? s, r = {
			locale: n,
			message: e,
			options: createLookupOptions(n, t, `ICU`)
		};
		return [r, getTranslateListenerKey(r)];
	}) : [], [
		s,
		m,
		n$4
	...
	m && h.forEach(([, e]) => {
		...
		m && h.filter(([e]) => d.getTranslateSnapshot(e, p) == null).forEach(([e]) => d.translate(e));
	}, [
		m,
		d,
		h,
...
function o(o) {
	let { locale: s, shouldTranslate: c } = l$1(), l = r$2(), u = d(o, s, c);
	return useCallback((e, t = {}) => {
		let n = createLookupOptions(t.$locale ?? s, t, `ICU`);
		return interpolateMessage(c ? {
			source: e,
			target: u({
				locale: n.$locale,
				message: e,
...
function c$1() {
	let c = i$2(), l = n(), u = process.env.NODE_ENV !== `production` && getI18nConfig().isDevHotReloadEnabled(), d = b(), f = useRef(null);
	return f.current ??= /* @__PURE__ */ new Set(), i$1(f, l.subscribeToDictionaryEntryEvents, getDictionaryListenerKey), useCallback((e) => {
		let t = getDictionaryListenerKey(e);
		u && f.current.add(t);
		let n = l.getDictionaryEntrySnapshot(e, c);
		return n == null && u && d(e), n;
	}, [
		l,
		c,
		u,
...
function c() {
	let c = i$2(), l = n(), u = process.env.NODE_ENV !== `production` && getI18nConfig().isDevHotReloadEnabled(), d = x(), f = useRef(null);
	return f.current ??= /* @__PURE__ */ new Set(), i$1(f, l.subscribeToDictionaryObjectEvents, getDictionaryListenerKey), useCallback((e) => {
		let t = getDictionaryListenerKey(e);
		u && f.current.add(t);
		let n = l.getDictionaryObjectSnapshot(e, c);
		return n == null && u && d(e), n;
	}, [
		l,
		c,
...
function f(a) {
	let c = i$3(), f = r$2(), h = c$3(), g = o(), _ = c$1(), v = p(a), y = useCallback((e, t = {}) => {
		let n = m(a, e), r = _({
			locale: f,
			id: n
		});
		if (r === void 0) throw Error(`Dictionary entry ${n} cannot be found`);
		let i = resolveDictionaryLookupOptions(r.options);
		if (!h) return g(r.entry, {
			...i,
			...extractVariables(t),
			$locale: f
		});
	...
	return useMemo(() => Object.assign(y, { obj: v }), [y, v]);
}
function p(i) {
	let o$4 = i$3(), s = r$2(), l = c$3(), d = o(), f = c();
	return useCallback((e) => {
		let t = m(i, e), n = f({
			locale: s,
			id: t
		});
		if (n === void 0) throw Error(`Dictionary entry ${t} cannot be found`);
		let r;
		return l && (r = f({
			locale: o$4,
			id: t
...
initializeGTClient();
var _jsxFileName$3 = "/Users/aymericpineau/Documents/benchmark-bloom/apps-benchmark/nextjs-static/gt-next-app/scripts/EmptyComponent.tsx";
var TestComponent = () => {
	f();
	return null;
};
function EmptyComponent() {
	return jsxDEV(BrowserGTProvider, { children: jsxDEV(TestComponent, {}, void 0, false, {
...
var _jsxFileName = "/Users/aymericpineau/Documents/benchmark-bloom/apps-benchmark/nextjs-static/gt-next-app/scripts/EmptyComponent.wrapper.tsx";
function Wrapped() {
