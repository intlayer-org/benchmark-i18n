...
var Jt = ({ locale: e, setLocale: t, availableLocales: n, namespace: r, keyPrefix: i, dictionaryContent: a }) => {
	...
	let { locales: l, defaultLocale: u } = g ?? {}, [d, p] = c(e ?? Yt ?? t ?? u);
	i(() => {
		e && e !== d && p(e);
	}, [e]), i(() => {
		tn();
...
}, cn = ({ children: e, i18n: t }) => (t !== void 0 && w({ log: v })(`${T("I18nextProvider", b)}: the \`i18n\` prop has no effect with intlayer. Intlayer manages its own i18n instance — you can safely remove the prop.`), f(rn, { children: e }));
...
function mn({ children: e }) {
	...
	return a(() => {
		...
		i18n: fn,
		children: [f(pn, { locale: t }), e]
	});
}
function hn({ children: e }) {
	return f(mn, { children: e });
}
function gn() {
	return f(hn, { children: f(ee, {}) });
}
