...
function x() {
	...
	function i() {
		...
		r(e), b(e), window.localStorage.setItem("theme", e);
	}
...
`)), R(s) && J.test(s) ? Y(s) : R(s) ? s : pe(s, this._locale, this._locales)(t, n?.formats);
	}
	t = this._.bind(this);
	date(e, t) {
...
function _e({ children: e }) {
	let t = s().locale ?? "en", c = i(() => ge(t), [t]), l = i(() => $(t, c), [t, c]), [u] = a(() => typeof performance < "u" ? performance.now() : 0);
	return r(() => {
		C("AppRoot", u);
	}, [u]), n(() => {
...
function ve({ children: e }) {
	return o(_e, { children: e });
}
function ye() {
	return o(ve, { children: o(x, {}) });
}
