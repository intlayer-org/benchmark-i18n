...
function I18nProvider({ children, language, resources, supportedLngs, defaultNS = "common", fallbackLng, localePath = "/locales", localeStructure = "{{lng}}/{{ns}}", localeExtension = "json", use = [], i18nextOptions = {}, ssrBackend = false }) {
	const [instance] = useState(() => {
		const inst = createInstance();
		inst.use(initReactI18next);
		const isBrowser = typeof window !== "undefined";
		const applyBackends = isBrowser || ssrBackend;
		const plugins = applyBackends ? use : use.filter((p) => p.type !== "backend");
		const userHasBackend = plugins.some((b) => b.type === "backend");
		...
		if (isBrowser && !userHasBackend) inst.use(resourcesToBackend((lng, ns) => {
		...
		plugins.forEach((plugin) => inst.use(plugin));
