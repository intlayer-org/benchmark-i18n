...
var ue = /(?:[Eec]{1,6}|G{1,5}|[Qq]{1,5}|(?:[yYur]+|U{1,5})|[ML]{1,5}|d{1,2}|D{1,3}|F{1}|[abB]{1,5}|[hkHK]{1,2}|w{1,2}|W{1}|m{1,2}|s{1,2}|[zZOvVxX]{1,4})(?=([^']*'[^']*')*[^']*$)/g;
function de(e) {
	let t = {};
...
function pe(e) {
	...
	let t = e.split(fe).filter((e) => e.length > 0), n = [];
	for (let e of t) {
		let t = e.split("/");
...
var C = /^\.(?:(0+)(\*)?|(#+)|(0+)(#+))$/g, w = /^(@+)?(\+|#+)?[rs]?$/g, he = /(\*)(0+)|(#+)(0+)|(0+)/g, T = /^(0+)$/;
function E(e) {
	let t = {};
...
function _e(e) {
	...
	for (let n of e) {
		switch (n.stem) {
			...
			case "measure-unit":
			case "unit":
				t.style = "unit", t.unit = me(n.options[0]);
				continue;
			case "compact-short":
			case "K":
...
var Pe = class {
	...
	parseMessage(e, t, n) {
		...
		for (; !this.isEOF();) {
			...
			if (i === 123) {
				...
				if (n) break;
				return this.error(k.UNMATCHED_CLOSING_TAG, V(this.clonePosition(), this.clonePosition()));
	...
	parseTag(e, t) {
		...
		if (this.bumpIf(">")) {
			...
			if (this.bumpIf("</")) {
				if (this.isEOF() || !H(this.char())) return this.error(k.INVALID_TAG, V(o, this.clonePosition()));
				let e = this.clonePosition();
				return r === this.parseTagName() ? (this.bumpSpace(), this.bumpIf(">") ? {
					...
					err: null
				} : this.error(k.INVALID_TAG, V(o, this.clonePosition()))) : this.error(k.UNMATCHED_CLOSING_TAG, V(e, this.clonePosition()));
			}
			return this.error(k.UNCLOSED_TAG, V(n, this.clonePosition()));
		}
		return this.error(k.INVALID_TAG, V(n, this.clonePosition()));
	}
	parseTagName() {
		let e = this.offset();
	...
	parseArgument(e, t) {
		let n = this.clonePosition();
		if (this.bump(), this.bumpSpace(), this.isEOF()) return this.error(k.EXPECT_ARGUMENT_CLOSING_BRACE, V(n, this.clonePosition()));
		if (this.char() === 125) return this.bump(), this.error(k.EMPTY_ARGUMENT, V(n, this.clonePosition()));
		let r = this.parseIdentifierIfPossible().value;
		if (!r) return this.error(k.MALFORMED_ARGUMENT, V(n, this.clonePosition()));
		if (this.bumpSpace(), this.isEOF()) return this.error(k.EXPECT_ARGUMENT_CLOSING_BRACE, V(n, this.clonePosition()));
		switch (this.char()) {
			...
			case 44: return this.bump(), this.bumpSpace(), this.isEOF() ? this.error(k.EXPECT_ARGUMENT_CLOSING_BRACE, V(n, this.clonePosition())) : this.parseArgumentOptions(e, t, r, n);
			default: return this.error(k.MALFORMED_ARGUMENT, V(n, this.clonePosition()));
		}
	}
	...
	parseArgumentOptions(e, t, n, r) {
		...
		switch (a) {
			case "": return this.error(k.EXPECT_ARGUMENT_TYPE, V(i, o));
			case "number":
			case "date":
			case "time": {
				this.bumpSpace();
				...
				if (this.bumpIf(",")) {
					...
					if (n.err) return n;
					let r = je(n.val);
					if (r.length === 0) return this.error(k.EXPECT_ARGUMENT_STYLE, V(this.clonePosition(), this.clonePosition()));
					e = {
						style: r,
			...
			case "select": {
				...
				if (this.bumpSpace(), !this.bumpIf(",")) return this.error(k.EXPECT_SELECT_ARGUMENT_OPTIONS, V(i, { ...i }));
				this.bumpSpace();
				let o = this.parseIdentifierIfPossible(), s = 0;
				if (a !== "select" && o.value === "offset") {
					if (!this.bumpIf(":")) return this.error(k.EXPECT_PLURAL_ARGUMENT_OFFSET_VALUE, V(this.clonePosition(), this.clonePosition()));
					this.bumpSpace();
			...
			default: return this.error(k.INVALID_ARGUMENT_TYPE, V(i, o));
		}
	}
	tryParseArgumentClose(e) {
		return this.isEOF() || this.char() !== 125 ? this.error(k.EXPECT_ARGUMENT_CLOSING_BRACE, V(e, this.clonePosition())) : (this.bump(), {
			val: !0,
			err: null
		});
	}
	parseSimpleArgStyleIfPossible() {
		...
		for (; !this.isEOF();) switch (this.char()) {
			case 39: {
				this.bump();
				let e = this.clonePosition();
				if (!this.bumpUntil("'")) return this.error(k.UNCLOSED_QUOTE_IN_ARGUMENT_STYLE, V(e, this.clonePosition()));
				this.bump();
				break;
			}
			case 123:
	...
	tryParsePluralOrSelectOptions(e, t, n, r) {
		...
		for (;;) {
			...
			if (!this.bumpIf("{")) return this.error(t === "select" ? k.EXPECT_SELECT_ARGUMENT_SELECTOR_FRAGMENT : k.EXPECT_PLURAL_ARGUMENT_SELECTOR_FRAGMENT, V(this.clonePosition(), this.clonePosition()));
		...
		return a.length === 0 ? this.error(t === "select" ? k.EXPECT_SELECT_ARGUMENT_SELECTOR : k.EXPECT_PLURAL_ARGUMENT_SELECTOR, V(this.clonePosition(), this.clonePosition())) : this.requiresOtherClause && !i ? this.error(k.MISSING_OTHER_CLAUSE, V(this.clonePosition(), this.clonePosition())) : {
			val: a,
			err: null
		};
	}
...
function U(e) {
	e.forEach((e) => {
		if (delete e.location, I(e) || L(e)) for (let t in e.options) delete e.options[t].location, U(e.options[t].value);
		else N(e) && be(e.style) || (P(e) || F(e)) && z(e.style) ? delete e.style.location : R(e) && U(e.children);
	});
}
function Re(e, t = {}) {
	t = {
...
function q(e, t, n, r, i, a, o) {
	...
	for (let c of e) {
		...
		if (P(c)) {
			let e = typeof c.style == "string" ? r.date[c.style] : z(c.style) ? c.style.parsedOptions : void 0;
			s.push({
				type: K.literal,
		...
		if (F(c)) {
			let e = typeof c.style == "string" ? r.time[c.style] : z(c.style) ? c.style.parsedOptions : r.time.medium;
			s.push({
				type: K.literal,
				value: n.getDateTimeFormat(t, e).format(l)
			});
...
function Xe(t) {
	...
	return function({ cache: t, formats: r, formatters: i, getMessageFallback: a = Je, locale: o, messagesOrError: s, namespace: c, onError: l, timeZone: u }) {
		...
		function p(p, m, h, ee) {
			...
			try {
				return qe(X(c, p), _, m && function(t) {
					let r = {};
...
var Qe = 86400;
7 * Qe, 365 * Qe;
function $e({ formats: e, getMessageFallback: t, messages: n, onError: r, ...i }) {
	return {
		...i,
		formats: e || void 0,
		messages: n || void 0,
...
var Q = t(void 0);
function et({ children: e, formats: t, getMessageFallback: n, locale: i, messages: a, now: s, onError: c, timeZone: l }) {
	let d = r(Q), f = o((() => d?.cache || se()), [i, d?.cache]), p = o((() => d?.formatters || le(f)), [f, d?.formatters]), m = o((() => ({
		...$e({
			locale: i,
...
var $ = !1, nt = typeof window > "u";
function rt(e) {
	return function(e, t, n) {
		let { cache: r, formats: i, formatters: a, getMessageFallback: s, locale: c, onError: l, timeZone: u } = tt(), d = e[n], f = Ze(t, n);
		return u || $ || !nt || ($ = !0, l(new b(x.ENVIRONMENT_FALLBACK, void 0))), o((() => Xe({
			cache: r,
			formatters: a,
