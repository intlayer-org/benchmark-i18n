...
function hn() {
	return qt().use(ze()).updateDefaults({ staticData: Object.fromEntries(mn.map((e) => [e, () => Promise.resolve(pn(e))])) });
}
var gn = hn().init(), _n = ({ language: e, staticData: t, children: n }) => {
	let r = d();
	return i(() => {
		let { unsubscribe: e } = gn.on("permanentChange", () => {
			r.refresh();
		});
		return () => {
			e();
...
function bn({ children: e, locale: t, staticData: n }) {
