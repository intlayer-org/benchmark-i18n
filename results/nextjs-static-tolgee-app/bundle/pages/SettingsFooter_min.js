...
function Se({ options: e }) {
	let t = de(), n = H(!1, () => o.isFetching(), t.onFetchingChange.emit), r = H(!1, () => S.isLoading(), t.onLoadingChange.emit), i = ye(t.onLanguageChange, t.onPendingLanguageChange, t.onRunningChange), a = ve(i.getLanguage, i.getInitialOptions, i.getAvailableLanguages, u, _, v, f, t), o = he(t, a.getBackendRecord, a.getBackendDevRecord, i.withDefaultNs, i.isInitialLoading, n, r);
	e && p(e);
	let s;
	t.onUpdate.listen(() => {
...
var G = 0, K = 1, q = 2, J = 3, Y = 4, Oe = new Set([
	q,
	K,
	G
]), X = "'", ke = new Set([
...
function je(e) {
	...
	for (s = 0; s < e.length; s++) switch (i = e[s], t) {
		case G:
			...
			break;
		case K:
			ke.has(i) ? (n = n.slice(0, -1) + i, t = q) : (n += i, t = G);
			break;
		case q:
			i === X ? t = G : (n += i, t = q);
			break;
		case J:
			i === "}" ? (d(), t = G) : De(i) ? r !== "" && (d(), t = Y) : (u(), t = J);
			break;
		case Y: i == "}" ? t = G : De(i) ? t = Y : c(W);
	}
...
(T = console.assert) == null || T.call(console, Le), RegExp(`([${["‌", "‍"].join("")}]{9})+`, "g");
function Re(e, t) {
	let n = (n) => {
...
var it = { useSuspense: !1 }, at, ot = () => (at ||= e.createContext(void 0), at), Z = void 0, st = ({ tolgee: n, options: r, children: a, fallback: o, ssr: s }) => {
	i(() => {
	...
	l = rt(n, u, d, !!s);
	let [f, p] = c(!l.isLoaded()), m = Object.assign(Object.assign({}, it), r), h = ot();
...
var ut = () => {
	...
	let { tolgee: r, options: a } = ut(), o = A(e), c = j(o).join(":"), l = Object.assign(Object.assign({}, a), t), { rerender: u, instance: d } = dt(), f = s([]);
	f.current = [];
	let p = r.isLoaded(o);
	i(() => {
		let e = r.on("update", u);
...
function Ct() {
	return tt().use(Pe()).updateDefaults({ staticData: Object.fromEntries(St.map((e) => [e, () => Promise.resolve(xt(e))])) });
}
var $ = Ct().init(), wt = ({ language: e, staticData: t, children: n }) => {
	let r = d();
	return i(() => {
...
function Dt({ children: e, locale: t, staticData: n }) {
