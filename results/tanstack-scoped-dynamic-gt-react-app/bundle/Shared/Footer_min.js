...
var yt = /[\t-\r \x85\u200E\u200F\u2028\u2029]/u, bt = /[\t-\r \x85\xA0\u1680\u2000-\u200A\u2028\u2029\u202F\u205F\u3000\x21-\x2F\x3A-\x40\x5B-\x5E\x60\x7B-\x7E\xA1-\xA7\xA9\xAB\xAC\xAE\xB0\xB1\xB6\xBB\xBF\xD7\xF7\u2010-\u2027\u2030-\u203E\u2041-\u2053\u2055-\u205E\u2190-\u245F\u2500-\u2775\u2794-\u2BFF\u2E00-\u2E7F\u3001-\u3003\u3008-\u3020\u3030\uFD3E-\uFD3F\uFE45-\uFE46]/u, xt = class {
	...
	readTagName() {
		let e = this.index;
		for (w(this.current()) || this.fail("INVALID_TAG", e), this.index += 1; !this.atEnd() && vt(this.current());) this.index += this.current().length;
...
function At(e, t, n) {
	...
	for (let r of e) switch (r.type) {
		...
		case C.number: {
			let e = T(t.variables, r.value), { scale: n, ...a } = typeof r.style == "string" ? Tt[r.style] ?? {} : r.style?.type === Ye.number ? r.style.parsedOptions : {}, o = jt(e, n);
			i(Mt(t, a).format(o));
			break;
		}
		case C.date:
		case C.time: {
			let e = T(t.variables, r.value), n = r.type === C.date ? Et : Ot;
			i(Nt(t, typeof r.style == "string" ? n[r.style] : r.style?.type === Ye.dateTime ? r.style.parsedOptions : r.type === C.time ? Ot.medium : void 0).format(e));
			break;
		}
		case C.select: {
...
function Ht(e, t, n) {
	return e.map((r, i) => {
		switch (r.type) {
			case C.literal: return Gt(r, t, i > 0, i < e.length - 1 || n);
			case C.argument: return `{${r.value}}`;
			case C.date:
			case C.time:
			case C.number: return qt(r);
			case C.select: return `{${r.value},select,${Yt(r.options, !1)}}`;
			case C.plural: {
				let e = r.pluralType === "cardinal" ? "plural" : "selectordinal", t = r.offset ? `offset:${r.offset} ` : "";
				return `{${r.value},${e},${t}${Yt(r.options, !0)}}`;
			}
			case C.pound: return "#";
			case C.tag: return `<${r.value}>${Ht(r.children, t, !0)}</${r.value}>`;
		}
	}).join("");
}
...
function Wt(e, t, n, r) {
	...
	let o = "", s = 0;
	for (let [r, a] of i) o += Ut(e.slice(s, r), t, s !== 0, s === 0 && n, !0), o += `'${e.slice(r, a).replace(/'/g, "''")}'`, s = a;
	return o += Ut(e.slice(s), t, s !== 0, s === 0 && n, r), o;
}
function Gt({ value: e }, t, n, r) {
	return Kt(e) ? e : Wt(e, t, n, r);
}
function Kt(e) {
	if (e[0] !== "<" || e.slice(-2) !== "/>") return !1;
	let t = e.slice(1, -2);
	if (!w(t[0])) return !1;
	for (let e of t.slice(1)) if (!vt(e)) return !1;
	return !0;
}
function qt(e) {
	let t = e.type === C.number ? "number" : e.type === C.date ? "date" : "time";
	return `{${e.value}, ${t}${e.style ? `, ${Jt(e.style)}` : ""}}`;
}
function Jt(e) {
	return typeof e == "string" ? e : e.type === Ye.dateTime ? `::${e.pattern}` : `::${e.tokens.map(({ stem: e, options: t }) => e + t.map((e) => `/${e}`).join("")).join(" ")}`;
}
function Yt(e, t) {
	return Object.entries(e).map(([e, n]) => `${e}{${Ht(n.value, t, !0)}}`).join(" ");
}
function Xt(e) {
...
var E = "_gt_", Qt = RegExp(`^${E}\\d+$`), $t = RegExp(`^${E}$`);
function en(e) {
...
function tn(e) {
	return e.type === C.select && $t.test(e.value) && !!e.options.other && (e.options.other.value.length === 0 || e.options.other.value.length > 0 && e.options.other.value[0]?.type === C.literal);
}
function nn(e) {
	let t = e;
...
var rn = (e) => `generaltranslation Formatting Error: Invalid cutoff style: ${e}.`, an = "DEFAULT_TERMINATOR_KEY", on = {
	ellipsis: {
		fr: {
			terminator: "…",
			separator: " "
	...
	constructor(t, n = {}) {
		...
		if (!on[r]) throw Error(rn(r));
		let i = n.maxChars === void 0 ? void 0 : on[r][new Intl.Locale(this.locale).language] || on[r].DEFAULT_TERMINATOR_KEY, a = n.terminator ?? i?.terminator, o = a == null ? void 0 : n.separator ?? i?.separator;
	...
	get(e, ...t) {
		...
		let o = a[i];
		return o === void 0 && (o = new cn[e](...t), a[i] = o), o;
	}
}();
function ln(e) {
...
function fn(e, t = un, n = ["en"]) {
	let r = ln(n).select(e), i = Math.abs(e);
...
var jn = (e, t, n) => {
	...
	try {
		...
		return !(e.split("-").length !== i || D.get("DisplayNames", ["en"], { type: "language" }).of(t) === t && !Nn(t) || n && D.get("DisplayNames", ["en"], { type: "region" }).of(n) === n || r && D.get("DisplayNames", ["en"], { type: "script" }).of(r) === r && !Mn.has(r));
	} catch {
		return !1;
	}
}, A = (e) => {
...
function $n(e, t = "en", n) {
	...
	e = j(e, n), t ||= "en";
	try {
		let i = A(e), a = D.get("Locale", e), o = a.language, s = Qn([
			r,
			e,
			i,
			o
			...
			t,
			"en"
		], g = D.get("DisplayNames", m, { type: "language" }), _ = D.get("DisplayNames", h, { type: "language" }), v = s?.name, y = s?.nativeName || s?.name, ee = v || g.of(e) || e, b = y || _.of(e) || e, te = s?.maximizedName || v || g.of(u) || e, ne = s?.nativeMaximizedName || y || _.of(u) || e, x = s?.minimizedName || v || g.of(p) || e, re = s?.nativeMinimizedName || y || _.of(p) || e, ie = s?.languageName || v || g.of(o) || e, S = s?.nativeLanguageName || y || _.of(o) || e, ae = s?.nameWithRegionCode || (c ? `${ie} (${c})` : ee), oe = s?.nativeNameWithRegionCode || (c ? `${S} (${c})` : b) || ae, se = D.get("DisplayNames", m, { type: "region" }), ce = D.get("DisplayNames", h, { type: "region" }), le = s?.regionName || (d ? se.of(d) : "") || "", ue = s?.nativeRegionName || (d ? ce.of(d) : "") || "", de = D.get("DisplayNames", m, { type: "script" }), fe = D.get("DisplayNames", h, { type: "script" });
		return {
			code: i,
			name: ee,
			nativeName: b,
...
function cr(e, t = "en", n) {
	...
	let r = e, i = Gn;
	try {
		r = D.get("DisplayNames", [t, "en"], { type: "region" }).of(e) || e, i = Xn(e);
	} catch {}
	return {
		code: e,
		name: r,
...
function ur(e, t) {
	return t?.dataFormat === "STRING" ? e : bn(e, t?.locales, t?.variables);
}
function M(e, t) {
	return k(e, t);
}
...
function Li(e, t) {
	...
	try {
		let i = gn(n || "");
		return lr(Ii(Object.keys(i).length ? vn(e) : e, {
			...r,
			...i,
			[E]: "other"
...
function $a(e, t, n) {
	let r = "", i = null;
	return typeof e == "number" && !i && n && (r = fn(e, Object.keys(n).filter(dn), t)), r && !i && (i = n[r]), i;
}
	...
	function i({ source: i, target: a, locales: o = ["en"], enableI18n: s }) {
		...
		if (Array.isArray(a) && !Array.isArray(i) && i && (i = [i]), Array.isArray(i) && Array.isArray(a)) {
			...
			return a.map((i, a) => {
				...
				if (nn(i)) return m(e.Fragment, { children: t({
					variableType: i.v || "v",
					variableValue: n[i.k],
...
function Xo(e) {
	let t = Ao(), n = jo();
	return Po(), d((n) => t.subscribeToTranslate(e, n), () => t.getTranslateSnapshot(e, n), () => t.getTranslateSnapshot(e, n));
}
	...
	let r = n, i = (e) => {
		...
		if (a) {
			...
			if ((e[1] === "automatic" || e[2] === "automatic") && (i.injectionType = "automatic"), e[0] === "translate" && (e[0] = "fragment"), e[0] === "variable" && (i.variableType = e?.[1] || "variable"), e[0] === "plural") {
				let e = Object.entries(n).reduce((e, [t, n]) => (dn(t) && (e[t] = ts(n, r)), e), {});
...
function rs(e, t) {
	...
	if (a) {
		...
		if (n === "plural") return r(e, { ...Object.entries(i).reduce((e, [n, r]) => (e[n] = dn(n) || n === "children" ? is(r, t) : r, e), {}) });
...
var ls = (e) => {
	...
	if (t["data-_gt"]) {
		...
		if (r === "variable") {
			let n = e.variableType || "variable", r = Ya(t, n), i = mn(n);
			return {
				i: e.id,
				k: r,
				v: i
...
function bs({ children: e, _locale: t, _enableI18n: n, _renderPreparedT: r = es, ...i }) {
	let { defaultLocale: a, locale: o, enableI18n: s, targetOptions: c, taggedSourceChildren: l, sourceJsxChildren: d, shouldTranslate: f } = ys({
		sourceChildren: e,
		params: i,
		_locale: t,
		_enableI18n: n
...
var Ts = class {
	constructor(e) {
		...
		}, this.getRegion = () => Cs({ cookieName: q().getRegionCookieName() }) || this.customGetRegion?.(), this.setRegion = (e) => {
			this.updateRegion(e), this.reload();
		}, this.getEnableI18n = () => {
			let e = Cs({ cookieName: q().getEnableI18nCookieName() });
...
function zs({ _getRegion: e, region: t }) {
	return Cs({ cookieName: q().getRegionCookieName() }) || e?.() || t;
}
function Bs({ enableI18n: e, _getEnableI18n: t }) {
	if (e !== void 0) return e;
	let n = Cs({ cookieName: q().getEnableI18nCookieName() });
			...
			children: [h("div", {
				...
				children: [
					h("div", { children: [m("h3", {
						...
						children: m(Z, { children: "i18n Benchmark" })
					}), m("p", {
						className: "text-sm text-muted-foreground",
						children: m(Z, { children: "An open-source test application for measuring the real-world impact of internationalization libraries on bundle size, loading time, and app reactivity." })
					})] }),
					h("div", { children: [m("h3", {
						className: "mb-2 text-sm font-semibold text-foreground",
						children: m(Z, { children: "Resources" })
					}), m("ul", {
						className: "space-y-1",
						children: [
							{
								label: m(Z, { children: "GitHub" }),
								href: "https://github.com/intlayer-org/benchmark-i18n",
								isInternal: !1
							},
							{
								label: m(Z, { children: "Methodology" }),
								to: "/$locale/about",
								isInternal: !0
							},
							{
								label: m(Z, { children: "Contributing" }),
								to: "/$locale/contact",
								isInternal: !0
							}
						].map((t) => m("li", { children: t.isInternal ? m(f, {
							preload: !1,
							to: t.to,
							params: { locale: e },
				...
				children: m(Z, { children: "i18n Benchmark — Open-source project. Built with React, Vite & TanStack Router." })
			})]
		})
	});
}
...
function lc(e, t, n = new URL(e.url).pathname) {
	...
	o && a.push(o), a.push(...Yi(e.headers.get("accept-language"))), a.length === 0 && console.warn(cc);
