...
import { useNavigate as f, useParams as p, useRouter as m } from "@tanstack/react-router";
import { jsx as h } from "react/jsx-runtime";
import g, { locales as _ } from "../../gt.config.json";
import { createIsomorphicFn as v, createMiddleware as y } from "@tanstack/react-start";
import { getRequest as ee, setCookie as te } from "@tanstack/react-start/server";
...
var se = "https://cdn.gtx.dev";
function ce(e) {
	return typeof e == "object" && !!e && "error" in e && typeof e.error == "string";
}
function le(e) {
	return ce(e.error) || typeof e.error == "string";
}
function ue(e) {
	if (e.data !== void 0) return e.data;
	if (e.response) {
		let t = ce(e.error) ? e.error.error : typeof e.error == "string" ? e.error : e.response.statusText;
		throw new ne(t, e.response.status, t);
	}
	throw e.error;
}
var de = (e) => e.client.post({
	security: [{
		scheme: "bearer",
		type: "http"
	}],
...
}), fe = 3, pe = 500, me = 6e4, he = /* @__PURE__ */ new Set([
	"GET",
	"HEAD",
	"OPTIONS",
	"PUT",
...
function _e({ fetch: e = globalThis.fetch, timeoutMs: t = ge } = {}) {
	return async (n, r) => {
		if (t === !1) return e(n, r);
...
var ve = (e) => new Promise((t) => setTimeout(t, e));
function ye(e) {
	if (!e) return;
...
function xe(e, t, n) {
	return e?.status === 429 ? be(e.headers.get("Retry-After")) ?? ye(e.headers.get("RateLimit-Reset")) ?? me : pe * (n === "linear" ? t + 1 : 2 ** t);
}
function Se({ fetch: e = globalThis.fetch, retryPolicy: t = "exponential" } = {}) {
	return async (n, r) => {
		let i = new Request(n, r), a = t === "none" ? 0 : fe, o = he.has(i.method);
		for (let n = 0; n <= a; n++) {
			let r;
			try {
				r = await e(i.clone());
			...
			r?.body?.cancel(), await ve(xe(r, n, t));
		}
		throw Error("Max retries exceeded");
	};
}
var Ce = { bodySerializer: (e) => JSON.stringify(e, (e, t) => typeof t == "bigint" ? t.toString() : t) }, we = ({ onRequest: e, onSseError: t, onSseEvent: n, responseTransformer: r, responseValidator: i, sseDefaultRetryDelay: a, sseMaxRetryAttempts: o, sseMaxRetryDelay: s, sseSleepFn: c, url: l, ...u }) => {
...
}, Oe = ({ allowReserved: e, explode: t, name: n, style: r, value: i }) => {
	if (!t) {
		let t = (e ? i : i.map((e) => encodeURIComponent(e))).join(Ee(r));
		switch (r) {
			case "label": return `.${t}`;
	...
	let a = Te(r), o = i.map((t) => r === "label" || r === "simple" ? e ? t : encodeURIComponent(t) : ke({
		allowReserved: e,
		name: n,
		value: t
	})).join(a);
	return r === "label" || r === "matrix" ? a + o : o;
}, ke = ({ allowReserved: e, name: t, value: n }) => {
	if (n == null) return "";
...
}, Ae = ({ allowReserved: e, explode: t, name: n, style: r, value: i, valueOnly: a }) => {
	...
	let o = De(r), s = Object.entries(i).map(([t, i]) => ke({
		allowReserved: e,
		name: r === "deepObject" ? `${n}[${t}]` : t,
		value: i
	})).join(o);
	return r === "label" || r === "matrix" ? o + s : s;
}, je = /\{[^{}]+\}/g, Me = ({ path: e, url: t }) => {
	let n = t, r = t.match(je);
	if (r) for (let t of r) {
	...
	return n;
}, Ne = ({ baseUrl: e, path: t, query: n, querySerializer: r, url: i }) => {
...
var Fe = async (e, t) => {
...
}, Re = (e, t) => t ? !!(e.headers.has(t) || e.query?.[t] || e.headers.get("Cookie")?.includes(`${t}=`)) : !1, ze = async ({ security: e, ...t }) => {
	for (let n of e) {
		if (Re(t, n.name)) continue;
		let e = await Fe(n, t.auth);
		if (!e) continue;
	...
	return n.baseUrl?.endsWith("/") && (n.baseUrl = n.baseUrl.substring(0, n.baseUrl.length - 1)), n.headers = Ue(e.headers, t.headers), n;
}, He = (e) => {
	let t = [];
	...
	for (let n of e) {
		if (!n) continue;
		let e = n instanceof Headers ? He(n) : Object.entries(n);
...
}), qe = { "Content-Type": "application/json" }, Je = (e = {}) => ({
	...Ce,
	headers: qe,
	parseAs: "auto",
	querySerializer: Ke,
	...
	let t = Ve(Je(), e), n = () => ({ ...t }), r = (e) => (t = Ve(t, e), n()), i = Ge(), a = async (e) => {
		let n = {
			...t,
			...e,
			fetch: e.fetch ?? t.fetch ?? globalThis.fetch,
			headers: Ue(t.headers, e.headers),
			serializedBody: void 0
		};
		return n.security && await ze({
			...n,
			security: n.security
		...
		if (c.ok) {
			let e = (t.parseAs === "auto" ? Le(c.headers.get("Content-Type")) : t.parseAs) ?? "json";
...
function Xe(e) {
	...
	return e.apiKey && t.set("Authorization", `Bearer ${e.apiKey}`), e.projectId && t.set("gt-project-id", e.projectId), Ye({
		baseUrl: e.baseUrl,
		fetch: Se({
			fetch: _e({
				fetch: e.fetch,
...
var S = {
	...
	number: 0,
	dateTime: 1
}, Ze = /^\.(?:(0+)(\*)?|(#+)|(0+)(#+))$/, Qe = /^@+(\+|#+)?[rs]?$/, $e = /^(?:(?:\+|#+)[rs]?|[rs])$/, et = /(\*)(0+)|(#+)(0+)|(0+)/g, tt = /[\t-\r \x85\u200E\u200F\u2028\u2029]+/u, nt = /(?:[Eec]{1,6}|G{1,5}|[Qq]{1,5}|(?:[yYur]+|U{1,5})|[ML]{1,5}|d{1,2}|D{1,3}|F|[abB]{1,5}|[hHkK]{1,2}|w{1,2}|W|m{1,2}|s{1,2}|[zZOvVxX]{1,4})(?=([^']*'[^']*')*[^']*$)/g, rt = {
	floor: "floor",
	ceiling: "ceil",
	down: "trunc",
	up: "expand",
...
function at(e) {
	let t = e.split(tt).filter(Boolean);
...
function ot(e) {
	...
	for (let n of e) {
		...
		switch (n.stem) {
			...
			case "measure-unit":
			case "unit":
				st(n), t.style = "unit", t.unit = e.replace(/^(.*?)-/, "");
				continue;
			case "compact-short":
			case "K":
			...
			case "scale":
				...
				continue;
			case "integer-width":
				if (st(n), n.options.length > 1) throw RangeError("integer-width stems only accept a single optional option");
				ut(t, e);
				continue;
		}
		if (n.stem.slice(0, 14) === "rounding-mode-") {
			let e = rt[n.stem.slice(14)];
		...
		if (!dt(t, n)) {
			if (Qe.test(n.stem)) {
				Object.assign(t, ft(n.stem));
				continue;
			}
			if ($e.test(n.stem)) throw SyntaxError("Significant precision must start with @.");
			ct(t, n.stem) || lt(t, n.stem);
		}
	}
	return t;
}
...
function lt(e, t) {
	...
	return e.notation = n[1] === "EE" ? "engineering" : "scientific", n[2] && ct(e, n[2]), e.minimumIntegerDigits = n[3].length, !0;
}
function ut(e, t) {
	t.replace(et, (t, n, r, i, a, o) => {
...
function ft(e) {
	let t = {};
	if (e.endsWith("r") && (t.roundingPriority = "morePrecision"), e.endsWith("s") && (t.roundingPriority = "lessPrecision"), $e.test(e)) throw SyntaxError("Significant precision must start with @.");
...
function pt(e) {
	...
	for (let [n] of e.matchAll(nt)) {
		...
		switch (n[0]) {
			...
			case "K":
			case "k":
				t.hourCycle = it[n[0]], t.hour = e === 2 ? "2-digit" : "numeric";
				break;
			case "m":
...
var _t = /^[A-Za-z]$/u, vt = /^[-.0-9_A-Za-z\u00B7\u00C0-\u00D6\u00D8-\u00F6\u00F8-\u037D\u037F-\u1FFF\u200C-\u200D\u203F-\u2040\u2070-\u218F\u2C00-\u2FEF\u3001-\uD7FF\uF900-\uFDCF\uFDF0-\uFFFD\u{10000}-\u{EFFFF}]$/u;
function w(e) {
	return _t.test(e ?? "");
}
function yt(e) {
...
var bt = /[\t-\r \x85\u200E\u200F\u2028\u2029]/u, xt = /[\t-\r \x85\xA0\u1680\u2000-\u200A\u2028\u2029\u202F\u205F\u3000\x21-\x2F\x3A-\x40\x5B-\x5E\x60\x7B-\x7E\xA1-\xA7\xA9\xAB\xAC\xAE\xB0\xB1\xB6\xBB\xBF\xD7\xF7\u2010-\u2027\u2030-\u203E\u2041-\u2053\u2055-\u205E\u2190-\u245F\u2500-\u2775\u2794-\u2BFF\u2E00-\u2E7F\u3001-\u3003\u3008-\u3020\u3030\uFD3E-\uFD3F\uFE45-\uFE46]/u, St = class {
	constructor(e, t) {
	...
	readTagName() {
		let e = this.index;
		for (w(this.current()) || this.fail("INVALID_TAG", e), this.index += 1; !this.atEnd() && yt(this.current());) this.index += this.current().length;
			...
			if (i.slice(0, 2) === "::") {
				...
				if (n === "number") {
					...
					r = {
						...
						tokens: e,
						...s ? { location: s } : {},
						parsedOptions: this.options.shouldParseSkeletons ? ot(e) : {}
					};
				} else {
					t || this.failAt("EXPECT_DATE_TIME_SKELETON", e, this.index);
					let n = mt(t, this.options.locale);
					r = {
						type: C.dateTime,
						pattern: n,
						...s ? { location: s } : {},
						parsedOptions: this.options.shouldParseSkeletons ? pt(n) : {}
					};
				}
			} else r = i;
		}
	...
	readIdentifier() {
		let e = this.index;
		for (; !this.atEnd() && !xt.test(this.current());) this.index += this.current().length;
		return this.message.slice(e, this.index);
	}
	skipSpace() {
	...
	current() {
		return wt(this.message, this.index);
	}
	peek() {
		let e = this.current();
		return wt(this.message, this.index + e.length);
	}
	atEnd() {
	...
	position(e) {
		let t = this.positions ??= Ct(this.message);
		return {
			offset: e,
			line: t[0][e],
			column: t[1][e]
...
function At(e, t = "en", n = {}) {
	let r = jt(Tt(e, { locale: Vt(t) }), {
		message: e,
		locales: t,
		variables: n
	});
...
function jt(e, t, n) {
	...
	for (let r of e) switch (r.type) {
		...
		case S.number: {
			let e = T(t.variables, r.value), { scale: n, ...a } = typeof r.style == "string" ? Et[r.style] ?? {} : r.style?.type === C.number ? r.style.parsedOptions : {}, o = Mt(e, n);
			i(Nt(t, a).format(o));
			break;
		}
		case S.date:
		case S.time: {
			let e = T(t.variables, r.value), n = r.type === S.date ? Dt : kt;
			i(Pt(t, typeof r.style == "string" ? n[r.style] : r.style?.type === C.dateTime ? r.style.parsedOptions : r.type === S.time ? kt.medium : void 0).format(e));
			break;
		}
		case S.select: {
			let e = String(T(t.variables, r.value)), n = zt(r.options, e) ?? r.options.other;
			if (!n) throw Bt(r.value, e, r.options);
			jt(n.value, t).forEach(i);
			break;
		}
		case S.plural: {
			let e = T(t.variables, r.value), n = `=${String(e)}`, a = zt(r.options, n), o = typeof e == "bigint" ? e : Number(e), s = typeof o == "bigint" ? o - BigInt(r.offset) : o - r.offset;
			if (!a && Rt(r.options)) {
				let e = Ft(t, r.pluralType ?? "cardinal").select(Lt(s));
				a = zt(r.options, e);
			}
			if (a ??= r.options.other, !a) throw Bt(r.value, e, r.options);
			jt(a.value, t, s).forEach(i);
			break;
		}
		case S.tag: {
			let e = T(t.variables, r.value);
			if (typeof e != "function") throw TypeError(`The ICU tag variable "${r.value}" must be a function.`);
			let a = e(jt(r.children, t, n));
...
function Nt(e, t = {}) {
	return It(e.numberFormats ??= /* @__PURE__ */ new Map(), JSON.stringify(t), () => new Intl.NumberFormat(e.locales, t));
}
function Pt(e, t) {
	return It(e.dateTimeFormats ??= /* @__PURE__ */ new Map(), t ? JSON.stringify(t) : "", () => new Intl.DateTimeFormat(e.locales, t));
}
function Ft(e, t) {
	...
	return It(e.pluralRules ??= /* @__PURE__ */ new Map(), t, () => new Intl.PluralRules(e.locales, { type: t }));
}
function It(e, t, n) {
	let r = e.get(t);
	if (r) return r;
...
function Ut(e, t, n) {
	return e.map((r, i) => {
		switch (r.type) {
			case S.literal: return Kt(r, t, i > 0, i < e.length - 1 || n);
			case S.argument: return `{${r.value}}`;
			case S.date:
			case S.time:
			case S.number: return Jt(r);
			case S.select: return `{${r.value},select,${Xt(r.options, !1)}}`;
			case S.plural: {
				let e = r.pluralType === "cardinal" ? "plural" : "selectordinal", t = r.offset ? `offset:${r.offset} ` : "";
				return `{${r.value},${e},${t}${Xt(r.options, !0)}}`;
			}
			case S.pound: return "#";
			case S.tag: return `<${r.value}>${Ut(r.children, t, !0)}</${r.value}>`;
		}
	}).join("");
}
...
function Gt(e, t, n, r) {
	...
	let o = "", s = 0;
	for (let [r, a] of i) o += Wt(e.slice(s, r), t, s !== 0, s === 0 && n, !0), o += `'${e.slice(r, a).replace(/'/g, "''")}'`, s = a;
	return o += Wt(e.slice(s), t, s !== 0, s === 0 && n, r), o;
}
function Kt({ value: e }, t, n, r) {
	return qt(e) ? e : Gt(e, t, n, r);
}
function qt(e) {
	if (e[0] !== "<" || e.slice(-2) !== "/>") return !1;
	let t = e.slice(1, -2);
	if (!w(t[0])) return !1;
	for (let e of t.slice(1)) if (!yt(e)) return !1;
	return !0;
}
function Jt(e) {
	let t = e.type === S.number ? "number" : e.type === S.date ? "date" : "time";
	return `{${e.value}, ${t}${e.style ? `, ${Yt(e.style)}` : ""}}`;
}
function Yt(e) {
	return typeof e == "string" ? e : e.type === C.dateTime ? `::${e.pattern}` : `::${e.tokens.map(({ stem: e, options: t }) => e + t.map((e) => `/${e}`).join("")).join(" ")}`;
}
function Xt(e, t) {
	return Object.entries(e).map(([e, n]) => `${e}{${Ut(n.value, t, !0)}}`).join(" ");
}
function Zt(e) {
...
var E = "_gt_", $t = RegExp(`^${E}\\d+$`), en = RegExp(`^${E}$`);
function tn(e) {
	return e.type === S.select && $t.test(e.value) && !!e.options.other && (e.options.other.value.length === 0 || e.options.other.value.length > 0 && e.options.other.value[0]?.type === S.literal);
}
function nn(e) {
	return e.type === S.select && en.test(e.value) && !!e.options.other && (e.options.other.value.length === 0 || e.options.other.value.length > 0 && e.options.other.value[0]?.type === S.literal);
}
function rn(e) {
	let t = e;
...
var an = (e) => `generaltranslation Formatting Error: Invalid cutoff style: ${e}.`, on = "DEFAULT_TERMINATOR_KEY", sn = {
	ellipsis: {
		fr: {
			terminator: "…",
			separator: " "
	...
	constructor(t, n = {}) {
		...
		if (!sn[r]) throw Error(an(r));
		let i = n.maxChars === void 0 ? void 0 : sn[r][new Intl.Locale(this.locale).language] || sn[r].DEFAULT_TERMINATOR_KEY, a = n.terminator ?? i?.terminator, o = a == null ? void 0 : n.separator ?? i?.separator;
	...
	get(e, ...t) {
		...
		let o = a[i];
		return o === void 0 && (o = new ln[e](...t), a[i] = o), o;
	}
}();
function un(e) {
...
function pn(e, t = dn, n = ["en"]) {
	let r = un(n).select(e), i = Math.abs(e);
...
function wn({ value: e, locales: t = ["en"], currency: n = "USD", options: r = {} }) {
	return D.get("NumberFormat", t, {
		style: "currency",
		currency: n,
		numberingSystem: "latn",
...
function On({ value: e, unit: t, locales: n = ["en"], options: r = {} }) {
...
var Mn = (e, t, n) => {
	...
	let n = e?.[t];
	return jn(n) && typeof n.code == "string" ? n.code : void 0;
}, Nn = /* @__PURE__ */ new Set([
	"Cham",
	"Jamo",
	"Kawi",
...
function Ln(e, t, n, r) {
	if (n && !n.allValid || !k(e, r) || !k(t, r) || In(e, t)) return !1;
	if (!n) return !0;
	let i = kn(t);
	return i !== void 0 && n.languages.has(i);
}
function Rn(e, t, n, r) {
	return Ln(e, t, n ? Fn(n, r) : void 0, r);
}
function zn(e) {
	try {
...
function Bn(e, t) {
	if (t.has(e)) return e;
	let { languageCode: n, regionCode: r, scriptCode: i, minimizedCode: a } = zn(e), o = `${n}-${r}`;
	if (t.has(o)) return o;
	let s = `${n}-${i}`;
	if (t.has(s)) return s;
	if (t.has(a)) return a;
...
function Un(e, t) {
	...
	try {
		...
		if (s) return s;
		let c = i.maximize();
		return qn[c.language] || Zn(c.region || "");
	} catch {
		return Kn;
	}
}
var Wn = "🌍", Gn = "🌏", Kn = Wn, qn = {
	...
	EU: "🇪🇺",
	419: "🌎"
}, Yn = /* @__PURE__ */ new Set(/* @__PURE__ */ "AF.AX.AL.DZ.AS.AD.AO.AI.AQ.AG.AR.AM.AW.AU.AT.AZ.BS.BH.BD.BB.BY.BE.BZ.BJ.BM.BT.BO.BQ.BA.BW.BV.BR.IO.BN.BG.BF.BI.CV.KH.CM.CA.KY.CF.TD.CL.CN.CX.CC.CO.KM.CD.CG.CK.CR.CI.HR.CU.CW.CY.CZ.DK.DJ.DM.DO.EC.EG.SV.GQ.ER.EE.SZ.ET.FK.FO.FJ.FI.FR.GF.PF.TF.GA.GM.GE.DE.GH.GI.GR.GL.GD.GP.GU.GT.GG.GN.GW.GY.HT.HM.VA.HN.HK.HU.IS.IN.ID.IR.IQ.IE.IM.IL.IT.JM.JP.JE.JO.KZ.KE.KI.KP.KR.KW.KG.LA.LV.LB.LS.LR.LY.LI.LT.LU.MO.MG.MW.MY.MV.ML.MT.MH.MQ.MR.MU.YT.MX.FM.MD.MC.MN.ME.MS.MA.MZ.MM.NA.NR.NP.NL.NC.NZ.NI.NE.NG.NU.NF.MK.MP.NO.OM.PK.PW.PS.PA.PG.PY.PE.PH.PN.PL.PT.PR.QA.RE.RO.RU.RW.BL.SH.KN.LC.MF.PM.VC.WS.SM.ST.SA.SN.RS.SC.SL.SG.SX.SK.SI.SB.SO.ZA.GS.SS.ES.LK.SD.SR.SJ.SE.CH.SY.TW.TJ.TZ.TH.TL.TG.TK.TO.TT.TN.TR.TM.TC.TV.UG.UA.AE.GB.US.UM.UY.UZ.VU.VE.VN.VG.VI.WF.EH.YE.ZM.ZW".split(".")), Xn = 127397;
function Zn(e) {
	return Qn(e) || "🌍";
}
function Qn(e) {
	let t = e.toUpperCase(), n = Jn[t];
	if (n) return n;
	if (Yn.has(t)) return String.fromCodePoint(t.charCodeAt(0) + Xn, t.charCodeAt(1) + Xn);
}
function $n(e, t) {
	if (!t) return;
	let n = {};
...
function er(e, t = "en", n) {
	...
	e = j(e, n), t ||= "en";
	try {
		let i = A(e), a = D.get("Locale", e), o = a.language, s = $n([
			r,
			e,
			i,
			o
			...
			t,
			"en"
		], g = D.get("DisplayNames", m, { type: "language" }), _ = D.get("DisplayNames", h, { type: "language" }), v = s?.name, y = s?.nativeName || s?.name, ee = v || g.of(e) || e, te = y || _.of(e) || e, ne = s?.maximizedName || v || g.of(u) || e, re = s?.nativeMaximizedName || y || _.of(u) || e, ie = s?.minimizedName || v || g.of(p) || e, b = s?.nativeMinimizedName || y || _.of(p) || e, ae = s?.languageName || v || g.of(o) || e, oe = s?.nativeLanguageName || y || _.of(o) || e, x = s?.nameWithRegionCode || (c ? `${ae} (${c})` : ee), se = s?.nativeNameWithRegionCode || (c ? `${oe} (${c})` : te) || x, ce = D.get("DisplayNames", m, { type: "region" }), le = D.get("DisplayNames", h, { type: "region" }), ue = s?.regionName || (d ? ce.of(d) : "") || "", de = s?.nativeRegionName || (d ? le.of(d) : "") || "", fe = D.get("DisplayNames", m, { type: "script" }), pe = D.get("DisplayNames", h, { type: "script" });
		return {
			code: i,
			name: ee,
			nativeName: te,
			maximizedCode: u,
			maximizedName: ne,
			nativeMaximizedName: re,
			minimizedCode: p,
			minimizedName: ie,
			nativeMinimizedName: b,
			languageCode: o,
			languageName: ae,
			nativeLanguageName: oe,
			nameWithRegionCode: x,
			nativeNameWithRegionCode: se,
			regionCode: d,
			regionName: ue,
			nativeRegionName: de,
			scriptCode: f,
			...
			emoji: s?.emoji || Un(i, n)
		};
	} catch {
		let t = k(e) ? A(e) : e, r = t.split("-"), i = r[0] || t, a = r.length > 2 ? r[2] : r[1] || "", o = r[3] || "", s = $n([t, i], n);
		t = s?.code || t;
		...
		o = s?.scriptCode || o;
		let ee = s?.scriptName || "", te = s?.nativeScriptName || "", ne = s?.nameWithRegionCode || (v ? `${g} (${v})` : c), re = s?.nativeNameWithRegionCode || (y ? `${_} (${y})` : l), ie = s?.emoji || "🌍";
		return {
			code: t,
			name: c,
			nativeName: l,
...
function nr(e) {
	try {
		let t = ar(D.get("Locale", e));
		if (t) return t;
	} catch {}
	let { scriptCode: t, languageCode: n } = er(e);
	return t ? rr.has(t.toLowerCase()) ? "rtl" : "ltr" : n && ir.has(n.toLowerCase()) ? "rtl" : "ltr";
}
...
var cr = class {
	...
	buildResolutionScope(e) {
		...
		return {
			...
			approved: Fn(t.map(({ canonicalLocale: e }) => e), this.customMapping)
		};
	}
	...
	formatRelativeTimeFromDate(e, t, n = {}) {
		let { locales: r, baseDate: i, ...a } = n, { value: o, unit: s } = Dn(e, i ?? /* @__PURE__ */ new Date());
		return On({
			value: o,
			unit: s,
			locales: this.getFormattingLocales(t, r),
	...
	formatMessage(e, t, n = {}) {
		...
		return a === "STRING" ? e : xn(e, this.getFormattingLocales(t, r), i);
	}
	formatList(e, t, n = {}) {
		let { locales: r, ...i } = n;
		return Tn({
	...
	getLocaleEmoji(e) {
		return Un(e, this.customMapping);
	}
	...
	requiresTranslation(e, t = this.defaultLocale, n = this.locales.length ? this.locales : void 0) {
		let r = n ? n === this.locales ? this.getResolutionScope().approved : Fn(n.map((e) => this.resolveCanonicalLocale(e)), this.customMapping) : void 0;
		return Ln(this.resolveCanonicalLocale(t), this.resolveCanonicalLocale(e), r, this.customMapping);
	}
	determineLocale(e, t = this.locales) {
		let { approvedLocalePairs: n, approved: r } = t === this.locales ? this.getResolutionScope() : this.buildResolutionScope(t), i = Vn(Array.isArray(e) ? e.map((e) => this.resolveCanonicalLocale(e)) : this.resolveCanonicalLocale(e), r, this.customMapping);
		if (i) return n.find(({ canonicalLocale: e }) => e === i)?.locale ?? this.resolveAliasLocale(i);
	}
	getLocaleDirection(e) {
		return nr(this.resolveCanonicalLocale(e));
	}
	isValidLocale(e) {
	...
	resolveAliasLocale(e) {
		return sr(e, this.customMapping);
	}
	standardizeLocale(e) {
		return A(e);
	}
	isSameDialect(...e) {
		return In(...e.map((e) => Array.isArray(e) ? e.map((e) => this.resolveCanonicalLocale(e)) : this.resolveCanonicalLocale(e)));
	}
	isSameLanguage(...e) {
	...
	isSupersetLocale(e, t) {
		return or(this.resolveCanonicalLocale(e), this.resolveCanonicalLocale(t));
	}
};
function lr(e, t = "en", n) {
	t ||= "en";
	let r = e, i = Kn;
	try {
		r = D.get("DisplayNames", [t, "en"], { type: "region" }).of(e) || e, i = Zn(e);
	} catch {}
	return {
		code: e,
		name: r,
...
function dr(e, t) {
	return t?.dataFormat === "STRING" ? e : xn(e, t?.locales, t?.variables);
}
function M(e, t) {
	return k(e, t);
}
...
function vr(e, t, n = "") {
	let r = _r(e), i = e?.length, a = t !== void 0;
	if (!r || a && i !== t) {
...
var Cr = typeof Uint8Array.from([]).toHex == "function" && typeof Uint8Array.fromHex == "function", wr = Array.from({ length: 256 }, (e, t) => t.toString(16).padStart(2, "0"));
function Tr(e) {
	if (vr(e), Cr) return e.toHex();
	let t = "";
...
var jr = class {
	...
	digestInto(e) {
		...
		t[a++] = 128, xr(this.buffer.subarray(a)), this.padOffset > r - a && (this.process(n, 0), a = 0);
	...
	3204031479,
	3329325298
]), F = /* @__PURE__ */ new Uint32Array(64), Nr = class extends jr {
	constructor(e) {
		super(64, e, 8, !1);
	}
	get() {
	...
	process(e, t) {
		...
		for (let e = 0; e < 64; e++) {
			let t = N(o, 6) ^ N(o, 11) ^ N(o, 25), u = l + t + kr(o, s, c) + Mr[e] + F[e] | 0, d = (N(n, 2) ^ N(n, 13) ^ N(n, 22)) + Ar(n, r, i) | 0;
	...
	destroy() {
		this.destroyed = !0, this.set(0, 0, 0, 0, 0, 0, 0, 0), xr(this.buffer);
	}
}, Pr = class extends Nr {
	A = P[0] | 0;
	B = P[1] | 0;
...
function Ir(e) {
	...
	if (Array.isArray(e)) {
		let t = "[";
		for (let n = 0; n < e.length; n++) n && (t += ","), t += Ir(e[n]) || "null";
		return t + "]";
	}
...
function Rr(e) {
	return Tr(Fr(Er(e))).slice(0, 16);
}
function zr({ source: e, context: t, id: n, maxChars: r, requiresReview: i, dataFormat: a }, o = Rr) {
	let s;
	return s = a === "JSX" ? Vr(e) : e, o(Lr({
		source: s,
		...n && { id: n },
		...t && { context: t },
...
var Br = (e) => {
	if (e && typeof e == "object") {
		let t = {};
		if ("c" in e && e.c && (t.c = Vr(e.c)), "d" in e) {
			let n = e?.d;
			n?.b && (t.b = Object.fromEntries(Object.entries(n.b).map(([e, t]) => [e, Vr(t)]))), n?.t && (t.t = n.t);
		}
		return rn(e) ? {
			k: e.k,
			...e.v && { v: e.v }
...
function Vr(e) {
	return Array.isArray(e) ? e.map(Br) : Br(e);
}
var I = "GT", Hr = (e) => x({
	source: I,
	severity: "Error",
...
async function ii(e, t, n) {
	...
	try {
		...
		throw e instanceof Error && e.name === "AbortError" ? Hr(n) : e;
	} finally {
		o && clearTimeout(o);
	}
}
async function ai(e) {
	if (!e.ok) {
		...
		throw new ne(Ur(e.status, e.statusText, t), e.status, t);
	}
}
async function oi(e, t, n, r) {
	let i = Array.isArray(e), a = i ? [] : void 0, o = {}, s = i ? e.map((e) => [void 0, e]) : Object.entries(e);
	for (let [e, t] of s) {
		let { source: n, metadata: r } = typeof t == "string" ? { source: t } : t, i = e ?? r?.hash ?? zr({
			source: n,
	...
	if (l.data === void 0 && l.response && !le(l)) throw await ai(l.response), l.error;
	let u = ue(l);
...
var si = class {
	...
	setConfig({ apiKey: e, devApiKey: t, sourceLocale: n, targetLocale: r, locales: i, projectId: a, customMapping: o, baseUrl: s }) {
		if (e && (this.apiKey = e), t && (this.devApiKey = t), a && (this.projectId = a), n && (this.sourceLocale = pr(n), !M(this.sourceLocale, o))) throw Error(qr(this.sourceLocale));
		if (r && (this.targetLocale = pr(r), !M(this.targetLocale, o))) throw Error(qr(this.targetLocale));
		if (i) {
			let e = [], t = [];
			if (i.forEach((n) => {
				let r = pr(n);
		...
		s && (this.baseUrl = s), o && (this.customMapping = o, this.reverseCustomMapping = Object.fromEntries(Object.entries(o).filter(([, e]) => e && typeof e == "object" && "code" in e).map(([e, t]) => [t.code, e]))), this._localeConfig = new cr({
	...
	getRegionProperties(e = this.getLocaleProperties().regionCode, t) {
		...
		return lr(e, this.targetLocale, t);
	}
	requiresTranslation(e = this.sourceLocale, t = this.targetLocale, n = this.locales, r = this.customMapping) {
		...
		return r === this.customMapping ? this.localeConfig.requiresTranslation(t, e, n) : mr(e, t, n, r);
	}
	determineLocale(e, t = this.locales || [], n = this.customMapping) {
		return n === this.customMapping ? this.localeConfig.determineLocale(e, t ?? []) : hr(e, t, n);
	}
	...
	resolveCanonicalLocale(e = this.targetLocale, t = this.customMapping) {
		...
		return t === this.customMapping ? this.localeConfig.resolveCanonicalLocale(e) : fr(e, t);
	}
	resolveAliasLocale(e, t = this.customMapping) {
		if (!e) throw Error(L("resolveAliasLocale"));
		return t === this.customMapping ? this.localeConfig.resolveAliasLocale(e) : gr(e, t);
	}
...
function hi() {
	...
	return gi(e) ? e.isDebugLoggingEnabled() : ui(li());
}
function gi(e) {
...
function wi(e, t) {
	if (!t) return;
	let n = Ti(e), r = Ei(e), i = [...n, ...r];
	if (i.forEach((e) => {
		z.error(`I18nConfig: ${Di(e)}`);
...
function Ti({ defaultLocale: e, locales: t, customMapping: n }) {
	let r = /* @__PURE__ */ new Set([...e ? [e] : [], ...t || []]);
	return Array.from(r).filter((e) => !M(e, n));
}
function Ei({ customMapping: e }) {
...
var Oi = class extends cr {
	constructor(e = {}) {
		let t = Mi(e);
		super(ki(e, t)), this.runtimeConfig = {
	...
	getLocaleConfig(e) {
		return !e || !ji(e) ? this : new cr(Ai(e));
	}
...
function Ai({ defaultLocale: e = "en", locales: t = [], customMapping: n } = {}) {
	return {
		defaultLocale: e,
...
var Ii = (e) => `String interpolation failed for message: "${e}".`;
function Li(e, t, n, r) {
	try {
		return dr(e, {
			variables: t,
		...
		return z.warn(Ii(e)), e;
	}
}
function Ri(e, t) {
	if (!e) return e;
	let n = t.$_fallback, r = Fi(t);
	try {
		let i = _n(n || "");
		return ur(Li(Object.keys(i).length ? yn(e) : e, {
			...r,
			...i,
			[E]: "other"
		}, t.$locale, t.$format), { maxChars: t.$maxChars });
	} catch {
		return z.warn(Ii(e)), t.$_fallback == null ? ur(e, { maxChars: t.$maxChars }) : Ri(t.$_fallback, {
			...t,
			$_fallback: void 0
		});
	}
...
function Bi({ source: e, target: t, options: n, sourceLocale: r }) {
	return t == null ? Vi(e, Hi(n, r)) : Vi(t, {
		$_fallback: e,
		...n
	});
}
function Vi(e, t) {
	switch (t.$format ?? "STRING") {
		case "ICU": return Ri(e, t);
		case "I18NEXT":
		case "STRING": return zi(e, t);
		default: return e;
	}
}
function Hi(e, t) {
...
var { getConditionStore: Gi, setConditionStore: Ki } = Wi(x({
	source: "gt-i18n",
	severity: "Error",
...
function qi(e, t) {
	let n = t;
	return n.$_hash == null ? zr({
		source: t.$format === "ICU" ? gn(e) : e,
...
async function Zi({ locale: e, enableI18n: t }, n) {
	let r = vi(), i = B().getDefaultLocale(), a = await r.getLookupTranslation(t ? e : i);
	return (n, r = {}) => {
		let o = Ui(t ? r.$locale ?? e : B().getDefaultLocale(), r, "ICU");
		return Bi({
			source: n,
			target: a(n, o),
			options: o,
...
async function Qi({ locale: e, enableI18n: t }) {
	let n = await Zi({
		locale: e,
		enableI18n: t
	});
	return (e, t = {}) => {
		if (e == null) return e;
		let r = Ji(e) ?? {};
		return Yi(r) ? n(r.$_source, r) : n(e, t);
	};
}
function $i({ sourceLocale: e, targetLocale: t, sourceEntry: n, target: r, dictionaryOptions: i, options: a = {} }) {
	let o = Ui(t, {
		...i,
		...Fi(a)
	}, i.$format);
...
function oa(e) {
	return typeof e == "string" ? !0 : !Array.isArray(e) || typeof e[0] != "string" ? !1 : e.length === 1 || e.length === 2 && sa(e[1]);
}
function sa(e) {
	if (typeof e != "object" || !e || Array.isArray(e)) return !1;
	let t = e;
	return (t.$context === void 0 || typeof t.$context == "string") && (t.$format === void 0 || ca(t.$format)) && (t.$maxChars === void 0 || typeof t.$maxChars == "number");
}
function ca(e) {
...
function da({ sourceObject: e, targetObject: t, translate: n }) {
	let r = W(t);
	if (r !== void 0) return r.entry;
	if (H(t)) return H(e) ? fa({
		sourceObject: e,
		targetObject: t,
		translate: n
	}) : da({
...
function fa({ sourceObject: e, targetObject: t, translate: n }) {
	if (!H(e)) return da({
		sourceObject: e,
		targetObject: t,
		translate: n
...
async function pa({ locale: e, enableI18n: t, rootId: n }) {
	let r = vi(), i = B().getDefaultLocale(), a = t ? e : i, [o, s, c] = await Promise.all([
...
function _a(e) {
	let { cacheUrl: t = se, projectId: n, _versionId: r, _branchId: i } = e, a = r ? `/${r}` : "", o = i ? `?branchId=${i}` : "";
	return `${t}/${n}/[locale]` + a + o;
}
function va({ type: e, remoteTranslationLoaderParams: t, loadTranslations: n }) {
	let { cacheUrl: r, projectId: i, _versionId: a, _branchId: o } = t;
	switch (e) {
		case "remote":
		case "gt-remote": return i ? ga({
			cacheUrl: r,
			projectId: i,
			_versionId: a,
			_branchId: o,
		...
		case "disabled": return r === null ? async () => ({}) : ya(x({
...
var xa = class {
	...
	load(e) {
		return ba(this.pendingLoads, e, () => this.loadResource(e).then((t) => (this.set(e, t), t)));
	}
	getExpiresAt() {
...
var Ta = class {
	constructor({ init: e, translateMany: t, onMiss: n, batchConfig: r }) {
		this.pendingTranslations = /* @__PURE__ */ new Map(), this.queue = [], this.batchTimer = null, this.activeRequests = 0, this.cache = structuredClone(e), this.translateMany = t, this.batchConfig = wa(r), this.onMiss = n;
	}
	get(e) {
	...
	async miss(e) {
		let t = this.getCacheKey(e), n = await ba(this.pendingTranslations, t, () => this.translate(e));
	...
	async sendBatchRequest(e) {
		this.activeRequests++;
		let t = Ea(e), n = await this.sendBatchRequestWithErrorHandling(e, t);
...
async function Da({ key: e, sourceValue: t, targetValue: n, translateEntry: r }) {
	...
	let a = H(n) ? n : {}, o = /* @__PURE__ */ new Set([...Object.keys(t), ...Object.keys(a)]), s = await Promise.all(Array.from(o).map(async (n) => {
		...
		ta(n, i);
		let o = t[n];
		return o === void 0 ? [n, U(a[n])] : [n, await Da({
			key: i,
			sourceValue: o,
			targetValue: a[n],
			translateEntry: r
...
var ka = class {
	...
	getEntry(e) {
		let t = W(na(this.cache, e));
		if (t !== void 0) return Oa(t);
	}
	getValue(e) {
		let t = na(this.cache, e);
	...
	async materializeValue(e, t, n = na(this.cache, e)) {
		return ba(this.pendingMaterializations, e, () => Da({
			key: e,
			sourceValue: t,
			targetValue: n,
			translateEntry: async (e, t) => ia(await this.materializeEntry(e, t))
		}).then((t) => (this.setValue(e, t), t)));
	}
	async materializeEntry(e, t) {
		return Oa(await ba(this.pendingTranslations, e, () => this.runtimeTranslate(e, t).then((t) => {
			ra(this.cache, e, t);
			let n = W(t);
...
function Aa(e, t) {
	for (let [n, r] of Object.entries(t)) {
		let t = e[n];
		H(t) && H(r) ? Aa(t, r) : e[n] = U(r);
	}
}
var ja = 12e3, Ma = class {
	constructor(e) {
		Na(e), this.config = {
			projectId: e.projectId,
			devApiKey: e.devApiKey,
		...
		this.createTranslateMany = ha(B().getGTClass(), this.config.runtimeTranslation?.timeout ?? ja, {
	...
	async translateDictionaryEntry(e, t, n) {
		let r = await this.lookupTranslationWithFallbackResolved(e, n.entry, aa(n.options));
...
function Na(e) {
	if (e.runtimeUrl && e.runtimeUrl !== "https://api.gtx.dev" && (e.projectId || z.warn("I18nCache: " + x({
	...
	})), !e.devApiKey && !e.apiKey && z.warn("I18nCache: " + x({
	...
	}))), e.loadDictionary && !e.dictionary) throw z.error("I18nCache: " + x({
...
var Fa = [];
function Ia({ locale: e, enableI18n: t, localesProp: n = Fa }) {
...
var Ra = "generaltranslation.locale-reset", za = "server-render", Ba = Symbol.for("generaltranslation.react-core.ReactI18nConfig"), Va = class extends Oi {
	renderStrategy;
	localeCookieName;
	regionCookieName;
	enableI18nCookieName;
	constructor(e = {}, t = za) {
		super(e), Wa(t), Object.defineProperty(this, Ba, { value: !0 }), this.renderStrategy = t, this.localeCookieName = e.localeCookieName ?? "generaltranslation.locale", this.regionCookieName = e.regionCookieName ?? "generaltranslation.region", this.enableI18nCookieName = e.enableI18nCookieName ?? "generaltranslation.enable-i18n";
	}
	getRenderStrategy() {
...
function Ga(e) {
	...
	let t = e;
	return t[Ba] === !0 && typeof t.getRenderStrategy == "function" && typeof t.getLocaleCookieName == "function" && typeof t.getRegionCookieName == "function" && typeof t.getEnableI18nCookieName == "function";
}
var { getConditionStore: J, setConditionStore: Ka, isConditionStoreInitialized: qa } = Wi(x({
	source: "@generaltranslation/react-core",
	severity: "Error",
...
function Ja({ Currency: e, GtInternalCurrency: t, DateTime: n, GtInternalDateTime: r, Num: i, GtInternalNum: a, RelativeTime: o, GtInternalRelativeTime: s, Var: c, GtInternalVar: l }) {
...
function Xa(e = {}, t) {
	return typeof e.name == "string" ? e.name : `_gt_${Ya[t] || "value"}_${e["data-_gt"]?.id}`;
}
function Za(e) {
...
function eo(e, t, n) {
	let r = "", i = null;
	return typeof e == "number" && !i && n && (r = pn(e, Object.keys(n).filter(fn), t)), r && !i && (i = n[r]), i;
}
function to({ renderVariable: t }) {
	return function({ children: n, defaultLocale: r = "en", enableI18n: i }) {
		let a = (n) => {
			let a = $a(n);
			if (Za(n.props)) {
				let { variableType: e, variableValue: a, variableOptions: o, injectionType: s } = Qa(n.props);
				return t({
					variableType: e,
					variableValue: a,
					variableOptions: o,
			...
			if (a?.transformation === "plural") {
				let e = a.branches || {};
				return typeof n.props.n == "number" ? s(eo(n.props.n, [r], e) ?? n.props.children) : n.props.children == null ? null : s(n.props.children);
			}
...
function no({ renderVariable: t }) {
	let n = to({ renderVariable: t });
	function r({ sourceElement: t, targetElement: r, locales: a = ["en"], enableI18n: o }) {
		...
		if (u && Object.entries(bn).forEach(([e, t]) => {
			u[e] && (d[t] = u[e]);
		}), l === "plural") {
			let e = t.props.n;
			return typeof e == "number" ? i({
				source: eo(e, a, c.branches || {}) ?? t.props.children,
				target: eo(e, a, r.d?.b || {}) ?? r.c,
				locales: a,
				enableI18n: o
			}) : n({
				children: t,
	...
	function i({ source: i, target: a, locales: o = ["en"], enableI18n: s }) {
		...
		if (Array.isArray(a) && !Array.isArray(i) && i && (i = [i]), Array.isArray(i) && Array.isArray(a)) {
			let n = {}, c = {}, l = {}, u = i.filter((t) => {
				if (e.isValidElement(t)) {
					if (Za(t.props)) {
						let { variableName: e, variableValue: r, variableOptions: i, injectionType: a } = Qa(t.props);
						n[e] = r, c[e] = i, l[e] = a;
					} else return !0;
				}
				return !1;
			...
			return a.map((i, a) => {
				...
				if (rn(i)) return h(e.Fragment, { children: t({
					variableType: i.v || "v",
					variableValue: n[i.k],
		...
		if (a && typeof a == "object" && !Array.isArray(a)) {
			...
			if (e.isValidElement(i)) {
				...
				if (Za(i.props)) {
					let { variableValue: e, variableOptions: n, variableType: r, injectionType: a } = Qa(i.props);
					return t({
						variableType: r,
						variableValue: e,
						variableOptions: n,
...
var ao = { display: "contents" }, oo = globalThis.navigator?.product === "ReactNative";
function so(e) {
	if (e == null || typeof e == "boolean" || e === "") return !0;
	if (Array.isArray(e)) return !e.some((e) => !so(e));
	if (o(e) && e.type === n) {
...
function co(e, t) {
	return oo || !ro() ? e : o(e) && typeof e.type == "string" ? r(e, { "data-_gt-hash": t }) : so(e) ? e : a("span", {
		"data-_gt-hash": t,
		style: ao
	}, e);
}
function lo({ renderDefaultChildren: e, renderTranslatedChildren: t }) {
...
function uo(e) {
	let t = Ja(e), n = to({ renderVariable: t }), r = no({ renderVariable: t });
	return {
		renderVariable: t,
...
var fo = class extends Ma {};
function po(e) {
	Ua(e, "server-render"), La(new fo(e));
}
var mo = R({
	namespace: "reactCore",
	key: "i18nStore",
...
function xo(e, { locale: t, id: n }) {
	let r = e?.[t];
	if (!r) return;
	if (!n) return r;
	let i = r;
	for (let e of n.split(".")) {
		if (!So(e) || !H(i) || !Object.prototype.hasOwnProperty.call(i, e)) return;
		i = i[e];
	}
	return i;
}
...
var wo = class {
	...
	subscribeToTranslate = (e, t) => {
		let n = Pa(e);
		return _o(this.translateListeners, (e) => {
			Pa(e) === n && t();
		});
	};
	subscribeToTranslationEvents = (e) => _o(this.translateListeners, e);
	subscribeToDictionaryEntryEvents = (e) => _o(this.dictionaryEntryListeners, e);
	subscribeToDictionaryObjectEvents = (e) => _o(this.dictionaryObjectListeners, e);
	getTranslateSnapshot = (e, t = {}) => vo(t, e) ?? K().lookupTranslation(e.locale, e.message, e.options);
	getDictionaryEntrySnapshot = (e, t = {}) => yo(t, e) ?? K().lookupDictionary(e.locale, e.id);
	getDictionaryObjectSnapshot = (e, t = {}) => bo(t, e) ?? K().lookupDictionaryObj(e.locale, e.id);
...
function Eo() {
	return To.isInitialized() || To.set(i(void 0)), To.get();
}
function Do() {
	let e = s(Eo());
...
function ko() {
	return Do()?.conditionStore ?? J();
}
function Y() {
	return ko().getLocale();
}
function X() {
	return ko().getEnableI18n();
}
function Ao() {
	return l(() => B().getDefaultLocale(), []);
}
function jo() {
	return Do()?.i18nStore || ho();
}
function Mo() {
	return Do()?.translationsSnapshot || {};
}
var No = () => {};
function Po() {
	return No;
...
var Fo = Po;
function Io({ _enableI18n: e, _locale: t, children: n, currency: r = "USD", options: i = {}, locales: a = [] }) {
	let o = Ia({
		locale: t,
		enableI18n: e,
		localesProp: a
...
function Lo({ _enableI18n: e, _locale: t, ...n }) {
	return Io({
		...n,
		_enableI18n: e ?? X(),
		_locale: t ?? Y()
...
Lo._gtt = "variable-currency-automatic", Ro._gtt = "variable-currency";
function zo({ _enableI18n: e, _locale: t, children: n, options: r = {}, locales: i = [] }) {
	let a = Ia({
		locale: t,
		enableI18n: e,
		localesProp: i
...
Bo._gtt = "variable-datetime-automatic", Vo._gtt = "variable-datetime";
...
function Uo({ _enableI18n: e, _locale: t, ...n }) {
	return Ho({
		...n,
		_enableI18n: e ?? X(),
		_locale: t ?? Y()
...
Uo._gtt = "variable-number-automatic", Wo._gtt = "variable-number";
function Go({ _enableI18n: e, _locale: t, date: n, children: r, value: i, unit: a, baseDate: o, locales: s = [], options: c = {} }) {
	let l = Ia({
		locale: t,
		enableI18n: e,
		localesProp: s
...
Ko._gtt = "variable-relative-time-automatic", qo._gtt = "variable-relative-time";
...
Yo._gtt = "variable-variable", Xo._gtt = "variable-variable-automatic";
function Zo(e) {
	let t = jo(), n = Mo();
	return Fo(), d((n) => t.subscribeToTranslate(e, n), () => t.getTranslateSnapshot(e, n), () => t.getTranslateSnapshot(e, n));
}
var { renderVariable: Qo, renderDefaultChildren: $o, renderTranslatedChildren: es, renderPreparedT: ts } = uo({
	Currency: Ro,
	GtInternalCurrency: Lo,
	DateTime: Vo,
	GtInternalDateTime: Bo,
	Num: Wo,
	GtInternalNum: Uo,
	RelativeTime: qo,
	...
	let r = n, i = (e) => {
		...
		if (a) {
			...
			if ((e[1] === "automatic" || e[2] === "automatic") && (i.injectionType = "automatic"), e[0] === "translate" && (e[0] = "fragment"), e[0] === "variable" && (i.variableType = e?.[1] || "variable"), e[0] === "plural") {
				let e = Object.entries(n).reduce((e, [t, n]) => (fn(t) && (e[t] = ns(n, r)), e), {});
				Object.keys(e).length && (i.branches = e);
			}
			if (e[0] === "branch") {
				let { children: e, branch: t, ...a } = n, o = Object.fromEntries(Object.entries(a).filter(([e]) => !e.startsWith("data-"))), s = Object.entries(o).reduce((e, [t, n]) => (e[t] = ns(n, r), e), {});
...
function is(e, t) {
	let { type: n, props: i } = e, a = ss(n);
	if (typeof i != "object" || !i) return e;
	if (a) {
		let { componentType: n, injectionType: o } = a;
		if (n === "variable") return e;
		if (n === "branch") return r(e, { ...Object.entries(i).reduce((e, [n, r]) => (e[n] = n !== "branch" && !n.startsWith("data-") ? as(r, t) : r, e), {}) });
		if (n === "plural") return r(e, { ...Object.entries(i).reduce((e, [n, r]) => (e[n] = fn(n) || n === "children" ? as(r, t) : r, e), {}) });
		if (n === "derive") return r(e, {
			...i,
			..."children" in i && { children: os(i.children, t + 1) }
		});
		if (n === "translate" && o === "automatic" && t > 0) return "children" in i ? os(i.children, t) : void 0;
		n === "translate" && o === "automatic" && console.warn(cs);
	}
	return r(e, {
		...i,
		..."children" in i && { children: os(i.children, t) }
	});
}
function as(e, t) {
	return o(e) ? is(e, t) : e;
...
function os(e, n) {
	return Array.isArray(e) ? t.map(e, (e) => as(e, n)) : as(e, n);
}
function ss(e) {
...
var cs = "'@generaltranslation/react-core Warning: A <_T> component was found injected outside of a <Derive> boundary. This may affect translation resolution for this component.";
function ls(t) {
...
var us = (e) => {
	...
	let r = Object.entries(bn).reduce((e, [n, r]) => {
		let i = t[r];
	...
	let { props: t } = e, n = { t: us(e) };
	if (t["data-_gt"]) {
		let e = t["data-_gt"], r = e.transformation;
		if (r === "variable") {
			let n = e.variableType || "variable", r = Xa(t, n), i = hn(n);
			return {
				i: e.id,
				k: r,
				v: i
		...
		n.i = e.id, n.d = ds(r, t, e.branches);
	}
	return t.children && (n.c = ms(t.children)), n;
}, ps = (e) => ls(e) ? fs(e) : typeof e == "number" ? e.toString() : e;
function ms(e) {
	return Array.isArray(e) ? e.map(ps) : ps(e);
}
function hs({ sourceChildren: e, params: t, locale: n }) {
	let r = gs(e), i = _s(r), a = vs({
		options: ys(t),
		locale: n
	});
	return io(i, a), {
...
function vs({ options: e, locale: t }) {
	return {
		...e,
		$locale: t
	};
...
function bs({ sourceChildren: e, params: t, _locale: n, _enableI18n: r }) {
	let i = Y(), a = X(), o = Ao(), s = n ?? i, c = r ?? a;
	return {
		defaultLocale: o,
		enableI18n: c,
		locale: s,
...
xs._gtt = "translate-client";
function Ss({ children: e, _locale: t, _enableI18n: n, _renderPreparedT: r = ts, ...i }) {
	let { defaultLocale: a, locale: o, enableI18n: s, targetOptions: c, taggedSourceChildren: l, sourceJsxChildren: d, shouldTranslate: f } = bs({
		sourceChildren: e,
		params: i,
		_locale: t,
		_enableI18n: n
...
var Cs = Eo();
function ws({ children: e, translations: t, dictionaries: n, conditionStore: r, i18nStore: i, onMissingTranslation: a, onMissingDictionaryEntry: o, onMissingDictionaryObj: s }) {
	let u = l(() => ({
...
function As(e) {
	if (!e) return "";
	let t = Array.isArray(e) ? e.join(", ") : e;
	return t.trim() ? Os(`Details: ${t}`) : "";
}
function js({ source: e, severity: t, whatHappened: n, reassurance: r, why: i, fix: a, wayOut: o, details: s, docsUrl: c }) {
	let l = e ? t ? `${e} ${t}:` : `${e}:` : t ? `${t}:` : "", u = i ? `${ks(n)} because ${ks(i)}` : n, d = !!a && !!o && /^[a-z]/.test(ks(o)), f = [
		u,
		r,
		d ? `${ks(a)}, or ${ks(o)}` : a,
		d ? void 0 : o,
...
}), { setConditionStore: Ns, isConditionStoreInitialized: Ps } = Wi(Ms), { getConditionStore: Fs, setConditionStore: Is, isConditionStoreInitialized: Ls } = Wi(Ms);
function Rs(e) {
	let t = zs(e), n = Bs(e), r = Vs(e);
	if (Ls()) {
		let e = Fs();
...
function Us() {
	return {
		projectId: Ws(() => void 0) || Gs(),
		devApiKey: Si() === "development" ? Ws(() => void 0) || Ks() : void 0
	};
}
function Ws(e) {
	try {
...
function Xs() {
	...
	return h("div", {
		...
		children: h("select", {
			...
			children: _.map((e) => h("option", {
				value: e,
				children: h(xs, { children: t(e) })
			}, e))
		})
	});
}
...
function $s(e) {
	if (!e) return "";
	let t = Array.isArray(e) ? e.join(", ") : e;
	return t.trim() ? Zs(`Details: ${t}`) : "";
}
function ec({ source: e, severity: t, whatHappened: n, reassurance: r, why: i, fix: a, wayOut: o, details: s, docsUrl: c }) {
	let l = e ? t ? `${e} ${t}:` : `${e}:` : t ? `${t}:` : "", u = i ? `${Qs(n)} because ${Qs(i)}` : n, d = !!a && !!o && /^[a-z]/.test(Qs(o)), f = [
		u,
		r,
		d ? `${Qs(a)}, or ${Qs(o)}` : a,
		d ? void 0 : o,
...
nc.set;
var rc = nc.isInitialized;
function ic(e, t = oc()) {
	let { pathname: n } = sc(e, t), r = n.match(/^\/([^/]+)(?:\/|$)/);
	if (!r) return;
	let i;
	try {
...
function ac(e, t, n = oc()) {
	let r = q(), { basepath: i, pathname: a } = sc(e, n), o = ic(a, "/") ? a.replace(/^\/[^/]+/, "") || "/" : a, s = r.resolveSupportedLocale(t);
...
function uc(e, t, n = new URL(e.url).pathname) {
...
var dc = v().server(() => $().getLocale()).client(() => J().getLocale());
v().server(() => $().getEnableI18n()).client(() => J().getEnableI18n()), v().server((e) => {
	let t = $();
	return Zi({
		locale: t.getLocale(),
...
}), v().server(fc).client(() => dc());
function fc({ defaultLocale: e, locales: t, customMapping: n }) {
	if (rc()) {
		let e = $();
...
function pc({ defaultLocale: e, locales: t, customMapping: n }) {
...
y().server(({ request: e, pathname: t, next: n }) => $().run(e, () => n(), t));
function mc(e) {
	let t = e.localeRouting && !e._reload ? {
		...e,
		_reload: ({ locale: e }) => {
			let t = ac(window.location.pathname, e), n = new URL(window.location.href);
...
async function gc(e, t = ["shared"]) {
	let n = {};
	return await Promise.all(t.map(async (t) => {
		let r = hc[`./src/_gt/${e}/${t}.json`];
		if (!r) {
