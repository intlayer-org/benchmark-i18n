...
var Dn = (e, t, n) => {
	...
	"Toto",
	"Thai"
]), kn = (e) => e >= "qaa" && e <= "qtz", A = (e, t) => {
	e = k(t, e) || e;
	try {
		let { language: t, region: n, script: r } = O.get("Locale", e), i = 1 + Number(!!n) + Number(!!r);
		return !(e.split("-").length !== i || O.get("DisplayNames", ["en"], { type: "language" }).of(t) === t && !kn(t) || n && O.get("DisplayNames", ["en"], { type: "region" }).of(n) === n || r && O.get("DisplayNames", ["en"], { type: "script" }).of(r) === r && !On.has(r));
	} catch {
		return !1;
	}
}, j = (e) => {
...
function Mn(e, t, n, r) {
	if (n && !n.allValid || !A(e, r) || !A(t, r) || jn(e, t)) return !1;
	if (!n) return !0;
	let i = wn(t);
...
function Yn(e, t = "en", n) {
	...
	e = M(e, n), t ||= "en";
	try {
		let i = j(e), a = O.get("Locale", e), o = a.language, s = Jn([
			r,
			e,
			i,
			o
		...
		let t = A(e) ? j(e) : e, r = t.split("-"), i = r[0] || t, a = r.length > 2 ? r[2] : r[1] || "", o = r[3] || "", s = Jn([t, i], n);
		t = s?.code || t;
...
var Er = class {
	...
	3204031479,
	3329325298
]), I = /* @__PURE__ */ new Uint32Array(64), Or = class extends Er {
	constructor(e) {
		super(64, e, 8, !1);
	}
	get() {
	...
	process(e, t) {
		...
		for (let e = 0; e < 64; e++) {
			let t = P(o, 6) ^ P(o, 11) ^ P(o, 25), u = l + t + wr(o, s, c) + Dr[e] + I[e] | 0, d = (P(n, 2) ^ P(n, 13) ^ P(n, 22)) + Tr(n, r, i) | 0;
...
async function ei(e) {
	if (!e.ok) {
		...
		throw new v(Rr(e.status, e.statusText, t), e.status, t);
	}
}
...
var ni = class {
	...
	setConfig({ apiKey: e, devApiKey: t, sourceLocale: n, targetLocale: r, locales: i, projectId: a, customMapping: o, baseUrl: s }) {
		if (e && (this.apiKey = e), t && (this.devApiKey = t), a && (this.projectId = a), n && (this.sourceLocale = cr(n), !N(this.sourceLocale, o))) throw Error(Hr(this.sourceLocale));
		if (r && (this.targetLocale = cr(r), !N(this.targetLocale, o))) throw Error(Hr(this.targetLocale));
		if (i) {
			let e = [], t = [];
			if (i.forEach((n) => {
				let r = cr(n);
...
function Ni(e, t) {
	...
	try {
		...
		return B.warn(ji(e)), t.$_fallback == null ? ar(e, { maxChars: t.$maxChars }) : Ni(t.$_fallback, {
			...t,
			$_fallback: void 0
		});
	}
...
var { getConditionStore: Bi, setConditionStore: Vi } = zi(S({
	source: "gt-i18n",
	severity: "Error",
...
async function Ki({ locale: e, enableI18n: t }, n) {
	let r = pi(), i = V().getDefaultLocale(), a = await r.getLookupTranslation(t ? e : i);
	return (n, r = {}) => {
		let o = Ri(t ? r.$locale ?? e : V().getDefaultLocale(), r, "ICU");
		return Fi({
			source: n,
			target: a(n, o),
			options: o,
...
async function ca({ locale: e, enableI18n: t, rootId: n }) {
	let r = pi(), i = V().getDefaultLocale(), a = t ? e : i, [o, s, c] = await Promise.all([
...
function pa({ type: e, remoteTranslationLoaderParams: t, loadTranslations: n }) {
	...
	switch (e) {
		...
		case "disabled": return r === null ? async () => ({}) : ma(S({
...
async function Sa({ key: e, sourceValue: t, targetValue: n, translateEntry: r }) {
	...
	let a = U(n) ? n : {}, o = /* @__PURE__ */ new Set([...Object.keys(t), ...Object.keys(a)]), s = await Promise.all(Array.from(o).map(async (n) => {
		...
		Xi(n, i);
		let o = t[n];
		return o === void 0 ? [n, W(a[n])] : [n, await Sa({
			key: i,
			sourceValue: o,
			targetValue: a[n],
			translateEntry: r
...
var Ea = 12e3, Da = class {
	constructor(e) {
		...
		this.createTranslateMany = ua(V().getGTClass(), this.config.runtimeTranslation?.timeout ?? Ea, {
...
var { getConditionStore: Y, setConditionStore: Va, isConditionStoreInitialized: Ha } = zi(S({
	source: "@generaltranslation/react-core",
	severity: "Error",
...
function go(e, { locale: t, id: n }) {
	...
	for (let e of n.split(".")) {
		if (!_o(e) || !U(i) || !Object.prototype.hasOwnProperty.call(i, e)) return;
		i = i[e];
	}
	return i;
}
...
var yo = class {
	...
	getTranslateSnapshot = (e, t = {}) => po(t, e) ?? q().lookupTranslation(e.locale, e.message, e.options);
	getDictionaryEntrySnapshot = (e, t = {}) => mo(t, e) ?? q().lookupDictionary(e.locale, e.id);
	getDictionaryObjectSnapshot = (e, t = {}) => ho(t, e) ?? q().lookupDictionaryObj(e.locale, e.id);
...
var Ss = class {
	constructor(e) {
		...
		}, this.getRegion = () => bs({ cookieName: J().getRegionCookieName() }) || this.customGetRegion?.(), this.setRegion = (e) => {
			this.updateRegion(e), this.reload();
		}, this.getEnableI18n = () => {
			let e = bs({ cookieName: J().getEnableI18nCookieName() });
...
function Cs(e) {
	let t = J(), n = xs(t.getLocaleCookieName());
...
}), { setConditionStore: ks, isConditionStoreInitialized: As } = zi(Os), { getConditionStore: js, setConditionStore: Ms, isConditionStoreInitialized: Ns } = zi(Os);
function Ps(e) {
...
function Is({ _getRegion: e, region: t }) {
	return bs({ cookieName: J().getRegionCookieName() }) || e?.() || t;
}
function Ls({ enableI18n: e, _getEnableI18n: t }) {
	if (e !== void 0) return e;
	let n = bs({ cookieName: J().getEnableI18nCookieName() });
...
function zs() {
	return {
		...
		devApiKey: _i() === "development" ? Bs(() => void 0) || Hs() : void 0
	};
}
function Bs(e) {
	try {
...
function tc(e, t, n = nc()) {
	let r = J(), { basepath: i, pathname: a } = rc(e, n), o = ec(a, "/") ? a.replace(/^\/[^/]+/, "") || "/" : a, s = r.resolveSupportedLocale(t);
...
var sc = p().server(() => $().getLocale()).client(() => Y().getLocale());
p().server(() => $().getEnableI18n()).client(() => Y().getEnableI18n()), p().server((e) => {
	let t = $();
	return Ki({
		locale: t.getLocale(),
...
async function fc(e, t = ["shared"]) {
	let n = {};
	return await Promise.all(t.map(async (t) => {
		let r = dc[`./src/_gt/${e}/${t}.json`];
		if (!r) {
