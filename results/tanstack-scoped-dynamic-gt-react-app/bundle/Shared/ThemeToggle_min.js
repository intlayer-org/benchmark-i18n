...
function Ys() {
	...
	c(() => {
		...
		if (e !== "auto") return;
		let t = window.matchMedia("(prefers-color-scheme: dark)"), n = () => Js("auto");
	...
	function n() {
		...
		t(n), Js(n), window.localStorage.setItem("theme", n);
	}
...
function ic(e, t, n = ac()) {
	let r = J(), { basepath: i, pathname: a } = oc(e, n), o = rc(a, "/") ? a.replace(/^\/[^/]+/, "") || "/" : a, s = r.resolveSupportedLocale(t);
...
var uc = m().server(() => $().getLocale()).client(() => Y().getLocale());
