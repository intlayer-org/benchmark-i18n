...
import { createIsomorphicFn as g, createMiddleware as _ } from "@tanstack/react-start";
import { getRequest as v, setCookie as y } from "@tanstack/react-start/server";
import ee from "../../../../gt.config.json";
...
var ae = "https://cdn.gtx.dev";
function oe(e) {
	return typeof e == "object" && !!e && "error" in e && typeof e.error == "string";
}
function se(e) {
	return oe(e.error) || typeof e.error == "string";
}
function ce(e) {
	if (e.data !== void 0) return e.data;
	if (e.response) {
		let t = oe(e.error) ? e.error.error : typeof e.error == "string" ? e.error : e.response.statusText;
		throw new b(t, e.response.status, t);
	}
	throw e.error;
}
var le = (e) => e.client.post({
	security: [{
		scheme: "bearer",
		type: "http"
	}],
...
}), ue = 3, de = 500, fe = 6e4, pe = /* @__PURE__ */ new Set([
	"GET",
	"HEAD",
	"OPTIONS",
	"PUT",
...
function he({ fetch: e = globalThis.fetch, timeoutMs: t = me } = {}) {
	return async (n, r) => {
		if (t === !1) return e(n, r);
...
function ye(e, t, n) {
	return e?.status === 429 ? ve(e.headers.get("Retry-After")) ?? _e(e.headers.get("RateLimit-Reset")) ?? fe : de * (n === "linear" ? t + 1 : 2 ** t);
}
function be({ fetch: e = globalThis.fetch, retryPolicy: t = "exponential" } = {}) {
	return async (n, r) => {
		let i = new Request(n, r), a = t === "none" ? 0 : ue, o = pe.has(i.method);
		for (let n = 0; n <= a; n++) {
			let r;
			try {
				r = await e(i.clone());
			...
			r?.body?.cancel(), await ge(ye(r, n, t));
		}
		throw Error("Max retries exceeded");
	};
}
var xe = { bodySerializer: (e) => JSON.stringify(e, (e, t) => typeof t == "bigint" ? t.toString() : t) }, Se = ({ onRequest: e, onSseError: t, onSseEvent: n, responseTransformer: r, responseValidator: i, sseDefaultRetryDelay: a, sseMaxRetryAttempts: o, sseMaxRetryDelay: s, sseSleepFn: c, url: l, ...u }) => {
...
}, Ee = ({ allowReserved: e, explode: t, name: n, style: r, value: i }) => {
	if (!t) {
		let t = (e ? i : i.map((e) => encodeURIComponent(e))).join(we(r));
		switch (r) {
			case "label": return `.${t}`;
	...
	let a = Ce(r), o = i.map((t) => r === "label" || r === "simple" ? e ? t : encodeURIComponent(t) : De({
		allowReserved: e,
		name: n,
		value: t
	})).join(a);
	return r === "label" || r === "matrix" ? a + o : o;
}, De = ({ allowReserved: e, name: t, value: n }) => {
	if (n == null) return "";
...
}, Oe = ({ allowReserved: e, explode: t, name: n, style: r, value: i, valueOnly: a }) => {
	...
	let o = Te(r), s = Object.entries(i).map(([t, i]) => De({
		allowReserved: e,
		name: r === "deepObject" ? `${n}[${t}]` : t,
		value: i
	})).join(o);
	return r === "label" || r === "matrix" ? o + s : s;
}, ke = /\{[^{}]+\}/g, Ae = ({ path: e, url: t }) => {
	let n = t, r = t.match(ke);
	if (r) for (let t of r) {
	...
	return n;
}, je = ({ baseUrl: e, path: t, query: n, querySerializer: r, url: i }) => {
...
var Ne = async (e, t) => {
...
}, Ie = (e, t) => t ? !!(e.headers.has(t) || e.query?.[t] || e.headers.get("Cookie")?.includes(`${t}=`)) : !1, Le = async ({ security: e, ...t }) => {
	for (let n of e) {
		if (Ie(t, n.name)) continue;
		let e = await Ne(n, t.auth);
		if (!e) continue;
	...
	return n.baseUrl?.endsWith("/") && (n.baseUrl = n.baseUrl.substring(0, n.baseUrl.length - 1)), n.headers = Ve(e.headers, t.headers), n;
}, Be = (e) => {
	let t = [];
	...
	for (let n of e) {
		if (!n) continue;
		let e = n instanceof Headers ? Be(n) : Object.entries(n);
...
}), Ge = { "Content-Type": "application/json" }, Ke = (e = {}) => ({
	...xe,
	headers: Ge,
	parseAs: "auto",
	querySerializer: We,
	...
	let t = ze(Ke(), e), n = () => ({ ...t }), r = (e) => (t = ze(t, e), n()), i = Ue(), a = async (e) => {
		let n = {
			...t,
			...e,
			fetch: e.fetch ?? t.fetch ?? globalThis.fetch,
			headers: Ve(t.headers, e.headers),
			serializedBody: void 0
		};
		return n.security && await Le({
			...n,
			security: n.security
		...
		if (c.ok) {
			let e = (t.parseAs === "auto" ? Fe(c.headers.get("Content-Type")) : t.parseAs) ?? "json";
...
var vt = /[\t-\r \x85\u200E\u200F\u2028\u2029]/u, yt = /[\t-\r \x85\xA0\u1680\u2000-\u200A\u2028\u2029\u202F\u205F\u3000\x21-\x2F\x3A-\x40\x5B-\x5E\x60\x7B-\x7E\xA1-\xA7\xA9\xAB\xAC\xAE\xB0\xB1\xB6\xBB\xBF\xD7\xF7\u2010-\u2027\u2030-\u203E\u2041-\u2053\u2055-\u205E\u2190-\u245F\u2500-\u2775\u2794-\u2BFF\u2E00-\u2E7F\u3001-\u3003\u3008-\u3020\u3030\uFD3E-\uFD3F\uFE45-\uFE46]/u, bt = class {
	...
	readTagName() {
		let e = this.index;
		for (T(this.current()) || this.fail("INVALID_TAG", e), this.index += 1; !this.atEnd() && _t(this.current());) this.index += this.current().length;
...
function kt(e, t, n) {
	...
	for (let r of e) switch (r.type) {
		...
		case C.number: {
			let e = E(t.variables, r.value), { scale: n, ...a } = typeof r.style == "string" ? wt[r.style] ?? {} : r.style?.type === w.number ? r.style.parsedOptions : {}, o = At(e, n);
			i(jt(t, a).format(o));
			break;
		}
		case C.date:
		case C.time: {
			let e = E(t.variables, r.value), n = r.type === C.date ? Tt : Dt;
			i(Mt(t, typeof r.style == "string" ? n[r.style] : r.style?.type === w.dateTime ? r.style.parsedOptions : r.type === C.time ? Dt.medium : void 0).format(e));
			break;
		}
		case C.select: {
			let e = String(E(t.variables, r.value)), n = Lt(r.options, e) ?? r.options.other;
			if (!n) throw Rt(r.value, e, r.options);
			kt(n.value, t).forEach(i);
			break;
		}
		case C.plural: {
			let e = E(t.variables, r.value), n = `=${String(e)}`, a = Lt(r.options, n), o = typeof e == "bigint" ? e : Number(e), s = typeof o == "bigint" ? o - BigInt(r.offset) : o - r.offset;
			if (!a && It(r.options)) {
...
function Vt(e, t, n) {
	return e.map((r, i) => {
		switch (r.type) {
			case C.literal: return Wt(r, t, i > 0, i < e.length - 1 || n);
			case C.argument: return `{${r.value}}`;
			case C.date:
			case C.time:
			case C.number: return Kt(r);
			case C.select: return `{${r.value},select,${Jt(r.options, !1)}}`;
			case C.plural: {
				let e = r.pluralType === "cardinal" ? "plural" : "selectordinal", t = r.offset ? `offset:${r.offset} ` : "";
				return `{${r.value},${e},${t}${Jt(r.options, !0)}}`;
			}
			case C.pound: return "#";
			case C.tag: return `<${r.value}>${Vt(r.children, t, !0)}</${r.value}>`;
		}
	}).join("");
}
...
function Ut(e, t, n, r) {
	...
	let o = "", s = 0;
	for (let [r, a] of i) o += Ht(e.slice(s, r), t, s !== 0, s === 0 && n, !0), o += `'${e.slice(r, a).replace(/'/g, "''")}'`, s = a;
	return o += Ht(e.slice(s), t, s !== 0, s === 0 && n, r), o;
}
function Wt({ value: e }, t, n, r) {
	return Gt(e) ? e : Ut(e, t, n, r);
}
function Gt(e) {
	if (e[0] !== "<" || e.slice(-2) !== "/>") return !1;
	let t = e.slice(1, -2);
	if (!T(t[0])) return !1;
	for (let e of t.slice(1)) if (!_t(e)) return !1;
	return !0;
}
function Kt(e) {
	let t = e.type === C.number ? "number" : e.type === C.date ? "date" : "time";
	return `{${e.value}, ${t}${e.style ? `, ${qt(e.style)}` : ""}}`;
}
function qt(e) {
	return typeof e == "string" ? e : e.type === w.dateTime ? `::${e.pattern}` : `::${e.tokens.map(({ stem: e, options: t }) => e + t.map((e) => `/${e}`).join("")).join(" ")}`;
}
function Jt(e, t) {
	return Object.entries(e).map(([e, n]) => `${e}{${Vt(n.value, t, !0)}}`).join(" ");
}
function Yt(e) {
...
function Xt({ icuString: e, shouldVisit: t, visitor: n, options: { recurseIntoVisited: r = !0, ...i } }) {
	let a = Ct(e, i);
	return o(a), a;
	function o(e) {
		e.map(s);
...
var D = "_gt_", Zt = RegExp(`^${D}\\d+$`), Qt = RegExp(`^${D}$`);
function $t(e) {
	return e.type === C.select && Zt.test(e.value) && !!e.options.other && (e.options.other.value.length === 0 || e.options.other.value.length > 0 && e.options.other.value[0]?.type === C.literal);
}
function en(e) {
	return e.type === C.select && Qt.test(e.value) && !!e.options.other && (e.options.other.value.length === 0 || e.options.other.value.length > 0 && e.options.other.value[0]?.type === C.literal);
}
function tn(e) {
	let t = e;
...
var An = (e, t, n) => {
	...
	"Toto",
	"Thai"
]), Nn = (e) => e >= "qaa" && e <= "qtz", k = (e, t) => {
	e = jn(t, e) || e;
	try {
		let { language: t, region: n, script: r } = O.get("Locale", e), i = 1 + Number(!!n) + Number(!!r);
		return !(e.split("-").length !== i || O.get("DisplayNames", ["en"], { type: "language" }).of(t) === t && !Nn(t) || n && O.get("DisplayNames", ["en"], { type: "region" }).of(n) === n || r && O.get("DisplayNames", ["en"], { type: "script" }).of(r) === r && !Mn.has(r));
	} catch {
		return !1;
	}
}, A = (e) => {
...
function Fn(...e) {
	try {
		let t = e.flat().map((e) => O.get("Locale", A(e))), [n] = t, r = new Set(t.map(({ region: e }) => e).filter(Boolean)), i = new Set(t.map(({ script: e }) => e).filter(Boolean));
...
function In(e, t, n, r) {
	if (n && !n.allValid || !k(e, r) || !k(t, r) || Fn(e, t)) return !1;
	if (!n) return !0;
	let i = Dn(t);
	return i !== void 0 && n.languages.has(i);
}
function Ln(e, t, n, r) {
	return In(e, t, n ? Pn(n, r) : void 0, r);
}
function Rn(e) {
	try {
...
function zn(e, t) {
	if (t.has(e)) return e;
	let { languageCode: n, regionCode: r, scriptCode: i, minimizedCode: a } = Rn(e), o = `${n}-${r}`;
	if (t.has(o)) return o;
	let s = `${n}-${i}`;
	if (t.has(s)) return s;
	if (t.has(a)) return a;
...
function Hn(e, t) {
	...
	e = j(e, t);
	try {
		let r = A(e), i = O.get("Locale", r), { language: a, region: o } = i;
		if (t) for (let i of [
			n,
			e,
			r,
...
function $n(e, t = "en", n) {
	...
	e = j(e, n), t ||= "en";
	try {
		let i = A(e), a = O.get("Locale", e), o = a.language, s = Qn([
			r,
			e,
			i,
			o
			...
			t,
			"en"
		], g = O.get("DisplayNames", m, { type: "language" }), _ = O.get("DisplayNames", h, { type: "language" }), v = s?.name, y = s?.nativeName || s?.name, ee = v || g.of(e) || e, b = y || _.of(e) || e, te = s?.maximizedName || v || g.of(u) || e, ne = s?.nativeMaximizedName || y || _.of(u) || e, x = s?.minimizedName || v || g.of(p) || e, re = s?.nativeMinimizedName || y || _.of(p) || e, ie = s?.languageName || v || g.of(o) || e, S = s?.nativeLanguageName || y || _.of(o) || e, ae = s?.nameWithRegionCode || (c ? `${ie} (${c})` : ee), oe = s?.nativeNameWithRegionCode || (c ? `${S} (${c})` : b) || ae, se = O.get("DisplayNames", m, { type: "region" }), ce = O.get("DisplayNames", h, { type: "region" }), le = s?.regionName || (d ? se.of(d) : "") || "", ue = s?.nativeRegionName || (d ? ce.of(d) : "") || "", de = O.get("DisplayNames", m, { type: "script" }), fe = O.get("DisplayNames", h, { type: "script" });
		return {
			code: i,
			name: ee,
			nativeName: b,
		...
		let t = k(e) ? A(e) : e, r = t.split("-"), i = r[0] || t, a = r.length > 2 ? r[2] : r[1] || "", o = r[3] || "", s = Qn([t, i], n);
		t = s?.code || t;
		...
		o = s?.scriptCode || o;
		let ee = s?.scriptName || "", b = s?.nativeScriptName || "", te = s?.nameWithRegionCode || (v ? `${g} (${v})` : c), ne = s?.nativeNameWithRegionCode || (y ? `${_} (${y})` : l), x = s?.emoji || "🌍";
		return {
			code: t,
			name: c,
			nativeName: l,
...
function ar(e, t) {
	try {
		let { language: n, region: r, script: i } = O.get("Locale", A(e)), { language: a, region: o, script: s } = O.get("Locale", A(t));
...
function or(e, t) {
	return t ? Object.keys(t).find((n) => jn(t, n) === e) ?? e : e;
}
var sr = class {
	getResolutionScope() {
	...
	isResolutionScopeCurrent(e) {
		...
		for (let t = 0; t < this.locales.length; t++) {
			...
			if (r.locale !== n || r.canonicalLocale !== this.resolveCanonicalLocale(n) || e.canonicalMappingCodes[t] !== jn(this.customMapping, r.canonicalLocale)) return !1;
		}
		return !0;
	}
	buildResolutionScope(e) {
		...
		return {
			...
			canonicalMappingCodes: t.map(({ canonicalLocale: e }) => jn(this.customMapping, e)),
			approved: Pn(t.map(({ canonicalLocale: e }) => e), this.customMapping)
		};
	}
	...
	requiresTranslation(e, t = this.defaultLocale, n = this.locales.length ? this.locales : void 0) {
		let r = n ? n === this.locales ? this.getResolutionScope().approved : Pn(n.map((e) => this.resolveCanonicalLocale(e)), this.customMapping) : void 0;
		return In(this.resolveCanonicalLocale(t), this.resolveCanonicalLocale(e), r, this.customMapping);
	}
	determineLocale(e, t = this.locales) {
		let { approvedLocalePairs: n, approved: r } = t === this.locales ? this.getResolutionScope() : this.buildResolutionScope(t), i = Bn(Array.isArray(e) ? e.map((e) => this.resolveCanonicalLocale(e)) : this.resolveCanonicalLocale(e), r, this.customMapping);
	...
	isSameDialect(...e) {
		return Fn(...e.map((e) => Array.isArray(e) ? e.map((e) => this.resolveCanonicalLocale(e)) : this.resolveCanonicalLocale(e)));
	}
	isSameLanguage(...e) {
...
function ur(e, t) {
	return t?.dataFormat === "STRING" ? e : yn(e, t?.locales, t?.variables);
}
function M(e, t) {
	return k(e, t);
}
...
function _r(e, t, n = "") {
	let r = gr(e), i = e?.length, a = t !== void 0;
	if (!r || a && i !== t) {
...
var Ar = class {
	...
	destroyed = !1;
	constructor(e, t, n, r) {
		this.blockLen = e, this.outputLen = t, this.padOffset = n, this.isLE = r, this.buffer = new Uint8Array(e), this.view = xr(this.buffer);
	}
	update(e) {
		vr(this), _r(e);
	...
	digestInto(e) {
		vr(this), yr(e, this), this.finished = !0;
		let { buffer: t, view: n, blockLen: r, isLE: i } = this, { pos: a } = this;
		t[a++] = 128, br(this.buffer.subarray(a)), this.padOffset > r - a && (this.process(n, 0), a = 0);
		for (let e = a; e < r; e++) t[e] = 0;
		n.setBigUint64(r - 8, BigInt(this.length * 8), i), this.process(n, 0);
		let o = xr(e), s = this.outputLen;
	...
	3204031479,
	3329325298
]), F = /* @__PURE__ */ new Uint32Array(64), Mr = class extends Ar {
	constructor(e) {
		super(64, e, 8, !1);
	}
	get() {
	...
	process(e, t) {
		...
		for (let e = 0; e < 64; e++) {
			let t = N(o, 6) ^ N(o, 11) ^ N(o, 25), u = l + t + Or(o, s, c) + jr[e] + F[e] | 0, d = (N(n, 2) ^ N(n, 13) ^ N(n, 22)) + kr(n, r, i) | 0;
	...
	destroy() {
		this.destroyed = !0, this.set(0, 0, 0, 0, 0, 0, 0, 0), br(this.buffer);
	}
}, Nr = class extends Mr {
	A = P[0] | 0;
	B = P[1] | 0;
...
function Fr(e) {
	...
	if (Array.isArray(e)) {
		let t = "[";
		for (let n = 0; n < e.length; n++) n && (t += ","), t += Fr(e[n]) || "null";
		return t + "]";
	}
...
function Lr(e) {
	return wr(Pr(Tr(e))).slice(0, 16);
}
function Rr({ source: e, context: t, id: n, maxChars: r, requiresReview: i, dataFormat: a }, o = Lr) {
	let s;
	return s = a === "JSX" ? Br(e) : e, o(Ir({
		source: s,
		...n && { id: n },
		...t && { context: t },
...
var zr = (e) => {
	if (e && typeof e == "object") {
		let t = {};
		if ("c" in e && e.c && (t.c = Br(e.c)), "d" in e) {
			let n = e?.d;
			n?.b && (t.b = Object.fromEntries(Object.entries(n.b).map(([e, t]) => [e, Br(t)]))), n?.t && (t.t = n.t);
		}
		return tn(e) ? {
			k: e.k,
			...e.v && { v: e.v }
...
function Br(e) {
	return Array.isArray(e) ? e.map(zr) : zr(e);
}
var I = "GT", Vr = (e) => S({
	source: I,
	severity: "Error",
...
async function ii(e) {
	if (!e.ok) {
		...
		throw new b(Hr(e.status, e.statusText, t), e.status, t);
	}
}
async function ai(e, t, n, r) {
	let i = Array.isArray(e), a = i ? [] : void 0, o = {}, s = i ? e.map((e) => [void 0, e]) : Object.entries(e);
	for (let [e, t] of s) {
		let { source: n, metadata: r } = typeof t == "string" ? { source: t } : t, i = e ?? r?.hash ?? Rr({
			source: n,
	...
	if (l.data === void 0 && l.response && !se(l)) throw await ii(l.response), l.error;
	let u = ce(l);
...
var oi = class {
	...
	setConfig({ apiKey: e, devApiKey: t, sourceLocale: n, targetLocale: r, locales: i, projectId: a, customMapping: o, baseUrl: s }) {
		if (e && (this.apiKey = e), t && (this.devApiKey = t), a && (this.projectId = a), n && (this.sourceLocale = fr(n), !M(this.sourceLocale, o))) throw Error(Kr(this.sourceLocale));
		if (r && (this.targetLocale = fr(r), !M(this.targetLocale, o))) throw Error(Kr(this.targetLocale));
		if (i) {
			let e = [], t = [];
			if (i.forEach((n) => {
				let r = fr(n);
	...
	requiresTranslation(e = this.sourceLocale, t = this.targetLocale, n = this.locales, r = this.customMapping) {
		...
		return r === this.customMapping ? this.localeConfig.requiresTranslation(t, e, n) : pr(e, t, n, r);
	}
	determineLocale(e, t = this.locales || [], n = this.customMapping) {
		return n === this.customMapping ? this.localeConfig.determineLocale(e, t ?? []) : mr(e, t, n);
	}
	...
	resolveCanonicalLocale(e = this.targetLocale, t = this.customMapping) {
		...
		return t === this.customMapping ? this.localeConfig.resolveCanonicalLocale(e) : dr(e, t);
	}
	resolveAliasLocale(e, t = this.customMapping) {
		if (!e) throw Error(L("resolveAliasLocale"));
		return t === this.customMapping ? this.localeConfig.resolveAliasLocale(e) : hr(e, t);
	}
...
function mi() {
	...
	return hi(e) ? e.isDebugLoggingEnabled() : li(ci());
}
function hi(e) {
...
function xi() {
	let e = Si(() => "production");
	return e ? e === "development" ? "development" : "production" : Si(() => !1) === !0 ? "development" : "production";
}
function Si(e) {
	try {
		return e();
...
var Di = class extends sr {
	...
	isDevHotReloadEnabled() {
		return !this.runtimeConfig._disableDevHotReload && !!this.runtimeConfig.devApiKey && !!this.runtimeConfig.projectId && this.runtimeConfig.runtimeUrl !== null && this.runtimeConfig.runtimeUrl !== "" && xi() === "development";
	}
	isGTServicesEnabled() {
	...
	getLocaleConfig(e) {
		return !e || !Ai(e) ? this : new sr(ki(e));
	}
...
var Fi = (e) => `String interpolation failed for message: "${e}".`;
function Ii(e, t, n, r) {
	try {
		return ur(e, {
			variables: t,
		...
		return z.warn(Fi(e)), e;
	}
}
function Li(e, t) {
	if (!e) return e;
	let n = t.$_fallback, r = Pi(t);
	try {
		let i = hn(n || "");
		return lr(Ii(Object.keys(i).length ? _n(e) : e, {
			...r,
			...i,
			[D]: "other"
		}, t.$locale, t.$format), { maxChars: t.$maxChars });
	} catch {
		return z.warn(Fi(e)), t.$_fallback == null ? lr(e, { maxChars: t.$maxChars }) : Li(t.$_fallback, {
			...t,
			$_fallback: void 0
		});
	}
...
function zi({ source: e, target: t, options: n, sourceLocale: r }) {
	return t == null ? Bi(e, Vi(n, r)) : Bi(t, {
		$_fallback: e,
		...n
	});
}
function Bi(e, t) {
	switch (t.$format ?? "STRING") {
		case "ICU": return Li(e, t);
		case "I18NEXT":
		case "STRING": return Ri(e, t);
		default: return e;
	}
}
function Vi(e, t) {
...
var { getConditionStore: Wi, setConditionStore: Gi } = Ui(S({
	source: "gt-i18n",
	severity: "Error",
...
async function Xi({ locale: e, enableI18n: t }, n) {
	let r = _i(), i = B().getDefaultLocale(), a = await r.getLookupTranslation(t ? e : i);
	return (n, r = {}) => {
		let o = Hi(t ? r.$locale ?? e : B().getDefaultLocale(), r, "ICU");
		return zi({
			source: n,
			target: a(n, o),
			options: o,
...
async function Zi({ locale: e, enableI18n: t }) {
	let n = await Xi({
		locale: e,
		enableI18n: t
	});
	return (e, t = {}) => {
		if (e == null) return e;
		let r = qi(e) ?? {};
		return Ji(r) ? n(r.$_source, r) : n(e, t);
	};
}
function Qi({ sourceLocale: e, targetLocale: t, sourceEntry: n, target: r, dictionaryOptions: i, options: a = {} }) {
	let o = Hi(t, {
		...i,
		...Pi(a)
	}, i.$format);
...
function aa(e) {
	return typeof e == "string" ? !0 : !Array.isArray(e) || typeof e[0] != "string" ? !1 : e.length === 1 || e.length === 2 && oa(e[1]);
}
function oa(e) {
	if (typeof e != "object" || !e || Array.isArray(e)) return !1;
	let t = e;
	return (t.$context === void 0 || typeof t.$context == "string") && (t.$format === void 0 || sa(t.$format)) && (t.$maxChars === void 0 || typeof t.$maxChars == "number");
}
function sa(e) {
...
function ua({ sourceObject: e, targetObject: t, translate: n }) {
	let r = W(t);
	if (r !== void 0) return r.entry;
	if (H(t)) return H(e) ? da({
		sourceObject: e,
		targetObject: t,
		translate: n
	}) : ua({
...
async function fa({ locale: e, enableI18n: t, rootId: n }) {
	let r = _i(), i = B().getDefaultLocale(), a = t ? e : i, [o, s, c] = await Promise.all([
...
function ga(e) {
	let { cacheUrl: t = ae, projectId: n, _versionId: r, _branchId: i } = e, a = r ? `/${r}` : "", o = i ? `?branchId=${i}` : "";
	return `${t}/${n}/[locale]` + a + o;
}
function _a({ type: e, remoteTranslationLoaderParams: t, loadTranslations: n }) {
	let { cacheUrl: r, projectId: i, _versionId: a, _branchId: o } = t;
	switch (e) {
		case "remote":
		case "gt-remote": return i ? ha({
			cacheUrl: r,
			projectId: i,
			_versionId: a,
			_branchId: o,
		...
		case "disabled": return r === null ? async () => ({}) : va(S({
...
var ba = class {
	...
	load(e) {
		return ya(this.pendingLoads, e, () => this.loadResource(e).then((t) => (this.set(e, t), t)));
	}
	getExpiresAt() {
...
function Ca(e) {
	return {
		maxConcurrentRequests: Sa(e?.maxConcurrentRequests, xa.maxConcurrentRequests, !0),
		maxBatchSize: Sa(e?.maxBatchSize, xa.maxBatchSize, !0),
		batchInterval: Sa(e?.batchInterval, xa.batchInterval)
	};
}
var wa = class {
	...
	async miss(e) {
		let t = this.getCacheKey(e), n = await ya(this.pendingTranslations, t, () => this.translate(e));
	...
	async sendBatchRequest(e) {
		this.activeRequests++;
		let t = Ta(e), n = await this.sendBatchRequestWithErrorHandling(e, t);
...
async function Ea({ key: e, sourceValue: t, targetValue: n, translateEntry: r }) {
	...
	let a = H(n) ? n : {}, o = /* @__PURE__ */ new Set([...Object.keys(t), ...Object.keys(a)]), s = await Promise.all(Array.from(o).map(async (n) => {
		...
		ea(n, i);
		let o = t[n];
		return o === void 0 ? [n, U(a[n])] : [n, await Ea({
			key: i,
			sourceValue: o,
			targetValue: a[n],
			translateEntry: r
...
var Oa = class {
	...
	getEntry(e) {
		let t = W(ta(this.cache, e));
		if (t !== void 0) return Da(t);
	}
	getValue(e) {
		let t = ta(this.cache, e);
	...
	async materializeValue(e, t, n = ta(this.cache, e)) {
		return ya(this.pendingMaterializations, e, () => Ea({
			key: e,
			sourceValue: t,
			targetValue: n,
			translateEntry: async (e, t) => ra(await this.materializeEntry(e, t))
		}).then((t) => (this.setValue(e, t), t)));
	}
	async materializeEntry(e, t) {
		return Da(await ya(this.pendingTranslations, e, () => this.runtimeTranslate(e, t).then((t) => {
			na(this.cache, e, t);
			let n = W(t);
...
function ka(e, t) {
	for (let [n, r] of Object.entries(t)) {
		let t = e[n];
		H(t) && H(r) ? ka(t, r) : e[n] = U(r);
	}
}
var Aa = 12e3, ja = class {
	constructor(e) {
		Ma(e), this.config = {
			projectId: e.projectId,
			devApiKey: e.devApiKey,
		...
		this.createTranslateMany = ma(B().getGTClass(), this.config.runtimeTranslation?.timeout ?? Aa, {
	...
	async translateDictionaryEntry(e, t, n) {
		let r = await this.lookupTranslationWithFallbackResolved(e, n.entry, ia(n.options));
...
function Na(e) {
	let t = "hash" in e ? e.hash : Ki(e.message, e.options);
	return `${e.locale}:${t}`;
}
var Pa = [];
function Fa({ locale: e, enableI18n: t, localesProp: n = Pa }) {
...
var La = "generaltranslation.locale-reset", Ra = "server-render", za = Symbol.for("generaltranslation.react-core.ReactI18nConfig"), Ba = class extends Di {
	renderStrategy;
	localeCookieName;
	regionCookieName;
	enableI18nCookieName;
	constructor(e = {}, t = Ra) {
		super(e), Ua(t), Object.defineProperty(this, za, { value: !0 }), this.renderStrategy = t, this.localeCookieName = e.localeCookieName ?? "generaltranslation.locale", this.regionCookieName = e.regionCookieName ?? "generaltranslation.region", this.enableI18nCookieName = e.enableI18nCookieName ?? "generaltranslation.enable-i18n";
	}
	getRenderStrategy() {
...
function Wa(e) {
	...
	let t = e;
	return t[za] === !0 && typeof t.getRenderStrategy == "function" && typeof t.getLocaleCookieName == "function" && typeof t.getRegionCookieName == "function" && typeof t.getEnableI18nCookieName == "function";
}
var { getConditionStore: J, setConditionStore: Ga, isConditionStoreInitialized: Ka } = Ui(S({
	source: "@generaltranslation/react-core",
	severity: "Error",
...
function qa({ Currency: e, GtInternalCurrency: t, DateTime: n, GtInternalDateTime: r, Num: i, GtInternalNum: a, RelativeTime: o, GtInternalRelativeTime: s, Var: c, GtInternalVar: l }) {
	return function({ variableType: u, variableValue: d, variableOptions: f, locales: p, enableI18n: h, injectionType: g }) {
		let _ = {
			_locale: p[0] ?? "en",
			_enableI18n: h
		};
		...
		if (u === "rt") {
			...
			if (typeof d == "number" && t?.unit) return m(e, {
				..._,
				value: d,
				unit: t.unit,
				baseDate: t?.baseDate,
...
function Ya(e = {}, t) {
	return typeof e.name == "string" ? e.name : `_gt_${Ja[t] || "value"}_${e["data-_gt"]?.id}`;
}
function Xa(e) {
...
function eo({ renderVariable: t }) {
	return function({ children: n, defaultLocale: r = "en", enableI18n: i }) {
		let a = (n) => {
			let a = Qa(n);
			if (Xa(n.props)) {
				let { variableType: e, variableValue: a, variableOptions: o, injectionType: s } = Za(n.props);
				return t({
					variableType: e,
					variableValue: a,
					variableOptions: o,
			...
			if (a?.transformation === "plural") {
				let e = a.branches || {};
				return typeof n.props.n == "number" ? s($a(n.props.n, [r], e) ?? n.props.children) : n.props.children == null ? null : s(n.props.children);
			}
	...
	function r({ sourceElement: t, targetElement: r, locales: a = ["en"], enableI18n: o }) {
		...
		if (u && Object.entries(vn).forEach(([e, t]) => {
			...
			return typeof e == "number" ? i({
				source: $a(e, a, c.branches || {}) ?? t.props.children,
				target: $a(e, a, r.d?.b || {}) ?? r.c,
				locales: a,
				enableI18n: o
			}) : n({
				children: t,
	...
	function i({ source: i, target: a, locales: o = ["en"], enableI18n: s }) {
		...
		if (Array.isArray(a) && !Array.isArray(i) && i && (i = [i]), Array.isArray(i) && Array.isArray(a)) {
			let n = {}, c = {}, l = {}, u = i.filter((t) => {
				if (e.isValidElement(t)) {
					if (Xa(t.props)) {
						let { variableName: e, variableValue: r, variableOptions: i, injectionType: a } = Za(t.props);
						n[e] = r, c[e] = i, l[e] = a;
					} else return !0;
				}
				return !1;
			...
			return a.map((i, a) => {
				if (typeof i == "string") return m(e.Fragment, { children: i }, `string_${a}`);
				if (tn(i)) return m(e.Fragment, { children: t({
					variableType: i.v || "v",
					variableValue: n[i.k],
				...
				let u = d(i);
				return u ? m(e.Fragment, { children: r({
					sourceElement: u,
					targetElement: i,
					locales: o,
					enableI18n: s
		...
		if (a && typeof a == "object" && !Array.isArray(a)) {
			...
			if (e.isValidElement(i)) {
				...
				if (Xa(i.props)) {
					let { variableValue: e, variableOptions: n, variableType: r, injectionType: a } = Za(i.props);
					return t({
						variableType: r,
						variableValue: e,
						variableOptions: n,
...
var io = { display: "contents" }, ao = globalThis.navigator?.product === "ReactNative";
function oo(e) {
	if (e == null || typeof e == "boolean" || e === "") return !0;
	if (Array.isArray(e)) return !e.some((e) => !oo(e));
	if (o(e) && e.type === n) {
...
function so(e, t) {
	return ao || !no() ? e : o(e) && typeof e.type == "string" ? r(e, { "data-_gt-hash": t }) : oo(e) ? e : a("span", {
		"data-_gt-hash": t,
		style: io
	}, e);
}
function co({ renderDefaultChildren: e, renderTranslatedChildren: t }) {
...
function lo(e) {
	let t = qa(e), n = eo({ renderVariable: t }), r = to({ renderVariable: t });
	return {
		renderVariable: t,
...
var uo = class extends ja {};
function fo(e) {
	Ha(e, "server-render"), Ia(new uo(e));
}
var po = R({
	namespace: "reactCore",
	key: "i18nStore",
...
function _o(e, t) {
	let n = t.options.$_hash ?? Ki(t.message, t.options);
	return e?.[t.locale]?.[n];
}
function vo(e, t) {
	return W(bo(e, t));
...
function bo(e, { locale: t, id: n }) {
	let r = e?.[t];
	if (!r) return;
	if (!n) return r;
	let i = r;
	for (let e of n.split(".")) {
		if (!xo(e) || !H(i) || !Object.prototype.hasOwnProperty.call(i, e)) return;
		i = i[e];
	}
	return i;
}
...
var Co = class {
	...
	subscribeToTranslate = (e, t) => {
		let n = Na(e);
		return go(this.translateListeners, (e) => {
			Na(e) === n && t();
		});
	};
	subscribeToTranslationEvents = (e) => go(this.translateListeners, e);
	subscribeToDictionaryEntryEvents = (e) => go(this.dictionaryEntryListeners, e);
	subscribeToDictionaryObjectEvents = (e) => go(this.dictionaryObjectListeners, e);
	getTranslateSnapshot = (e, t = {}) => _o(t, e) ?? K().lookupTranslation(e.locale, e.message, e.options);
	getDictionaryEntrySnapshot = (e, t = {}) => vo(t, e) ?? K().lookupDictionary(e.locale, e.id);
	getDictionaryObjectSnapshot = (e, t = {}) => yo(t, e) ?? K().lookupDictionaryObj(e.locale, e.id);
...
function To() {
	return wo.isInitialized() || wo.set(i(void 0)), wo.get();
}
function Eo() {
	let e = s(To());
...
function Oo() {
	return Eo()?.conditionStore ?? J();
}
function Y() {
	return Oo().getLocale();
}
function X() {
	return Oo().getEnableI18n();
}
function ko() {
	return u(() => B().getDefaultLocale(), []);
}
function Ao() {
	return Eo()?.i18nStore || mo();
}
function jo() {
	return Eo()?.translationsSnapshot || {};
}
var Mo = () => {};
function No() {
	return Mo;
...
Io._gtt = "variable-currency-automatic", Lo._gtt = "variable-currency";
...
zo._gtt = "variable-datetime-automatic", Bo._gtt = "variable-datetime";
...
Ho._gtt = "variable-number-automatic", Uo._gtt = "variable-number";
function Wo({ _enableI18n: e, _locale: t, date: n, children: r, value: i, unit: a, baseDate: o, locales: s = [], options: c = {} }) {
	let l = Fa({
		locale: t,
		enableI18n: e,
		localesProp: s
...
Go._gtt = "variable-relative-time-automatic", Ko._gtt = "variable-relative-time";
function qo({ children: e }) {
	return e;
}
function Jo({ children: e }) {
	return qo({ children: e });
}
function Yo({ children: e }) {
	return qo({ children: e });
}
Jo._gtt = "variable-variable", Yo._gtt = "variable-variable-automatic";
function Xo(e) {
	let t = Ao(), n = jo();
	return Po(), f((n) => t.subscribeToTranslate(e, n), () => t.getTranslateSnapshot(e, n), () => t.getTranslateSnapshot(e, n));
}
var { renderVariable: Zo, renderDefaultChildren: Qo, renderTranslatedChildren: $o, renderPreparedT: es } = lo({
	Currency: Lo,
	GtInternalCurrency: Io,
	DateTime: Bo,
	GtInternalDateTime: zo,
	Num: Uo,
	GtInternalNum: Ho,
	RelativeTime: Ko,
	GtInternalRelativeTime: Go,
	Var: Jo,
	GtInternalVar: Yo
});
function ts(t, n = 0) {
	let r = n, i = (e) => {
		...
		if (a) {
			...
			if ((e[1] === "automatic" || e[2] === "automatic") && (i.injectionType = "automatic"), e[0] === "translate" && (e[0] = "fragment"), e[0] === "variable" && (i.variableType = e?.[1] || "variable"), e[0] === "plural") {
				let e = Object.entries(n).reduce((e, [t, n]) => (un(t) && (e[t] = ts(n, r)), e), {});
				Object.keys(e).length && (i.branches = e);
			}
			if (e[0] === "branch") {
				let { children: e, branch: t, ...a } = n, o = Object.fromEntries(Object.entries(a).filter(([e]) => !e.startsWith("data-"))), s = Object.entries(o).reduce((e, [t, n]) => (e[t] = ts(n, r), e), {});
...
function rs(e, t) {
	let { type: n, props: i } = e, a = os(n);
	if (typeof i != "object" || !i) return e;
	if (a) {
		let { componentType: n, injectionType: o } = a;
		if (n === "variable") return e;
		if (n === "branch") return r(e, { ...Object.entries(i).reduce((e, [n, r]) => (e[n] = n !== "branch" && !n.startsWith("data-") ? is(r, t) : r, e), {}) });
		if (n === "plural") return r(e, { ...Object.entries(i).reduce((e, [n, r]) => (e[n] = un(n) || n === "children" ? is(r, t) : r, e), {}) });
		if (n === "derive") return r(e, {
			...i,
			..."children" in i && { children: as(i.children, t + 1) }
		});
		if (n === "translate" && o === "automatic" && t > 0) return "children" in i ? as(i.children, t) : void 0;
		n === "translate" && o === "automatic" && console.warn(ss);
	}
	return r(e, {
		...i,
		..."children" in i && { children: as(i.children, t) }
	});
}
function is(e, t) {
	return o(e) ? rs(e, t) : e;
...
function as(e, n) {
	return Array.isArray(e) ? t.map(e, (e) => is(e, n)) : is(e, n);
}
function os(e) {
...
var ss = "'@generaltranslation/react-core Warning: A <_T> component was found injected outside of a <Derive> boundary. This may affect translation resolution for this component.";
function cs(t) {
...
var ls = (e) => {
	...
	let { props: t } = e, n = { t: ls(e) };
	if (t["data-_gt"]) {
		let e = t["data-_gt"], r = e.transformation;
		if (r === "variable") {
			let n = e.variableType || "variable", r = Ya(t, n), i = pn(n);
			return {
				i: e.id,
				k: r,
				v: i
		...
		n.i = e.id, n.d = us(r, t, e.branches);
	}
	return t.children && (n.c = ps(t.children)), n;
}, fs = (e) => cs(e) ? ds(e) : typeof e == "number" ? e.toString() : e;
function ps(e) {
	return Array.isArray(e) ? e.map(fs) : fs(e);
}
function ms({ sourceChildren: e, params: t, locale: n }) {
	let r = hs(e), i = gs(r), a = _s({
		options: vs(t),
		locale: n
	});
	return ro(i, a), {
...
function _s({ options: e, locale: t }) {
	return {
		...e,
		$locale: t
	};
...
function ys({ sourceChildren: e, params: t, _locale: n, _enableI18n: r }) {
	let i = Y(), a = X(), o = ko(), s = n ?? i, c = r ?? a;
	return {
		defaultLocale: o,
		enableI18n: c,
		locale: s,
...
bs._gtt = "translate-client";
function xs({ children: e, _locale: t, _enableI18n: n, _renderPreparedT: r = es, ...i }) {
	let { defaultLocale: a, locale: o, enableI18n: s, targetOptions: c, taggedSourceChildren: l, sourceJsxChildren: u, shouldTranslate: f } = ys({
		sourceChildren: e,
		params: i,
		_locale: t,
		_enableI18n: n
...
var Ts = class {
	constructor(e) {
		this.getLocale = () => Es(this.customGetLocale), this.setLocale = (e) => {
...
function Es(e) {
	let t = q(), n = ws(t.getLocaleCookieName());
...
function As({ source: e, severity: t, whatHappened: n, reassurance: r, why: i, fix: a, wayOut: o, details: s, docsUrl: c }) {
	let l = e ? t ? `${e} ${t}:` : `${e}:` : t ? `${t}:` : "", u = i ? `${Os(n)} because ${Os(i)}` : n, d = !!a && !!o && /^[a-z]/.test(Os(o)), f = [
		u,
		r,
		d ? `${Os(a)}, or ${Os(o)}` : a,
		d ? void 0 : o,
...
}), { setConditionStore: Ms, isConditionStoreInitialized: Ns } = Ui(js), { getConditionStore: Ps, setConditionStore: Fs, isConditionStoreInitialized: Is } = Ui(js);
function Ls(e) {
	let t = Rs(e), n = zs(e), r = Bs(e);
	if (Is()) {
		let e = Ps();
...
function Rs({ _getLocale: e, locale: t }) {
	let n = q(), r = [];
	return t && r.push(...Array.isArray(t) ? t : [t]), e && r.push(e()), r.push(...ws(n.getLocaleCookieName())), n.resolveSupportedLocale(r);
}
function zs({ _getRegion: e, region: t }) {
	return Z({ cookieName: q().getRegionCookieName() }) || e?.() || t;
}
function Bs({ enableI18n: e, _getEnableI18n: t }) {
...
function Hs() {
	return {
		projectId: Us(() => void 0) || Ws(),
		devApiKey: xi() === "development" ? Us(() => void 0) || Gs() : void 0
	};
}
function Us(e) {
	try {
...
function Ws() {
	try {
		return Ks(process.env.VITE_GT_PROJECT_ID);
	} catch {
		return;
	}
}
function Gs() {
	try {
		return Ks(process.env.VITE_GT_DEV_API_KEY);
	} catch {
		return;
	}
}
...
function Xs() {
	return Ys("AboutHeader"), h(p, { children: [m("h1", {
		className: "mb-4 text-3xl font-bold text-foreground",
		children: m(bs, { children: "About This Benchmark" })
	}), m("p", {
		className: "mb-8 max-w-3xl text-muted-foreground",
		children: m(bs, { children: "This is an open-source test application — not a product or a company. Its sole purpose is to provide a realistic, multi-page React app where different i18n libraries can be integrated and measured under identical conditions." })
	})] });
}
function Zs(e) {
	let t = e.trim();
...
var dc = g().server(() => $().getLocale()).client(() => J().getLocale());
g().server(() => $().getEnableI18n()).client(() => J().getEnableI18n()), g().server((e) => {
	let t = $();
	return Xi({
		locale: t.getLocale(),
...
_().server(({ request: e, pathname: t, next: n }) => $().run(e, () => n(), t));
function mc(e) {
