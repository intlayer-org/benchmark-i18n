...
var _t = /[\t-\r \x85\u200E\u200F\u2028\u2029]/u, vt = /[\t-\r \x85\xA0\u1680\u2000-\u200A\u2028\u2029\u202F\u205F\u3000\x21-\x2F\x3A-\x40\x5B-\x5E\x60\x7B-\x7E\xA1-\xA7\xA9\xAB\xAC\xAE\xB0\xB1\xB6\xBB\xBF\xD7\xF7\u2010-\u2027\u2030-\u203E\u2041-\u2053\u2055-\u205E\u2190-\u245F\u2500-\u2775\u2794-\u2BFF\u2E00-\u2E7F\u3001-\u3003\u3008-\u3020\u3030\uFD3E-\uFD3F\uFE45-\uFE46]/u, yt = class {
	constructor(e, t) {
	...
	readTagName() {
		let e = this.index;
		for (T(this.current()) || this.fail("INVALID_TAG", e), this.index += 1; !this.atEnd() && gt(this.current());) this.index += this.current().length;
	...
	current() {
		return xt(this.message, this.index);
	}
	peek() {
		let e = this.current();
		return xt(this.message, this.index + e.length);
	}
	atEnd() {
	...
	position(e) {
		let t = this.positions ??= bt(this.message);
		return {
			offset: e,
			line: t[0][e],
			column: t[1][e]
...
function Dt(e, t = "en", n = {}) {
	let r = Ot(St(e, { locale: Rt(t) }), {
		message: e,
		locales: t,
		variables: n
	});
...
function Ot(e, t, n) {
	...
	for (let r of e) switch (r.type) {
		...
		case C.number: {
			let e = E(t.variables, r.value), { scale: n, ...a } = typeof r.style == "string" ? Ct[r.style] ?? {} : r.style?.type === w.number ? r.style.parsedOptions : {}, o = kt(e, n);
			i(At(t, a).format(o));
			break;
		}
		case C.date:
		case C.time: {
			let e = E(t.variables, r.value), n = r.type === C.date ? wt : Et;
			i(jt(t, typeof r.style == "string" ? n[r.style] : r.style?.type === w.dateTime ? r.style.parsedOptions : r.type === C.time ? Et.medium : void 0).format(e));
			break;
		}
		case C.select: {
			let e = String(E(t.variables, r.value)), n = It(r.options, e) ?? r.options.other;
			if (!n) throw Lt(r.value, e, r.options);
			Ot(n.value, t).forEach(i);
			break;
		}
		case C.plural: {
			let e = E(t.variables, r.value), n = `=${String(e)}`, a = It(r.options, n), o = typeof e == "bigint" ? e : Number(e), s = typeof o == "bigint" ? o - BigInt(r.offset) : o - r.offset;
			if (!a && Ft(r.options)) {
				let e = Mt(t, r.pluralType ?? "cardinal").select(Pt(s));
				a = It(r.options, e);
			}
			if (a ??= r.options.other, !a) throw Lt(r.value, e, r.options);
			Ot(a.value, t, s).forEach(i);
			break;
		}
		case C.tag: {
			let e = E(t.variables, r.value);
			if (typeof e != "function") throw TypeError(`The ICU tag variable "${r.value}" must be a function.`);
			let a = e(Ot(r.children, t, n));
...
function At(e, t = {}) {
	return Nt(e.numberFormats ??= /* @__PURE__ */ new Map(), JSON.stringify(t), () => new Intl.NumberFormat(e.locales, t));
}
function jt(e, t) {
	return Nt(e.dateTimeFormats ??= /* @__PURE__ */ new Map(), t ? JSON.stringify(t) : "", () => new Intl.DateTimeFormat(e.locales, t));
}
function Mt(e, t) {
	...
	return Nt(e.pluralRules ??= /* @__PURE__ */ new Map(), t, () => new Intl.PluralRules(e.locales, { type: t }));
}
function Nt(e, t, n) {
	let r = e.get(t);
	if (r) return r;
...
function Bt(e, t, n) {
	return e.map((r, i) => {
		switch (r.type) {
			case C.literal: return Ut(r, t, i > 0, i < e.length - 1 || n);
			case C.argument: return `{${r.value}}`;
			case C.date:
			case C.time:
			case C.number: return Gt(r);
			case C.select: return `{${r.value},select,${qt(r.options, !1)}}`;
			case C.plural: {
				let e = r.pluralType === "cardinal" ? "plural" : "selectordinal", t = r.offset ? `offset:${r.offset} ` : "";
				return `{${r.value},${e},${t}${qt(r.options, !0)}}`;
			}
			case C.pound: return "#";
			case C.tag: return `<${r.value}>${Bt(r.children, t, !0)}</${r.value}>`;
		}
	}).join("");
}
...
function Ht(e, t, n, r) {
	...
	let o = "", s = 0;
	for (let [r, a] of i) o += Vt(e.slice(s, r), t, s !== 0, s === 0 && n, !0), o += `'${e.slice(r, a).replace(/'/g, "''")}'`, s = a;
	return o += Vt(e.slice(s), t, s !== 0, s === 0 && n, r), o;
}
function Ut({ value: e }, t, n, r) {
	return Wt(e) ? e : Ht(e, t, n, r);
}
function Wt(e) {
	if (e[0] !== "<" || e.slice(-2) !== "/>") return !1;
	let t = e.slice(1, -2);
	if (!T(t[0])) return !1;
	for (let e of t.slice(1)) if (!gt(e)) return !1;
	return !0;
}
function Gt(e) {
	let t = e.type === C.number ? "number" : e.type === C.date ? "date" : "time";
	return `{${e.value}, ${t}${e.style ? `, ${Kt(e.style)}` : ""}}`;
}
function Kt(e) {
	return typeof e == "string" ? e : e.type === w.dateTime ? `::${e.pattern}` : `::${e.tokens.map(({ stem: e, options: t }) => e + t.map((e) => `/${e}`).join("")).join(" ")}`;
}
function qt(e, t) {
	return Object.entries(e).map(([e, n]) => `${e}{${Bt(n.value, t, !0)}}`).join(" ");
}
function Jt(e) {
...
function Yt({ icuString: e, shouldVisit: t, visitor: n, options: { recurseIntoVisited: r = !0, ...i } }) {
	let a = St(e, i);
	return o(a), a;
	function o(e) {
		e.map(s);
...
var D = "_gt_", Xt = RegExp(`^${D}\\d+$`), Zt = RegExp(`^${D}$`);
function Qt(e) {
...
var tn = (e) => `generaltranslation Formatting Error: Invalid cutoff style: ${e}.`, nn = "DEFAULT_TERMINATOR_KEY", rn = {
	ellipsis: {
		fr: {
			terminator: "…",
			separator: " "
	...
	constructor(t, n = {}) {
		...
		if (!rn[r]) throw Error(tn(r));
		let i = n.maxChars === void 0 ? void 0 : rn[r][new Intl.Locale(this.locale).language] || rn[r].DEFAULT_TERMINATOR_KEY, a = n.terminator ?? i?.terminator, o = a == null ? void 0 : n.separator ?? i?.separator;
	...
	get(e, ...t) {
		...
		let o = a[i];
		return o === void 0 && (o = new on[e](...t), a[i] = o), o;
	}
}();
function sn(e) {
...
function un(e, t = cn, n = ["en"]) {
	let r = sn(n).select(e), i = Math.abs(e);
...
function vn(e, t = "en", n = {}) {
	return Dt(e, t, n)?.toString() ?? "";
}
...
function xn({ value: e, locales: t = ["en"], currency: n = "USD", options: r = {} }) {
	return O.get("NumberFormat", t, {
		style: "currency",
		currency: n,
		numberingSystem: "latn",
...
function Tn({ value: e, unit: t, locales: n = ["en"], options: r = {} }) {
...
var kn = (e, t, n) => {
	...
	"Toto",
	"Thai"
]), jn = (e) => e >= "qaa" && e <= "qtz", A = (e, t) => {
	e = k(t, e) || e;
	try {
		let { language: t, region: n, script: r } = O.get("Locale", e), i = 1 + Number(!!n) + Number(!!r);
		return !(e.split("-").length !== i || O.get("DisplayNames", ["en"], { type: "language" }).of(t) === t && !jn(t) || n && O.get("DisplayNames", ["en"], { type: "region" }).of(n) === n || r && O.get("DisplayNames", ["en"], { type: "script" }).of(r) === r && !An.has(r));
	} catch {
		return !1;
	}
}, j = (e) => {
...
function Pn(e, t, n, r) {
	if (n && !n.allValid || !A(e, r) || !A(t, r) || Nn(e, t)) return !1;
	if (!n) return !0;
	let i = En(t);
	return i !== void 0 && n.languages.has(i);
}
function Fn(e, t, n, r) {
	return Pn(e, t, n ? Mn(n, r) : void 0, r);
}
function In(e) {
	try {
...
function Ln(e, t) {
	if (t.has(e)) return e;
	let { languageCode: n, regionCode: r, scriptCode: i, minimizedCode: a } = In(e), o = `${n}-${r}`;
	if (t.has(o)) return o;
	let s = `${n}-${i}`;
	if (t.has(s)) return s;
	if (t.has(a)) return a;
...
function Qn(e, t = "en", n) {
	...
	e = Bn(e, n), t ||= "en";
	try {
		let i = j(e), a = O.get("Locale", e), o = a.language, s = Zn([
			r,
			e,
			i,
			o
			...
			t,
			"en"
		], g = O.get("DisplayNames", m, { type: "language" }), _ = O.get("DisplayNames", h, { type: "language" }), v = s?.name, y = s?.nativeName || s?.name, b = v || g.of(e) || e, ee = y || _.of(e) || e, te = s?.maximizedName || v || g.of(u) || e, x = s?.nativeMaximizedName || y || _.of(u) || e, ne = s?.minimizedName || v || g.of(p) || e, re = s?.nativeMinimizedName || y || _.of(p) || e, S = s?.languageName || v || g.of(o) || e, ie = s?.nativeLanguageName || y || _.of(o) || e, ae = s?.nameWithRegionCode || (c ? `${S} (${c})` : b), oe = s?.nativeNameWithRegionCode || (c ? `${ie} (${c})` : ee) || ae, se = O.get("DisplayNames", m, { type: "region" }), ce = O.get("DisplayNames", h, { type: "region" }), le = s?.regionName || (d ? se.of(d) : "") || "", ue = s?.nativeRegionName || (d ? ce.of(d) : "") || "", de = O.get("DisplayNames", m, { type: "script" }), fe = O.get("DisplayNames", h, { type: "script" });
		return {
			code: i,
			name: b,
			nativeName: ee,
		...
		let t = A(e) ? j(e) : e, r = t.split("-"), i = r[0] || t, a = r.length > 2 ? r[2] : r[1] || "", o = r[3] || "", s = Zn([t, i], n);
		t = s?.code || t;
...
var or = class {
	...
	buildResolutionScope(e) {
		...
		return {
			...
			approved: Mn(t.map(({ canonicalLocale: e }) => e), this.customMapping)
		};
	}
	...
	formatRelativeTimeFromDate(e, t, n = {}) {
		let { locales: r, baseDate: i, ...a } = n, { value: o, unit: s } = wn(e, i ?? /* @__PURE__ */ new Date());
		return Tn({
			value: o,
			unit: s,
			locales: this.getFormattingLocales(t, r),
	...
	formatMessage(e, t, n = {}) {
		...
		return a === "STRING" ? e : vn(e, this.getFormattingLocales(t, r), i);
	}
	formatList(e, t, n = {}) {
		let { locales: r, ...i } = n;
		return Sn({
	...
	requiresTranslation(e, t = this.defaultLocale, n = this.locales.length ? this.locales : void 0) {
		let r = n ? n === this.locales ? this.getResolutionScope().approved : Mn(n.map((e) => this.resolveCanonicalLocale(e)), this.customMapping) : void 0;
		return Pn(this.resolveCanonicalLocale(t), this.resolveCanonicalLocale(e), r, this.customMapping);
	}
	determineLocale(e, t = this.locales) {
		let { approvedLocalePairs: n, approved: r } = t === this.locales ? this.getResolutionScope() : this.buildResolutionScope(t), i = Rn(Array.isArray(e) ? e.map((e) => this.resolveCanonicalLocale(e)) : this.resolveCanonicalLocale(e), r, this.customMapping);
	...
	resolveCanonicalLocale(e) {
		return Bn(e, this.customMapping);
	}
	...
	isSameDialect(...e) {
		return Nn(...e.map((e) => Array.isArray(e) ? e.map((e) => this.resolveCanonicalLocale(e)) : this.resolveCanonicalLocale(e)));
	}
	isSameLanguage(...e) {
...
function lr(e, t) {
	return t?.dataFormat === "STRING" ? e : vn(e, t?.locales, t?.variables);
}
function M(e, t) {
	return A(e, t);
}
...
var xr = typeof Uint8Array.from([]).toHex == "function" && typeof Uint8Array.fromHex == "function", Sr = Array.from({ length: 256 }, (e, t) => t.toString(16).padStart(2, "0"));
function Cr(e) {
	if (gr(e), xr) return e.toHex();
	let t = "";
	for (let n = 0; n < e.length; n++) t += Sr[e[n]];
	return t;
}
function wr(e) {
...
var kr = class {
	...
	3204031479,
	3329325298
]), F = /* @__PURE__ */ new Uint32Array(64), jr = class extends kr {
	constructor(e) {
		super(64, e, 8, !1);
	}
	get() {
	...
	process(e, t) {
		...
		for (let e = 0; e < 64; e++) {
			let t = N(o, 6) ^ N(o, 11) ^ N(o, 25), u = l + t + Dr(o, s, c) + Ar[e] + F[e] | 0, d = (N(n, 2) ^ N(n, 13) ^ N(n, 22)) + Or(n, r, i) | 0;
...
async function ri(e) {
	if (!e.ok) {
		...
		throw new b(Vr(e.status, e.statusText, t), e.status, t);
	}
}
async function ii(e, t, n, r) {
	...
	if (l.data === void 0 && l.response && !oe(l)) throw await ri(l.response), l.error;
	let u = se(l);
...
var ai = class {
	...
	setConfig({ apiKey: e, devApiKey: t, sourceLocale: n, targetLocale: r, locales: i, projectId: a, customMapping: o, baseUrl: s }) {
		if (e && (this.apiKey = e), t && (this.devApiKey = t), a && (this.projectId = a), n && (this.sourceLocale = dr(n), !M(this.sourceLocale, o))) throw Error(Gr(this.sourceLocale));
		if (r && (this.targetLocale = dr(r), !M(this.targetLocale, o))) throw Error(Gr(this.targetLocale));
		if (i) {
			let e = [], t = [];
			if (i.forEach((n) => {
				let r = dr(n);
...
function pi() {
	...
	return mi(e) ? e.isDebugLoggingEnabled() : ci(si());
}
function mi(e) {
...
function bi() {
	let e = xi(() => "production");
	return e ? e === "development" ? "development" : "production" : xi(() => !1) === !0 ? "development" : "production";
}
function xi(e) {
	try {
		return e();
...
var Ei = class extends or {
	...
	getLocaleConfig(e) {
		return !e || !ki(e) ? this : new or(Oi(e));
	}
...
var Pi = (e) => `String interpolation failed for message: "${e}".`;
function Fi(e, t, n, r) {
	try {
		return lr(e, {
			variables: t,
		...
		return z.warn(Pi(e)), e;
	}
}
function Ii(e, t) {
	if (!e) return e;
	let n = t.$_fallback, r = Ni(t);
	try {
		let i = mn(n || "");
		return cr(Fi(Object.keys(i).length ? gn(e) : e, {
			...r,
			...i,
			[D]: "other"
		}, t.$locale, t.$format), { maxChars: t.$maxChars });
	} catch {
		return z.warn(Pi(e)), t.$_fallback == null ? cr(e, { maxChars: t.$maxChars }) : Ii(t.$_fallback, {
			...t,
			$_fallback: void 0
		});
	}
...
function Ri({ source: e, target: t, options: n, sourceLocale: r }) {
	return t == null ? zi(e, Bi(n, r)) : zi(t, {
		$_fallback: e,
		...n
	});
}
function zi(e, t) {
	switch (t.$format ?? "STRING") {
		case "ICU": return Ii(e, t);
		case "I18NEXT":
		case "STRING": return Li(e, t);
		default: return e;
	}
}
function Bi(e, t) {
...
var { getConditionStore: Ui, setConditionStore: Wi } = Hi(S({
	source: "gt-i18n",
	severity: "Error",
...
async function Yi({ locale: e, enableI18n: t }, n) {
	let r = gi(), i = B().getDefaultLocale(), a = await r.getLookupTranslation(t ? e : i);
	return (n, r = {}) => {
		let o = Vi(t ? r.$locale ?? e : B().getDefaultLocale(), r, "ICU");
		return Ri({
			source: n,
			target: a(n, o),
			options: o,
...
async function Xi({ locale: e, enableI18n: t }) {
	let n = await Yi({
		locale: e,
		enableI18n: t
	});
	return (e, t = {}) => {
		if (e == null) return e;
		let r = Ki(e) ?? {};
		return qi(r) ? n(r.$_source, r) : n(e, t);
	};
}
...
function ia(e) {
	return typeof e == "string" ? !0 : !Array.isArray(e) || typeof e[0] != "string" ? !1 : e.length === 1 || e.length === 2 && aa(e[1]);
}
function aa(e) {
	if (typeof e != "object" || !e || Array.isArray(e)) return !1;
	let t = e;
	return (t.$context === void 0 || typeof t.$context == "string") && (t.$format === void 0 || oa(t.$format)) && (t.$maxChars === void 0 || typeof t.$maxChars == "number");
}
function oa(e) {
...
function la({ sourceObject: e, targetObject: t, translate: n }) {
	let r = W(t);
	if (r !== void 0) return r.entry;
	if (H(t)) return H(e) ? ua({
		sourceObject: e,
		targetObject: t,
		translate: n
	}) : la({
...
async function da({ locale: e, enableI18n: t, rootId: n }) {
	let r = gi(), i = B().getDefaultLocale(), a = t ? e : i, [o, s, c] = await Promise.all([
...
function ga({ type: e, remoteTranslationLoaderParams: t, loadTranslations: n }) {
	let { cacheUrl: r, projectId: i, _versionId: a, _branchId: o } = t;
	switch (e) {
		case "remote":
		case "gt-remote": return i ? ma({
			cacheUrl: r,
			projectId: i,
			_versionId: a,
			_branchId: o,
		...
		case "disabled": return r === null ? async () => ({}) : _a(S({
...
var ya = class {
	...
	load(e) {
		return va(this.pendingLoads, e, () => this.loadResource(e).then((t) => (this.set(e, t), t)));
	}
	getExpiresAt() {
...
function Sa(e) {
	return {
		maxConcurrentRequests: xa(e?.maxConcurrentRequests, ba.maxConcurrentRequests, !0),
		maxBatchSize: xa(e?.maxBatchSize, ba.maxBatchSize, !0),
		batchInterval: xa(e?.batchInterval, ba.batchInterval)
	};
}
var Ca = class {
	constructor({ init: e, translateMany: t, onMiss: n, batchConfig: r }) {
		this.pendingTranslations = /* @__PURE__ */ new Map(), this.queue = [], this.batchTimer = null, this.activeRequests = 0, this.cache = structuredClone(e), this.translateMany = t, this.batchConfig = Sa(r), this.onMiss = n;
	}
	get(e) {
	...
	async miss(e) {
		let t = this.getCacheKey(e), n = await va(this.pendingTranslations, t, () => this.translate(e));
	...
	async sendBatchRequest(e) {
		this.activeRequests++;
		let t = wa(e), n = await this.sendBatchRequestWithErrorHandling(e, t);
...
async function Ta({ key: e, sourceValue: t, targetValue: n, translateEntry: r }) {
	...
	let a = H(n) ? n : {}, o = /* @__PURE__ */ new Set([...Object.keys(t), ...Object.keys(a)]), s = await Promise.all(Array.from(o).map(async (n) => {
		...
		$i(n, i);
		let o = t[n];
		return o === void 0 ? [n, U(a[n])] : [n, await Ta({
			key: i,
			sourceValue: o,
			targetValue: a[n],
			translateEntry: r
...
var Da = class {
	...
	getEntry(e) {
		let t = W(ea(this.cache, e));
		if (t !== void 0) return Ea(t);
	}
	getValue(e) {
		let t = ea(this.cache, e);
	...
	async materializeValue(e, t, n = ea(this.cache, e)) {
		return va(this.pendingMaterializations, e, () => Ta({
			key: e,
			sourceValue: t,
			targetValue: n,
			translateEntry: async (e, t) => na(await this.materializeEntry(e, t))
		}).then((t) => (this.setValue(e, t), t)));
	}
	async materializeEntry(e, t) {
		return Ea(await va(this.pendingTranslations, e, () => this.runtimeTranslate(e, t).then((t) => {
			ta(this.cache, e, t);
			let n = W(t);
...
function Oa(e, t) {
	for (let [n, r] of Object.entries(t)) {
		let t = e[n];
		H(t) && H(r) ? Oa(t, r) : e[n] = U(r);
	}
}
var ka = 12e3, Aa = class {
	constructor(e) {
		ja(e), this.config = {
			projectId: e.projectId,
			devApiKey: e.devApiKey,
		...
		this.createTranslateMany = pa(B().getGTClass(), this.config.runtimeTranslation?.timeout ?? ka, {
	...
	async translateDictionaryEntry(e, t, n) {
		let r = await this.lookupTranslationWithFallbackResolved(e, n.entry, ra(n.options));
...
function Ma(e) {
	let t = "hash" in e ? e.hash : Gi(e.message, e.options);
	return `${e.locale}:${t}`;
}
var Na = [];
function Pa({ locale: e, enableI18n: t, localesProp: n = Na }) {
...
var Ia = "generaltranslation.locale-reset", La = "server-render", Ra = Symbol.for("generaltranslation.react-core.ReactI18nConfig"), za = class extends Ei {
	renderStrategy;
	localeCookieName;
	regionCookieName;
	enableI18nCookieName;
	constructor(e = {}, t = La) {
		super(e), Ha(t), Object.defineProperty(this, Ra, { value: !0 }), this.renderStrategy = t, this.localeCookieName = e.localeCookieName ?? "generaltranslation.locale", this.regionCookieName = e.regionCookieName ?? "generaltranslation.region", this.enableI18nCookieName = e.enableI18nCookieName ?? "generaltranslation.enable-i18n";
	}
	getRenderStrategy() {
...
function Ua(e) {
	...
	let t = e;
	return t[Ra] === !0 && typeof t.getRenderStrategy == "function" && typeof t.getLocaleCookieName == "function" && typeof t.getRegionCookieName == "function" && typeof t.getEnableI18nCookieName == "function";
}
var { getConditionStore: J, setConditionStore: Wa, isConditionStoreInitialized: Ga } = Hi(S({
	source: "@generaltranslation/react-core",
	severity: "Error",
...
function Ka({ Currency: e, GtInternalCurrency: t, DateTime: n, GtInternalDateTime: r, Num: i, GtInternalNum: a, RelativeTime: o, GtInternalRelativeTime: s, Var: c, GtInternalVar: l }) {
...
function Ja(e = {}, t) {
	return typeof e.name == "string" ? e.name : `_gt_${qa[t] || "value"}_${e["data-_gt"]?.id}`;
}
function Ya(e) {
...
function Qa(e, t, n) {
	let r = "", i = null;
	return typeof e == "number" && !i && n && (r = un(e, Object.keys(n).filter(ln), t)), r && !i && (i = n[r]), i;
}
function $a({ renderVariable: t }) {
	return function({ children: n, defaultLocale: r = "en", enableI18n: i }) {
		let a = (n) => {
			let a = Za(n);
			if (Ya(n.props)) {
				let { variableType: e, variableValue: a, variableOptions: o, injectionType: s } = Xa(n.props);
				return t({
					variableType: e,
					variableValue: a,
					variableOptions: o,
			...
			if (a?.transformation === "plural") {
				let e = a.branches || {};
				return typeof n.props.n == "number" ? s(Qa(n.props.n, [r], e) ?? n.props.children) : n.props.children == null ? null : s(n.props.children);
			}
...
function eo({ renderVariable: t }) {
	...
	function r({ sourceElement: t, targetElement: r, locales: a = ["en"], enableI18n: o }) {
		...
		if (u && Object.entries(_n).forEach(([e, t]) => {
			u[e] && (d[t] = u[e]);
		}), l === "plural") {
			let e = t.props.n;
			return typeof e == "number" ? i({
				source: Qa(e, a, c.branches || {}) ?? t.props.children,
				target: Qa(e, a, r.d?.b || {}) ?? r.c,
				locales: a,
				enableI18n: o
			}) : n({
				children: t,
	...
	function i({ source: i, target: a, locales: o = ["en"], enableI18n: s }) {
		...
		if (Array.isArray(a) && !Array.isArray(i) && i && (i = [i]), Array.isArray(i) && Array.isArray(a)) {
			let n = {}, c = {}, l = {}, u = i.filter((t) => {
				if (e.isValidElement(t)) {
					if (Ya(t.props)) {
						let { variableName: e, variableValue: r, variableOptions: i, injectionType: a } = Xa(t.props);
						n[e] = r, c[e] = i, l[e] = a;
					} else return !0;
				}
				return !1;
			...
			return a.map((i, a) => {
				...
				if (en(i)) return p(e.Fragment, { children: t({
					variableType: i.v || "v",
					variableValue: n[i.k],
		...
		if (a && typeof a == "object" && !Array.isArray(a)) {
			...
			if (e.isValidElement(i)) {
				...
				if (Ya(i.props)) {
					let { variableValue: e, variableOptions: n, variableType: r, injectionType: a } = Xa(i.props);
					return t({
						variableType: r,
						variableValue: e,
						variableOptions: n,
...
var ro = { display: "contents" }, io = globalThis.navigator?.product === "ReactNative";
function ao(e) {
	if (e == null || typeof e == "boolean" || e === "") return !0;
	if (Array.isArray(e)) return !e.some((e) => !ao(e));
	if (o(e) && e.type === n) {
...
function oo(e, t) {
	return io || !to() ? e : o(e) && typeof e.type == "string" ? r(e, { "data-_gt-hash": t }) : ao(e) ? e : a("span", {
		"data-_gt-hash": t,
		style: ro
	}, e);
}
function so({ renderDefaultChildren: e, renderTranslatedChildren: t }) {
...
function co(e) {
	let t = Ka(e), n = $a({ renderVariable: t }), r = eo({ renderVariable: t });
	return {
		renderVariable: t,
...
var lo = class extends Aa {};
function uo(e) {
	Va(e, "server-render"), Fa(new lo(e));
}
var fo = R({
	namespace: "reactCore",
	key: "i18nStore",
...
function go(e, t) {
	let n = t.options.$_hash ?? Gi(t.message, t.options);
	return e?.[t.locale]?.[n];
}
function _o(e, t) {
	return W(yo(e, t));
...
function yo(e, { locale: t, id: n }) {
	let r = e?.[t];
	if (!r) return;
	if (!n) return r;
	let i = r;
	for (let e of n.split(".")) {
		if (!bo(e) || !H(i) || !Object.prototype.hasOwnProperty.call(i, e)) return;
		i = i[e];
	}
	return i;
}
...
var So = class {
	...
	subscribeToTranslate = (e, t) => {
		let n = Ma(e);
		return ho(this.translateListeners, (e) => {
			Ma(e) === n && t();
		});
	};
	subscribeToTranslationEvents = (e) => ho(this.translateListeners, e);
	subscribeToDictionaryEntryEvents = (e) => ho(this.dictionaryEntryListeners, e);
	subscribeToDictionaryObjectEvents = (e) => ho(this.dictionaryObjectListeners, e);
	getTranslateSnapshot = (e, t = {}) => go(t, e) ?? K().lookupTranslation(e.locale, e.message, e.options);
	getDictionaryEntrySnapshot = (e, t = {}) => _o(t, e) ?? K().lookupDictionary(e.locale, e.id);
	getDictionaryObjectSnapshot = (e, t = {}) => vo(t, e) ?? K().lookupDictionaryObj(e.locale, e.id);
...
function wo() {
	return Co.isInitialized() || Co.set(i(void 0)), Co.get();
}
function To() {
	let e = s(wo());
...
function Do() {
	return To()?.conditionStore ?? J();
}
function Y() {
	return Do().getLocale();
}
function X() {
	return Do().getEnableI18n();
}
function Oo() {
	return l(() => B().getDefaultLocale(), []);
}
function ko() {
	return To()?.i18nStore || po();
}
function Ao() {
	return To()?.translationsSnapshot || {};
}
var jo = () => {};
function Mo() {
	return jo;
...
function Fo({ _enableI18n: e, _locale: t, ...n }) {
	return Po({
		...n,
		_enableI18n: e ?? X(),
		_locale: t ?? Y()
...
Fo._gtt = "variable-currency-automatic", Io._gtt = "variable-currency";
function Lo({ _enableI18n: e, _locale: t, children: n, options: r = {}, locales: i = [] }) {
	let a = Pa({
		locale: t,
		enableI18n: e,
		localesProp: i
...
function Ro({ _enableI18n: e, _locale: t, ...n }) {
	return Lo({
		...n,
		_enableI18n: e ?? X(),
		_locale: t ?? Y()
...
Ro._gtt = "variable-datetime-automatic", zo._gtt = "variable-datetime";
...
Vo._gtt = "variable-number-automatic", Ho._gtt = "variable-number";
function Uo({ _enableI18n: e, _locale: t, date: n, children: r, value: i, unit: a, baseDate: o, locales: s = [], options: c = {} }) {
	let l = Pa({
		locale: t,
		enableI18n: e,
		localesProp: s
...
Wo._gtt = "variable-relative-time-automatic", Go._gtt = "variable-relative-time";
function Ko({ children: e }) {
	return e;
}
function qo({ children: e }) {
	return Ko({ children: e });
}
function Jo({ children: e }) {
	return Ko({ children: e });
}
qo._gtt = "variable-variable", Jo._gtt = "variable-variable-automatic";
function Yo(e) {
	let t = ko(), n = Ao();
	return No(), d((n) => t.subscribeToTranslate(e, n), () => t.getTranslateSnapshot(e, n), () => t.getTranslateSnapshot(e, n));
}
var { renderVariable: Xo, renderDefaultChildren: Zo, renderTranslatedChildren: Qo, renderPreparedT: $o } = co({
	Currency: Io,
	GtInternalCurrency: Fo,
	DateTime: zo,
	GtInternalDateTime: Ro,
	Num: Ho,
	GtInternalNum: Vo,
	RelativeTime: Go,
	...
	let r = n, i = (e) => {
		...
		if (a) {
			...
			if ((e[1] === "automatic" || e[2] === "automatic") && (i.injectionType = "automatic"), e[0] === "translate" && (e[0] = "fragment"), e[0] === "variable" && (i.variableType = e?.[1] || "variable"), e[0] === "plural") {
				let e = Object.entries(n).reduce((e, [t, n]) => (ln(t) && (e[t] = es(n, r)), e), {});
				Object.keys(e).length && (i.branches = e);
			}
			if (e[0] === "branch") {
				let { children: e, branch: t, ...a } = n, o = Object.fromEntries(Object.entries(a).filter(([e]) => !e.startsWith("data-"))), s = Object.entries(o).reduce((e, [t, n]) => (e[t] = es(n, r), e), {});
...
function ns(e, t) {
	let { type: n, props: i } = e, a = as(n);
	if (typeof i != "object" || !i) return e;
	if (a) {
		let { componentType: n, injectionType: o } = a;
		if (n === "variable") return e;
		if (n === "branch") return r(e, { ...Object.entries(i).reduce((e, [n, r]) => (e[n] = n !== "branch" && !n.startsWith("data-") ? rs(r, t) : r, e), {}) });
		if (n === "plural") return r(e, { ...Object.entries(i).reduce((e, [n, r]) => (e[n] = ln(n) || n === "children" ? rs(r, t) : r, e), {}) });
		if (n === "derive") return r(e, {
			...i,
			..."children" in i && { children: is(i.children, t + 1) }
		});
		if (n === "translate" && o === "automatic" && t > 0) return "children" in i ? is(i.children, t) : void 0;
		n === "translate" && o === "automatic" && console.warn(os);
	}
	return r(e, {
		...i,
		..."children" in i && { children: is(i.children, t) }
	});
}
function rs(e, t) {
	return o(e) ? ns(e, t) : e;
...
function is(e, n) {
	return Array.isArray(e) ? t.map(e, (e) => rs(e, n)) : rs(e, n);
}
function as(e) {
...
var os = "'@generaltranslation/react-core Warning: A <_T> component was found injected outside of a <Derive> boundary. This may affect translation resolution for this component.";
function ss(t) {
...
var cs = (e) => {
	...
	let r = Object.entries(_n).reduce((e, [n, r]) => {
		let i = t[r];
	...
	let { props: t } = e, n = { t: cs(e) };
	if (t["data-_gt"]) {
		let e = t["data-_gt"], r = e.transformation;
		if (r === "variable") {
			let n = e.variableType || "variable", r = Ja(t, n), i = fn(n);
			return {
				i: e.id,
				k: r,
				v: i
		...
		n.i = e.id, n.d = ls(r, t, e.branches);
	}
	return t.children && (n.c = fs(t.children)), n;
}, ds = (e) => ss(e) ? us(e) : typeof e == "number" ? e.toString() : e;
function fs(e) {
	return Array.isArray(e) ? e.map(ds) : ds(e);
}
function ps({ sourceChildren: e, params: t, locale: n }) {
	let r = ms(e), i = hs(r), a = gs({
		options: _s(t),
		locale: n
	});
	return no(i, a), {
...
function gs({ options: e, locale: t }) {
	return {
		...e,
		$locale: t
	};
...
function vs({ sourceChildren: e, params: t, _locale: n, _enableI18n: r }) {
	let i = Y(), a = X(), o = Oo(), s = n ?? i, c = r ?? a;
	return {
		defaultLocale: o,
		enableI18n: c,
		locale: s,
...
function ys({ children: e, _locale: t, _enableI18n: n, _renderPreparedT: r = $o, ...i }) {
	let { defaultLocale: a, locale: o, enableI18n: s, targetOptions: c, taggedSourceChildren: l, sourceJsxChildren: d, shouldTranslate: f } = vs({
		sourceChildren: e,
		params: i,
		_locale: t,
		_enableI18n: n
...
var bs = wo();
function xs({ children: e, translations: t, dictionaries: n, conditionStore: r, i18nStore: i, onMissingTranslation: a, onMissingDictionaryEntry: o, onMissingDictionaryObj: s }) {
	let u = l(() => ({
...
var ws = class {
	constructor(e) {
		this.getLocale = () => Ts(this.customGetLocale), this.setLocale = (e) => {
			this.updateLocale(e), Q({
				cookieName: Ia,
				value: "true"
			}), this.reload();
		}, this.getRegion = () => Ss({ cookieName: q().getRegionCookieName() }) || this.customGetRegion?.(), this.setRegion = (e) => {
			this.updateRegion(e), this.reload();
		}, this.getEnableI18n = () => {
			let e = Ss({ cookieName: q().getEnableI18nCookieName() });
...
function Os(e) {
	if (!e) return "";
	let t = Array.isArray(e) ? e.join(", ") : e;
	return t.trim() ? Es(`Details: ${t}`) : "";
}
function ks({ source: e, severity: t, whatHappened: n, reassurance: r, why: i, fix: a, wayOut: o, details: s, docsUrl: c }) {
	let l = e ? t ? `${e} ${t}:` : `${e}:` : t ? `${t}:` : "", u = i ? `${Ds(n)} because ${Ds(i)}` : n, d = !!a && !!o && /^[a-z]/.test(Ds(o)), f = [
		u,
		r,
		d ? `${Ds(a)}, or ${Ds(o)}` : a,
		d ? void 0 : o,
...
}), { setConditionStore: js, isConditionStoreInitialized: Ms } = Hi(As), { getConditionStore: Ns, setConditionStore: Ps, isConditionStoreInitialized: Fs } = Hi(As);
function Is(e) {
	let t = Ls(e), n = Rs(e), r = zs(e);
	if (Fs()) {
		let e = Ns();
...
function Ls({ _getLocale: e, locale: t }) {
	let n = q(), r = [];
	return t && r.push(...Array.isArray(t) ? t : [t]), e && r.push(e()), r.push(...Cs(n.getLocaleCookieName())), n.resolveSupportedLocale(r);
}
function Rs({ _getRegion: e, region: t }) {
	return Ss({ cookieName: q().getRegionCookieName() }) || e?.() || t;
}
function zs({ enableI18n: e, _getEnableI18n: t }) {
	if (e !== void 0) return e;
	let n = Ss({ cookieName: q().getEnableI18nCookieName() });
...
function Vs() {
	return {
		projectId: Hs(() => void 0) || Us(),
		devApiKey: bi() === "development" ? Hs(() => void 0) || Ws() : void 0
	};
}
function Hs(e) {
	try {
...
function Us() {
	try {
		return Gs(process.env.VITE_GT_PROJECT_ID);
	} catch {
		return;
	}
}
function Ws() {
	try {
		return Gs(process.env.VITE_GT_DEV_API_KEY);
	} catch {
		return;
	}
}
...
function Js() {
	return m(f, { children: [p("h2", {
		className: "mb-6 text-2xl font-bold text-foreground",
		children: p(Z, { children: "Open Positions" })
	}), p("div", {
		className: "space-y-4",
		children: [
			{
				title: p(Z, { children: "Senior Frontend Engineer" }),
				location: p(Z, { children: "Remote" }),
				type: p(Z, { children: "Full-time" }),
				dept: p(Z, { children: "Engineering" }),
				desc: p(Z, { children: "Build and maintain our benchmarking dashboard and developer tools using React, TypeScript, and Vite." })
			},
			{
				title: p(Z, { children: "Backend Engineer" }),
				location: p(Z, { children: "Remote" }),
				type: p(Z, { children: "Full-time" }),
				dept: p(Z, { children: "Engineering" }),
				desc: p(Z, { children: "Design and scale our cloud benchmarking infrastructure handling thousands of automated runs daily." })
			},
			{
				title: p(Z, { children: "Technical Writer" }),
				location: p(Z, { children: "Remote" }),
				type: p(Z, { children: "Part-time" }),
				dept: p(Z, { children: "Documentation" }),
				desc: p(Z, { children: "Create comprehensive guides, API references, and tutorials for our benchmarking platform." })
			},
			{
				title: p(Z, { children: "DevRel Engineer" }),
				location: p(Z, { children: "San Francisco / Remote" }),
				type: p(Z, { children: "Full-time" }),
				dept: p(Z, { children: "Community" }),
				desc: p(Z, { children: "Engage with the i18n community through talks, workshops, blog posts, and open source contributions." })
			},
			{
				title: p(Z, { children: "QA Engineer" }),
				location: p(Z, { children: "Remote" }),
				type: p(Z, { children: "Full-time" }),
				dept: p(Z, { children: "Engineering" }),
				desc: p(Z, { children: "Ensure the accuracy and reliability of benchmark results through rigorous testing and validation." })
			}
		].map((e) => m("div", {
			...
			children: [m("div", { children: [
				...
				children: p(Z, { children: "Apply Now" })
			})]
		}, e.title?.toString() || ""))
	})] });
}
...
function rc(e, t, n = ic()) {
	let r = q(), { basepath: i, pathname: a } = ac(e, n), o = nc(a, "/") ? a.replace(/^\/[^/]+/, "") || "/" : a, s = r.resolveSupportedLocale(t);
...
function cc(e, t, n = new URL(e.url).pathname) {
	...
	o && a.push(o), a.push(...Ji(e.headers.get("accept-language"))), a.length === 0 && console.warn(sc);
...
var lc = h().server(() => $().getLocale()).client(() => J().getLocale());
h().server(() => $().getEnableI18n()).client(() => J().getEnableI18n()), h().server((e) => {
	let t = $();
	return Yi({
		locale: t.getLocale(),
...
async function mc(e, t = ["shared"]) {
	let n = {};
	return await Promise.all(t.map(async (t) => {
		let r = pc[`./src/_gt/${e}/${t}.json`];
		if (!r) {
