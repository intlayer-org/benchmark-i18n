...
var C = /^\.(?:(0+)(\*)?|(#+)|(0+)(#+))$/g, w = /^(@+)?(\+|#+)?[rs]?$/g, de = /(\*)(0+)|(#+)(0+)|(0+)/g, T = /^(0+)$/;
function E(e) {
	let t = {};
	return e[e.length - 1] === "r" ? t.roundingPriority = "morePrecision" : e[e.length - 1] === "s" && (t.roundingPriority = "lessPrecision"), e.replace(w, function(e, n, r) {
...
function fe(e) {
	...
	if (e[0] === "E" && e[1] === "E" ? (t = { notation: "engineering" }, e = e.slice(2)) : e[0] === "E" && (t = { notation: "scientific" }, e = e.slice(1)), t) {
		let n = e.slice(0, 2);
		if (n === "+!" ? (t.signDisplay = "always", e = e.slice(2)) : n === "+?" && (t.signDisplay = "exceptZero", e = e.slice(2)), !T.test(e)) throw Error("Malformed concise eng/scientific notation");
...
function pe(e) {
	...
	for (let n of e) {
		...
		if (C.test(n.stem)) {
			...
			n.stem.replace(C, function(e, n, r, i, a, o) {
...
var ye = RegExp(`^${ge.source}*`), be = RegExp(`${ge.source}*$`);
function H(e, t) {
	return {
		start: e,
		end: t
...
var xe = !!Object.fromEntries, Se = !!String.prototype.trimStart, Ce = !!String.prototype.trimEnd, we = xe ? Object.fromEntries : function(e) {
	let t = {};
	...
	return e.replace(be, "");
}, De = /* @__PURE__ */ RegExp("([^\\p{White_Space}\\p{Pattern_Syntax}]*)", "yu");
function Oe(e, t) {
	return De.lastIndex = t, De.exec(e)[1] ?? "";
}
var ke = class {
	constructor(e, t = {}) {
	...
	parseMessage(e, t, n) {
		...
		for (; !this.isEOF();) {
			...
			if (i === 123) {
				...
				if (n) break;
				return this.error(k.UNMATCHED_CLOSING_TAG, H(this.clonePosition(), this.clonePosition()));
			} else if (i === 60 && !this.ignoreTag && U(this.peek() || 0)) {
	...
	parseTag(e, t) {
		...
		if (this.bumpIf(">")) {
			...
			if (this.bumpIf("</")) {
				if (this.isEOF() || !U(this.char())) return this.error(k.INVALID_TAG, H(o, this.clonePosition()));
				let e = this.clonePosition();
				return r === this.parseTagName() ? (this.bumpSpace(), this.bumpIf(">") ? {
					...
					err: null
				} : this.error(k.INVALID_TAG, H(o, this.clonePosition()))) : this.error(k.UNMATCHED_CLOSING_TAG, H(e, this.clonePosition()));
			}
			return this.error(k.UNCLOSED_TAG, H(n, this.clonePosition()));
		}
	...
	parseArgument(e, t) {
		let n = this.clonePosition();
		if (this.bump(), this.bumpSpace(), this.isEOF()) return this.error(k.EXPECT_ARGUMENT_CLOSING_BRACE, H(n, this.clonePosition()));
		if (this.char() === 125) return this.bump(), this.error(k.EMPTY_ARGUMENT, H(n, this.clonePosition()));
		let r = this.parseIdentifierIfPossible().value;
		if (!r) return this.error(k.MALFORMED_ARGUMENT, H(n, this.clonePosition()));
		if (this.bumpSpace(), this.isEOF()) return this.error(k.EXPECT_ARGUMENT_CLOSING_BRACE, H(n, this.clonePosition()));
		switch (this.char()) {
			...
			case 44: return this.bump(), this.bumpSpace(), this.isEOF() ? this.error(k.EXPECT_ARGUMENT_CLOSING_BRACE, H(n, this.clonePosition())) : this.parseArgumentOptions(e, t, r, n);
			default: return this.error(k.MALFORMED_ARGUMENT, H(n, this.clonePosition()));
		}
	}
	...
	parseArgumentOptions(e, t, n, r) {
		...
		switch (a) {
			case "": return this.error(k.EXPECT_ARGUMENT_TYPE, H(i, o));
			case "number":
			case "date":
			case "time": {
				this.bumpSpace();
				...
				if (this.bumpIf(",")) {
					...
					if (n.err) return n;
					let r = Ee(n.val);
					if (r.length === 0) return this.error(k.EXPECT_ARGUMENT_STYLE, H(this.clonePosition(), this.clonePosition()));
					e = {
						style: r,
						styleLocation: H(t, this.clonePosition())
					};
				}
				let t = this.tryParseArgumentClose(r);
				if (t.err) return t;
				let i = H(r, this.clonePosition());
				if (e && e.style.startsWith("::")) {
					...
					{
						if (t.length === 0) return this.error(k.EXPECT_DATE_TIME_SKELETON, i);
						let r = t;
			...
			case "select": {
				...
				if (this.bumpSpace(), !this.bumpIf(",")) return this.error(k.EXPECT_SELECT_ARGUMENT_OPTIONS, H(i, { ...i }));
				this.bumpSpace();
				let o = this.parseIdentifierIfPossible(), s = 0;
				if (a !== "select" && o.value === "offset") {
					if (!this.bumpIf(":")) return this.error(k.EXPECT_PLURAL_ARGUMENT_OFFSET_VALUE, H(this.clonePosition(), this.clonePosition()));
					this.bumpSpace();
					let e = this.tryParseDecimalInteger(k.EXPECT_PLURAL_ARGUMENT_OFFSET_VALUE, k.INVALID_PLURAL_ARGUMENT_OFFSET_VALUE);
					if (e.err) return e;
			...
			default: return this.error(k.INVALID_ARGUMENT_TYPE, H(i, o));
		}
	}
	tryParseArgumentClose(e) {
		return this.isEOF() || this.char() !== 125 ? this.error(k.EXPECT_ARGUMENT_CLOSING_BRACE, H(e, this.clonePosition())) : (this.bump(), {
			val: !0,
			err: null
		});
	}
	parseSimpleArgStyleIfPossible() {
		...
		for (; !this.isEOF();) switch (this.char()) {
			case 39: {
				this.bump();
				let e = this.clonePosition();
				if (!this.bumpUntil("'")) return this.error(k.UNCLOSED_QUOTE_IN_ARGUMENT_STYLE, H(e, this.clonePosition()));
				this.bump();
				break;
			}
			case 123:
	...
	tryParsePluralOrSelectOptions(e, t, n, r) {
		...
		for (;;) {
			if (s.length === 0) {
				...
				if (t !== "select" && this.bumpIf("=")) {
					let t = this.tryParseDecimalInteger(k.EXPECT_PLURAL_ARGUMENT_SELECTOR, k.INVALID_PLURAL_ARGUMENT_SELECTOR);
					if (t.err) return t;
					c = H(e, this.clonePosition()), s = this.message.slice(e.offset, this.offset());
				} else break;
			}
			if (o.has(s)) return this.error(t === "select" ? k.DUPLICATE_SELECT_ARGUMENT_SELECTOR : k.DUPLICATE_PLURAL_ARGUMENT_SELECTOR, c);
			s === "other" && (i = !0), this.bumpSpace();
			let r = this.clonePosition();
			if (!this.bumpIf("{")) return this.error(t === "select" ? k.EXPECT_SELECT_ARGUMENT_SELECTOR_FRAGMENT : k.EXPECT_PLURAL_ARGUMENT_SELECTOR_FRAGMENT, H(this.clonePosition(), this.clonePosition()));
		...
		return a.length === 0 ? this.error(t === "select" ? k.EXPECT_SELECT_ARGUMENT_SELECTOR : k.EXPECT_PLURAL_ARGUMENT_SELECTOR, H(this.clonePosition(), this.clonePosition())) : this.requiresOtherClause && !i ? this.error(k.MISSING_OTHER_CLAUSE, H(this.clonePosition(), this.clonePosition())) : {
			val: a,
			err: null
		};
	}
...
function W(e) {
	e.forEach((e) => {
		if (delete e.location, I(e) || L(e)) for (let t in e.options) delete e.options[t].location, W(e.options[t].value);
		else N(e) && z(e.style) || (P(e) || F(e)) && B(e.style) ? delete e.style.location : R(e) && W(e.children);
	});
}
function Ne(e, t = {}) {
	t = {
	...
	if (n.err) {
		let e = SyntaxError(k[n.err.kind]);
...
var G = function(e) {
	...
	constructor(e, t, n, r) {
		super(`Invalid values for "${e}": "${t}". Options are "${Object.keys(n).join("\", \"")}"`, G.INVALID_VALUE, r);
	}
}, Fe = class extends K {
	constructor(e, t, n) {
		super(`Value for "${e}" must be of type ${t}`, G.INVALID_VALUE, n);
	}
}, Ie = class extends K {
	constructor(e, t) {
		super(`The intl string context variable "${e}" was not provided to the string "${t}"`, G.MISSING_VALUE, t);
	}
}, q = function(e) {
...
function J(e, t, n, r, i, a, o) {
	...
	for (let c of e) {
		...
		if (P(c)) {
			let e = typeof c.style == "string" ? r.date[c.style] : B(c.style) ? c.style.parsedOptions : void 0;
			s.push({
				type: q.literal,
		...
		if (F(c)) {
			let e = typeof c.style == "string" ? r.time[c.style] : B(c.style) ? c.style.parsedOptions : r.time.medium;
			s.push({
				type: q.literal,
				value: n.getDateTimeFormat(t, e).format(l)
			});
		...
		if (N(c)) {
			let e = typeof c.style == "string" ? r.number[c.style] : z(c.style) ? c.style.parsedOptions : void 0;
			if (e && e.scale) {
				let t = e.scale || 1;
				if (typeof l == "bigint") {
...
function Ge(e) {
	...
	return function({ cache: e, formats: n, formatters: i, getMessageFallback: a = Ue, locale: o, messagesOrError: s, namespace: c, onError: l, timeZone: u }) {
		...
		function p(p, m, h, g) {
			...
			try {
				return He(Z(c, p), v, m && function(e) {
					let n = {};
...
function $e(e) {
	return function(e, t, n) {
		...
		return u || Ze || !Qe || (Ze = !0, l(new b(x.ENVIRONMENT_FALLBACK, void 0))), s((() => Ge({
			cache: r,
			formatters: a,
