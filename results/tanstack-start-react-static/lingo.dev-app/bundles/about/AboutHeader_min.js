...
}({ enableConsole: !0 }), f = function(e, t) {
...
var F = /[ \xA0\u1680\u2000-\u200A\u202F\u205F\u3000]/, ce = /(?:[Eec]{1,6}|G{1,5}|[Qq]{1,5}|(?:[yYur]+|U{1,5})|[ML]{1,5}|d{1,2}|D{1,3}|F{1}|[abB]{1,5}|[hkHK]{1,2}|w{1,2}|W{1}|m{1,2}|s{1,2}|[zZOvVxX]{1,4})(?=([^']*'[^']*')*[^']*$)/g;
function le(e) {
	var t = {};
...
var I = /^\.(?:(0+)(\*)?|(#+)|(0+)(#+))$/g, L = /^(@+)?(\+|#+)?[rs]?$/g, pe = /(\*)(0+)|(#+)(0+)|(0+)/g, R = /^(0+)$/;
function z(e) {
	var t = {};
...
function he(e) {
	for (var t = {}, n = 0, r = e; n < r.length; n++) {
		...
		if (I.test(i.stem)) {
			...
			var a = i.options[0];
			a === "w" ? t = m(m({}, t), { trailingZeroDisplay: "stripIfInteger" }) : a && (t = m(m({}, t), z(a)));
			continue;
		}
		if (L.test(i.stem)) {
			t = m(m({}, t), z(i.stem));
...
var ve = RegExp(`^${F.source}*`), ye = RegExp(`${F.source}*\$`);
function U(e, t) {
	return {
		start: e,
		end: t
...
var be = !!Object.fromEntries, xe = !!String.prototype.trimStart, Se = !!String.prototype.trimEnd, W = be ? Object.fromEntries : function(e) {
	for (var t = {}, n = 0, r = e; n < r.length; n++) {
		var i = r[n], a = i[0];
		t[a] = i[1];
	}
	...
	return e.prototype.parse = function() {
		...
		for (var r = []; !this.isEOF();) {
			...
			if (i === 123) {
				...
				if (n) break;
				return this.error(C.UNMATCHED_CLOSING_TAG, U(this.clonePosition(), this.clonePosition()));
		...
		if (this.bumpIf(">")) {
			...
			if (this.bumpIf("</")) {
				if (this.isEOF() || !K(this.char())) return this.error(C.INVALID_TAG, U(o, this.clonePosition()));
				...
				return r === this.parseTagName() ? (this.bumpSpace(), this.bumpIf(">") ? {
					...
					err: null
				} : this.error(C.INVALID_TAG, U(o, this.clonePosition()))) : this.error(C.UNMATCHED_CLOSING_TAG, U(s, this.clonePosition()));
			} else return this.error(C.UNCLOSED_TAG, U(n, this.clonePosition()));
		} else return this.error(C.INVALID_TAG, U(n, this.clonePosition()));
		...
		var n = this.clonePosition();
		if (this.bump(), this.bumpSpace(), this.isEOF()) return this.error(C.EXPECT_ARGUMENT_CLOSING_BRACE, U(n, this.clonePosition()));
		if (this.char() === 125) return this.bump(), this.error(C.EMPTY_ARGUMENT, U(n, this.clonePosition()));
		var r = this.parseIdentifierIfPossible().value;
		if (!r) return this.error(C.MALFORMED_ARGUMENT, U(n, this.clonePosition()));
		if (this.bumpSpace(), this.isEOF()) return this.error(C.EXPECT_ARGUMENT_CLOSING_BRACE, U(n, this.clonePosition()));
		switch (this.char()) {
			...
			case 44: return this.bump(), this.bumpSpace(), this.isEOF() ? this.error(C.EXPECT_ARGUMENT_CLOSING_BRACE, U(n, this.clonePosition())) : this.parseArgumentOptions(e, t, r, n);
			default: return this.error(C.MALFORMED_ARGUMENT, U(n, this.clonePosition()));
		}
		...
		switch (a) {
			case "": return this.error(C.EXPECT_ARGUMENT_TYPE, U(i, o));
			case "number":
			case "date":
			case "time":
				this.bumpSpace();
				...
				if (this.bumpIf(",")) {
					...
					if (l.err) return l;
					var u = we(l.val);
					if (u.length === 0) return this.error(C.EXPECT_ARGUMENT_STYLE, U(this.clonePosition(), this.clonePosition()));
					s = {
						style: u,
				...
				if (s && s.style.startsWith("::")) {
					...
					if (a === "number") {
						...
						var h = p;
						this.locale && (h = ge(p, this.locale));
						var u = {
							type: T.dateTime,
							pattern: h,
			...
			case "select":
				...
				if (this.bumpSpace(), !this.bumpIf(",")) return this.error(C.EXPECT_SELECT_ARGUMENT_OPTIONS, U(g, m({}, g)));
				this.bumpSpace();
				var _ = this.parseIdentifierIfPossible(), v = 0;
				if (a !== "select" && _.value === "offset") {
					if (!this.bumpIf(":")) return this.error(C.EXPECT_PLURAL_ARGUMENT_OFFSET_VALUE, U(this.clonePosition(), this.clonePosition()));
					this.bumpSpace();
			...
			default: return this.error(C.INVALID_ARGUMENT_TYPE, U(i, o));
		}
	}, e.prototype.tryParseArgumentClose = function(e) {
		return this.isEOF() || this.char() !== 125 ? this.error(C.EXPECT_ARGUMENT_CLOSING_BRACE, U(e, this.clonePosition())) : (this.bump(), {
			val: !0,
			err: null
		});
		...
		for (var e = 0, t = this.clonePosition(); !this.isEOF();) switch (this.char()) {
			case 39:
				this.bump();
				var n = this.clonePosition();
				if (!this.bumpUntil("'")) return this.error(C.UNCLOSED_QUOTE_IN_ARGUMENT_STYLE, U(n, this.clonePosition()));
				this.bump();
				break;
			case 123:
				e += 1, this.bump();
		...
		for (var i, a = !1, o = [], s = /* @__PURE__ */ new Set(), c = r.value, l = r.location;;) {
			...
			if (!this.bumpIf("{")) return this.error(t === "select" ? C.EXPECT_SELECT_ARGUMENT_SELECTOR_FRAGMENT : C.EXPECT_PLURAL_ARGUMENT_SELECTOR_FRAGMENT, U(this.clonePosition(), this.clonePosition()));
		...
		return o.length === 0 ? this.error(t === "select" ? C.EXPECT_SELECT_ARGUMENT_SELECTOR : C.EXPECT_PLURAL_ARGUMENT_SELECTOR, U(this.clonePosition(), this.clonePosition())) : this.requiresOtherClause && !a ? this.error(C.MISSING_OTHER_CLAUSE, U(this.clonePosition(), this.clonePosition())) : {
			val: o,
			err: null
		};
...
function q(e) {
	e.forEach(function(e) {
		...
		else D(e) && N(e.style) || (O(e) || k(e)) && P(e.style) ? delete e.style.location : M(e) && q(e.children);
	});
}
function Ae(e, t) {
	t === void 0 && (t = {}), t = m({
...
function Ve(t, n, r) {
	try {
		let i = new Re(t, r), a = Be(n), o = i.format(a);
...
function We() {
	let { t: e } = He(["d69a92e868dc", "4371fc3273ff"]);
	return Ue("AboutHeader"), /* @__PURE__ */ c(o, { children: [/* @__PURE__ */ s("h1", {
