...
function te(e, t) {
	let n = e.length === 1 ? v : ee;
	return y(e, this, n, t.cache.create(), t.serializer);
}
function ne(e, t) {
	return y(e, this, ee, t.cache.create(), t.serializer);
}
function re(e, t) {
	...
	for (let n of e) {
		switch (n.stem) {
			...
			case "integer-width":
				...
				n.options[0].replace(_e, function(e, n, r, i, a, o) {
		...
		if (he.test(n.stem)) {
			...
			n.stem.replace(he, function(e, n, r, i, a, o) {
...
var De = RegExp(`^${we.source}*`), Oe = RegExp(`${we.source}*$`);
function z(e, t) {
	return {
		start: e,
		end: t
...
var ke = !!Object.fromEntries, Ae = !!String.prototype.trimStart, je = !!String.prototype.trimEnd, Me = ke ? Object.fromEntries : function(e) {
	let t = {};
	...
	return e.replace(Oe, "");
}, Fe = /* @__PURE__ */ RegExp("([^\\p{White_Space}\\p{Pattern_Syntax}]*)", "yu");
function Ie(e, t) {
	return Fe.lastIndex = t, Fe.exec(e)[1] ?? "";
}
var Le = class {
	message;
	position;
	...
	parseMessage(e, t, n) {
		...
		for (; !this.isEOF();) {
			...
			if (i === 123) {
				...
				if (n) break;
				return this.error(L.UNMATCHED_CLOSING_TAG, z(this.clonePosition(), this.clonePosition()));
	...
	parseTag(e, t) {
		...
		if (this.bumpIf(">")) {
			...
			if (this.bumpIf("</")) {
				if (this.isEOF() || !B(this.char())) return this.error(L.INVALID_TAG, z(o, this.clonePosition()));
				let e = this.clonePosition();
				return r === this.parseTagName() ? (this.bumpSpace(), this.bumpIf(">") ? {
					...
					err: null
				} : this.error(L.INVALID_TAG, z(o, this.clonePosition()))) : this.error(L.UNMATCHED_CLOSING_TAG, z(e, this.clonePosition()));
			} else return this.error(L.UNCLOSED_TAG, z(n, this.clonePosition()));
		} else return this.error(L.INVALID_TAG, z(n, this.clonePosition()));
	}
	parseTagName() {
		let e = this.offset();
	...
	parseArgument(e, t) {
		let n = this.clonePosition();
		if (this.bump(), this.bumpSpace(), this.isEOF()) return this.error(L.EXPECT_ARGUMENT_CLOSING_BRACE, z(n, this.clonePosition()));
		if (this.char() === 125) return this.bump(), this.error(L.EMPTY_ARGUMENT, z(n, this.clonePosition()));
		let r = this.parseIdentifierIfPossible().value;
		if (!r) return this.error(L.MALFORMED_ARGUMENT, z(n, this.clonePosition()));
		if (this.bumpSpace(), this.isEOF()) return this.error(L.EXPECT_ARGUMENT_CLOSING_BRACE, z(n, this.clonePosition()));
		switch (this.char()) {
			...
			case 44: return this.bump(), this.bumpSpace(), this.isEOF() ? this.error(L.EXPECT_ARGUMENT_CLOSING_BRACE, z(n, this.clonePosition())) : this.parseArgumentOptions(e, t, r, n);
			default: return this.error(L.MALFORMED_ARGUMENT, z(n, this.clonePosition()));
		}
	}
	...
	parseArgumentOptions(e, t, n, r) {
		...
		switch (a) {
			case "": return this.error(L.EXPECT_ARGUMENT_TYPE, z(i, o));
			case "number":
			case "date":
			case "time": {
				this.bumpSpace();
				...
				if (this.bumpIf(",")) {
					...
					if (n.err) return n;
					let r = Pe(n.val);
					if (r.length === 0) return this.error(L.EXPECT_ARGUMENT_STYLE, z(this.clonePosition(), this.clonePosition()));
					e = {
						style: r,
			...
			case "select": {
				...
				if (this.bumpSpace(), !this.bumpIf(",")) return this.error(L.EXPECT_SELECT_ARGUMENT_OPTIONS, z(i, { ...i }));
				this.bumpSpace();
				let o = this.parseIdentifierIfPossible(), s = 0;
				if (a !== "select" && o.value === "offset") {
					if (!this.bumpIf(":")) return this.error(L.EXPECT_PLURAL_ARGUMENT_OFFSET_VALUE, z(this.clonePosition(), this.clonePosition()));
					this.bumpSpace();
			...
			default: return this.error(L.INVALID_ARGUMENT_TYPE, z(i, o));
		}
	}
	tryParseArgumentClose(e) {
		return this.isEOF() || this.char() !== 125 ? this.error(L.EXPECT_ARGUMENT_CLOSING_BRACE, z(e, this.clonePosition())) : (this.bump(), {
			val: !0,
			err: null
		});
	}
	parseSimpleArgStyleIfPossible() {
		...
		for (; !this.isEOF();) switch (this.char()) {
			case 39: {
				this.bump();
				let e = this.clonePosition();
				if (!this.bumpUntil("'")) return this.error(L.UNCLOSED_QUOTE_IN_ARGUMENT_STYLE, z(e, this.clonePosition()));
				this.bump();
				break;
			}
			case 123:
	...
	tryParsePluralOrSelectOptions(e, t, n, r) {
		...
		for (;;) {
			...
			if (!this.bumpIf("{")) return this.error(t === "select" ? L.EXPECT_SELECT_ARGUMENT_SELECTOR_FRAGMENT : L.EXPECT_PLURAL_ARGUMENT_SELECTOR_FRAGMENT, z(this.clonePosition(), this.clonePosition()));
		...
		return a.length === 0 ? this.error(t === "select" ? L.EXPECT_SELECT_ARGUMENT_SELECTOR : L.EXPECT_PLURAL_ARGUMENT_SELECTOR, z(this.clonePosition(), this.clonePosition())) : this.requiresOtherClause && !i ? this.error(L.MISSING_OTHER_CLAUSE, z(this.clonePosition(), this.clonePosition())) : {
			val: a,
			err: null
		};
	}
...
function V(e) {
	e.forEach((e) => {
		if (delete e.location, M(e) || N(e)) for (let t in e.options) delete e.options[t].location, V(e.options[t].value);
		else k(e) && F(e.style) || (A(e) || j(e)) && I(e.style) ? delete e.style.location : P(e) && V(e.children);
	});
}
function Ve(e, t = {}) {
	t = {
...
function G(e, t, n, r, i, a, o) {
	...
	for (let c of e) {
		...
		if (M(c)) {
			...
			if (!a) throw new He(c.value, l, Object.keys(c.options), o);
...
function $e(e) {
	...
	return function({ cache: e, formats: n, formatters: i, getMessageFallback: a = Ze, locale: o, messagesOrError: s, namespace: c, onError: l, timeZone: u }) {
		...
		function p(p, m, h, g) {
			...
			try {
				return Xe(J(c, p), v, m && function(e) {
					let n = {};
...
var X = n(void 0);
function rt({ children: e, formats: t, getMessageFallback: n, locale: r, messages: i, now: o, onError: s, timeZone: l }) {
	let u = a(X), d = c((() => u?.cache || se()), [r, u?.cache]), f = c((() => u?.formatters || le(d)), [d, u?.formatters]), p = c((() => ({
		...nt({
			locale: r,
...
function ot(e) {
	return function(e, t, n) {
		let { cache: r, formats: i, formatters: a, getMessageFallback: o, locale: s, onError: l, timeZone: u } = Z(), d = e[n], f = et(t, n);
		return u || it || !at || (it = !0, l(new x(S.ENVIRONMENT_FALLBACK, void 0))), c((() => $e({
			cache: r,
			formatters: a,
...
function ct() {
	let e = ot("theme-toggle"), [t, n] = l("auto");
	o(() => {
		let e = st();
		n(e), Q(e);
	}, []), o(() => {
...
function dt() {
	...
	return /* @__PURE__ */ m("div", {
		...
		children: /* @__PURE__ */ m("select", {
			...
			children: lt.map((e) => /* @__PURE__ */ m("option", {
				value: e,
				children: ut(e)
			}, e))
		})
...
function _t() {
	return /* @__PURE__ */ m(gt, { children: /* @__PURE__ */ m(pt, {}) });
}
//#endregion
