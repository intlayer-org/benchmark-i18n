...
var { getConditionStore: n$1, setConditionStore: r$2, isConditionStoreInitialized: i$2 } = createConditionStoreSingleton(createDiagnosticMessage$2({
	source: `@generaltranslation/react-core`,
	severity: `Error`,
...
function r$1(r) {
	f(r, `server-render`), r$3(new t$1(r));
}
function e(e, t) {
	return e.add(t), () => {
		e.delete(t);
...
var l = class {
	...
	updateTranslations = (t) => {
		n$2().updateTranslations(t);
	};
	updateDictionaries = (t) => {
		n$2().updateDictionaries(t);
	};
	translate = async (t) => n$2().lookupTranslationWithFallback(t.locale, t.message, t.options).then(() => {
		this.emitTranslateEvent(t);
	}).catch((e) => this.logRuntimeTranslationError(e));
	translateDictionaryEntry = (t) => {
		n$2().lookupDictionaryWithFallback(t.locale, t.id).then(() => {
			this.emitDictionaryEvent(t);
		}).catch((e) => this.logRuntimeTranslationError(e));
	};
	translateDictionaryObject = (t) => {
		n$2().lookupDictionaryObjWithFallback(t.locale, t.id).then(() => {
			this.emitDictionaryEvent(t);
		}).catch((e) => this.logRuntimeTranslationError(e));
	};
	...
	getTranslateSnapshot = (t$2, r = {}) => t(r, t$2) ?? n$2().lookupTranslation(t$2.locale, t$2.message, t$2.options);
	getDictionaryEntrySnapshot = (t, n$3 = {}) => n(n$3, t) ?? n$2().lookupDictionary(t.locale, t.id);
	getDictionaryObjectSnapshot = (t, n = {}) => r(n, t) ?? n$2().lookupDictionaryObj(t.locale, t.id);
...
function getLocaleFromPath(pathname, basepath = getRouterBasepath()) {
	...
	return u().determineSupportedLocale(segment);
}
...
function resolveRequestConditions(request, localeConfig, pathname = new URL(request.url).pathname) {
	...
	(0, server_exports.setCookie)(i18nConfig.getLocaleCookieName(), locale, localeCookieOptions);
...
var getLocale = createIsomorphicFn().server(() => getConditionStore().getLocale()).client(() => n$1().getLocale());
createIsomorphicFn().server(() => getConditionStore().getEnableI18n()).client(() => n$1().getEnableI18n());
...
function determineLocaleServer({ defaultLocale, locales, customMapping }) {
	...
	return resolveRequestConditions((0, server_exports.getRequest)(), {
		defaultLocale,
		locales,
		customMapping
	}).locale;
