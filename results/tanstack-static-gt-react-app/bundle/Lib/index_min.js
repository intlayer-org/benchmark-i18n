...
import f from "../gt.config.json";
import p from "../src/_gt/en.json";
import m from "../src/_gt/fr.json";
import h from "../src/_gt/de.json";
import g from "../src/_gt/es.json";
import _ from "../src/_gt/it.json";
import v from "../src/_gt/ja.json";
import y from "../src/_gt/ko.json";
import ee from "../src/_gt/pt.json";
import te from "../src/_gt/ru.json";
import ne from "../src/_gt/zh.json";
var b = Object.defineProperty, re = Object.getOwnPropertyDescriptor, ie = Object.getOwnPropertyNames, ae = Object.prototype.hasOwnProperty, oe = (e, t) => {
	let n = {};
	...
	if (t && typeof t == "object" || typeof t == "function") for (var i = ie(t), a = 0, o = i.length, s; a < o; a++) s = i[a], !ae.call(e, s) && s !== n && b(e, s, {
...
var le = c(({ request: e, router: t, responseHeaders: n }) => l({
	request: e,
	router: t,
	responseHeaders: n,
	children: s(x, { router: t })
})), ue = c(({ router: e, responseHeaders: t }) => u({
	router: e,
	responseHeaders: t,
	children: s(x, { router: e })
})), de = oe({
	StartServer: () => x,
	defaultRenderHandler: () => ue,
	defaultStreamHandler: () => le
});
import * as fe from "@tanstack/start-server-core";
ce(de, fe);
var pe = oe({
	StartServer: () => x,
	defaultRenderHandler: () => ue,
...
var ye = "https://cdn.gtx.dev";
function be(e) {
	return typeof e == "object" && !!e && "error" in e && typeof e.error == "string";
}
function xe(e) {
	return be(e.error) || typeof e.error == "string";
}
function Se(e) {
	if (e.data !== void 0) return e.data;
	if (e.response) {
		let t = be(e.error) ? e.error.error : typeof e.error == "string" ? e.error : e.response.statusText;
		throw new me(t, e.response.status, t);
	}
	throw e.error;
}
...
}), we = 3, Te = 500, Ee = 6e4, De = /* @__PURE__ */ new Set([
	"GET",
	"HEAD",
	"OPTIONS",
	"PUT",
...
function ke({ fetch: e = globalThis.fetch, timeoutMs: t = Oe } = {}) {
	return async (n, r) => {
		if (t === !1) return e(n, r);
...
var Ae = (e) => new Promise((t) => setTimeout(t, e));
function je(e) {
	if (!e) return;
...
function Ne(e, t, n) {
	return e?.status === 429 ? Me(e.headers.get("Retry-After")) ?? je(e.headers.get("RateLimit-Reset")) ?? Ee : Te * (n === "linear" ? t + 1 : 2 ** t);
}
function Pe({ fetch: e = globalThis.fetch, retryPolicy: t = "exponential" } = {}) {
	return async (n, r) => {
		let i = new Request(n, r), a = t === "none" ? 0 : we, o = De.has(i.method);
		for (let n = 0; n <= a; n++) {
			let r;
			try {
				r = await e(i.clone());
			...
			r?.body?.cancel(), await Ae(Ne(r, n, t));
		}
		throw Error("Max retries exceeded");
	};
}
var Fe = { bodySerializer: (e) => JSON.stringify(e, (e, t) => typeof t == "bigint" ? t.toString() : t) }, Ie = ({ onRequest: e, onSseError: t, onSseEvent: n, responseTransformer: r, responseValidator: i, sseDefaultRetryDelay: a, sseMaxRetryAttempts: o, sseMaxRetryDelay: s, sseSleepFn: c, url: l, ...u }) => {
	...
	if (!t) {
		let t = (e ? i : i.map((e) => encodeURIComponent(e))).join(Re(r));
		switch (r) {
			case "label": return `.${t}`;
	...
	let a = Le(r), o = i.map((t) => r === "label" || r === "simple" ? e ? t : encodeURIComponent(t) : Ve({
		allowReserved: e,
		name: n,
		value: t
	})).join(a);
	return r === "label" || r === "matrix" ? a + o : o;
}, Ve = ({ allowReserved: e, name: t, value: n }) => {
	if (n == null) return "";
	...
	let o = ze(r), s = Object.entries(i).map(([t, i]) => Ve({
		allowReserved: e,
		name: r === "deepObject" ? `${n}[${t}]` : t,
		value: i
	})).join(o);
	return r === "label" || r === "matrix" ? o + s : s;
}, Ue = /\{[^{}]+\}/g, We = ({ path: e, url: t }) => {
	let n = t, r = t.match(Ue);
	if (r) for (let t of r) {
	...
	return n;
}, Ge = ({ baseUrl: e, path: t, query: n, querySerializer: r, url: i }) => {
...
var qe = async (e, t) => {
...
}, Xe = (e, t) => t ? !!(e.headers.has(t) || e.query?.[t] || e.headers.get("Cookie")?.includes(`${t}=`)) : !1, Ze = async ({ security: e, ...t }) => {
	for (let n of e) {
		if (Xe(t, n.name)) continue;
		let e = await qe(n, t.auth);
		if (!e) continue;
	...
	return n.baseUrl?.endsWith("/") && (n.baseUrl = n.baseUrl.substring(0, n.baseUrl.length - 1)), n.headers = tt(e.headers, t.headers), n;
}, et = (e) => {
	let t = [];
	...
	for (let n of e) {
		if (!n) continue;
		let e = n instanceof Headers ? et(n) : Object.entries(n);
...
}), at = { "Content-Type": "application/json" }, ot = (e = {}) => ({
	...Fe,
	headers: at,
	parseAs: "auto",
	querySerializer: it,
	...
	let t = $e(ot(), e), n = () => ({ ...t }), r = (e) => (t = $e(t, e), n()), i = rt(), a = async (e) => {
		let n = {
			...t,
			...e,
			fetch: e.fetch ?? t.fetch ?? globalThis.fetch,
			headers: tt(t.headers, e.headers),
		...
		if (c.ok) {
			let e = (t.parseAs === "auto" ? Ye(c.headers.get("Content-Type")) : t.parseAs) ?? "json";
...
function ct(e) {
	...
	return e.apiKey && t.set("Authorization", `Bearer ${e.apiKey}`), e.projectId && t.set("gt-project-id", e.projectId), st({
		baseUrl: e.baseUrl,
		fetch: Pe({
			fetch: ke({
				fetch: e.fetch,
...
var w = {
	...
	number: 0,
	dateTime: 1
}, lt = /^\.(?:(0+)(\*)?|(#+)|(0+)(#+))$/, ut = /^@+(\+|#+)?[rs]?$/, dt = /^(?:(?:\+|#+)[rs]?|[rs])$/, ft = /(\*)(0+)|(#+)(0+)|(0+)/g, pt = /[\t-\r \x85\u200E\u200F\u2028\u2029]+/u, mt = /(?:[Eec]{1,6}|G{1,5}|[Qq]{1,5}|(?:[yYur]+|U{1,5})|[ML]{1,5}|d{1,2}|D{1,3}|F|[abB]{1,5}|[hHkK]{1,2}|w{1,2}|W|m{1,2}|s{1,2}|[zZOvVxX]{1,4})(?=([^']*'[^']*')*[^']*$)/g, ht = {
	floor: "floor",
	ceiling: "ceil",
	down: "trunc",
	up: "expand",
...
function vt(e) {
	...
	for (let n of e) {
		...
		switch (n.stem) {
			...
			case "measure-unit":
			case "unit":
				yt(n), t.style = "unit", t.unit = e.replace(/^(.*?)-/, "");
				continue;
			case "compact-short":
			case "K":
			...
			case "scale":
				...
				continue;
			case "integer-width":
				if (yt(n), n.options.length > 1) throw RangeError("integer-width stems only accept a single optional option");
				St(t, e);
				continue;
		}
		if (n.stem.slice(0, 14) === "rounding-mode-") {
			let e = ht[n.stem.slice(14)];
		...
		if (!Ct(t, n)) {
			if (ut.test(n.stem)) {
				Object.assign(t, wt(n.stem));
				continue;
			}
			if (dt.test(n.stem)) throw SyntaxError("Significant precision must start with @.");
			bt(t, n.stem) || xt(t, n.stem);
		}
	}
	return t;
}
...
function xt(e, t) {
	...
	return e.notation = n[1] === "EE" ? "engineering" : "scientific", n[2] && bt(e, n[2]), e.minimumIntegerDigits = n[3].length, !0;
}
function St(e, t) {
	t.replace(ft, (t, n, r, i, a, o) => {
...
function wt(e) {
	let t = {};
	if (e.endsWith("r") && (t.roundingPriority = "morePrecision"), e.endsWith("s") && (t.roundingPriority = "lessPrecision"), dt.test(e)) throw SyntaxError("Significant precision must start with @.");
	if (!ut.test(e)) return t;
...
function Tt(e) {
	...
	for (let [n] of e.matchAll(mt)) {
		...
		switch (n[0]) {
			...
			case "K":
			case "k":
				t.hourCycle = gt[n[0]], t.hour = e === 2 ? "2-digit" : "numeric";
				break;
			case "m":
...
var kt = /^[A-Za-z]$/u, At = /^[-.0-9_A-Za-z\u00B7\u00C0-\u00D6\u00D8-\u00F6\u00F8-\u037D\u037F-\u1FFF\u200C-\u200D\u203F-\u2040\u2070-\u218F\u2C00-\u2FEF\u3001-\uD7FF\uF900-\uFDCF\uFDF0-\uFFFD\u{10000}-\u{EFFFF}]$/u;
function E(e) {
	return kt.test(e ?? "");
}
function jt(e) {
...
var Mt = /[\t-\r \x85\u200E\u200F\u2028\u2029]/u, Nt = /[\t-\r \x85\xA0\u1680\u2000-\u200A\u2028\u2029\u202F\u205F\u3000\x21-\x2F\x3A-\x40\x5B-\x5E\x60\x7B-\x7E\xA1-\xA7\xA9\xAB\xAC\xAE\xB0\xB1\xB6\xBB\xBF\xD7\xF7\u2010-\u2027\u2030-\u203E\u2041-\u2053\u2055-\u205E\u2190-\u245F\u2500-\u2775\u2794-\u2BFF\u2E00-\u2E7F\u3001-\u3003\u3008-\u3020\u3030\uFD3E-\uFD3F\uFE45-\uFE46]/u, Pt = class {
	...
	readTagName() {
		let e = this.index;
		for (E(this.current()) || this.fail("INVALID_TAG", e), this.index += 1; !this.atEnd() && jt(this.current());) this.index += this.current().length;
			...
			if (i.slice(0, 2) === "::") {
				...
				if (n === "number") {
					...
					r = {
						...
						tokens: e,
						...s ? { location: s } : {},
						parsedOptions: this.options.shouldParseSkeletons ? vt(e) : {}
					};
				} else {
					t || this.failAt("EXPECT_DATE_TIME_SKELETON", e, this.index);
					let n = Et(t, this.options.locale);
					r = {
						type: T.dateTime,
						pattern: n,
						...s ? { location: s } : {},
						parsedOptions: this.options.shouldParseSkeletons ? Tt(n) : {}
					};
				}
			} else r = i;
		}
...
function Ut(e, t, n) {
	...
	for (let r of e) switch (r.type) {
		...
		case w.number: {
			let e = D(t.variables, r.value), { scale: n, ...a } = typeof r.style == "string" ? Rt[r.style] ?? {} : r.style?.type === T.number ? r.style.parsedOptions : {}, o = Wt(e, n);
			i(Gt(t, a).format(o));
			break;
		}
		case w.date:
		case w.time: {
			let e = D(t.variables, r.value), n = r.type === w.date ? zt : Vt;
			i(Kt(t, typeof r.style == "string" ? n[r.style] : r.style?.type === T.dateTime ? r.style.parsedOptions : r.type === w.time ? Vt.medium : void 0).format(e));
			break;
		}
		case w.select: {
			let e = String(D(t.variables, r.value)), n = Zt(r.options, e) ?? r.options.other;
			if (!n) throw Qt(r.value, e, r.options);
			Ut(n.value, t).forEach(i);
			break;
		}
		case w.plural: {
			let e = D(t.variables, r.value), n = `=${String(e)}`, a = Zt(r.options, n), o = typeof e == "bigint" ? e : Number(e), s = typeof o == "bigint" ? o - BigInt(r.offset) : o - r.offset;
			if (!a && Xt(r.options)) {
				let e = qt(t, r.pluralType ?? "cardinal").select(Yt(s));
				a = Zt(r.options, e);
			}
			if (a ??= r.options.other, !a) throw Qt(r.value, e, r.options);
...
function tn(e, t, n) {
	return e.map((r, i) => {
		switch (r.type) {
			case w.literal: return an(r, t, i > 0, i < e.length - 1 || n);
			case w.argument: return `{${r.value}}`;
			case w.date:
			case w.time:
			case w.number: return sn(r);
			case w.select: return `{${r.value},select,${ln(r.options, !1)}}`;
			case w.plural: {
				let e = r.pluralType === "cardinal" ? "plural" : "selectordinal", t = r.offset ? `offset:${r.offset} ` : "";
				return `{${r.value},${e},${t}${ln(r.options, !0)}}`;
			}
			case w.pound: return "#";
			case w.tag: return `<${r.value}>${tn(r.children, t, !0)}</${r.value}>`;
		}
	}).join("");
}
...
function rn(e, t, n, r) {
	...
	let o = "", s = 0;
	for (let [r, a] of i) o += nn(e.slice(s, r), t, s !== 0, s === 0 && n, !0), o += `'${e.slice(r, a).replace(/'/g, "''")}'`, s = a;
	return o += nn(e.slice(s), t, s !== 0, s === 0 && n, r), o;
}
function an({ value: e }, t, n, r) {
	return on(e) ? e : rn(e, t, n, r);
}
function on(e) {
	if (e[0] !== "<" || e.slice(-2) !== "/>") return !1;
	let t = e.slice(1, -2);
	if (!E(t[0])) return !1;
	for (let e of t.slice(1)) if (!jt(e)) return !1;
	return !0;
}
function sn(e) {
	let t = e.type === w.number ? "number" : e.type === w.date ? "date" : "time";
	return `{${e.value}, ${t}${e.style ? `, ${cn(e.style)}` : ""}}`;
}
function cn(e) {
	return typeof e == "string" ? e : e.type === T.dateTime ? `::${e.pattern}` : `::${e.tokens.map(({ stem: e, options: t }) => e + t.map((e) => `/${e}`).join("")).join(" ")}`;
}
function ln(e, t) {
	return Object.entries(e).map(([e, n]) => `${e}{${tn(n.value, t, !0)}}`).join(" ");
}
function un(e) {
...
function dn({ icuString: e, shouldVisit: t, visitor: n, options: { recurseIntoVisited: r = !0, ...i } }) {
	let a = Lt(e, i);
	return o(a), a;
	function o(e) {
		e.map(s);
...
var O = "_gt_", fn = RegExp(`^${O}\\d+$`), pn = RegExp(`^${O}$`);
function mn(e) {
...
function hn(e) {
	return e.type === w.select && pn.test(e.value) && !!e.options.other && (e.options.other.value.length === 0 || e.options.other.value.length > 0 && e.options.other.value[0]?.type === w.literal);
}
function gn(e) {
	let t = e;
...
var _n = (e) => `generaltranslation Formatting Error: Invalid cutoff style: ${e}.`, vn = "DEFAULT_TERMINATOR_KEY", yn = {
	ellipsis: {
		fr: {
			terminator: "…",
			separator: " "
	...
	constructor(t, n = {}) {
		...
		if (!yn[r]) throw Error(_n(r));
		let i = n.maxChars === void 0 ? void 0 : yn[r][new Intl.Locale(this.locale).language] || yn[r].DEFAULT_TERMINATOR_KEY, a = n.terminator ?? i?.terminator, o = a == null ? void 0 : n.separator ?? i?.separator;
	...
	get(e, ...t) {
		...
		let o = a[i];
		return o === void 0 && (o = new xn[e](...t), a[i] = o), o;
	}
}();
function Sn(e) {
...
function On({ value: e, locales: t = ["en"], options: n = {} }) {
...
function kn({ value: e, locales: t = ["en"], currency: n = "USD", options: r = {} }) {
	return k.get("NumberFormat", t, {
		style: "currency",
		currency: n,
		numberingSystem: "latn",
...
function jn({ value: e, locales: t = ["en"], options: n = {} }) {
...
var Ln = (e, t, n) => {
	...
	let n = e?.[t];
	return In(n) && typeof n.code == "string" ? n.code : void 0;
}, Rn = /* @__PURE__ */ new Set([
	"Cham",
	"Jamo",
	"Kawi",
	...
	"Toto",
	"Thai"
]), zn = (e) => e >= "qaa" && e <= "qtz", j = (e, t) => {
	e = A(t, e) || e;
	try {
		let { language: t, region: n, script: r } = k.get("Locale", e), i = 1 + Number(!!n) + Number(!!r);
		return !(e.split("-").length !== i || k.get("DisplayNames", ["en"], { type: "language" }).of(t) === t && !zn(t) || n && k.get("DisplayNames", ["en"], { type: "region" }).of(n) === n || r && k.get("DisplayNames", ["en"], { type: "script" }).of(r) === r && !Rn.has(r));
	} catch {
		return !1;
	}
}, M = (e) => {
...
function Hn(e, t, n, r) {
	if (n && !n.allValid || !j(e, r) || !j(t, r) || Vn(e, t)) return !1;
	if (!n) return !0;
	let i = Pn(t);
	return i !== void 0 && n.languages.has(i);
}
function Un(e, t, n, r) {
	return Hn(e, t, n ? Bn(n, r) : void 0, r);
}
function Wn(e) {
	try {
...
function ar(e, t = "en", n) {
	...
	try {
		let i = M(e), a = k.get("Locale", e), o = a.language, s = ir([
			...
			t,
			"en"
		], g = k.get("DisplayNames", m, { type: "language" }), _ = k.get("DisplayNames", h, { type: "language" }), v = s?.name, y = s?.nativeName || s?.name, ee = v || g.of(e) || e, te = y || _.of(e) || e, ne = s?.maximizedName || v || g.of(u) || e, b = s?.nativeMaximizedName || y || _.of(u) || e, re = s?.minimizedName || v || g.of(p) || e, ie = s?.nativeMinimizedName || y || _.of(p) || e, ae = s?.languageName || v || g.of(o) || e, oe = s?.nativeLanguageName || y || _.of(o) || e, se = s?.nameWithRegionCode || (c ? `${ae} (${c})` : ee), ce = s?.nativeNameWithRegionCode || (c ? `${oe} (${c})` : te) || se, x = k.get("DisplayNames", m, { type: "region" }), le = k.get("DisplayNames", h, { type: "region" }), ue = s?.regionName || (d ? x.of(d) : "") || "", de = s?.nativeRegionName || (d ? le.of(d) : "") || "", fe = k.get("DisplayNames", m, { type: "script" }), pe = k.get("DisplayNames", h, { type: "script" });
		return {
			code: i,
			name: ee,
			nativeName: te,
...
var pr = class {
	...
	formatRelativeTimeFromDate(e, t, n = {}) {
		let { locales: r, baseDate: i, ...a } = n, { value: o, unit: s } = Mn(e, i ?? /* @__PURE__ */ new Date());
		return Nn({
			value: o,
			unit: s,
			locales: this.getFormattingLocales(t, r),
	...
	formatMessage(e, t, n = {}) {
		...
		return a === "STRING" ? e : En(e, this.getFormattingLocales(t, r), i);
	}
	formatList(e, t, n = {}) {
		let { locales: r, ...i } = n;
		return An({
	...
	requiresTranslation(e, t = this.defaultLocale, n = this.locales.length ? this.locales : void 0) {
		...
		return Hn(this.resolveCanonicalLocale(t), this.resolveCanonicalLocale(e), r, this.customMapping);
	}
...
function gr(e, t) {
	return t?.dataFormat === "STRING" ? e : En(e, t?.locales, t?.variables);
}
function P(e, t) {
	return j(e, t);
}
...
var Ir = class {
	...
	3204031479,
	3329325298
]), L = /* @__PURE__ */ new Uint32Array(64), Rr = class extends Ir {
	constructor(e) {
		super(64, e, 8, !1);
	}
	get() {
	...
	process(e, t) {
		...
		for (let e = 0; e < 64; e++) {
			let t = F(o, 6) ^ F(o, 11) ^ F(o, 25), u = l + t + Pr(o, s, c) + Lr[e] + L[e] | 0, d = (F(n, 2) ^ F(n, 13) ^ F(n, 22)) + Fr(n, r, i) | 0;
...
async function ui(e) {
	if (!e.ok) {
		...
		throw new me(Jr(e.status, e.statusText, t), e.status, t);
	}
}
async function di(e, t, n, r) {
	...
	if (l.data === void 0 && l.response && !xe(l)) throw await ui(l.response), l.error;
	let u = Se(l);
...
var fi = class {
	...
	setConfig({ apiKey: e, devApiKey: t, sourceLocale: n, targetLocale: r, locales: i, projectId: a, customMapping: o, baseUrl: s }) {
		if (e && (this.apiKey = e), t && (this.devApiKey = t), a && (this.projectId = a), n && (this.sourceLocale = vr(n), !P(this.sourceLocale, o))) throw Error(Qr(this.sourceLocale));
		if (r && (this.targetLocale = vr(r), !P(this.targetLocale, o))) throw Error(Qr(this.targetLocale));
		if (i) {
			let e = [], t = [];
			if (i.forEach((n) => {
				let r = vr(n);
...
function Ui(e, t) {
	...
	try {
		let i = Cn(n || "");
		return hr(Hi(Object.keys(i).length ? Tn(e) : e, {
			...r,
			...i,
			[O]: "other"
		}, t.$locale, t.$format), { maxChars: t.$maxChars });
	} catch {
		return V.warn(Vi(e)), t.$_fallback == null ? hr(e, { maxChars: t.$maxChars }) : Ui(t.$_fallback, {
			...t,
			$_fallback: void 0
		});
	}
...
async function na({ locale: e, enableI18n: t }, n) {
	let r = Ci(), i = H().getDefaultLocale(), a = H().isDevHotReloadEnabled(), o = await r.getLookupTranslation(t ? e : i);
	return a && o.prefetchEntries && await o.prefetchEntries(n?.map(({ message: e, ...t }) => ({
		message: e,
		options: {
			$format: "ICU",
			...t
		...
		let c = Ji(t ? s.$locale ?? e : H().getDefaultLocale(), s, "ICU"), l = o(n, c);
		return a && l == null && r.lookupTranslationWithFallback(c.$locale, n, c).catch(() => {}), Gi({
			source: n,
			target: l,
			options: c,
			sourceLocale: i
...
function _a({ sourceObject: e, targetObject: t, translate: n }) {
	...
	let r = {}, i = /* @__PURE__ */ new Set([...Object.keys(e), ...W(t) ? Object.keys(t) : []]);
	for (let a of Array.from(i)) {
		let i = ga({
			sourceObject: e[a],
			targetObject: W(t) ? t[a] : void 0,
			translate: n
		});
		i !== void 0 && (r[a] = i);
	}
...
async function va({ locale: e, enableI18n: t, rootId: n }) {
	let r = Ci(), i = H().getDefaultLocale(), a = t ? e : i, [o, s, c] = await Promise.all([
...
function Sa(e) {
	let { cacheUrl: t = ye, projectId: n, _versionId: r, _branchId: i } = e, a = r ? `/${r}` : "", o = i ? `?branchId=${i}` : "";
...
async function Ma({ key: e, sourceValue: t, targetValue: n, translateEntry: r }) {
	if (K(n) !== void 0 || W(n) && !W(t)) return G(n);
	let i = K(t);
	if (i !== void 0) return await r(e, i);
	if (!W(t)) throw new q(e);
	let a = W(n) ? n : {}, o = /* @__PURE__ */ new Set([...Object.keys(t), ...Object.keys(a)]), s = await Promise.all(Array.from(o).map(async (n) => {
		let i = e ? `${e}.${n}` : n;
		oa(n, i);
		let o = t[n];
		return o === void 0 ? [n, G(a[n])] : [n, await Ma({
			key: i,
			sourceValue: o,
			targetValue: a[n],
			translateEntry: r
...
var Ia = 12e3, La = class {
	constructor(e) {
		...
		this.createTranslateMany = ba(H().getGTClass(), this.config.runtimeTranslation?.timeout ?? Ia, {
	...
	async getLookupTranslation(e) {
		return this.guardAsync((e) => e, async () => {
			...
			let n = await this.translations.getOrLoad(t), r = async (e = []) => {
				if (H().isDevHotReloadEnabled()) {
					let r = za(e, t, (e) => this._resolveCacheLocale(e) ?? this._resolveLocale(e));
...
function Ra(e) {
	if (e.runtimeUrl && e.runtimeUrl !== "https://api.gtx.dev" && (e.projectId || V.warn("I18nCache: " + C({
	...
	})), !e.devApiKey && !e.apiKey && V.warn("I18nCache: " + C({
	...
	}))), e.loadDictionary && !e.dictionary) throw V.error("I18nCache: " + C({
...
var Ha = "generaltranslation.locale-reset", Ua = "server-render", Wa = Symbol.for("generaltranslation.react-core.ReactI18nConfig"), Ga = class extends Ni {
	renderStrategy;
	localeCookieName;
	regionCookieName;
...
var { getConditionStore: X, setConditionStore: Xa, isConditionStoreInitialized: Za } = Yi(C({
	source: "@generaltranslation/react-core",
	severity: "Error",
...
function $a(e) {
	qa(e, "server-render"), Va(new Qa(e));
}
function eo(e, t) {
	return e.add(t), () => {
		e.delete(t);
...
function io(e, { locale: t, id: n }) {
	let r = e?.[t];
	if (!r) return;
	if (!n) return r;
	let i = r;
	for (let e of n.split(".")) {
		if (!ao(e) || !W(i) || !Object.prototype.hasOwnProperty.call(i, e)) return;
		i = i[e];
	}
	return i;
}
...
var so = class {
	...
	constructor() {}
	updateTranslations = (e) => {
		J().updateTranslations(e);
	};
	updateDictionaries = (e) => {
		J().updateDictionaries(e);
	};
	translate = async (e) => J().lookupTranslationWithFallback(e.locale, e.message, e.options).then(() => {
		this.emitTranslateEvent(e);
	}).catch((e) => this.logRuntimeTranslationError(e));
	translateDictionaryEntry = (e) => {
		J().lookupDictionaryWithFallback(e.locale, e.id).then(() => {
			this.emitDictionaryEvent(e);
		}).catch((e) => this.logRuntimeTranslationError(e));
	};
	translateDictionaryObject = (e) => {
		J().lookupDictionaryObjWithFallback(e.locale, e.id).then(() => {
			this.emitDictionaryEvent(e);
		}).catch((e) => this.logRuntimeTranslationError(e));
	};
	...
	subscribeToTranslate = (e, t) => {
		let n = Ba(e);
		return eo(this.translateListeners, (e) => {
			Ba(e) === n && t();
		});
	};
	subscribeToTranslationEvents = (e) => eo(this.translateListeners, e);
	subscribeToDictionaryEntryEvents = (e) => eo(this.dictionaryEntryListeners, e);
	subscribeToDictionaryObjectEvents = (e) => eo(this.dictionaryObjectListeners, e);
	getTranslateSnapshot = (e, t = {}) => to(t, e) ?? J().lookupTranslation(e.locale, e.message, e.options);
	getDictionaryEntrySnapshot = (e, t = {}) => no(t, e) ?? J().lookupDictionary(e.locale, e.id);
	getDictionaryObjectSnapshot = (e, t = {}) => ro(t, e) ?? J().lookupDictionaryObj(e.locale, e.id);
...
function lo() {
	return co.isInitialized() || co.set(e(void 0)), co.get();
}
var uo = lo();
function fo({ children: e, translations: r, dictionaries: i, conditionStore: a, i18nStore: o, onMissingTranslation: c, onMissingDictionaryEntry: l, onMissingDictionaryObj: u }) {
	let d = n(() => ({
...
function Z({ cookieName: e }) {
	if (!(typeof document > "u")) return U(document.cookie, e);
}
...
var mo = class {
	constructor(e) {
		this.getLocale = () => ho(this.customGetLocale), this.setLocale = (e) => {
			this.updateLocale(e), Q({
				cookieName: Ha,
				value: "true"
			}), this.reload();
		}, this.getRegion = () => Z({ cookieName: Y().getRegionCookieName() }) || this.customGetRegion?.(), this.setRegion = (e) => {
			this.updateRegion(e), this.reload();
		}, this.getEnableI18n = () => {
			let e = Z({ cookieName: Y().getEnableI18nCookieName() });
			...
			Q({
				cookieName: Y().getRegionCookieName(),
				value: e ?? ""
			});
		}, this.updateEnableI18n = (e) => {
			Q({
				cookieName: Y().getEnableI18nCookieName(),
				value: e ? "true" : "false"
			});
		}, this.reload = () => {
			let e = {
...
function ho(e) {
	let t = Y(), n = po(t.getLocaleCookieName());
...
function yo({ source: e, severity: t, whatHappened: n, reassurance: r, why: i, fix: a, wayOut: o, details: s, docsUrl: c }) {
	let l = e ? t ? `${e} ${t}:` : `${e}:` : t ? `${t}:` : "", u = i ? `${_o(n)} because ${_o(i)}` : n, d = !!a && !!o && /^[a-z]/.test(_o(o)), f = [
		u,
		r,
		d ? `${_o(a)}, or ${_o(o)}` : a,
		d ? void 0 : o,
...
}), { setConditionStore: xo, isConditionStoreInitialized: So } = Yi(bo), { getConditionStore: Co, setConditionStore: wo, isConditionStoreInitialized: To } = Yi(bo);
function Eo(e) {
	let t = Do(e), n = Oo(e), r = ko(e);
	if (To()) {
		let e = Co();
...
function Do({ _getLocale: e, locale: t }) {
	let n = Y(), r = [];
	return t && r.push(...Array.isArray(t) ? t : [t]), e && r.push(e()), r.push(...po(n.getLocaleCookieName())), n.resolveSupportedLocale(r);
}
function Oo({ _getRegion: e, region: t }) {
	return Z({ cookieName: Y().getRegionCookieName() }) || e?.() || t;
}
function ko({ enableI18n: e, _getEnableI18n: t }) {
	if (e !== void 0) return e;
	let n = Z({ cookieName: Y().getEnableI18nCookieName() });
...
function jo() {
	return {
		projectId: Mo(() => void 0) || No(),
		devApiKey: Di() === "development" ? Mo(() => void 0) || Po() : void 0
	};
}
function Mo(e) {
	try {
...
function No() {
	try {
		return Fo(process.env.VITE_GT_PROJECT_ID);
	} catch {
		return;
	}
}
function Po() {
	try {
		return Fo(process.env.VITE_GT_DEV_API_KEY);
	} catch {
		return;
	}
}
...
Uo.set;
var Wo = Uo.isInitialized;
function Go(e, t = qo()) {
	let { pathname: n } = Jo(e, t), r = n.match(/^\/([^/]+)(?:\/|$)/);
	if (!r) return;
	let i;
	try {
	...
	return Y().determineSupportedLocale(i);
}
function Ko(e, t, n = qo()) {
	let r = Y(), { basepath: i, pathname: a } = Jo(e, n), o = Go(a, "/") ? a.replace(/^\/[^/]+/, "") || "/" : a, s = r.resolveSupportedLocale(t);
...
function Zo(e, t, n = new URL(e.url).pathname) {
	let r = Y(), i = e.headers.get("cookie"), a = [];
	if (t?.localeRouting) {
		let e = Go(n);
		e && a.push(e);
	}
	let o = U(i, r.getLocaleCookieName());
	o && a.push(o), a.push(...ta(e.headers.get("accept-language"))), a.length === 0 && console.warn(Xo);
	let s = r.resolveSupportedLocale(a, t ?? {
		defaultLocale: r.getDefaultLocale(),
		locales: r.getLocales(),
		customMapping: r.getCustomMapping()
	});
	(0, pe.setCookie)(r.getLocaleCookieName(), s, Yo);
	let c = U(i, r.getEnableI18nCookieName());
	return {
		locale: s,
		region: U(i, r.getRegionCookieName()) || void 0,
		enableI18n: c === void 0 || c === "true"
	};
}
var Qo = a().server(() => $().getLocale()).client(() => X().getLocale());
a().server(() => $().getEnableI18n()).client(() => X().getEnableI18n()), a().server((e) => {
	let t = $();
	return na({
		locale: t.getLocale(),
...
}), a().server($o).client(() => Qo());
function $o({ defaultLocale: e, locales: t, customMapping: n }) {
	if (Wo()) {
		let e = $();
		if (e.hasActiveScope()) return e.getLocale();
	}
	return Zo((0, pe.getRequest)(), {
		defaultLocale: e,
		locales: t,
		customMapping: n
	}).locale;
}
function es({ defaultLocale: e, locales: t, customMapping: n }) {
	let r = Y(), i = r.getLocaleCookieName(), a = [], o = U(document.cookie, i);
...
function ts(e) {
	let t = e.localeRouting && !e._reload ? {
		...e,
		_reload: ({ locale: e }) => {
			let t = Ko(window.location.pathname, e), n = new URL(window.location.href);
...
var ns = "/Users/aymericpineau/Documents/benchmark-bloom/apps-benchmark/tanstack-start-react-static/gt-react-app/scripts/EmptyComponent.tsx", rs = () => null;
function is() {
	return d(Lo, {
		locale: "en",
		translations: {},
		children: d(rs, {}, void 0, !1, {
			fileName: ns,
			lineNumber: 12,
			columnNumber: 7
		}, this)
...
var ss = "/Users/aymericpineau/Documents/benchmark-bloom/apps-benchmark/tanstack-start-react-static/gt-react-app/scripts/Wrapper.tsx";
ts({
	...f,
	loadTranslations: os
});
...
var ls = "/Users/aymericpineau/Documents/benchmark-bloom/apps-benchmark/tanstack-start-react-static/gt-react-app/scripts/EmptyComponent.wrapper.tsx";
function us() {
	return d(cs, { children: d(is, {}, void 0, !1, {
		fileName: ls,
		lineNumber: 9,
		columnNumber: 11
	}, this) }, void 0, !1, {
