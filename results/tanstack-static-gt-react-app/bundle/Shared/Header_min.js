...
import { createIsomorphicFn as b, createMiddleware as ee } from "@tanstack/react-start";
import { getRequest as te, setCookie as ne } from "@tanstack/react-start/server";
...
function de(e) {
	...
	if (e.response) {
		...
		throw new re(t, e.response.status, t);
	}
	throw e.error;
}
...
var C = {
	...
	number: 0,
	dateTime: 1
}, Qe = /^\.(?:(0+)(\*)?|(#+)|(0+)(#+))$/, $e = /^@+(\+|#+)?[rs]?$/, et = /^(?:(?:\+|#+)[rs]?|[rs])$/, tt = /(\*)(0+)|(#+)(0+)|(0+)/g, nt = /[\t-\r \x85\u200E\u200F\u2028\u2029]+/u, rt = /(?:[Eec]{1,6}|G{1,5}|[Qq]{1,5}|(?:[yYur]+|U{1,5})|[ML]{1,5}|d{1,2}|D{1,3}|F|[abB]{1,5}|[hHkK]{1,2}|w{1,2}|W|m{1,2}|s{1,2}|[zZOvVxX]{1,4})(?=([^']*'[^']*')*[^']*$)/g, it = {
	floor: "floor",
	ceiling: "ceil",
	down: "trunc",
	up: "expand",
...
function ot(e) {
	let t = e.split(nt).filter(Boolean);
...
function st(e) {
	...
	for (let n of e) {
		...
		switch (n.stem) {
			...
			case "measure-unit":
			case "unit":
				ct(n), t.style = "unit", t.unit = e.replace(/^(.*?)-/, "");
				continue;
			case "compact-short":
			case "K":
			...
			case "scale":
				...
				continue;
			case "integer-width":
				if (ct(n), n.options.length > 1) throw RangeError("integer-width stems only accept a single optional option");
				dt(t, e);
				continue;
		}
		if (n.stem.slice(0, 14) === "rounding-mode-") {
			let e = it[n.stem.slice(14)];
		...
		if (!ft(t, n)) {
			if ($e.test(n.stem)) {
				Object.assign(t, pt(n.stem));
				continue;
			}
			if (et.test(n.stem)) throw SyntaxError("Significant precision must start with @.");
			lt(t, n.stem) || ut(t, n.stem);
		}
	}
	return t;
}
...
function ut(e, t) {
	...
	return e.notation = n[1] === "EE" ? "engineering" : "scientific", n[2] && lt(e, n[2]), e.minimumIntegerDigits = n[3].length, !0;
}
function dt(e, t) {
	t.replace(tt, (t, n, r, i, a, o) => {
...
function pt(e) {
	let t = {};
	if (e.endsWith("r") && (t.roundingPriority = "morePrecision"), e.endsWith("s") && (t.roundingPriority = "lessPrecision"), et.test(e)) throw SyntaxError("Significant precision must start with @.");
	if (!$e.test(e)) return t;
...
function mt(e) {
	...
	for (let [n] of e.matchAll(rt)) {
		...
		switch (n[0]) {
			...
			case "K":
			case "k":
				t.hourCycle = at[n[0]], t.hour = e === 2 ? "2-digit" : "numeric";
				break;
			case "m":
...
var vt = /^[A-Za-z]$/u, yt = /^[-.0-9_A-Za-z\u00B7\u00C0-\u00D6\u00D8-\u00F6\u00F8-\u037D\u037F-\u1FFF\u200C-\u200D\u203F-\u2040\u2070-\u218F\u2C00-\u2FEF\u3001-\uD7FF\uF900-\uFDCF\uFDF0-\uFFFD\u{10000}-\u{EFFFF}]$/u;
function T(e) {
	return vt.test(e ?? "");
}
function bt(e) {
...
var xt = /[\t-\r \x85\u200E\u200F\u2028\u2029]/u, St = /[\t-\r \x85\xA0\u1680\u2000-\u200A\u2028\u2029\u202F\u205F\u3000\x21-\x2F\x3A-\x40\x5B-\x5E\x60\x7B-\x7E\xA1-\xA7\xA9\xAB\xAC\xAE\xB0\xB1\xB6\xBB\xBF\xD7\xF7\u2010-\u2027\u2030-\u203E\u2041-\u2053\u2055-\u205E\u2190-\u245F\u2500-\u2775\u2794-\u2BFF\u2E00-\u2E7F\u3001-\u3003\u3008-\u3020\u3030\uFD3E-\uFD3F\uFE45-\uFE46]/u, Ct = class {
	constructor(e, t) {
	...
	readTagName() {
		let e = this.index;
		for (T(this.current()) || this.fail("INVALID_TAG", e), this.index += 1; !this.atEnd() && bt(this.current());) this.index += this.current().length;
			...
			if (i.slice(0, 2) === "::") {
				...
				if (n === "number") {
					...
					r = {
						...
						tokens: e,
						...s ? { location: s } : {},
						parsedOptions: this.options.shouldParseSkeletons ? st(e) : {}
					};
				} else {
					t || this.failAt("EXPECT_DATE_TIME_SKELETON", e, this.index);
					let n = ht(t, this.options.locale);
					r = {
						type: w.dateTime,
						pattern: n,
						...s ? { location: s } : {},
						parsedOptions: this.options.shouldParseSkeletons ? mt(n) : {}
					};
				}
			} else r = i;
		}
	...
	readIdentifier() {
		let e = this.index;
		for (; !this.atEnd() && !St.test(this.current());) this.index += this.current().length;
		return this.message.slice(e, this.index);
	}
	skipSpace() {
	...
	current() {
		return Tt(this.message, this.index);
	}
	peek() {
		let e = this.current();
		return Tt(this.message, this.index + e.length);
	}
	atEnd() {
	...
	position(e) {
		let t = this.positions ??= wt(this.message);
		return {
			offset: e,
			line: t[0][e],
			column: t[1][e]
...
function jt(e, t = "en", n = {}) {
	let r = Mt(Et(e, { locale: Ht(t) }), {
		message: e,
		locales: t,
		variables: n
	});
...
function Mt(e, t, n) {
	...
	for (let r of e) switch (r.type) {
		...
		case C.number: {
			let e = E(t.variables, r.value), { scale: n, ...a } = typeof r.style == "string" ? Dt[r.style] ?? {} : r.style?.type === w.number ? r.style.parsedOptions : {}, o = Nt(e, n);
			i(Pt(t, a).format(o));
			break;
		}
		case C.date:
		case C.time: {
			let e = E(t.variables, r.value), n = r.type === C.date ? Ot : At;
			i(Ft(t, typeof r.style == "string" ? n[r.style] : r.style?.type === w.dateTime ? r.style.parsedOptions : r.type === C.time ? At.medium : void 0).format(e));
			break;
		}
		case C.select: {
			let e = String(E(t.variables, r.value)), n = Bt(r.options, e) ?? r.options.other;
			if (!n) throw Vt(r.value, e, r.options);
			Mt(n.value, t).forEach(i);
			break;
		}
		case C.plural: {
			let e = E(t.variables, r.value), n = `=${String(e)}`, a = Bt(r.options, n), o = typeof e == "bigint" ? e : Number(e), s = typeof o == "bigint" ? o - BigInt(r.offset) : o - r.offset;
			if (!a && zt(r.options)) {
				let e = It(t, r.pluralType ?? "cardinal").select(Rt(s));
				a = Bt(r.options, e);
			}
			if (a ??= r.options.other, !a) throw Vt(r.value, e, r.options);
			Mt(a.value, t, s).forEach(i);
			break;
		}
		case C.tag: {
			let e = E(t.variables, r.value);
			if (typeof e != "function") throw TypeError(`The ICU tag variable "${r.value}" must be a function.`);
			let a = e(Mt(r.children, t, n));
...
function Pt(e, t = {}) {
	return Lt(e.numberFormats ??= /* @__PURE__ */ new Map(), JSON.stringify(t), () => new Intl.NumberFormat(e.locales, t));
}
function Ft(e, t) {
	return Lt(e.dateTimeFormats ??= /* @__PURE__ */ new Map(), t ? JSON.stringify(t) : "", () => new Intl.DateTimeFormat(e.locales, t));
}
function It(e, t) {
	...
	return Lt(e.pluralRules ??= /* @__PURE__ */ new Map(), t, () => new Intl.PluralRules(e.locales, { type: t }));
}
function Lt(e, t, n) {
	let r = e.get(t);
	if (r) return r;
...
function Wt(e, t, n) {
	return e.map((r, i) => {
		switch (r.type) {
			case C.literal: return qt(r, t, i > 0, i < e.length - 1 || n);
			case C.argument: return `{${r.value}}`;
			case C.date:
			case C.time:
			case C.number: return Yt(r);
			case C.select: return `{${r.value},select,${Zt(r.options, !1)}}`;
			case C.plural: {
				let e = r.pluralType === "cardinal" ? "plural" : "selectordinal", t = r.offset ? `offset:${r.offset} ` : "";
				return `{${r.value},${e},${t}${Zt(r.options, !0)}}`;
			}
			case C.pound: return "#";
			case C.tag: return `<${r.value}>${Wt(r.children, t, !0)}</${r.value}>`;
		}
	}).join("");
}
...
function Kt(e, t, n, r) {
	...
	let o = "", s = 0;
	for (let [r, a] of i) o += Gt(e.slice(s, r), t, s !== 0, s === 0 && n, !0), o += `'${e.slice(r, a).replace(/'/g, "''")}'`, s = a;
	return o += Gt(e.slice(s), t, s !== 0, s === 0 && n, r), o;
}
function qt({ value: e }, t, n, r) {
	return Jt(e) ? e : Kt(e, t, n, r);
}
function Jt(e) {
	if (e[0] !== "<" || e.slice(-2) !== "/>") return !1;
	let t = e.slice(1, -2);
	if (!T(t[0])) return !1;
	for (let e of t.slice(1)) if (!bt(e)) return !1;
	return !0;
}
function Yt(e) {
	let t = e.type === C.number ? "number" : e.type === C.date ? "date" : "time";
	return `{${e.value}, ${t}${e.style ? `, ${Xt(e.style)}` : ""}}`;
}
function Xt(e) {
	return typeof e == "string" ? e : e.type === w.dateTime ? `::${e.pattern}` : `::${e.tokens.map(({ stem: e, options: t }) => e + t.map((e) => `/${e}`).join("")).join(" ")}`;
}
function Zt(e, t) {
	return Object.entries(e).map(([e, n]) => `${e}{${Wt(n.value, t, !0)}}`).join(" ");
}
function Qt(e) {
...
function $t({ icuString: e, shouldVisit: t, visitor: n, options: { recurseIntoVisited: r = !0, ...i } }) {
	let a = Et(e, i);
	return o(a), a;
	function o(e) {
		e.map(s);
...
var D = "_gt_", en = RegExp(`^${D}\\d+$`), tn = RegExp(`^${D}$`);
function nn(e) {
	return e.type === C.select && en.test(e.value) && !!e.options.other && (e.options.other.value.length === 0 || e.options.other.value.length > 0 && e.options.other.value[0]?.type === C.literal);
}
function rn(e) {
	return e.type === C.select && tn.test(e.value) && !!e.options.other && (e.options.other.value.length === 0 || e.options.other.value.length > 0 && e.options.other.value[0]?.type === C.literal);
}
function an(e) {
	let t = e;
...
var on = (e) => `generaltranslation Formatting Error: Invalid cutoff style: ${e}.`, sn = "DEFAULT_TERMINATOR_KEY", cn = {
	ellipsis: {
		fr: {
			terminator: "…",
			separator: " "
	...
	constructor(t, n = {}) {
		...
		if (!cn[r]) throw Error(on(r));
		let i = n.maxChars === void 0 ? void 0 : cn[r][new Intl.Locale(this.locale).language] || cn[r].DEFAULT_TERMINATOR_KEY, a = n.terminator ?? i?.terminator, o = a == null ? void 0 : n.separator ?? i?.separator;
	...
	get(e, ...t) {
		...
		let o = a[i];
		return o === void 0 && (o = new un[e](...t), a[i] = o), o;
	}
}();
function dn(e) {
...
function mn(e, t = fn, n = ["en"]) {
	let r = dn(n).select(e), i = Math.abs(e);
...
function Tn({ value: e, locales: t = ["en"], currency: n = "USD", options: r = {} }) {
	return O.get("NumberFormat", t, {
		style: "currency",
		currency: n,
		numberingSystem: "latn",
...
function kn({ value: e, unit: t, locales: n = ["en"], options: r = {} }) {
...
var Nn = (e, t, n) => {
	...
	let n = e?.[t];
	return Mn(n) && typeof n.code == "string" ? n.code : void 0;
}, Pn = /* @__PURE__ */ new Set([
	"Cham",
	"Jamo",
	"Kawi",
	...
	"Toto",
	"Thai"
]), Fn = (e) => e >= "qaa" && e <= "qtz", A = (e, t) => {
	e = k(t, e) || e;
	try {
		let { language: t, region: n, script: r } = O.get("Locale", e), i = 1 + Number(!!n) + Number(!!r);
		return !(e.split("-").length !== i || O.get("DisplayNames", ["en"], { type: "language" }).of(t) === t && !Fn(t) || n && O.get("DisplayNames", ["en"], { type: "region" }).of(n) === n || r && O.get("DisplayNames", ["en"], { type: "script" }).of(r) === r && !Pn.has(r));
	} catch {
		return !1;
	}
}, j = (e) => {
...
function Rn(e, t, n, r) {
	if (n && !n.allValid || !A(e, r) || !A(t, r) || Ln(e, t)) return !1;
	if (!n) return !0;
	let i = An(t);
	return i !== void 0 && n.languages.has(i);
}
function zn(e, t, n, r) {
	return Rn(e, t, n ? In(n, r) : void 0, r);
}
function Bn(e) {
	try {
...
function Vn(e, t) {
	if (t.has(e)) return e;
	let { languageCode: n, regionCode: r, scriptCode: i, minimizedCode: a } = Bn(e), o = `${n}-${r}`;
	if (t.has(o)) return o;
	let s = `${n}-${i}`;
	if (t.has(s)) return s;
	if (t.has(a)) return a;
...
function Wn(e, t) {
	...
	try {
		...
		if (s) return s;
		let c = i.maximize();
		return Jn[c.language] || Qn(c.region || "");
	} catch {
		return qn;
	}
}
var Gn = "🌍", Kn = "🌏", qn = Gn, Jn = {
	...
	EU: "🇪🇺",
	419: "🌎"
}, Xn = /* @__PURE__ */ new Set(/* @__PURE__ */ "AF.AX.AL.DZ.AS.AD.AO.AI.AQ.AG.AR.AM.AW.AU.AT.AZ.BS.BH.BD.BB.BY.BE.BZ.BJ.BM.BT.BO.BQ.BA.BW.BV.BR.IO.BN.BG.BF.BI.CV.KH.CM.CA.KY.CF.TD.CL.CN.CX.CC.CO.KM.CD.CG.CK.CR.CI.HR.CU.CW.CY.CZ.DK.DJ.DM.DO.EC.EG.SV.GQ.ER.EE.SZ.ET.FK.FO.FJ.FI.FR.GF.PF.TF.GA.GM.GE.DE.GH.GI.GR.GL.GD.GP.GU.GT.GG.GN.GW.GY.HT.HM.VA.HN.HK.HU.IS.IN.ID.IR.IQ.IE.IM.IL.IT.JM.JP.JE.JO.KZ.KE.KI.KP.KR.KW.KG.LA.LV.LB.LS.LR.LY.LI.LT.LU.MO.MG.MW.MY.MV.ML.MT.MH.MQ.MR.MU.YT.MX.FM.MD.MC.MN.ME.MS.MA.MZ.MM.NA.NR.NP.NL.NC.NZ.NI.NE.NG.NU.NF.MK.MP.NO.OM.PK.PW.PS.PA.PG.PY.PE.PH.PN.PL.PT.PR.QA.RE.RO.RU.RW.BL.SH.KN.LC.MF.PM.VC.WS.SM.ST.SA.SN.RS.SC.SL.SG.SX.SK.SI.SB.SO.ZA.GS.SS.ES.LK.SD.SR.SJ.SE.CH.SY.TW.TJ.TZ.TH.TL.TG.TK.TO.TT.TN.TR.TM.TC.TV.UG.UA.AE.GB.US.UM.UY.UZ.VU.VE.VN.VG.VI.WF.EH.YE.ZM.ZW".split(".")), Zn = 127397;
function Qn(e) {
	return $n(e) || "🌍";
}
function $n(e) {
	let t = e.toUpperCase(), n = Yn[t];
	if (n) return n;
	if (Xn.has(t)) return String.fromCodePoint(t.charCodeAt(0) + Zn, t.charCodeAt(1) + Zn);
}
function er(e, t) {
	if (!t) return;
	let n = {};
...
function tr(e, t = "en", n) {
	...
	e = M(e, n), t ||= "en";
	try {
		let i = j(e), a = O.get("Locale", e), o = a.language, s = er([
			r,
			e,
			i,
			o
			...
			t,
			"en"
		], g = O.get("DisplayNames", m, { type: "language" }), _ = O.get("DisplayNames", h, { type: "language" }), v = s?.name, y = s?.nativeName || s?.name, b = v || g.of(e) || e, ee = y || _.of(e) || e, te = s?.maximizedName || v || g.of(u) || e, ne = s?.nativeMaximizedName || y || _.of(u) || e, re = s?.minimizedName || v || g.of(p) || e, ie = s?.nativeMinimizedName || y || _.of(p) || e, ae = s?.languageName || v || g.of(o) || e, x = s?.nativeLanguageName || y || _.of(o) || e, oe = s?.nameWithRegionCode || (c ? `${ae} (${c})` : b), se = s?.nativeNameWithRegionCode || (c ? `${x} (${c})` : ee) || oe, S = O.get("DisplayNames", m, { type: "region" }), ce = O.get("DisplayNames", h, { type: "region" }), le = s?.regionName || (d ? S.of(d) : "") || "", ue = s?.nativeRegionName || (d ? ce.of(d) : "") || "", de = O.get("DisplayNames", m, { type: "script" }), fe = O.get("DisplayNames", h, { type: "script" });
		return {
			code: i,
			name: b,
			nativeName: ee,
			...
			languageCode: o,
			languageName: ae,
			nativeLanguageName: x,
			nameWithRegionCode: oe,
			...
			emoji: s?.emoji || Wn(i, n)
		};
	} catch {
		let t = A(e) ? j(e) : e, r = t.split("-"), i = r[0] || t, a = r.length > 2 ? r[2] : r[1] || "", o = r[3] || "", s = er([t, i], n);
		t = s?.code || t;
...
function rr(e) {
	try {
		let t = or(O.get("Locale", e));
		if (t) return t;
	} catch {}
	let { scriptCode: t, languageCode: n } = tr(e);
	return t ? ir.has(t.toLowerCase()) ? "rtl" : "ltr" : n && ar.has(n.toLowerCase()) ? "rtl" : "ltr";
}
...
var lr = class {
	...
	buildResolutionScope(e) {
		...
		return {
			...
			approved: In(t.map(({ canonicalLocale: e }) => e), this.customMapping)
		};
	}
	...
	formatMessage(e, t, n = {}) {
		...
		return a === "STRING" ? e : Sn(e, this.getFormattingLocales(t, r), i);
	}
	formatList(e, t, n = {}) {
		let { locales: r, ...i } = n;
		return En({
	...
	getLocaleName(e) {
		return nr(e, this.defaultLocale, this.customMapping);
	}
	getLocaleEmoji(e) {
		return Wn(e, this.customMapping);
	}
	...
	requiresTranslation(e, t = this.defaultLocale, n = this.locales.length ? this.locales : void 0) {
		let r = n ? n === this.locales ? this.getResolutionScope().approved : In(n.map((e) => this.resolveCanonicalLocale(e)), this.customMapping) : void 0;
		return Rn(this.resolveCanonicalLocale(t), this.resolveCanonicalLocale(e), r, this.customMapping);
	}
	determineLocale(e, t = this.locales) {
		let { approvedLocalePairs: n, approved: r } = t === this.locales ? this.getResolutionScope() : this.buildResolutionScope(t), i = Hn(Array.isArray(e) ? e.map((e) => this.resolveCanonicalLocale(e)) : this.resolveCanonicalLocale(e), r, this.customMapping);
		if (i) return n.find(({ canonicalLocale: e }) => e === i)?.locale ?? this.resolveAliasLocale(i);
	}
	getLocaleDirection(e) {
		return rr(this.resolveCanonicalLocale(e));
	}
	isValidLocale(e) {
	...
	resolveAliasLocale(e) {
		return cr(e, this.customMapping);
	}
	standardizeLocale(e) {
		return j(e);
	}
	...
	isSupersetLocale(e, t) {
		return sr(this.resolveCanonicalLocale(e), this.resolveCanonicalLocale(t));
	}
};
function ur(e, t = "en", n) {
	t ||= "en";
	let r = e, i = qn;
	try {
...
function fr(e, t) {
	return t?.dataFormat === "STRING" ? e : Sn(e, t?.locales, t?.variables);
}
function N(e, t) {
	return A(e, t);
}
...
function yr(e, t, n = "") {
	let r = vr(e), i = e?.length, a = t !== void 0;
	if (!r || a && i !== t) {
...
var wr = typeof Uint8Array.from([]).toHex == "function" && typeof Uint8Array.fromHex == "function", Tr = Array.from({ length: 256 }, (e, t) => t.toString(16).padStart(2, "0"));
function Er(e) {
	if (yr(e), wr) return e.toHex();
	let t = "";
...
var Mr = class {
	...
	digestInto(e) {
		br(this), xr(e, this), this.finished = !0;
	...
	3204031479,
	3329325298
]), I = /* @__PURE__ */ new Uint32Array(64), Pr = class extends Mr {
	constructor(e) {
		super(64, e, 8, !1);
	}
	get() {
	...
	process(e, t) {
		...
		for (let e = 0; e < 64; e++) {
			let t = P(o, 6) ^ P(o, 11) ^ P(o, 25), u = l + t + Ar(o, s, c) + Nr[e] + I[e] | 0, d = (P(n, 2) ^ P(n, 13) ^ P(n, 22)) + jr(n, r, i) | 0;
...
var ei = class {
	...
	constructor(e = {}) {
		this.config = {
			...
			handlers: [],
			...e
		}, this.handlers = [...this.config.handlers || []], this.config.enableConsole && this.handlers.push(new ei(this.config));
	}
	addHandler(e) {
...
async function ai(e, t, n) {
	...
	try {
		...
		throw e instanceof Error && e.name === "AbortError" ? Ur(n) : e;
	} finally {
		o && clearTimeout(o);
	}
}
async function oi(e) {
	if (!e.ok) {
		...
		throw new re(Wr(e.status, e.statusText, t), e.status, t);
	}
}
async function si(e, t, n, r) {
	...
	if (l.data === void 0 && l.response && !ue(l)) throw await oi(l.response), l.error;
	let u = de(l);
...
var ci = class {
	...
	setConfig({ apiKey: e, devApiKey: t, sourceLocale: n, targetLocale: r, locales: i, projectId: a, customMapping: o, baseUrl: s }) {
		if (e && (this.apiKey = e), t && (this.devApiKey = t), a && (this.projectId = a), n && (this.sourceLocale = mr(n), !N(this.sourceLocale, o))) throw Error(Jr(this.sourceLocale));
		if (r && (this.targetLocale = mr(r), !N(this.targetLocale, o))) throw Error(Jr(this.targetLocale));
		if (i) {
			let e = [], t = [];
			if (i.forEach((n) => {
				let r = mr(n);
				N(r) ? e.push(r) : t.push(n);
			}), t.length > 0) throw Error(Yr(t));
			this.locales = e;
		}
		s && (this.baseUrl = s), o && (this.customMapping = o, this.reverseCustomMapping = Object.fromEntries(Object.entries(o).filter(([, e]) => e && typeof e == "object" && "code" in e).map(([e, t]) => [t.code, e]))), this._localeConfig = new lr({
	...
	getRegionProperties(e = this.getLocaleProperties().regionCode, t) {
		...
		return ur(e, this.targetLocale, t);
	}
	requiresTranslation(e = this.sourceLocale, t = this.targetLocale, n = this.locales, r = this.customMapping) {
		...
		return r === this.customMapping ? this.localeConfig.requiresTranslation(t, e, n) : hr(e, t, n, r);
	}
	determineLocale(e, t = this.locales || [], n = this.customMapping) {
		return n === this.customMapping ? this.localeConfig.determineLocale(e, t ?? []) : gr(e, t, n);
	}
	...
	resolveCanonicalLocale(e = this.targetLocale, t = this.customMapping) {
		...
		return t === this.customMapping ? this.localeConfig.resolveCanonicalLocale(e) : pr(e, t);
	}
	resolveAliasLocale(e, t = this.customMapping) {
		if (!e) throw Error(R("resolveAliasLocale"));
		return t === this.customMapping ? this.localeConfig.resolveAliasLocale(e) : _r(e, t);
	}
...
function ui() {
	let e = fi();
	return e === void 0 ? pi(() => void 0) : e;
}
function di(e) {
	return e?.toUpperCase() === li;
}
function fi() {
...
function gi() {
	let e = hi("i18n")?.i18nConfig;
	return _i(e) ? e.isDebugLoggingEnabled() : di(ui());
}
function _i(e) {
...
function Ci() {
	let e = wi(() => "production");
	return e ? e === "development" ? "development" : "production" : wi(() => !1) === !0 ? "development" : "production";
}
function wi(e) {
	try {
		return e();
...
function Ti(e, t) {
	if (!t) return;
	let n = Ei(e), r = Di(e), i = [...n, ...r];
	if (i.forEach((e) => {
		B.error(`I18nConfig: ${Oi(e)}`);
...
var ki = class extends lr {
	constructor(e = {}) {
		let t = Ni(e);
		super(Ai(e, t)), this.runtimeConfig = {
			...
			_tagIds: e._tagIds
		}, this.gtServicesEnabled = t, this.logLevel = ui();
	}
	getDefaultLocale() {
	...
	getLocaleConfig(e) {
		return !e || !Mi(e) ? this : new lr(ji(e));
	}
...
function ji({ defaultLocale: e = "en", locales: t = [], customMapping: n } = {}) {
	return {
		defaultLocale: e,
...
var Li = (e) => `String interpolation failed for message: "${e}".`;
function Ri(e, t, n, r) {
	try {
		return fr(e, {
			variables: t,
		...
		return B.warn(Li(e)), e;
	}
}
function zi(e, t) {
	if (!e) return e;
	let n = t.$_fallback, r = Ii(t);
	try {
		let i = vn(n || "");
		return dr(Ri(Object.keys(i).length ? bn(e) : e, {
			...r,
			...i,
			[D]: "other"
		}, t.$locale, t.$format), { maxChars: t.$maxChars });
	} catch {
		return B.warn(Li(e)), t.$_fallback == null ? dr(e, { maxChars: t.$maxChars }) : zi(t.$_fallback, {
			...t,
			$_fallback: void 0
		});
	}
...
function Vi({ source: e, target: t, options: n, sourceLocale: r }) {
	return t == null ? Hi(e, Ui(n, r)) : Hi(t, {
		$_fallback: e,
		...n
	});
}
function Hi(e, t) {
	switch (t.$format ?? "STRING") {
		case "ICU": return zi(e, t);
		case "I18NEXT":
		case "STRING": return Bi(e, t);
		default: return e;
	}
}
function Ui(e, t) {
...
var { getConditionStore: Ki, setConditionStore: qi } = Gi(S({
	source: "gt-i18n",
	severity: "Error",
...
async function Qi({ locale: e, enableI18n: t }, n) {
	let r = yi(), i = V().getDefaultLocale(), a = await r.getLookupTranslation(t ? e : i);
	return (n, r = {}) => {
		let o = Wi(t ? r.$locale ?? e : V().getDefaultLocale(), r, "ICU");
		return Vi({
			source: n,
			target: a(n, o),
			options: o,
...
async function $i({ locale: e, enableI18n: t }) {
	let n = await Qi({
		locale: e,
		enableI18n: t
	});
	return (e, t = {}) => {
		if (e == null) return e;
		let r = Yi(e) ?? {};
		return Xi(r) ? n(r.$_source, r) : n(e, t);
	};
}
function ea({ sourceLocale: e, targetLocale: t, sourceEntry: n, target: r, dictionaryOptions: i, options: a = {} }) {
	let o = Wi(t, {
		...i,
		...Ii(a)
	}, i.$format);
...
function sa(e) {
	return typeof e == "string" ? !0 : !Array.isArray(e) || typeof e[0] != "string" ? !1 : e.length === 1 || e.length === 2 && ca(e[1]);
}
function ca(e) {
	if (typeof e != "object" || !e || Array.isArray(e)) return !1;
	let t = e;
	return (t.$context === void 0 || typeof t.$context == "string") && (t.$format === void 0 || la(t.$format)) && (t.$maxChars === void 0 || typeof t.$maxChars == "number");
}
function la(e) {
...
function pa({ sourceObject: e, targetObject: t, translate: n }) {
	if (!U(e)) return fa({
		sourceObject: e,
		targetObject: t,
		translate: n
...
async function ma({ locale: e, enableI18n: t, rootId: n }) {
	let r = yi(), i = V().getDefaultLocale(), a = t ? e : i, [o, s, c] = await Promise.all([
...
function ya({ type: e, remoteTranslationLoaderParams: t, loadTranslations: n }) {
	...
	switch (e) {
		...
		case "disabled": return r === null ? async () => ({}) : ba(S({
...
var Sa = class {
	...
	load(e) {
		return xa(this.pendingLoads, e, () => this.loadResource(e).then((t) => (this.set(e, t), t)));
	}
	getExpiresAt() {
...
function Ta(e) {
	return {
		maxConcurrentRequests: wa(e?.maxConcurrentRequests, Ca.maxConcurrentRequests, !0),
		maxBatchSize: wa(e?.maxBatchSize, Ca.maxBatchSize, !0),
		batchInterval: wa(e?.batchInterval, Ca.batchInterval)
	};
}
var Ea = class {
	...
	async miss(e) {
		let t = this.getCacheKey(e), n = await xa(this.pendingTranslations, t, () => this.translate(e));
...
async function Oa({ key: e, sourceValue: t, targetValue: n, translateEntry: r }) {
	...
	let a = U(n) ? n : {}, o = /* @__PURE__ */ new Set([...Object.keys(t), ...Object.keys(a)]), s = await Promise.all(Array.from(o).map(async (n) => {
		...
		na(n, i);
		let o = t[n];
		return o === void 0 ? [n, W(a[n])] : [n, await Oa({
			key: i,
			sourceValue: o,
			targetValue: a[n],
			translateEntry: r
...
var Aa = class {
	...
	getEntry(e) {
		let t = G(ra(this.cache, e));
		if (t !== void 0) return ka(t);
	}
	getValue(e) {
		let t = ra(this.cache, e);
	...
	async materializeValue(e, t, n = ra(this.cache, e)) {
		return xa(this.pendingMaterializations, e, () => Oa({
			key: e,
			sourceValue: t,
			targetValue: n,
			translateEntry: async (e, t) => aa(await this.materializeEntry(e, t))
		}).then((t) => (this.setValue(e, t), t)));
	}
	async materializeEntry(e, t) {
		return ka(await xa(this.pendingTranslations, e, () => this.runtimeTranslate(e, t).then((t) => {
			ia(this.cache, e, t);
			let n = G(t);
...
function ja(e, t) {
	for (let [n, r] of Object.entries(t)) {
		let t = e[n];
		U(t) && U(r) ? ja(t, r) : e[n] = W(r);
	}
}
var Ma = 12e3, Na = class {
	constructor(e) {
		Pa(e), this.config = {
			projectId: e.projectId,
			devApiKey: e.devApiKey,
		...
		this.createTranslateMany = ga(V().getGTClass(), this.config.runtimeTranslation?.timeout ?? Ma, {
	...
	async translateDictionaryEntry(e, t, n) {
		let r = await this.lookupTranslationWithFallbackResolved(e, n.entry, oa(n.options));
...
function Fa(e) {
	let t = "hash" in e ? e.hash : Ji(e.message, e.options);
	return `${e.locale}:${t}`;
}
var Ia = [];
function La({ locale: e, enableI18n: t, localesProp: n = Ia }) {
...
var za = "generaltranslation.locale-reset", Ba = "server-render", Va = Symbol.for("generaltranslation.react-core.ReactI18nConfig"), Ha = class extends ki {
	renderStrategy;
	localeCookieName;
	regionCookieName;
	enableI18nCookieName;
	constructor(e = {}, t = Ba) {
		super(e), Ga(t), Object.defineProperty(this, Va, { value: !0 }), this.renderStrategy = t, this.localeCookieName = e.localeCookieName ?? "generaltranslation.locale", this.regionCookieName = e.regionCookieName ?? "generaltranslation.region", this.enableI18nCookieName = e.enableI18nCookieName ?? "generaltranslation.enable-i18n";
	}
	getRenderStrategy() {
...
var { getConditionStore: Y, setConditionStore: qa, isConditionStoreInitialized: Ja } = Gi(S({
	source: "@generaltranslation/react-core",
	severity: "Error",
...
function to(e, t, n) {
	let r = "", i = null;
	return typeof e == "number" && !i && n && (r = mn(e, Object.keys(n).filter(pn), t)), r && !i && (i = n[r]), i;
}
function no({ renderVariable: t }) {
	return function({ children: n, defaultLocale: r = "en", enableI18n: i }) {
		let a = (n) => {
			let a = eo(n);
			if (Qa(n.props)) {
				let { variableType: e, variableValue: a, variableOptions: o, injectionType: s } = $a(n.props);
				return t({
					variableType: e,
					variableValue: a,
					variableOptions: o,
			...
			if (a?.transformation === "plural") {
				let e = a.branches || {};
				return typeof n.props.n == "number" ? s(to(n.props.n, [r], e) ?? n.props.children) : n.props.children == null ? null : s(n.props.children);
			}
...
function ro({ renderVariable: t }) {
	let n = no({ renderVariable: t });
	function r({ sourceElement: t, targetElement: r, locales: a = ["en"], enableI18n: o }) {
		...
		if (u && Object.entries(xn).forEach(([e, t]) => {
			u[e] && (d[t] = u[e]);
		}), l === "plural") {
			let e = t.props.n;
			return typeof e == "number" ? i({
				source: to(e, a, c.branches || {}) ?? t.props.children,
				target: to(e, a, r.d?.b || {}) ?? r.c,
				locales: a,
				enableI18n: o
			}) : n({
				children: t,
	...
	function i({ source: i, target: a, locales: o = ["en"], enableI18n: s }) {
		...
		if (Array.isArray(a) && !Array.isArray(i) && i && (i = [i]), Array.isArray(i) && Array.isArray(a)) {
			let n = {}, c = {}, l = {}, u = i.filter((t) => {
				if (e.isValidElement(t)) {
					if (Qa(t.props)) {
						let { variableName: e, variableValue: r, variableOptions: i, injectionType: a } = $a(t.props);
						n[e] = r, c[e] = i, l[e] = a;
					} else return !0;
				}
				return !1;
			...
			return a.map((i, a) => {
				...
				if (an(i)) return _(e.Fragment, { children: t({
					variableType: i.v || "v",
					variableValue: n[i.k],
		...
		if (a && typeof a == "object" && !Array.isArray(a)) {
			...
			if (e.isValidElement(i)) {
				...
				if (Qa(i.props)) {
					let { variableValue: e, variableOptions: n, variableType: r, injectionType: a } = $a(i.props);
					return t({
						variableType: r,
						variableValue: e,
						variableOptions: n,
...
var oo = { display: "contents" }, so = globalThis.navigator?.product === "ReactNative";
function co(e) {
	if (e == null || typeof e == "boolean" || e === "") return !0;
	if (Array.isArray(e)) return !e.some((e) => !co(e));
	if (o(e) && e.type === n) {
...
function lo(e, t) {
	return so || !io() ? e : o(e) && typeof e.type == "string" ? r(e, { "data-_gt-hash": t }) : co(e) ? e : a("span", {
		"data-_gt-hash": t,
		style: oo
	}, e);
}
function uo({ renderDefaultChildren: e, renderTranslatedChildren: t }) {
...
function fo(e) {
	let t = Ya(e), n = no({ renderVariable: t }), r = ro({ renderVariable: t });
	return {
		renderVariable: t,
...
function mo(e) {
	Wa(e, "server-render"), Ra(new po(e));
}
var ho = z({
	namespace: "reactCore",
	key: "i18nStore",
...
function yo(e, t) {
	let n = t.options.$_hash ?? Ji(t.message, t.options);
	return e?.[t.locale]?.[n];
}
function bo(e, t) {
	return G(So(e, t));
...
function So(e, { locale: t, id: n }) {
	let r = e?.[t];
	if (!r) return;
	if (!n) return r;
	let i = r;
	for (let e of n.split(".")) {
		if (!Co(e) || !U(i) || !Object.prototype.hasOwnProperty.call(i, e)) return;
		i = i[e];
	}
	return i;
}
...
var To = class {
	...
	subscribeToTranslate = (e, t) => {
		let n = Fa(e);
		return vo(this.translateListeners, (e) => {
			Fa(e) === n && t();
		});
	};
	subscribeToTranslationEvents = (e) => vo(this.translateListeners, e);
	subscribeToDictionaryEntryEvents = (e) => vo(this.dictionaryEntryListeners, e);
	subscribeToDictionaryObjectEvents = (e) => vo(this.dictionaryObjectListeners, e);
	getTranslateSnapshot = (e, t = {}) => yo(t, e) ?? q().lookupTranslation(e.locale, e.message, e.options);
	getDictionaryEntrySnapshot = (e, t = {}) => bo(t, e) ?? q().lookupDictionary(e.locale, e.id);
	getDictionaryObjectSnapshot = (e, t = {}) => xo(t, e) ?? q().lookupDictionaryObj(e.locale, e.id);
...
function Do() {
	return Eo.isInitialized() || Eo.set(i(void 0)), Eo.get();
}
function Oo() {
	let e = s(Do());
...
function Ao() {
	return Oo()?.conditionStore ?? Y();
}
function jo() {
	return Ao().getLocale();
}
function Mo() {
	return Ao().getEnableI18n();
}
function No() {
	return u(() => V().getDefaultLocale(), []);
}
function Po() {
	return Oo()?.i18nStore || go();
}
function Fo() {
...
var Ro = Lo;
function zo({ _enableI18n: e, _locale: t, children: n, currency: r = "USD", options: i = {}, locales: a = [] }) {
	let o = La({
		locale: t,
		enableI18n: e,
		localesProp: a
...
Bo._gtt = "variable-currency-automatic", Vo._gtt = "variable-currency";
...
Uo._gtt = "variable-datetime-automatic", Wo._gtt = "variable-datetime";
...
Ko._gtt = "variable-number-automatic", qo._gtt = "variable-number";
function Jo({ _enableI18n: e, _locale: t, date: n, children: r, value: i, unit: a, baseDate: o, locales: s = [], options: c = {} }) {
	let l = La({
		locale: t,
		enableI18n: e,
		localesProp: s
...
Yo._gtt = "variable-relative-time-automatic", Xo._gtt = "variable-relative-time";
function Zo({ children: e }) {
	return e;
}
function Qo({ children: e }) {
	return Zo({ children: e });
}
function $o({ children: e }) {
	return Zo({ children: e });
}
Qo._gtt = "variable-variable", $o._gtt = "variable-variable-automatic";
function es(e) {
	let t = Po(), n = Fo();
	return Ro(), p((n) => t.subscribeToTranslate(e, n), () => t.getTranslateSnapshot(e, n), () => t.getTranslateSnapshot(e, n));
}
var { renderVariable: ts, renderDefaultChildren: ns, renderTranslatedChildren: rs, renderPreparedT: is } = fo({
	Currency: Vo,
	GtInternalCurrency: Bo,
	DateTime: Wo,
	GtInternalDateTime: Uo,
	Num: qo,
	GtInternalNum: Ko,
	RelativeTime: Xo,
	GtInternalRelativeTime: Yo,
	Var: Qo,
	GtInternalVar: $o
});
function as(t, n = 0) {
	let r = n, i = (e) => {
		...
		if (a) {
			...
			if ((e[1] === "automatic" || e[2] === "automatic") && (i.injectionType = "automatic"), e[0] === "translate" && (e[0] = "fragment"), e[0] === "variable" && (i.variableType = e?.[1] || "variable"), e[0] === "plural") {
				let e = Object.entries(n).reduce((e, [t, n]) => (pn(t) && (e[t] = as(n, r)), e), {});
				Object.keys(e).length && (i.branches = e);
			}
			if (e[0] === "branch") {
				let { children: e, branch: t, ...a } = n, o = Object.fromEntries(Object.entries(a).filter(([e]) => !e.startsWith("data-"))), s = Object.entries(o).reduce((e, [t, n]) => (e[t] = as(n, r), e), {});
...
function ss(e, t) {
	let { type: n, props: i } = e, a = us(n);
	if (typeof i != "object" || !i) return e;
	if (a) {
		let { componentType: n, injectionType: o } = a;
		if (n === "variable") return e;
		if (n === "branch") return r(e, { ...Object.entries(i).reduce((e, [n, r]) => (e[n] = n !== "branch" && !n.startsWith("data-") ? cs(r, t) : r, e), {}) });
		if (n === "plural") return r(e, { ...Object.entries(i).reduce((e, [n, r]) => (e[n] = pn(n) || n === "children" ? cs(r, t) : r, e), {}) });
		if (n === "derive") return r(e, {
			...i,
			..."children" in i && { children: ls(i.children, t + 1) }
		});
		if (n === "translate" && o === "automatic" && t > 0) return "children" in i ? ls(i.children, t) : void 0;
		n === "translate" && o === "automatic" && console.warn(ds);
	}
	return r(e, {
		...i,
		..."children" in i && { children: ls(i.children, t) }
	});
}
function cs(e, t) {
	return o(e) ? ss(e, t) : e;
...
function ls(e, n) {
	return Array.isArray(e) ? t.map(e, (e) => cs(e, n)) : cs(e, n);
}
function us(e) {
...
var ds = "'@generaltranslation/react-core Warning: A <_T> component was found injected outside of a <Derive> boundary. This may affect translation resolution for this component.";
function fs(t) {
...
var ps = (e) => {
	...
	let r = Object.entries(xn).reduce((e, [n, r]) => {
		let i = t[r];
	...
	let { props: t } = e, n = { t: ps(e) };
	if (t["data-_gt"]) {
		let e = t["data-_gt"], r = e.transformation;
		if (r === "variable") {
			let n = e.variableType || "variable", r = Za(t, n), i = gn(n);
			return {
				i: e.id,
				k: r,
				v: i
		...
		n.i = e.id, n.d = ms(r, t, e.branches);
	}
	return t.children && (n.c = _s(t.children)), n;
}, gs = (e) => fs(e) ? hs(e) : typeof e == "number" ? e.toString() : e;
function _s(e) {
	return Array.isArray(e) ? e.map(gs) : gs(e);
}
function vs({ sourceChildren: e, params: t, locale: n }) {
	let r = ys(e), i = bs(r), a = xs({
		options: Ss(t),
		locale: n
	});
	return ao(i, a), {
...
function xs({ options: e, locale: t }) {
	return {
		...e,
		$locale: t
	};
...
function Cs({ sourceChildren: e, params: t, _locale: n, _enableI18n: r }) {
	let i = jo(), a = Mo(), o = No(), s = n ?? i, c = r ?? a;
	return {
		defaultLocale: o,
		enableI18n: c,
		locale: s,
...
function ws({ children: e, _locale: t, _enableI18n: n, _renderPreparedT: r = is, ...i }) {
	let { defaultLocale: a, locale: o, enableI18n: s, targetOptions: c, taggedSourceChildren: l, sourceJsxChildren: u, shouldTranslate: f } = Cs({
		sourceChildren: e,
		params: i,
		_locale: t,
		_enableI18n: n
...
var ks = class {
	constructor(e) {
		this.getLocale = () => As(this.customGetLocale), this.setLocale = (e) => {
			this.updateLocale(e), Z({
				cookieName: za,
				value: "true"
			}), this.reload();
		}, this.getRegion = () => Ds({ cookieName: J().getRegionCookieName() }) || this.customGetRegion?.(), this.setRegion = (e) => {
			this.updateRegion(e), this.reload();
		}, this.getEnableI18n = () => {
			let e = Ds({ cookieName: J().getEnableI18nCookieName() });
...
function As(e) {
	let t = J(), n = Os(t.getLocaleCookieName());
...
}), { setConditionStore: Is, isConditionStoreInitialized: Ls } = Gi(Fs), { getConditionStore: Rs, setConditionStore: zs, isConditionStoreInitialized: Bs } = Gi(Fs);
function Vs(e) {
	let t = Hs(e), n = Us(e), r = Ws(e);
	if (Bs()) {
		let e = Rs();
...
function Hs({ _getLocale: e, locale: t }) {
	let n = J(), r = [];
	return t && r.push(...Array.isArray(t) ? t : [t]), e && r.push(e()), r.push(...Os(n.getLocaleCookieName())), n.resolveSupportedLocale(r);
}
function Us({ _getRegion: e, region: t }) {
	return Ds({ cookieName: J().getRegionCookieName() }) || e?.() || t;
}
function Ws({ enableI18n: e, _getEnableI18n: t }) {
	if (e !== void 0) return e;
	let n = Ds({ cookieName: J().getEnableI18nCookieName() });
...
function Ks() {
	return {
		projectId: qs(() => void 0) || Js(),
		devApiKey: Ci() === "development" ? qs(() => void 0) || Ys() : void 0
	};
}
function qs(e) {
	try {
...
function Js() {
	try {
		return Xs(process.env.VITE_GT_PROJECT_ID);
	} catch {
		return;
	}
}
function Ys() {
	try {
		return Xs(process.env.VITE_GT_DEV_API_KEY);
	} catch {
		return;
	}
}
...
function tc() {
	...
	c(() => {
		...
		if (e !== "auto") return;
		let t = window.matchMedia("(prefers-color-scheme: dark)"), n = () => ec("auto");
	...
	function n() {
		...
		t(n), ec(n), window.localStorage.setItem("theme", n);
	}
	...
	return _("button", {
		...
		children: e === "auto" ? _(X, { children: "Theme: Auto" }) : e === "dark" ? _(X, { children: "Theme: Dark" }) : _(X, { children: "Theme: Light" })
	});
}
var nc = [
	"en",
...
function ic() {
	let e = g({ strict: !1 }).locale ?? "en", t = h(), n = (e) => {
		try {
				...
				children: [_(m, {
					...
					children: [
						...
						_(m, {
							...
							activeProps: { className: "is-active" },
							className: "nav-link",
							children: _(X, { children: "Methodology" })
						}),
						v("div", {
							className: "relative",
							children: [v("button", {
								...
								onMouseLeave: () => t(!1),
								onClick: () => t(!e),
								children: [_(X, { children: "Mock Pages" }), _(y, {
									size: 14,
								...
								children: _("div", {
									...
									children: [
										{
											to: "/$locale/products",
											label: _(X, { children: "Products" })
										},
										{
											to: "/$locale/pricing",
											label: _(X, { children: "Pricing" })
										},
										{
											to: "/$locale/team",
											label: _(X, { children: "Team" })
										},
										{
											to: "/$locale/blog",
											label: _(X, { children: "Blog" })
										},
										{
											to: "/$locale/careers",
											label: _(X, { children: "Careers" })
										},
										{
											to: "/$locale/faq",
											label: _(X, { children: "FAQ" })
										},
										{
											to: "/$locale/contact",
											label: _(X, { children: "Contact" })
										},
										{
											to: "/$locale/settings",
											label: _(X, { children: "Settings" })
										}
									].map((e) => _(m, {
										preload: !1,
										to: e.to,
...
function lc({ source: e, severity: t, whatHappened: n, reassurance: r, why: i, fix: a, wayOut: o, details: s, docsUrl: c }) {
...
dc.set;
var fc = dc.isInitialized;
function pc(e, t = hc()) {
	let { pathname: n } = gc(e, t), r = n.match(/^\/([^/]+)(?:\/|$)/);
	if (!r) return;
	let i;
	try {
...
function mc(e, t, n = hc()) {
	let r = J(), { basepath: i, pathname: a } = gc(e, n), o = pc(a, "/") ? a.replace(/^\/[^/]+/, "") || "/" : a, s = r.resolveSupportedLocale(t);
...
function yc(e, t, n = new URL(e.url).pathname) {
...
var bc = b().server(() => $().getLocale()).client(() => Y().getLocale());
...
}), b().server(xc).client(() => bc());
function xc({ defaultLocale: e, locales: t, customMapping: n }) {
	if (fc()) {
		let e = $();
...
ee().server(({ request: e, pathname: t, next: n }) => $().run(e, () => n(), t));
function Cc(e) {
	let t = e.localeRouting && !e._reload ? {
		...e,
		_reload: ({ locale: e }) => {
			let t = mc(window.location.pathname, e), n = new URL(window.location.href);
