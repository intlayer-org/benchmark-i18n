...
function Ks() {
	return p("section", {
		...
		children: [f("h2", {
			...
			children: f(Z, { children: "What We Measure" })
		}), f("ul", {
			className: "space-y-4",
			children: [
				{
					metric: f(Z, { children: "Bundle size impact" }),
					desc: f(Z, { children: "The additional JavaScript bytes sent to users when the i18n library and its translation files are included. This directly affects download time on slow networks." })
				},
				{
					metric: f(Z, { children: "Rendering overhead" }),
					desc: f(Z, { children: "How much extra time the library adds to React's render cycle. Libraries that inject translations via a single context provider can cause unnecessary re-renders across the component tree." })
				},
				{
					metric: f(Z, { children: "Hydration cost" }),
					desc: f(Z, { children: "During SSR, translation data is serialized into HTML. Large dictionaries increase the HTML payload and slow down hydration — the moment the page becomes interactive." })
				},
				{
					metric: f(Z, { children: "Lazy loading effectiveness" }),
					desc: f(Z, { children: "Whether splitting translations by route or namespace actually reduces the initial load, and what trade-offs it introduces (waterfall requests, FOUC, cache complexity)." })
				},
				{
					metric: f(Z, { children: "Locale switch speed" }),
					desc: f(Z, { children: "How fast the app can switch from one language to another at runtime — including fetching new translations, re-rendering components, and updating the DOM." })
				}
			].map((e) => p("li", {
