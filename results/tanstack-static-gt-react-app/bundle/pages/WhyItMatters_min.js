...
function ie(e) {
	return re(e.error) || typeof e.error == "string";
}
function ae(e) {
	if (e.data !== void 0) return e.data;
	if (e.response) {
		let t = re(e.error) ? e.error.error : typeof e.error == "string" ? e.error : e.response.statusText;
		throw new v(t, e.response.status, t);
	}
	throw e.error;
}
var oe = (e) => e.client.post({
	security: [{
		scheme: "bearer",
		type: "http"
	}],
...
}), se = 3, ce = 500, le = 6e4, ue = /* @__PURE__ */ new Set([
	"GET",
	"HEAD",
	"OPTIONS",
	"PUT",
...
function fe({ fetch: e = globalThis.fetch, timeoutMs: t = de } = {}) {
	return async (n, r) => {
		if (t === !1) return e(n, r);
...
var pe = (e) => new Promise((t) => setTimeout(t, e));
function me(e) {
	if (!e) return;
...
function ge(e, t, n) {
	return e?.status === 429 ? he(e.headers.get("Retry-After")) ?? me(e.headers.get("RateLimit-Reset")) ?? le : ce * (n === "linear" ? t + 1 : 2 ** t);
}
function _e({ fetch: e = globalThis.fetch, retryPolicy: t = "exponential" } = {}) {
	return async (n, r) => {
		let i = new Request(n, r), a = t === "none" ? 0 : se, o = ue.has(i.method);
		for (let n = 0; n <= a; n++) {
			let r;
			try {
				r = await e(i.clone());
			...
			r?.body?.cancel(), await pe(ge(r, n, t));
		}
		throw Error("Max retries exceeded");
	};
}
var ve = { bodySerializer: (e) => JSON.stringify(e, (e, t) => typeof t == "bigint" ? t.toString() : t) }, ye = ({ onRequest: e, onSseError: t, onSseEvent: n, responseTransformer: r, responseValidator: i, sseDefaultRetryDelay: a, sseMaxRetryAttempts: o, sseMaxRetryDelay: s, sseSleepFn: c, url: l, ...u }) => {
...
}, Ce = ({ allowReserved: e, explode: t, name: n, style: r, value: i }) => {
	if (!t) {
		let t = (e ? i : i.map((e) => encodeURIComponent(e))).join(xe(r));
		switch (r) {
			case "label": return `.${t}`;
	...
	let a = be(r), o = i.map((t) => r === "label" || r === "simple" ? e ? t : encodeURIComponent(t) : we({
		allowReserved: e,
		name: n,
		value: t
	})).join(a);
	return r === "label" || r === "matrix" ? a + o : o;
}, we = ({ allowReserved: e, name: t, value: n }) => {
	if (n == null) return "";
...
}, Te = ({ allowReserved: e, explode: t, name: n, style: r, value: i, valueOnly: a }) => {
	...
	let o = Se(r), s = Object.entries(i).map(([t, i]) => we({
		allowReserved: e,
		name: r === "deepObject" ? `${n}[${t}]` : t,
		value: i
	})).join(o);
	return r === "label" || r === "matrix" ? o + s : s;
}, Ee = /\{[^{}]+\}/g, De = ({ path: e, url: t }) => {
	let n = t, r = t.match(Ee);
	if (r) for (let t of r) {
	...
	return n;
}, Oe = ({ baseUrl: e, path: t, query: n, querySerializer: r, url: i }) => {
...
var Ae = async (e, t) => {
...
}, Ne = (e, t) => t ? !!(e.headers.has(t) || e.query?.[t] || e.headers.get("Cookie")?.includes(`${t}=`)) : !1, Pe = async ({ security: e, ...t }) => {
	for (let n of e) {
		if (Ne(t, n.name)) continue;
		let e = await Ae(n, t.auth);
		if (!e) continue;
	...
	path: e.path,
	query: e.query,
	querySerializer: typeof e.querySerializer == "function" ? e.querySerializer : je(e.querySerializer),
	url: e.url
}), Ie = (e, t) => {
	let n = {
		...e,
	...
	return n.baseUrl?.endsWith("/") && (n.baseUrl = n.baseUrl.substring(0, n.baseUrl.length - 1)), n.headers = Re(e.headers, t.headers), n;
}, Le = (e) => {
	let t = [];
	...
	for (let n of e) {
		if (!n) continue;
		let e = n instanceof Headers ? Le(n) : Object.entries(n);
...
}), He = { "Content-Type": "application/json" }, Ue = (e = {}) => ({
	...ve,
	headers: He,
	parseAs: "auto",
	querySerializer: Ve,
	...
	let t = Ie(Ue(), e), n = () => ({ ...t }), r = (e) => (t = Ie(t, e), n()), i = Be(), a = async (e) => {
		let n = {
			...t,
			...e,
			fetch: e.fetch ?? t.fetch ?? globalThis.fetch,
			headers: Re(t.headers, e.headers),
			serializedBody: void 0
		};
		return n.security && await Pe({
			...n,
			security: n.security
		...
		if (c.ok) {
			let e = (t.parseAs === "auto" ? Me(c.headers.get("Content-Type")) : t.parseAs) ?? "json";
...
function Ge(e) {
	...
	return e.apiKey && t.set("Authorization", `Bearer ${e.apiKey}`), e.projectId && t.set("gt-project-id", e.projectId), We({
		baseUrl: e.baseUrl,
		fetch: _e({
			fetch: fe({
				fetch: e.fetch,
...
var C = {
	...
	number: 0,
	dateTime: 1
}, Ke = /^\.(?:(0+)(\*)?|(#+)|(0+)(#+))$/, qe = /^@+(\+|#+)?[rs]?$/, Je = /^(?:(?:\+|#+)[rs]?|[rs])$/, Ye = /(\*)(0+)|(#+)(0+)|(0+)/g, Xe = /[\t-\r \x85\u200E\u200F\u2028\u2029]+/u, Ze = /(?:[Eec]{1,6}|G{1,5}|[Qq]{1,5}|(?:[yYur]+|U{1,5})|[ML]{1,5}|d{1,2}|D{1,3}|F|[abB]{1,5}|[hHkK]{1,2}|w{1,2}|W|m{1,2}|s{1,2}|[zZOvVxX]{1,4})(?=([^']*'[^']*')*[^']*$)/g, Qe = {
	floor: "floor",
	ceiling: "ceil",
	down: "trunc",
	up: "expand",
...
function et(e) {
	let t = e.split(Xe).filter(Boolean);
...
function tt(e) {
	...
	for (let n of e) {
		...
		switch (n.stem) {
			...
			case "measure-unit":
			case "unit":
				nt(n), t.style = "unit", t.unit = e.replace(/^(.*?)-/, "");
				continue;
			case "compact-short":
			case "K":
			...
			case "scale":
				...
				continue;
			case "integer-width":
				if (nt(n), n.options.length > 1) throw RangeError("integer-width stems only accept a single optional option");
				at(t, e);
				continue;
		}
		if (n.stem.slice(0, 14) === "rounding-mode-") {
			let e = Qe[n.stem.slice(14)];
		...
		if (!ot(t, n)) {
			if (qe.test(n.stem)) {
				Object.assign(t, st(n.stem));
				continue;
			}
			if (Je.test(n.stem)) throw SyntaxError("Significant precision must start with @.");
			rt(t, n.stem) || it(t, n.stem);
		}
	}
	return t;
}
...
function it(e, t) {
	...
	return e.notation = n[1] === "EE" ? "engineering" : "scientific", n[2] && rt(e, n[2]), e.minimumIntegerDigits = n[3].length, !0;
}
function at(e, t) {
	t.replace(Ye, (t, n, r, i, a, o) => {
...
function st(e) {
	let t = {};
	if (e.endsWith("r") && (t.roundingPriority = "morePrecision"), e.endsWith("s") && (t.roundingPriority = "lessPrecision"), Je.test(e)) throw SyntaxError("Significant precision must start with @.");
...
function ct(e) {
	...
	for (let [n] of e.matchAll(Ze)) {
		...
		switch (n[0]) {
			...
			case "K":
			case "k":
				t.hourCycle = $e[n[0]], t.hour = e === 2 ? "2-digit" : "numeric";
				break;
			case "m":
...
var ft = /^[A-Za-z]$/u, pt = /^[-.0-9_A-Za-z\u00B7\u00C0-\u00D6\u00D8-\u00F6\u00F8-\u037D\u037F-\u1FFF\u200C-\u200D\u203F-\u2040\u2070-\u218F\u2C00-\u2FEF\u3001-\uD7FF\uF900-\uFDCF\uFDF0-\uFFFD\u{10000}-\u{EFFFF}]$/u;
function T(e) {
	return ft.test(e ?? "");
}
function mt(e) {
...
var ht = /[\t-\r \x85\u200E\u200F\u2028\u2029]/u, gt = /[\t-\r \x85\xA0\u1680\u2000-\u200A\u2028\u2029\u202F\u205F\u3000\x21-\x2F\x3A-\x40\x5B-\x5E\x60\x7B-\x7E\xA1-\xA7\xA9\xAB\xAC\xAE\xB0\xB1\xB6\xBB\xBF\xD7\xF7\u2010-\u2027\u2030-\u203E\u2041-\u2053\u2055-\u205E\u2190-\u245F\u2500-\u2775\u2794-\u2BFF\u2E00-\u2E7F\u3001-\u3003\u3008-\u3020\u3030\uFD3E-\uFD3F\uFE45-\uFE46]/u, _t = class {
	constructor(e, t) {
	...
	readTagName() {
		let e = this.index;
		for (T(this.current()) || this.fail("INVALID_TAG", e), this.index += 1; !this.atEnd() && mt(this.current());) this.index += this.current().length;
			...
			if (i.slice(0, 2) === "::") {
				...
				if (n === "number") {
					...
					r = {
						...
						tokens: e,
						...s ? { location: s } : {},
						parsedOptions: this.options.shouldParseSkeletons ? tt(e) : {}
					};
				} else {
					t || this.failAt("EXPECT_DATE_TIME_SKELETON", e, this.index);
					let n = lt(t, this.options.locale);
					r = {
						type: w.dateTime,
						pattern: n,
						...s ? { location: s } : {},
						parsedOptions: this.options.shouldParseSkeletons ? ct(n) : {}
					};
				}
			} else r = i;
		}
	...
	current() {
		return yt(this.message, this.index);
	}
	peek() {
		let e = this.current();
		return yt(this.message, this.index + e.length);
	}
	atEnd() {
	...
	position(e) {
		let t = this.positions ??= vt(this.message);
		return {
			offset: e,
			line: t[0][e],
			column: t[1][e]
...
function Tt(e, t = "en", n = {}) {
	let r = Et(bt(e, { locale: It(t) }), {
		message: e,
		locales: t,
		variables: n
	});
...
function Et(e, t, n) {
	...
	for (let r of e) switch (r.type) {
		...
		case C.number: {
			let e = E(t.variables, r.value), { scale: n, ...a } = typeof r.style == "string" ? xt[r.style] ?? {} : r.style?.type === w.number ? r.style.parsedOptions : {}, o = Dt(e, n);
			i(Ot(t, a).format(o));
			break;
		}
		case C.date:
		case C.time: {
			let e = E(t.variables, r.value), n = r.type === C.date ? St : wt;
			i(kt(t, typeof r.style == "string" ? n[r.style] : r.style?.type === w.dateTime ? r.style.parsedOptions : r.type === C.time ? wt.medium : void 0).format(e));
			break;
		}
		case C.select: {
			let e = String(E(t.variables, r.value)), n = Pt(r.options, e) ?? r.options.other;
			if (!n) throw Ft(r.value, e, r.options);
			Et(n.value, t).forEach(i);
			break;
		}
		case C.plural: {
			let e = E(t.variables, r.value), n = `=${String(e)}`, a = Pt(r.options, n), o = typeof e == "bigint" ? e : Number(e), s = typeof o == "bigint" ? o - BigInt(r.offset) : o - r.offset;
			if (!a && Nt(r.options)) {
				let e = At(t, r.pluralType ?? "cardinal").select(Mt(s));
				a = Pt(r.options, e);
			}
			if (a ??= r.options.other, !a) throw Ft(r.value, e, r.options);
			Et(a.value, t, s).forEach(i);
			break;
		}
		case C.tag: {
			let e = E(t.variables, r.value);
			if (typeof e != "function") throw TypeError(`The ICU tag variable "${r.value}" must be a function.`);
			let a = e(Et(r.children, t, n));
...
function Ot(e, t = {}) {
	return jt(e.numberFormats ??= /* @__PURE__ */ new Map(), JSON.stringify(t), () => new Intl.NumberFormat(e.locales, t));
}
function kt(e, t) {
	return jt(e.dateTimeFormats ??= /* @__PURE__ */ new Map(), t ? JSON.stringify(t) : "", () => new Intl.DateTimeFormat(e.locales, t));
}
function At(e, t) {
	...
	return jt(e.pluralRules ??= /* @__PURE__ */ new Map(), t, () => new Intl.PluralRules(e.locales, { type: t }));
}
function jt(e, t, n) {
	let r = e.get(t);
	if (r) return r;
...
function Rt(e, t, n) {
	return e.map((r, i) => {
		switch (r.type) {
			case C.literal: return Vt(r, t, i > 0, i < e.length - 1 || n);
			case C.argument: return `{${r.value}}`;
			case C.date:
			case C.time:
			case C.number: return Ut(r);
			case C.select: return `{${r.value},select,${Gt(r.options, !1)}}`;
			case C.plural: {
				let e = r.pluralType === "cardinal" ? "plural" : "selectordinal", t = r.offset ? `offset:${r.offset} ` : "";
				return `{${r.value},${e},${t}${Gt(r.options, !0)}}`;
			}
			case C.pound: return "#";
			case C.tag: return `<${r.value}>${Rt(r.children, t, !0)}</${r.value}>`;
		}
	}).join("");
}
...
function Bt(e, t, n, r) {
	...
	let o = "", s = 0;
	for (let [r, a] of i) o += zt(e.slice(s, r), t, s !== 0, s === 0 && n, !0), o += `'${e.slice(r, a).replace(/'/g, "''")}'`, s = a;
	return o += zt(e.slice(s), t, s !== 0, s === 0 && n, r), o;
}
function Vt({ value: e }, t, n, r) {
	return Ht(e) ? e : Bt(e, t, n, r);
}
function Ht(e) {
	if (e[0] !== "<" || e.slice(-2) !== "/>") return !1;
	let t = e.slice(1, -2);
	if (!T(t[0])) return !1;
	for (let e of t.slice(1)) if (!mt(e)) return !1;
	return !0;
}
function Ut(e) {
	let t = e.type === C.number ? "number" : e.type === C.date ? "date" : "time";
	return `{${e.value}, ${t}${e.style ? `, ${Wt(e.style)}` : ""}}`;
}
function Wt(e) {
	return typeof e == "string" ? e : e.type === w.dateTime ? `::${e.pattern}` : `::${e.tokens.map(({ stem: e, options: t }) => e + t.map((e) => `/${e}`).join("")).join(" ")}`;
}
function Gt(e, t) {
	return Object.entries(e).map(([e, n]) => `${e}{${Rt(n.value, t, !0)}}`).join(" ");
}
function Kt(e) {
...
var D = "_gt_", Jt = RegExp(`^${D}\\d+$`), Yt = RegExp(`^${D}$`);
function Xt(e) {
...
function Xn(e, t = "en", n) {
	...
	e = j(e, n), t ||= "en";
	try {
		let i = A(e), a = O.get("Locale", e), o = a.language, s = Yn([
			r,
			e,
			i,
			o
			...
			t,
			"en"
		], g = O.get("DisplayNames", m, { type: "language" }), _ = O.get("DisplayNames", h, { type: "language" }), v = s?.name, y = s?.nativeName || s?.name, b = v || g.of(e) || e, x = y || _.of(e) || e, ee = s?.maximizedName || v || g.of(u) || e, te = s?.nativeMaximizedName || y || _.of(u) || e, S = s?.minimizedName || v || g.of(p) || e, ne = s?.nativeMinimizedName || y || _.of(p) || e, re = s?.languageName || v || g.of(o) || e, ie = s?.nativeLanguageName || y || _.of(o) || e, ae = s?.nameWithRegionCode || (c ? `${re} (${c})` : b), oe = s?.nativeNameWithRegionCode || (c ? `${ie} (${c})` : x) || ae, se = O.get("DisplayNames", m, { type: "region" }), ce = O.get("DisplayNames", h, { type: "region" }), le = s?.regionName || (d ? se.of(d) : "") || "", ue = s?.nativeRegionName || (d ? ce.of(d) : "") || "", de = O.get("DisplayNames", m, { type: "script" }), fe = O.get("DisplayNames", h, { type: "script" });
		return {
			code: i,
			name: b,
			nativeName: x,
...
function sr(e, t) {
	return t?.dataFormat === "STRING" ? e : gn(e, t?.locales, t?.variables);
}
function M(e, t) {
	return k(e, t);
}
...
var Dr = class {
	...
	3204031479,
	3329325298
]), F = /* @__PURE__ */ new Uint32Array(64), kr = class extends Dr {
	constructor(e) {
		super(64, e, 8, !1);
	}
	get() {
	...
	process(e, t) {
		...
		for (let e = 0; e < 64; e++) {
			let t = N(o, 6) ^ N(o, 11) ^ N(o, 25), u = l + t + Tr(o, s, c) + Or[e] + F[e] | 0, d = (N(n, 2) ^ N(n, 13) ^ N(n, 22)) + Er(n, r, i) | 0;
...
async function ti(e) {
	if (!e.ok) {
		...
		throw new v(zr(e.status, e.statusText, t), e.status, t);
	}
}
async function ni(e, t, n, r) {
	...
	if (l.data === void 0 && l.response && !ie(l)) throw await ti(l.response), l.error;
	let u = ae(l);
...
var ri = class {
	...
	setConfig({ apiKey: e, devApiKey: t, sourceLocale: n, targetLocale: r, locales: i, projectId: a, customMapping: o, baseUrl: s }) {
		if (e && (this.apiKey = e), t && (this.devApiKey = t), a && (this.projectId = a), n && (this.sourceLocale = lr(n), !M(this.sourceLocale, o))) throw Error(Ur(this.sourceLocale));
		if (r && (this.targetLocale = lr(r), !M(this.targetLocale, o))) throw Error(Ur(this.targetLocale));
		if (i) {
			let e = [], t = [];
			if (i.forEach((n) => {
				let r = lr(n);
...
function Pi(e, t) {
	...
	try {
		let i = fn(n || "");
		return or(Ni(Object.keys(i).length ? mn(e) : e, {
			...r,
			...i,
			[D]: "other"
		}, t.$locale, t.$format), { maxChars: t.$maxChars });
	} catch {
		return z.warn(Mi(e)), t.$_fallback == null ? or(e, { maxChars: t.$maxChars }) : Pi(t.$_fallback, {
			...t,
			$_fallback: void 0
		});
	}
...
async function Ji({ locale: e, enableI18n: t }, n) {
	let r = mi(), i = B().getDefaultLocale(), a = await r.getLookupTranslation(t ? e : i);
	return (n, r = {}) => {
		let o = zi(t ? r.$locale ?? e : B().getDefaultLocale(), r, "ICU");
		return Ii({
			source: n,
			target: a(n, o),
			options: o,
...
async function Yi({ locale: e, enableI18n: t }) {
	let n = await Ji({
		locale: e,
		enableI18n: t
	});
...
function Xi({ sourceLocale: e, targetLocale: t, sourceEntry: n, target: r, dictionaryOptions: i, options: a = {} }) {
	let o = zi(t, {
		...i,
		...ji(a)
	}, i.$format);
...
function ra(e) {
	return typeof e == "string" ? !0 : !Array.isArray(e) || typeof e[0] != "string" ? !1 : e.length === 1 || e.length === 2 && ia(e[1]);
}
function ia(e) {
	if (typeof e != "object" || !e || Array.isArray(e)) return !1;
	let t = e;
	return (t.$context === void 0 || typeof t.$context == "string") && (t.$format === void 0 || aa(t.$format)) && (t.$maxChars === void 0 || typeof t.$maxChars == "number");
}
function aa(e) {
...
async function ua({ locale: e, enableI18n: t, rootId: n }) {
	let r = mi(), i = B().getDefaultLocale(), a = t ? e : i, [o, s, c] = await Promise.all([
...
function ha({ type: e, remoteTranslationLoaderParams: t, loadTranslations: n }) {
	let { cacheUrl: r, projectId: i, _versionId: a, _branchId: o } = t;
	switch (e) {
		case "remote":
		case "gt-remote": return i ? pa({
			cacheUrl: r,
			projectId: i,
			_versionId: a,
			_branchId: o,
		...
		case "disabled": return r === null ? async () => ({}) : ga(S({
...
var va = class {
	...
	load(e) {
		return _a(this.pendingLoads, e, () => this.loadResource(e).then((t) => (this.set(e, t), t)));
	}
	getExpiresAt() {
...
function xa(e) {
	return {
		maxConcurrentRequests: ba(e?.maxConcurrentRequests, ya.maxConcurrentRequests, !0),
		maxBatchSize: ba(e?.maxBatchSize, ya.maxBatchSize, !0),
		batchInterval: ba(e?.batchInterval, ya.batchInterval)
	};
}
var Sa = class {
	constructor({ init: e, translateMany: t, onMiss: n, batchConfig: r }) {
		this.pendingTranslations = /* @__PURE__ */ new Map(), this.queue = [], this.batchTimer = null, this.activeRequests = 0, this.cache = structuredClone(e), this.translateMany = t, this.batchConfig = xa(r), this.onMiss = n;
	}
	get(e) {
	...
	async miss(e) {
		let t = this.getCacheKey(e), n = await _a(this.pendingTranslations, t, () => this.translate(e));
	...
	async sendBatchRequest(e) {
		this.activeRequests++;
		let t = Ca(e), n = await this.sendBatchRequestWithErrorHandling(e, t);
...
async function wa({ key: e, sourceValue: t, targetValue: n, translateEntry: r }) {
	...
	let a = V(n) ? n : {}, o = /* @__PURE__ */ new Set([...Object.keys(t), ...Object.keys(a)]), s = await Promise.all(Array.from(o).map(async (n) => {
		...
		Qi(n, i);
		let o = t[n];
		return o === void 0 ? [n, H(a[n])] : [n, await wa({
			key: i,
			sourceValue: o,
			targetValue: a[n],
			translateEntry: r
...
var Ea = class {
	...
	getEntry(e) {
		let t = U($i(this.cache, e));
		if (t !== void 0) return Ta(t);
	}
	getValue(e) {
		let t = $i(this.cache, e);
	...
	async materializeValue(e, t, n = $i(this.cache, e)) {
		return _a(this.pendingMaterializations, e, () => wa({
			key: e,
			sourceValue: t,
			targetValue: n,
			translateEntry: async (e, t) => ta(await this.materializeEntry(e, t))
		}).then((t) => (this.setValue(e, t), t)));
	}
	async materializeEntry(e, t) {
		return Ta(await _a(this.pendingTranslations, e, () => this.runtimeTranslate(e, t).then((t) => {
			ea(this.cache, e, t);
			let n = U(t);
...
function Da(e, t) {
	for (let [n, r] of Object.entries(t)) {
		let t = e[n];
		V(t) && V(r) ? Da(t, r) : e[n] = H(r);
	}
}
var Oa = 12e3, ka = class {
	constructor(e) {
		Aa(e), this.config = {
			projectId: e.projectId,
			devApiKey: e.devApiKey,
		...
		this.createTranslateMany = fa(B().getGTClass(), this.config.runtimeTranslation?.timeout ?? Oa, {
	...
	async translateDictionaryEntry(e, t, n) {
		let r = await this.lookupTranslationWithFallbackResolved(e, n.entry, na(n.options));
...
var Ma = [];
function Na({ locale: e, enableI18n: t, localesProp: n = Ma }) {
...
var Fa = "generaltranslation.locale-reset", Ia = "server-render", La = Symbol.for("generaltranslation.react-core.ReactI18nConfig"), Ra = class extends wi {
	renderStrategy;
	localeCookieName;
	regionCookieName;
	enableI18nCookieName;
	constructor(e = {}, t = Ia) {
		super(e), Va(t), Object.defineProperty(this, La, { value: !0 }), this.renderStrategy = t, this.localeCookieName = e.localeCookieName ?? "generaltranslation.locale", this.regionCookieName = e.regionCookieName ?? "generaltranslation.region", this.enableI18nCookieName = e.enableI18nCookieName ?? "generaltranslation.enable-i18n";
	}
	getRenderStrategy() {
...
function Ha(e) {
	if (e instanceof Ra) return !0;
	let t = e;
	return t[La] === !0 && typeof t.getRenderStrategy == "function" && typeof t.getLocaleCookieName == "function" && typeof t.getRegionCookieName == "function" && typeof t.getEnableI18nCookieName == "function";
}
var { getConditionStore: q, setConditionStore: Ua, isConditionStoreInitialized: Wa } = Bi(S({
	source: "@generaltranslation/react-core",
	severity: "Error",
...
function Ga({ Currency: e, GtInternalCurrency: t, DateTime: n, GtInternalDateTime: r, Num: i, GtInternalNum: a, RelativeTime: o, GtInternalRelativeTime: s, Var: c, GtInternalVar: l }) {
...
function Qa({ renderVariable: t }) {
	return function({ children: n, defaultLocale: r = "en", enableI18n: i }) {
		let a = (n) => {
			let a = Xa(n);
			if (Ja(n.props)) {
				let { variableType: e, variableValue: a, variableOptions: o, injectionType: s } = Ya(n.props);
				return t({
					variableType: e,
					variableValue: a,
					variableOptions: o,
			...
			if (a?.transformation === "plural") {
				let e = a.branches || {};
				return typeof n.props.n == "number" ? s(Za(n.props.n, [r], e) ?? n.props.children) : n.props.children == null ? null : s(n.props.children);
			}
	...
	function r({ sourceElement: t, targetElement: r, locales: a = ["en"], enableI18n: o }) {
		...
		if (u && Object.entries(hn).forEach(([e, t]) => {
			...
			return typeof e == "number" ? i({
				source: Za(e, a, c.branches || {}) ?? t.props.children,
				target: Za(e, a, r.d?.b || {}) ?? r.c,
				locales: a,
				enableI18n: o
			}) : n({
				children: t,
	...
	function i({ source: i, target: a, locales: o = ["en"], enableI18n: s }) {
		...
		if (Array.isArray(a) && !Array.isArray(i) && i && (i = [i]), Array.isArray(i) && Array.isArray(a)) {
			let n = {}, c = {}, l = {}, u = i.filter((t) => {
				if (e.isValidElement(t)) {
					if (Ja(t.props)) {
						let { variableName: e, variableValue: r, variableOptions: i, injectionType: a } = Ya(t.props);
						n[e] = r, c[e] = i, l[e] = a;
					} else return !0;
				}
				return !1;
			...
			return a.map((i, a) => {
				...
				if (Qt(i)) return f(e.Fragment, { children: t({
					variableType: i.v || "v",
					variableValue: n[i.k],
		...
		if (a && typeof a == "object" && !Array.isArray(a)) {
			...
			if (e.isValidElement(i)) {
				...
				if (Ja(i.props)) {
					let { variableValue: e, variableOptions: n, variableType: r, injectionType: a } = Ya(i.props);
					return t({
						variableType: r,
						variableValue: e,
						variableOptions: n,
...
var no = { display: "contents" }, ro = globalThis.navigator?.product === "ReactNative";
function io(e) {
	if (e == null || typeof e == "boolean" || e === "") return !0;
	if (Array.isArray(e)) return !e.some((e) => !io(e));
	if (o(e) && e.type === n) {
...
function ao(e, t) {
	return ro || !eo() ? e : o(e) && typeof e.type == "string" ? r(e, { "data-_gt-hash": t }) : io(e) ? e : a("span", {
		"data-_gt-hash": t,
		style: no
	}, e);
}
function oo({ renderDefaultChildren: e, renderTranslatedChildren: t }) {
...
function so(e) {
	let t = Ga(e), n = Qa({ renderVariable: t }), r = $a({ renderVariable: t });
	return {
		renderVariable: t,
...
var co = class extends ka {};
function lo(e) {
	Ba(e, "server-render"), Pa(new co(e));
}
var uo = R({
	namespace: "reactCore",
	key: "i18nStore",
...
function vo(e, { locale: t, id: n }) {
	let r = e?.[t];
	if (!r) return;
	if (!n) return r;
	let i = r;
	for (let e of n.split(".")) {
		if (!yo(e) || !V(i) || !Object.prototype.hasOwnProperty.call(i, e)) return;
		i = i[e];
	}
	return i;
}
...
var xo = class {
	...
	subscribeToTranslate = (e, t) => {
		let n = ja(e);
		return mo(this.translateListeners, (e) => {
			ja(e) === n && t();
		});
	};
	subscribeToTranslationEvents = (e) => mo(this.translateListeners, e);
	subscribeToDictionaryEntryEvents = (e) => mo(this.dictionaryEntryListeners, e);
	subscribeToDictionaryObjectEvents = (e) => mo(this.dictionaryObjectListeners, e);
	getTranslateSnapshot = (e, t = {}) => ho(t, e) ?? G().lookupTranslation(e.locale, e.message, e.options);
	getDictionaryEntrySnapshot = (e, t = {}) => go(t, e) ?? G().lookupDictionary(e.locale, e.id);
	getDictionaryObjectSnapshot = (e, t = {}) => _o(t, e) ?? G().lookupDictionaryObj(e.locale, e.id);
...
function Co() {
	return So.isInitialized() || So.set(i(void 0)), So.get();
}
function wo() {
	let e = s(Co());
...
function Oo() {
	return wo()?.i18nStore || fo();
}
function ko() {
	return wo()?.translationsSnapshot || {};
}
var Ao = () => {};
function jo() {
	return Ao;
...
var Mo = jo;
function No({ _enableI18n: e, _locale: t, children: n, currency: r = "USD", options: i = {}, locales: a = [] }) {
	let o = Na({
		locale: t,
		enableI18n: e,
		localesProp: a
...
Po._gtt = "variable-currency-automatic", Fo._gtt = "variable-currency";
function Io({ _enableI18n: e, _locale: t, children: n, options: r = {}, locales: i = [] }) {
	let a = Na({
		locale: t,
		enableI18n: e,
		localesProp: i
...
Lo._gtt = "variable-datetime-automatic", Ro._gtt = "variable-datetime";
...
Bo._gtt = "variable-number-automatic", Vo._gtt = "variable-number";
function Ho({ _enableI18n: e, _locale: t, date: n, children: r, value: i, unit: a, baseDate: o, locales: s = [], options: c = {} }) {
	let l = Na({
		locale: t,
		enableI18n: e,
		localesProp: s
...
Uo._gtt = "variable-relative-time-automatic", Wo._gtt = "variable-relative-time";
function Go({ children: e }) {
	return e;
}
function Ko({ children: e }) {
	return Go({ children: e });
}
function qo({ children: e }) {
	return Go({ children: e });
}
Ko._gtt = "variable-variable", qo._gtt = "variable-variable-automatic";
function Jo(e) {
	let t = Oo(), n = ko();
	return Mo(), d((n) => t.subscribeToTranslate(e, n), () => t.getTranslateSnapshot(e, n), () => t.getTranslateSnapshot(e, n));
}
var { renderVariable: Yo, renderDefaultChildren: Xo, renderTranslatedChildren: Zo, renderPreparedT: Qo } = so({
	Currency: Fo,
	GtInternalCurrency: Po,
	DateTime: Ro,
	GtInternalDateTime: Lo,
	Num: Vo,
	GtInternalNum: Bo,
	RelativeTime: Wo,
	GtInternalRelativeTime: Uo,
	Var: Ko,
	GtInternalVar: qo
});
function $o(t, n = 0) {
	let r = n, i = (e) => {
		...
		if (a) {
			...
			if ((e[1] === "automatic" || e[2] === "automatic") && (i.injectionType = "automatic"), e[0] === "translate" && (e[0] = "fragment"), e[0] === "variable" && (i.variableType = e?.[1] || "variable"), e[0] === "plural") {
				let e = Object.entries(n).reduce((e, [t, n]) => (sn(t) && (e[t] = $o(n, r)), e), {});
				Object.keys(e).length && (i.branches = e);
			}
			if (e[0] === "branch") {
				let { children: e, branch: t, ...a } = n, o = Object.fromEntries(Object.entries(a).filter(([e]) => !e.startsWith("data-"))), s = Object.entries(o).reduce((e, [t, n]) => (e[t] = $o(n, r), e), {});
...
function ts(e, t) {
	let { type: n, props: i } = e, a = is(n);
	if (typeof i != "object" || !i) return e;
	if (a) {
		let { componentType: n, injectionType: o } = a;
		if (n === "variable") return e;
		if (n === "branch") return r(e, { ...Object.entries(i).reduce((e, [n, r]) => (e[n] = n !== "branch" && !n.startsWith("data-") ? ns(r, t) : r, e), {}) });
		if (n === "plural") return r(e, { ...Object.entries(i).reduce((e, [n, r]) => (e[n] = sn(n) || n === "children" ? ns(r, t) : r, e), {}) });
		if (n === "derive") return r(e, {
			...i,
			..."children" in i && { children: rs(i.children, t + 1) }
		});
		if (n === "translate" && o === "automatic" && t > 0) return "children" in i ? rs(i.children, t) : void 0;
		n === "translate" && o === "automatic" && console.warn(as);
	}
	return r(e, {
		...i,
		..."children" in i && { children: rs(i.children, t) }
	});
}
function ns(e, t) {
	return o(e) ? ts(e, t) : e;
...
function rs(e, n) {
	return Array.isArray(e) ? t.map(e, (e) => ns(e, n)) : ns(e, n);
}
function is(e) {
...
var as = "'@generaltranslation/react-core Warning: A <_T> component was found injected outside of a <Derive> boundary. This may affect translation resolution for this component.";
function os(t) {
...
var ss = (e) => {
	...
	let { props: t } = e, n = { t: ss(e) };
	if (t["data-_gt"]) {
		let e = t["data-_gt"], r = e.transformation;
		if (r === "variable") {
			let n = e.variableType || "variable", r = qa(t, n), i = un(n);
			return {
				i: e.id,
				k: r,
				v: i
		...
		n.i = e.id, n.d = cs(r, t, e.branches);
	}
	return t.children && (n.c = ds(t.children)), n;
}, us = (e) => os(e) ? ls(e) : typeof e == "number" ? e.toString() : e;
function ds(e) {
	return Array.isArray(e) ? e.map(us) : us(e);
}
function fs({ sourceChildren: e, params: t, locale: n }) {
	let r = ps(e), i = ms(r), a = hs({
		options: gs(t),
		locale: n
	});
	return to(i, a), {
...
function hs({ options: e, locale: t }) {
	return {
		...e,
		$locale: t
	};
...
function _s({ sourceChildren: e, params: t, _locale: n, _enableI18n: r }) {
	let i = J(), a = Y(), o = Do(), s = n ?? i, c = r ?? a;
	return {
		defaultLocale: o,
		enableI18n: c,
		locale: s,
...
function vs({ children: e, _locale: t, _enableI18n: n, _renderPreparedT: r = Qo, ...i }) {
	let { defaultLocale: a, locale: o, enableI18n: s, targetOptions: c, taggedSourceChildren: l, sourceJsxChildren: d, shouldTranslate: f } = _s({
		sourceChildren: e,
		params: i,
		_locale: t,
		_enableI18n: n
...
function Z({ cookieName: e }) {
	if (!(typeof document > "u")) return qi(document.cookie, e);
}
...
var Ss = class {
	constructor(e) {
		this.getLocale = () => Cs(this.customGetLocale), this.setLocale = (e) => {
			this.updateLocale(e), Q({
				cookieName: Fa,
				value: "true"
			}), this.reload();
		}, this.getRegion = () => Z({ cookieName: K().getRegionCookieName() }) || this.customGetRegion?.(), this.setRegion = (e) => {
			this.updateRegion(e), this.reload();
		}, this.getEnableI18n = () => {
			let e = Z({ cookieName: K().getEnableI18nCookieName() });
...
function Cs(e) {
	let t = K(), n = xs(t.getLocaleCookieName());
...
function Es(e) {
	if (!e) return "";
	let t = Array.isArray(e) ? e.join(", ") : e;
	return t.trim() ? ws(`Details: ${t}`) : "";
}
function Ds({ source: e, severity: t, whatHappened: n, reassurance: r, why: i, fix: a, wayOut: o, details: s, docsUrl: c }) {
	let l = e ? t ? `${e} ${t}:` : `${e}:` : t ? `${t}:` : "", u = i ? `${Ts(n)} because ${Ts(i)}` : n, d = !!a && !!o && /^[a-z]/.test(Ts(o)), f = [
		u,
		r,
		d ? `${Ts(a)}, or ${Ts(o)}` : a,
		d ? void 0 : o,
...
}), { setConditionStore: ks, isConditionStoreInitialized: As } = Bi(Os), { getConditionStore: js, setConditionStore: Ms, isConditionStoreInitialized: Ns } = Bi(Os);
function Ps(e) {
	let t = Fs(e), n = Is(e), r = Ls(e);
	if (Ns()) {
		let e = js();
...
function Fs({ _getLocale: e, locale: t }) {
	let n = K(), r = [];
	return t && r.push(...Array.isArray(t) ? t : [t]), e && r.push(e()), r.push(...xs(n.getLocaleCookieName())), n.resolveSupportedLocale(r);
}
function Is({ _getRegion: e, region: t }) {
	return Z({ cookieName: K().getRegionCookieName() }) || e?.() || t;
}
function Ls({ enableI18n: e, _getEnableI18n: t }) {
	if (e !== void 0) return e;
	let n = Z({ cookieName: K().getEnableI18nCookieName() });
...
function zs() {
	return {
		projectId: Bs(() => void 0) || Vs(),
		devApiKey: vi() === "development" ? Bs(() => void 0) || Hs() : void 0
	};
}
function Bs(e) {
	try {
...
function Vs() {
	try {
		return Us(process.env.VITE_GT_PROJECT_ID);
	} catch {
		return;
	}
}
function Hs() {
	try {
		return Us(process.env.VITE_GT_DEV_API_KEY);
	} catch {
		return;
	}
}
...
function Ks() {
	return p("section", {
		...
		children: [f("h2", {
			...
			children: f(X, { children: "Why These Metrics Matter" })
		}), p("div", {
			...
			children: [
				p("div", {
					...
					children: [f("h3", {
						...
						children: f(X, { children: "Bundle Size" })
					}), f("p", {
						className: "text-sm text-muted-foreground",
						children: f(X, { children: "The bundle is the data shipped to every user across the globe. A larger bundle means longer download times — especially on slow 3G connections common in many regions. i18n libraries vary dramatically in their weight: from a few kilobytes to tens of kilobytes of runtime code, plus the translation files themselves." })
					})]
				}),
				p("div", {
					className: "rounded-lg border border-border bg-card p-6",
					children: [f("h3", {
						className: "mb-2 text-lg font-semibold text-foreground",
						children: f(X, { children: "Rendering & Hydration" })
					}), f("p", {
						className: "text-sm text-muted-foreground",
						children: f(X, { children: "Connecting a large JSON dictionary to every component creates a hidden dependency: any change in the translation context can trigger re-renders across the entire tree. During SSR hydration, parsing and attaching massive translation objects adds latency before the page becomes interactive — directly impacting Time to Interactive (TTI)." })
					})]
				}),
				p("div", {
					className: "rounded-lg border border-border bg-card p-6",
					children: [f("h3", {
						className: "mb-2 text-lg font-semibold text-foreground",
						children: f(X, { children: "Dynamic Loading" })
					}), f("p", {
						className: "text-sm text-muted-foreground",
						children: f(X, { children: "Loading all translations upfront overloads the initial payload. Dynamic (lazy) loading splits translations by route or namespace, sending only what the current page needs. However, lazy loading introduces its own trade-offs: waterfall requests, flash of untranslated content, and caching complexity. Measuring both strategies is essential." })
					})]
				})
			]
		})]
...
function Ys(e) {
	if (!e) return "";
	let t = Array.isArray(e) ? e.join(", ") : e;
	return t.trim() ? qs(`Details: ${t}`) : "";
}
function Xs({ source: e, severity: t, whatHappened: n, reassurance: r, why: i, fix: a, wayOut: o, details: s, docsUrl: c }) {
	let l = e ? t ? `${e} ${t}:` : `${e}:` : t ? `${t}:` : "", u = i ? `${Js(n)} because ${Js(i)}` : n, d = !!a && !!o && /^[a-z]/.test(Js(o)), f = [
		u,
		r,
		d ? `${Js(a)}, or ${Js(o)}` : a,
		d ? void 0 : o,
...
Qs.set;
var $s = Qs.isInitialized;
function ec(e, t = nc()) {
	let { pathname: n } = rc(e, t), r = n.match(/^\/([^/]+)(?:\/|$)/);
	if (!r) return;
	let i;
	try {
	...
	return K().determineSupportedLocale(i);
}
function tc(e, t, n = nc()) {
	let r = K(), { basepath: i, pathname: a } = rc(e, n), o = ec(a, "/") ? a.replace(/^\/[^/]+/, "") || "/" : a, s = r.resolveSupportedLocale(t);
...
function oc(e, t, n = new URL(e.url).pathname) {
	let r = K(), i = e.headers.get("cookie"), a = [];
	if (t?.localeRouting) {
		let e = ec(n);
		e && a.push(e);
	}
	let o = qi(i, r.getLocaleCookieName());
	o && a.push(o), a.push(...Ki(e.headers.get("accept-language"))), a.length === 0 && console.warn(ac);
	let s = r.resolveSupportedLocale(a, t ?? {
		defaultLocale: r.getDefaultLocale(),
		locales: r.getLocales(),
		customMapping: r.getCustomMapping()
	});
	_(r.getLocaleCookieName(), s, ic);
	let c = qi(i, r.getEnableI18nCookieName());
	return {
		locale: s,
		region: qi(i, r.getRegionCookieName()) || void 0,
		enableI18n: c === void 0 || c === "true"
	};
}
var sc = m().server(() => $().getLocale()).client(() => q().getLocale());
m().server(() => $().getEnableI18n()).client(() => q().getEnableI18n()), m().server((e) => {
	let t = $();
	return Ji({
		locale: t.getLocale(),
...
function lc({ defaultLocale: e, locales: t, customMapping: n }) {
	let r = K(), i = r.getLocaleCookieName(), a = [], o = qi(document.cookie, i);
...
function uc(e) {
	let t = e.localeRouting && !e._reload ? {
		...e,
		_reload: ({ locale: e }) => {
			let t = tc(window.location.pathname, e), n = new URL(window.location.href);
