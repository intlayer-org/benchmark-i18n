...
var f = () => {}, p = m;
function m({ initialLocale: e, initialTranslations: t = {}, router: r, children: c }) {
	let [p, m] = a(() => e || (typeof window < "u" ? u() : "en")), [h, g] = a(t), [_, v] = a(!1);
	l.debug(`LingoProvider initialized with locale: ${p}`, t), i(() => {
...
var F = /[ \xA0\u1680\u2000-\u200A\u202F\u205F\u3000]/, de = /(?:[Eec]{1,6}|G{1,5}|[Qq]{1,5}|(?:[yYur]+|U{1,5})|[ML]{1,5}|d{1,2}|D{1,3}|F{1}|[abB]{1,5}|[hkHK]{1,2}|w{1,2}|W{1}|m{1,2}|s{1,2}|[zZOvVxX]{1,4})(?=([^']*'[^']*')*[^']*$)/g;
function fe(e) {
	var t = {};
...
var I = /^\.(?:(0+)(\*)?|(#+)|(0+)(#+))$/g, L = /^(@+)?(\+|#+)?[rs]?$/g, ge = /(\*)(0+)|(#+)(0+)|(0+)/g, R = /^(0+)$/;
function z(e) {
	var t = {};
	...
	for (var t = {}, n = 0, r = e; n < r.length; n++) {
		...
		switch (i.stem) {
			...
			case "KK":
				...
				continue;
			case "scientific":
				t = _(_(_({}, t), { notation: "scientific" }), i.options.reduce(function(e, t) {
					return _(_({}, e), V(t));
				}, {}));
				continue;
			case "engineering":
				t = _(_(_({}, t), { notation: "engineering" }), i.options.reduce(function(e, t) {
					return _(_({}, e), V(t));
				}, {}));
				continue;
		...
		if (I.test(i.stem)) {
			...
			var a = i.options[0];
			a === "w" ? t = _(_({}, t), { trailingZeroDisplay: "stripIfInteger" }) : a && (t = _(_({}, t), z(a)));
			continue;
		}
		if (L.test(i.stem)) {
			t = _(_({}, t), z(i.stem));
...
function ye(e, t) {
	for (var n = "", r = 0; r < e.length; r++) {
		...
		if (i === "j") {
			...
			var o = 1 + (a & 1), s = a < 2 ? 1 : 3 + (a >> 1), c = "a", l = be(t);
...
var xe = RegExp(`^${F.source}*`), Se = RegExp(`${F.source}*\$`);
function U(e, t) {
	return {
		start: e,
		end: t
...
var Ce = !!Object.fromEntries, we = !!String.prototype.trimStart, Te = !!String.prototype.trimEnd, W = Ce ? Object.fromEntries : function(e) {
	for (var t = {}, n = 0, r = e; n < r.length; n++) {
		var i = r[n], a = i[0];
		t[a] = i[1];
	}
	...
	return e.prototype.parse = function() {
		...
		for (var r = []; !this.isEOF();) {
			...
			if (i === 123) {
				...
				if (n) break;
				return this.error(T.UNMATCHED_CLOSING_TAG, U(this.clonePosition(), this.clonePosition()));
		...
		if (this.bumpIf(">")) {
			...
			if (this.bumpIf("</")) {
				if (this.isEOF() || !K(this.char())) return this.error(T.INVALID_TAG, U(o, this.clonePosition()));
				...
				return r === this.parseTagName() ? (this.bumpSpace(), this.bumpIf(">") ? {
					...
					err: null
				} : this.error(T.INVALID_TAG, U(o, this.clonePosition()))) : this.error(T.UNMATCHED_CLOSING_TAG, U(s, this.clonePosition()));
			}
			return this.error(T.UNCLOSED_TAG, U(n, this.clonePosition()));
		}
		return this.error(T.INVALID_TAG, U(n, this.clonePosition()));
		...
		var n = this.clonePosition();
		if (this.bump(), this.bumpSpace(), this.isEOF()) return this.error(T.EXPECT_ARGUMENT_CLOSING_BRACE, U(n, this.clonePosition()));
		if (this.char() === 125) return this.bump(), this.error(T.EMPTY_ARGUMENT, U(n, this.clonePosition()));
		var r = this.parseIdentifierIfPossible().value;
		if (!r) return this.error(T.MALFORMED_ARGUMENT, U(n, this.clonePosition()));
		if (this.bumpSpace(), this.isEOF()) return this.error(T.EXPECT_ARGUMENT_CLOSING_BRACE, U(n, this.clonePosition()));
		switch (this.char()) {
			...
			case 44: return this.bump(), this.bumpSpace(), this.isEOF() ? this.error(T.EXPECT_ARGUMENT_CLOSING_BRACE, U(n, this.clonePosition())) : this.parseArgumentOptions(e, t, r, n);
			default: return this.error(T.MALFORMED_ARGUMENT, U(n, this.clonePosition()));
		}
		...
		switch (a) {
			case "": return this.error(T.EXPECT_ARGUMENT_TYPE, U(i, o));
			case "number":
			case "date":
			case "time":
				this.bumpSpace();
				...
				if (this.bumpIf(",")) {
					...
					if (l.err) return l;
					var u = De(l.val);
					if (u.length === 0) return this.error(T.EXPECT_ARGUMENT_STYLE, U(this.clonePosition(), this.clonePosition()));
					s = {
						style: u,
				...
				if (s && s.style.startsWith("::")) {
					...
					var m = p;
					this.locale && (m = ye(p, this.locale));
					var u = {
						type: D.dateTime,
						pattern: m,
			...
			case "select":
				...
				if (this.bumpSpace(), !this.bumpIf(",")) return this.error(T.EXPECT_SELECT_ARGUMENT_OPTIONS, U(h, _({}, h)));
				this.bumpSpace();
				var g = this.parseIdentifierIfPossible(), v = 0;
				if (a !== "select" && g.value === "offset") {
					if (!this.bumpIf(":")) return this.error(T.EXPECT_PLURAL_ARGUMENT_OFFSET_VALUE, U(this.clonePosition(), this.clonePosition()));
					this.bumpSpace();
					var l = this.tryParseDecimalInteger(T.EXPECT_PLURAL_ARGUMENT_OFFSET_VALUE, T.INVALID_PLURAL_ARGUMENT_OFFSET_VALUE);
					if (l.err) return l;
					this.bumpSpace(), g = this.parseIdentifierIfPossible(), v = l.val;
				}
			...
			default: return this.error(T.INVALID_ARGUMENT_TYPE, U(i, o));
		}
	}, e.prototype.tryParseArgumentClose = function(e) {
		return this.isEOF() || this.char() !== 125 ? this.error(T.EXPECT_ARGUMENT_CLOSING_BRACE, U(e, this.clonePosition())) : (this.bump(), {
			val: !0,
			err: null
		});
		...
		for (var e = 0, t = this.clonePosition(); !this.isEOF();) switch (this.char()) {
			case 39:
				this.bump();
				var n = this.clonePosition();
				if (!this.bumpUntil("'")) return this.error(T.UNCLOSED_QUOTE_IN_ARGUMENT_STYLE, U(n, this.clonePosition()));
				this.bump();
				break;
			case 123:
				e += 1, this.bump();
		...
		for (var i, a = !1, o = [], s = /* @__PURE__ */ new Set(), c = r.value, l = r.location;;) {
			...
			if (!this.bumpIf("{")) return this.error(t === "select" ? T.EXPECT_SELECT_ARGUMENT_SELECTOR_FRAGMENT : T.EXPECT_PLURAL_ARGUMENT_SELECTOR_FRAGMENT, U(this.clonePosition(), this.clonePosition()));
		...
		return o.length === 0 ? this.error(t === "select" ? T.EXPECT_SELECT_ARGUMENT_SELECTOR : T.EXPECT_PLURAL_ARGUMENT_SELECTOR, U(this.clonePosition(), this.clonePosition())) : this.requiresOtherClause && !a ? this.error(T.MISSING_OTHER_CLAUSE, U(this.clonePosition(), this.clonePosition())) : {
			val: o,
			err: null
		};
...
function q(e) {
	e.forEach(function(e) {
		...
		else le(e) && N(e.style) || (O(e) || k(e)) && P(e.style) ? delete e.style.location : M(e) && q(e.children);
	});
}
function Ne(e, t) {
	t === void 0 && (t = {}), t = _({
...
function Q(e, t, n, r, i, a, o) {
	...
	for (var s = [], c = 0, l = e; c < l.length; c++) {
		...
		if (M(u)) {
			...
			if (!Le(g)) throw new Pe(h, "function", o);
...
function Ue(e) {
	let t = {};
	for (let [n, r] of Object.entries(e)) t[n] = typeof r == "function" ? He(r) : r;
	return t;
}
function We(t, n, r) {
	try {
		let i = new Ve(t, r), a = Ue(n), s = i.format(a);
