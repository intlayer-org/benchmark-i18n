...
var xe = () => {
	...
	return t;
}, Se = 0, z = 1, Ce = 2, we = class extends Error {
	constructor(e, t, n) {
		let r, i = "";
		e === Se ? r = "Empty parameter" : e === z ? (r = "Unexpected character", i = "Did you forget to use FormatIcu to render ICU message syntax?") : r = "Unexpected end", super(`Tolgee parser: ${r} at ${t} in "${n}"` + (i ? "\n" + i : "")), this.code = e, this.index = t;
	}
};
function Te(e) {
	return /\s/.test(e);
...
function ke(e) {
	...
	for (s = 0; s < e.length; s++) switch (i = e[s], t) {
		...
		case H:
			...
			break;
		case U:
			i === "}" ? (d(), t = B) : Te(i) ? r !== "" && (d(), t = W) : (u(), t = U);
			break;
		case W: i == "}" ? t = B : Te(i) ? t = W : c(z);
	}
	return Ee.has(t) || c(Ce), l(), [a, o];
}
function Ae(e, t) {
	let [n, r] = ke(e), i = [n[0]];
...
}, Ke = 3e3, qe = 200, Je = 0, J = /* @__PURE__ */ new Map(), Y = !1, X, Z;
async function Ye({ type: e, replyType: t, payload: n, timeoutMs: r = Be, progressType: i, onProgress: a }) {
	Qe();
	let o = Date.now() + r;
	await $e(Math.min(o, Date.now() + Ke));
	let s = o - Date.now();
	if (s <= 0) throw Xe(e);
	let c = Ze();
...
var Xe = (e) => new q("unavailable", `the Tolgee browser extension did not answer ${e} in time`), Ze = () => `${Date.now()}-${Je++}-${Math.random()}`;
function Qe() {
...
function $e(e) {
	return X ||= new Promise((t, n) => {
		let r = () => window.postMessage({ type: Ue }, window.origin), i = () => {
			clearInterval(a), et(), Z = void 0, n(new q("unavailable", "the Tolgee browser extension did not answer"));
		}, a = setInterval(() => {
			if (Date.now() > e) {
				i();
				return;
...
function Nt() {
	let e = sessionStorage.getItem(_t) || void 0, t = sessionStorage.getItem(vt) || void 0, n = sessionStorage.getItem(yt) || void 0, r = sessionStorage.getItem(bt) || void 0, i = Ge(sessionStorage.getItem(xt));
	if (!t) return;
