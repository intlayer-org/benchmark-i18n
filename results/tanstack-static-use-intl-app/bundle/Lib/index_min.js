...
var fe = /^\.(?:(0+)(\*)?|(#+)|(0+)(#+))$/g, x = /^(@+)?(\+|#+)?[rs]?$/g, pe = /(\*)(0+)|(#+)(0+)|(0+)/g, S = /^(0+)$/;
function me(e) {
	let t = {};
...
var je = class {
	...
	parseMessage(e, t, n) {
		...
		for (; !this.isEOF();) {
			...
			if (i === 123) {
				...
				if (n) break;
				return this.error(C.UNMATCHED_CLOSING_TAG, L(this.clonePosition(), this.clonePosition()));
	...
	parseTag(e, t) {
		...
		if (this.bumpIf(">")) {
			...
			if (this.bumpIf("</")) {
				if (this.isEOF() || !B(this.char())) return this.error(C.INVALID_TAG, L(o, this.clonePosition()));
				let e = this.clonePosition();
				return r === this.parseTagName() ? (this.bumpSpace(), this.bumpIf(">") ? {
					...
					err: null
				} : this.error(C.INVALID_TAG, L(o, this.clonePosition()))) : this.error(C.UNMATCHED_CLOSING_TAG, L(e, this.clonePosition()));
			}
			return this.error(C.UNCLOSED_TAG, L(n, this.clonePosition()));
		}
		return this.error(C.INVALID_TAG, L(n, this.clonePosition()));
	}
	parseTagName() {
		let e = this.offset();
	...
	parseArgument(e, t) {
		let n = this.clonePosition();
		if (this.bump(), this.bumpSpace(), this.isEOF()) return this.error(C.EXPECT_ARGUMENT_CLOSING_BRACE, L(n, this.clonePosition()));
		if (this.char() === 125) return this.bump(), this.error(C.EMPTY_ARGUMENT, L(n, this.clonePosition()));
		let r = this.parseIdentifierIfPossible().value;
		if (!r) return this.error(C.MALFORMED_ARGUMENT, L(n, this.clonePosition()));
		if (this.bumpSpace(), this.isEOF()) return this.error(C.EXPECT_ARGUMENT_CLOSING_BRACE, L(n, this.clonePosition()));
		switch (this.char()) {
			...
			case 44: return this.bump(), this.bumpSpace(), this.isEOF() ? this.error(C.EXPECT_ARGUMENT_CLOSING_BRACE, L(n, this.clonePosition())) : this.parseArgumentOptions(e, t, r, n);
			default: return this.error(C.MALFORMED_ARGUMENT, L(n, this.clonePosition()));
		}
	}
	...
	parseArgumentOptions(e, t, n, r) {
		...
		switch (a) {
			case "": return this.error(C.EXPECT_ARGUMENT_TYPE, L(i, o));
			case "number":
			case "date":
			case "time": {
				this.bumpSpace();
				...
				if (this.bumpIf(",")) {
					...
					if (n.err) return n;
					let r = ke(n.val);
					if (r.length === 0) return this.error(C.EXPECT_ARGUMENT_STYLE, L(this.clonePosition(), this.clonePosition()));
					e = {
						style: r,
			...
			case "select": {
				...
				if (this.bumpSpace(), !this.bumpIf(",")) return this.error(C.EXPECT_SELECT_ARGUMENT_OPTIONS, L(i, { ...i }));
				this.bumpSpace();
				let o = this.parseIdentifierIfPossible(), s = 0;
				if (a !== "select" && o.value === "offset") {
					if (!this.bumpIf(":")) return this.error(C.EXPECT_PLURAL_ARGUMENT_OFFSET_VALUE, L(this.clonePosition(), this.clonePosition()));
					this.bumpSpace();
			...
			default: return this.error(C.INVALID_ARGUMENT_TYPE, L(i, o));
		}
	}
	tryParseArgumentClose(e) {
		return this.isEOF() || this.char() !== 125 ? this.error(C.EXPECT_ARGUMENT_CLOSING_BRACE, L(e, this.clonePosition())) : (this.bump(), {
			val: !0,
			err: null
		});
	}
	parseSimpleArgStyleIfPossible() {
		...
		for (; !this.isEOF();) switch (this.char()) {
			case 39: {
				this.bump();
				let e = this.clonePosition();
				if (!this.bumpUntil("'")) return this.error(C.UNCLOSED_QUOTE_IN_ARGUMENT_STYLE, L(e, this.clonePosition()));
				this.bump();
				break;
			}
			case 123:
	...
	tryParsePluralOrSelectOptions(e, t, n, r) {
		...
		for (;;) {
			...
			if (!this.bumpIf("{")) return this.error(t === "select" ? C.EXPECT_SELECT_ARGUMENT_SELECTOR_FRAGMENT : C.EXPECT_PLURAL_ARGUMENT_SELECTOR_FRAGMENT, L(this.clonePosition(), this.clonePosition()));
		...
		return a.length === 0 ? this.error(t === "select" ? C.EXPECT_SELECT_ARGUMENT_SELECTOR : C.EXPECT_PLURAL_ARGUMENT_SELECTOR, L(this.clonePosition(), this.clonePosition())) : this.requiresOtherClause && !i ? this.error(C.MISSING_OTHER_CLAUSE, L(this.clonePosition(), this.clonePosition())) : {
			val: a,
			err: null
		};
	}
...
function nt({ children: e, formats: t, getMessageFallback: n, locale: r, messages: i, now: c, onError: l, timeZone: u }) {
	let d = a($), f = o(() => d?.cache || re(), [r, d?.cache]), p = o(() => d?.formatters || oe(f), [f, d?.formatters]), m = o(() => ({
		...tt({
			locale: r,
...
var ct = "/Users/aymericpineau/Documents/benchmark-bloom/apps-benchmark/tanstack-start-react-static/use-intl-app/scripts/EmptyComponent.tsx", lt = () => (st(), null);
function ut() {
	return c(nt, {
		locale: "en",
		messages: {},
		timeZone: "UTC",
		children: c(lt, {}, void 0, !1, {
			fileName: ct,
			lineNumber: 13,
			columnNumber: 7
		}, this)
...
var gt = "/Users/aymericpineau/Documents/benchmark-bloom/apps-benchmark/tanstack-start-react-static/use-intl-app/scripts/EmptyComponent.wrapper.tsx";
function _t() {
