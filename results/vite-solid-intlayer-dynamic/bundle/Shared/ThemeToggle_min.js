...
var U = (e = W) => {
	...
	let { locale: r } = d(K) ?? {}, i = m.defaultLocale, a = n ?? r?.() ?? i;
	return q(X(`${String(t)}.${a}`, e[a]?.()), a);
}, Q = a("<button type=button class=\"rounded-md border border-border bg-accent px-3 py-1.5 text-xs font-medium text-foreground transition-colors hover:bg-accent/80\">");
function te() {
...
function ne() {
	let e = Z(ee, "theme-toggle"), [t, a] = l("auto");
	u(() => {
		let e = te();
		a(e), $(e);
	}), s(() => {
	...
	function o() {
		...
		a(n), $(n), window.localStorage.setItem("theme", n);
	}
