...
function OpenPositions() {
	...
	const openings = [
		{
			title: content$1().seniorFrontendEngineer.value,
			location: content$1().remote.value,
			type: "Full-time",
			dept: content$1().engineering.value,
			desc: content$1().buildAndMaintainOurBenchmarking.value
		},
		{
			title: content$1().backendEngineer.value,
			location: content$1().remote.value,
			type: "Full-time",
			dept: content$1().engineering.value,
			desc: content$1().designAndScaleOurCloud.value
		},
		{
			title: content$1().technicalWriter.value,
			location: content$1().remote.value,
			type: "Part-time",
			dept: content$1().documentation.value,
			desc: content$1().createComprehensiveGuidesApiReferences.value
		},
		{
			title: content$1().devrelEngineer.value,
			location: content$1().sanFranciscoRemote.value,
			type: "Full-time",
			dept: content$1().community.value,
			desc: content$1().engageWithTheI18nCommunity.value
		},
		{
			title: content$1().qaEngineer.value,
			location: content$1().remote.value,
			type: "Full-time",
			dept: content$1().engineering.value,
			desc: content$1().ensureTheAccuracyAndReliability.value
		}
	];
	return [(() => {
		var _el$ = _tmpl$();
		insert(_el$, () => content$1().openPositions);
		return _el$;
	})(), (() => {
		var _el$2 = _tmpl$2();
		insert(_el$2, createComponent(For, {
			...
			children: (o) => (() => {
				...
				insert(_el$0, () => o.type);
				insert(_el$1, () => content$1().applyNow);
				return _el$3;
			})()
		}));
		return _el$2;
