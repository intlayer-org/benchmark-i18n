...
import { For as S, Suspense as C, createContext as ne, createEffect as re, createMemo as ie, createSignal as w, lazy as T, onMount as E, useContext as ae } from "solid-js";
import { A as D, useLocation as oe, useNavigate as O, useParams as k } from "@solidjs/router";
var A = {
	key: "header",
	content: {
...
}, j = ({ children: e, value: t, additionalProps: n }) => {
	let r = [e];
...
}, N = "\x1B[0m", se = "\x1B[34m", ce = "\x1B[31m", le = "\x1B[32m", P = (e, t, n) => t ? `${t}${e}${n ? typeof n == "boolean" ? N : n : N}` : e;
P("✗", ce), P("✓", le), P("⏲", se);
var ue = (e = "") => new Proxy(() => e, {
	get: (t, n) => {
		if (n === "toJSON" || n === Symbol.toPrimitive || n === "toString") return () => e;
		if (n !== "then") return n === Symbol.iterator ? function* () {
			yield e;
		} : ue(e ? `${e}.${String(n)}` : String(n));
	},
	apply: () => e
}), de = (e, t, n = d(t)) => {
	let r = {
...
}, R = process.env.INTLAYER_NODE_TYPE_INTLAYER_NODE === "false", z = process.env.INTLAYER_NODE_TYPE_SOLID_NODE === "false", B = !0, V = !0, H = !0, U = !0, W = null, G = null, K = null, q = null, J = null;
U || (W = T(() => import("./ContentSelector--hRpRV2o.js").then((e) => ({ default: e.ContentSelector })))), B || (G = T(() => import("./MarkdownRenderer-B7hyvLDf.js").then((e) => ({ default: e.MarkdownMetadataRenderer }))), K = T(() => import("./MarkdownRenderer-B7hyvLDf.js").then((e) => ({ default: e.MarkdownRenderer }))), import("./markdown-Cvh9lBUG.js").then((e) => {
	J = e.getMarkdownMetadata;
})), V || (q = T(() => import("./HTMLRenderer-C-wPH0rg.js").then((e) => ({ default: e.HTMLRenderer }))));
var fe = R ? e : {
	...
	transform: (e, { plugins: t, ...r }) => j({
		...r,
		value: r.children,
		children: !U && n.enabled ? m(C, {
			get fallback() {
				return r.children;
			},
			get children() {
				return m(W, y(r, { get children() {
					return r.children;
				} }));
			}
		}) : r.children
		...
		let { plugins: i, ...a } = t, o = r(J?.(e) ?? {}, {
			plugins: [{
				...
				transform: (t, r) => j({
					...r,
					value: t,
					children: !U && n.enabled ? m(ContentSelector, y(a, { get children() {
						return m(C, {
							fallback: e,
							get children() {
								return m(G, y(a, {
	...
	return Y.set(r, o), o;
}, be = (e, t) => de(e, t, ye(t)), xe = process.env.INTLAYER_ROUTING_STORAGE_COOKIES === "false";
process.env.INTLAYER_ROUTING_STORAGE_HEADERS;
var Se = (e = X) => {
	let { locales: t } = c;
	if (e?.isCookieEnabled === !1) return;
	let n = (e) => !!e && t.includes(e);
	if (!xe) for (let t = 0; t < (r.storage.cookies ?? []).length; t++) try {
	...
	let n = ae(we) ?? {};
	return ie(() => be(e, t ?? n?.locale?.()));
}, { defaultLocale: Te, locales: Ee } = c;
function De(e) {
	typeof performance < "u" && performance.mark && performance.mark(`${e}-start`), E(() => {
...
}, ke = x("<div class=\"flex items-center gap-2\"><select class=\"h-8 rounded-md border border-border bg-card px-2 text-xs font-medium transition-colors focus:outline-none focus:ring-1 focus:ring-primary\">"), Ae = x("<option>");
function je() {
	let e = k(), t = O(), n = oe(), r = (e) => {
		t(`${n.pathname.replace(/^\/[^/]+/, `/${e}`)}${n.search}${n.hash}`);
	};
	return (() => {
		var t = ke(), n = t.firstChild;
		return n.addEventListener("change", (e) => r(e.currentTarget.value)), _(n, m(S, {
			each: Oe,
			children: (e) => (() => {
				var t = Ae();
				return t.value = e, _(t, () => Q(e)), t;
			})()
...
}, Ne = x("<button type=button class=\"rounded-md border border-border bg-accent px-3 py-1.5 text-xs font-medium text-foreground transition-colors hover:bg-accent/80\">");
function Pe() {
...
h(["click"]);
var Ie = x("<svg width=14 height=14 viewBox=\"0 0 24 24\"fill=none stroke=currentColor stroke-width=2 stroke-linecap=round stroke-linejoin=round aria-hidden=true><path d=\"m6 9 6 6 6-6\">"), Le = x("<header class=\"sticky top-0 z-50 border-b border-border bg-card/80 backdrop-blur-lg\"><nav class=\"container flex h-16 items-center justify-between\"><div class=\"flex items-center gap-8\"><div class=\"hidden items-center gap-6 text-sm font-medium md:flex\"><div class=relative><button type=button class=\"flex cursor-pointer items-center gap-1 border-none bg-transparent nav-link\"></button></div></div></div><div class=\"flex items-center gap-4\"><a href=https://github.com/intlayer-org/benchmark-i18n target=_blank rel=noreferrer class=\"text-muted-foreground transition hover:text-foreground\"><span class=sr-only></span><svg viewBox=\"0 0 16 16\"aria-hidden=true width=20 height=20><path fill=currentColor d=\"M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.012 8.012 0 0 0 16 8c0-4.42-3.58-8-8-8z\">"), Re = x("<div class=\"absolute left-0 top-full w-48 pt-2\"><div class=\"overflow-hidden rounded-md border border-border bg-card py-1 shadow-lg\">");
function ze(e) {
	return (() => {
		var t = Ie();
...
function Be() {
	let e = Z(A);
	De(e().header.value);
	let [t, n] = w(!1), r = k(), i = () => r.locale ?? "en", a = () => [
		{
			to: `/${i()}/products`,
	...
	return (() => {
		var r = Le(), o = r.firstChild.firstChild, s = o.firstChild, c = s.firstChild, l = c.firstChild, u = o.nextSibling, d = u.firstChild.firstChild;
		return _(o, m(D, {
			get href() {
				return `/${i()}`;
			},
			...
			return () => e() && (() => {
				...
				return e.addEventListener("mouseleave", () => n(!1)), e.addEventListener("mouseenter", () => n(!0)), _(t, m(S, {
					get each() {
						return a();
					},
					children: (e) => m(D, {
		...
		})(), null), _(d, () => e().goToGithub), _(u, m(je, {}), null), _(u, m(Fe, {}), null), r;
	})();
}
h(["click"]);
