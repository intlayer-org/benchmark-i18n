...
import { Dynamic as m, createComponent as h, delegateEvents as g, effect as _, insert as v, memo as y, mergeProps as b, setAttribute as x, template as S } from "solid-js/web";
import { For as C, Suspense as w, createContext as T, createEffect as te, createMemo as ne, createSignal as E, lazy as D, onMount as O, useContext as k } from "solid-js";
import { A, useLocation as re, useNavigate as ie, useParams as j } from "@solidjs/router";
var M = process.env.INTLAYER_EDITOR_ENABLED !== "false" && n?.enabled && typeof window < "u" && window.self !== window.top, N = process.env.INTLAYER_EDITOR_ENABLED === "false", P = (e) => N || !M ? e.children : h(m, {
...
}, ae = (e) => e != null && typeof e != "string" && typeof e != "number" && typeof e != "boolean", L = /\{\{\s*(.*?)\s*\}\}/g, R = (e, t = {}) => {
	if (!Object.values(t).some(ae)) return {
		isSimple: !0,
...
}, z = process.env.INTLAYER_NODE_TYPE_INTLAYER_NODE === "false", B = process.env.INTLAYER_NODE_TYPE_SOLID_NODE === "false", V = process.env.INTLAYER_NODE_TYPE_MARKDOWN === "false", H = process.env.INTLAYER_NODE_TYPE_HTML === "false", U = process.env.INTLAYER_NODE_TYPE_INSERTION === "false", W = process.env.INTLAYER_EDITOR_ENABLED === "false", G = V ? null : D(() => import("./MarkdownRenderer-iWMAOT1d.js").then((e) => ({ default: e.MarkdownMetadataRenderer }))), K = V ? null : D(() => import("./MarkdownRenderer-iWMAOT1d.js").then((e) => ({ default: e.MarkdownRenderer }))), q = H ? null : D(() => import("./HTMLRenderer-_RkJwpIh.js").then((e) => ({ default: e.HTMLRenderer }))), J = z ? c : {
	id: "intlayer-node-plugin",
	canHandle: (e) => typeof e == "bigint" || typeof e == "string" || typeof e == "number",
	transform: (e, { plugins: t, ...r }) => F({
		...r,
		value: r.children,
		children: !W && n.enabled ? h(P, b(r, { get children() {
			return r.children;
		} })) : r.children
	})
}, oe = B ? c : {
		...
		let { plugins: i, ...a } = t, o = r(ee(e) ?? {}, {
			plugins: [{
				...
				transform: (t, r) => F({
					...r,
					value: t,
					children: !W && n.enabled ? h(P, b(a, { get children() {
						return h(w, {
							fallback: e,
							get children() {
								return h(G, b(a, {
...
var me = process.env.INTLAYER_ROUTING_STORAGE_COOKIES === "false", he = process.env.INTLAYER_ROUTING_STORAGE_LOCALSTORAGE === "false", ge = process.env.INTLAYER_ROUTING_STORAGE_SESSIONSTORAGE === "false";
process.env.INTLAYER_ROUTING_STORAGE_HEADERS;
var _e = (n = X) => {
	let { locales: r } = e;
	if (n?.isCookieEnabled === !1) return;
	let i = (e) => !!e && r.includes(e);
	if (!me) for (let e = 0; e < (t.storage.cookies ?? []).length; e++) try {
		let r = n?.getCookie?.(t.storage.cookies[e].name);
		if (i(r)) return r;
	} catch {}
	if (!he) for (let e = 0; e < (t.storage.localStorage ?? []).length; e++) try {
		let r = n?.getLocaleStorage?.(t.storage.localStorage[e].name);
		if (i(r)) return r;
	} catch {}
	if (!ge && t.storage.sessionStorage) for (let e = 0; e < t.storage.sessionStorage.length; e++) try {
...
}, Te = S("<div class=\"flex items-center gap-2\"><select class=\"h-8 rounded-md border border-border bg-card px-2 text-xs font-medium transition-colors focus:outline-none focus:ring-1 focus:ring-primary\">"), Q = S("<option>");
function Ee() {
	let e = j(), t = ie(), n = re(), r = (e) => {
		t(`${n.pathname.replace(/^\/[^/]+/, `/${e}`)}${n.search}${n.hash}`);
	};
	return (() => {
		var t = Te(), n = t.firstChild;
		return n.addEventListener("change", (e) => r(e.currentTarget.value)), v(n, h(C, {
			each: Ce,
			children: (e) => (() => {
				var t = Q();
				return t.value = e, v(t, () => we(e)), t;
			})()
		})), _(() => n.value = e.locale ?? "en"), t;
	})();
}
var De = S("<button type=button class=\"rounded-md border border-border bg-accent px-3 py-1.5 text-xs font-medium text-foreground transition-colors hover:bg-accent/80\">");
function Oe() {
...
function ke() {
	let e = Z("theme-toggle"), [t, n] = E("auto");
	O(() => {
		let e = Oe();
		n(e), $(e);
	}), te(() => {
	...
	return (() => {
		...
		return e.$$click = r, v(e, a), _((t) => {
			var n = i(), r = i();
			return n !== t.e && x(e, "aria-label", t.e = n), r !== t.t && x(e, "title", t.t = r), t;
		}, {
			e: void 0,
			t: void 0
		}), e;
...
g(["click"]);
var Ae = S("<svg width=14 height=14 viewBox=\"0 0 24 24\"fill=none stroke=currentColor stroke-width=2 stroke-linecap=round stroke-linejoin=round aria-hidden=true><path d=\"m6 9 6 6 6-6\">"), je = S("<header class=\"sticky top-0 z-50 border-b border-border bg-card/80 backdrop-blur-lg\"><nav class=\"container flex h-16 items-center justify-between\"><div class=\"flex items-center gap-8\"><div class=\"hidden items-center gap-6 text-sm font-medium md:flex\"><div class=relative><button type=button class=\"flex cursor-pointer items-center gap-1 border-none bg-transparent nav-link\"></button></div></div></div><div class=\"flex items-center gap-4\"><a href=https://github.com/intlayer-org/benchmark-i18n target=_blank rel=noreferrer class=\"text-muted-foreground transition hover:text-foreground\"><span class=sr-only></span><svg viewBox=\"0 0 16 16\"aria-hidden=true width=20 height=20><path fill=currentColor d=\"M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.012 8.012 0 0 0 16 8c0-4.42-3.58-8-8-8z\">"), Me = S("<div class=\"absolute left-0 top-full w-48 pt-2\"><div class=\"overflow-hidden rounded-md border border-border bg-card py-1 shadow-lg\">");
function Ne(e) {
	return (() => {
		var t = Ae();
		return _(() => x(t, "class", e.class)), t;
	})();
}
function Pe() {
	let e = Z("header");
	Se(e().header.value);
	let [t, n] = E(!1), r = j(), i = () => r.locale ?? "en", a = () => [
		{
			to: `/${i()}/products`,
	...
	return (() => {
		var r = je(), o = r.firstChild.firstChild, s = o.firstChild, c = s.firstChild, l = c.firstChild, u = o.nextSibling, d = u.firstChild.firstChild;
		return v(o, h(A, {
			get href() {
				return `/${i()}`;
			},
		...
		}), c), l.$$click = () => n(!t()), l.addEventListener("mouseleave", () => n(!1)), l.addEventListener("mouseenter", () => n(!0)), v(l, () => e().mockPages, null), v(l, h(Ne, { get class() {
			return `transition-transform ${t() ? "rotate-180" : ""}`;
		} }), null), v(c, (() => {
			var e = y(() => !!t());
			return () => e() && (() => {
				var e = Me(), t = e.firstChild;
				return e.addEventListener("mouseleave", () => n(!1)), e.addEventListener("mouseenter", () => n(!0)), v(t, h(C, {
					get each() {
						return a();
					},
					children: (e) => h(A, {
		...
		})(), null), v(d, () => e().goToGithub), v(u, h(Ee, {}), null), v(u, h(ke, {}), null), r;
	})();
}
g(["click"]);
