...
import { Suspense, createContext, createMemo, lazy, onMount, useContext } from "solid-js";
var about_header_default = {
	...
	content: {
		...
		"translation": {
			...
			"es": {
				"aboutThisBenchmark": "Acerca de este benchmark",
				"thisIsAnOpenSource": "Esta es una aplicación de prueba de código abierto, no un producto o una empresa. Su único propósito es proporcionar una aplicación React realista de varias páginas donde se puedan integrar y medir diferentes bibliotecas i18n en condiciones idénticas."
			},
			"de": {
				"aboutThisBenchmark": "Über diesen Benchmark",
				"thisIsAnOpenSource": "Dies ist eine Open-Source-Testanwendung — kein Produkt oder Unternehmen. Ihr einziger Zweck besteht darin, eine realistische, mehrseitige React-App bereitzustellen, in die verschiedene i18n-Bibliotheken integriert und unter identischen Bedingungen gemessen werden können."
			},
			"it": {
				"aboutThisBenchmark": "Informazioni su questo benchmark",
				"thisIsAnOpenSource": "Questa è un'applicazione di test open source — non un prodotto o un'azienda. Il suo unico scopo è quello di fornire un'app React multipagina realistica in cui diverse librerie i18n possono essere integrate e misurate in condizioni identiche."
			},
			"pt": {
				"aboutThisBenchmark": "Sobre este benchmark",
				"thisIsAnOpenSource": "Este é um aplicativo de teste de código aberto — não um produto ou uma empresa. Seu único propósito é fornecer um aplicativo React de várias páginas realista, onde diferentes bibliotecas i18n possam ser integradas e medidas em condições idênticas."
			},
			"zh": {
				"aboutThisBenchmark": "关于此基准测试",
				"thisIsAnOpenSource": "这是一个开源测试应用程序——不是产品或公司。其唯一目的是提供一个真实的、多页面的 React 应用程序，可以在相同条件下集成和衡量不同的 i18n 库。"
			},
			"ja": {
				"aboutThisBenchmark": "このベンチマークについて",
				"thisIsAnOpenSource": "これはオープンソースのテストアプリケーションであり、製品や企業ではありません。その唯一の目的は、さまざまなi18nライブラリを統合し、同一条件下で測定できる現実的なマルチページReactアプリを提供することです。"
			},
			"ko": {
				"aboutThisBenchmark": "이 벤치마크 정보",
				"thisIsAnOpenSource": "이것은 오픈 소스 테스트 애플리케이션이며 제품이나 회사가 아닙니다. 유일한 목적은 동일한 조건에서 다양한 i18n 라이브러리를 통합하고 측정할 수 있는 현실적인 다중 페이지 React 앱을 제공하는 것입니다."
			},
			"ru": {
				"aboutThisBenchmark": "Об этом бенчмарке",
				"thisIsAnOpenSource": "Это открытое тестовое приложение — не продукт и не компания. Его единственная цель — предоставить реалистичное многостраничное React-приложение, в которое можно интегрировать и измерять различные библиотеки i18n в идентичных условиях."
			}
		}
	},
...
function AboutHeader() {
	const content = M(about_header_default);
	usePerformanceMeasure("AboutHeader");
	return [(() => {
		var _el$ = _tmpl$();
		insert(_el$, () => content().aboutThisBenchmark);
		return _el$;
	})(), (() => {
		var _el$2 = _tmpl$2();
		insert(_el$2, () => content().thisIsAnOpenSource);
		return _el$2;
	})()];
}
