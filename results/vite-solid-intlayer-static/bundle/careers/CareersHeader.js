...
var careers_header_default = {
	...
	content: {
		...
		"translation": {
			...
			"de": {
				"careers": "Karriere",
				"joinOurMissionToImprove": "Schließen Sie sich unserer Mission an, das Ökosystem der Internationalisierung zu verbessern. Wir sind ein Remote-First-Team, das Wert auf Wirkung, Transparenz und kontinuierliches Lernen legt."
			},
			"it": {
				"careers": "Carriere",
			...
			"pt": {
				"careers": "Carreiras",
				"joinOurMissionToImprove": "Junte-se à nossa missão para melhorar o ecossistema de internacionalização. Somos uma equipe que prioriza o trabalho remoto e valoriza o impacto, a transparência e o aprendizado contínuo."
			},
			"zh": {
				"careers": "职业生涯",
			...
			"ja": {
				"careers": "採用情報",
				"joinOurMissionToImprove": "国際化エコシステムを改善するという私たちの使命に参加してください。私たちは、インパクト、透明性、そして継続的な学習を大切にするリモートファーストのチームです。"
			},
			"ko": {
				"careers": "채용",
				"joinOurMissionToImprove": "국제화 생태계를 개선하려는 우리의 사명에 동참하십시오. 우리는 영향력, 투명성 및 지속적인 학습을 가치 있게 여기는 원격 우선 팀입니다."
			},
			"ru": {
				"careers": "Вакансии",
				"joinOurMissionToImprove": "Присоединяйтесь к нашей миссии по улучшению экосистемы интернационализации. Мы — команда, ориентированная на удаленную работу, которая ценит вклад, прозрачность и непрерывное обучение."
			}
		}
	},
	localIds: ["careers-header::local::src/components/pages/careers/careersHeader.content.ts"]
};
...
function MockBanner() {
	const content = M(mock_banner_default);
	return (() => {
		var _el$ = _tmpl$$1();
		insert(_el$, () => content().thisPageContainsMockData);
		return _el$;
	})();
}
var _tmpl$ = template(`<h1 class="mb-2 text-3xl font-bold text-foreground">`), _tmpl$2 = template(`<p class="mb-4 text-muted-foreground">`);
function CareersHeader() {
	const content = M(careers_header_default);
	return [
		createComponent(MockBanner, {}),
		(() => {
			var _el$ = _tmpl$();
			insert(_el$, () => content().careers);
			return _el$;
		})(),
		(() => {
			var _el$2 = _tmpl$2();
			insert(_el$2, () => content().joinOurMissionToImprove);
			return _el$2;
		})()
	];
}
