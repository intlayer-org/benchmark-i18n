...
import { c as d, o as f, s as p } from "./nodeType-CG6hvPHO.js";
import { n as ee } from "./markdown-BZysCahS.js";
...
var te = process.env.INTLAYER_EDITOR_ENABLED !== "false" && n?.enabled && typeof window < "u" && window.self !== window.top, w = process.env.INTLAYER_EDITOR_ENABLED === "false", T = (e) => w || !te ? e.children : h(m, {
...
}, O = (e) => e != null && typeof e != "string" && typeof e != "number" && typeof e != "boolean", k = /\{\{\s*(.*?)\s*\}\}/g, ne = (e, t = {}) => {
	if (!Object.values(t).some(O)) return {
		isSimple: !0,
...
}, A = process.env.INTLAYER_NODE_TYPE_INTLAYER_NODE === "false", j = process.env.INTLAYER_NODE_TYPE_SOLID_NODE === "false", M = process.env.INTLAYER_NODE_TYPE_MARKDOWN === "false", N = process.env.INTLAYER_NODE_TYPE_HTML === "false", P = process.env.INTLAYER_NODE_TYPE_INSERTION === "false", F = process.env.INTLAYER_EDITOR_ENABLED === "false", I = M ? null : S(() => import("./MarkdownRenderer-iWMAOT1d.js").then((e) => ({ default: e.MarkdownMetadataRenderer }))), L = M ? null : S(() => import("./MarkdownRenderer-iWMAOT1d.js").then((e) => ({ default: e.MarkdownRenderer }))), R = N ? null : S(() => import("./HTMLRenderer-_RkJwpIh.js").then((e) => ({ default: e.HTMLRenderer }))), z = A ? c : {
	id: "intlayer-node-plugin",
	canHandle: (e) => typeof e == "bigint" || typeof e == "string" || typeof e == "number",
	transform: (e, { plugins: t, ...r }) => E({
		...r,
		value: r.children,
		children: !F && n.enabled ? h(T, _(r, { get children() {
			return r.children;
		} })) : r.children
	})
}, B = j ? c : {
		...
		let { plugins: i, ...a } = t, o = r(ee(e) ?? {}, {
			plugins: [{
				...
				transform: (t, r) => E({
					...r,
					value: t,
					children: !F && n.enabled ? h(T, _(a, { get children() {
						return h(y, {
							fallback: e,
							get children() {
								return h(I, _(a, {
...
var Y = process.env.INTLAYER_ROUTING_STORAGE_COOKIES === "false", X = process.env.INTLAYER_ROUTING_STORAGE_LOCALSTORAGE === "false", Z = process.env.INTLAYER_ROUTING_STORAGE_SESSIONSTORAGE === "false";
process.env.INTLAYER_ROUTING_STORAGE_HEADERS;
var re = (n = Q) => {
	let { locales: r } = e;
	if (n?.isCookieEnabled === !1) return;
	let i = (e) => !!e && r.includes(e);
	if (!Y) for (let e = 0; e < (t.storage.cookies ?? []).length; e++) try {
		let r = n?.getCookie?.(t.storage.cookies[e].name);
		if (i(r)) return r;
	} catch {}
	if (!X) for (let e = 0; e < (t.storage.localStorage ?? []).length; e++) try {
		let r = n?.getLocaleStorage?.(t.storage.localStorage[e].name);
		if (i(r)) return r;
	} catch {}
	if (!Z && t.storage.sessionStorage) for (let e = 0; e < t.storage.sessionStorage.length; e++) try {
...
}, { defaultLocale: oe, locales: se } = e, ce = v("<div class=\"mb-6 rounded-md border border-border bg-muted px-4 py-3 text-center text-sm text-muted-foreground\">");
function le() {
	let e = $("mock-banner");
	return (() => {
		var t = ce();
...
var ue = v("<h1 class=\"mb-2 text-3xl font-bold text-foreground\">"), de = v("<p class=\"mb-8 text-muted-foreground\"> <a href=mailto:contact@intlayer.org class=\"text-primary hover:underline\">contact@intlayer.org</a>.");
function fe() {
