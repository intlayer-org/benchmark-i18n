	...
	content: {
		...
		translation: {
			...
			fr: {
				...
				chooseThePlanThatFits: "Choisissez le forfait qui convient à votre équipe. Pas de frais cachés."
			},
			es: {
				simpleTransparentPricing: "Precios simples y transparentes",
				chooseThePlanThatFits: "Elija el plan que mejor se adapte a su equipo. Sin cargos ocultos."
			},
			de: {
			...
			pt: {
				...
				chooseThePlanThatFits: "Escolha o plano que melhor se adapta à sua equipe. Sem taxas ocultas."
			},
			zh: {
			...
			ja: {
				...
				chooseThePlanThatFits: "あなたのチームに合ったプランをお選びください。隠れた費用はありません。"
			},
			ko: {
				simpleTransparentPricing: "단순하고 투명한 가격 책정",
				chooseThePlanThatFits: "팀에 맞는 플랜을 선택하십시오. 숨겨진 비용은 없습니다."
			},
			ru: {
				simpleTransparentPricing: "Простые и прозрачные цены",
				chooseThePlanThatFits: "Выберите план, который подходит вашей команде. Никаких скрытых платежей."
			}
		}
	},
...
}, D = "\x1B[0m", O = "\x1B[34m", k = "\x1B[31m", te = "\x1B[32m", A = (e, t, n) => t ? `${t}${e}${n ? typeof n == "boolean" ? D : n : D}` : e;
A("✗", k), A("✓", te), A("⏲", O);
...
}, F = process.env.INTLAYER_NODE_TYPE_INTLAYER_NODE === "false", I = process.env.INTLAYER_NODE_TYPE_SOLID_NODE === "false", L = !0, R = !0, z = !0, B = !0, V = null, H = null, U = null, W = null, G = null;
B || (V = S(() => import("./ContentSelector--hRpRV2o.js").then((e) => ({ default: e.ContentSelector })))), L || (H = S(() => import("./MarkdownRenderer-B7hyvLDf.js").then((e) => ({ default: e.MarkdownMetadataRenderer }))), U = S(() => import("./MarkdownRenderer-B7hyvLDf.js").then((e) => ({ default: e.MarkdownRenderer }))), import("./markdown-Cvh9lBUG.js").then((e) => {
	G = e.getMarkdownMetadata;
})), R || (W = S(() => import("./HTMLRenderer-C-wPH0rg.js").then((e) => ({ default: e.HTMLRenderer }))));
var K = F ? e : {
	...
	transform: (e, { plugins: t, ...r }) => T({
		...r,
		value: r.children,
		children: !B && n.enabled ? h(y, {
			get fallback() {
				return r.children;
			},
			get children() {
				return h(V, _(r, { get children() {
					return r.children;
				} }));
			}
		}) : r.children
		...
		let { plugins: i, ...a } = t, o = r(G?.(e) ?? {}, {
			plugins: [{
				...
				transform: (t, r) => T({
					...r,
					value: t,
					children: !B && n.enabled ? h(ContentSelector, _(a, { get children() {
						return h(y, {
							fallback: e,
							get children() {
								return h(H, _(a, {
	...
	transform: (e, t) => {
		let r = e[ee], { plugins: i, ...a } = t, o = (e) => T({
			...a,
			value: r,
			children: !B && n.enabled ? h(y, {
				fallback: r,
	...
	return Z.set(r, o), o;
}, oe = (e, t) => j(e, t, ae(t)), se = process.env.INTLAYER_ROUTING_STORAGE_COOKIES === "false";
process.env.INTLAYER_ROUTING_STORAGE_HEADERS;
var ce = (e = Q) => {
	let { locales: t } = c;
	if (e?.isCookieEnabled === !1) return;
	let n = (e) => !!e && t.includes(e);
	if (!se) for (let t = 0; t < (r.storage.cookies ?? []).length; t++) try {
	...
	locale: () => le ?? c?.defaultLocale,
	setLocale: () => null
}), $ = (e, t) => {
	let n = C(ue) ?? {};
	return x(() => oe(e, t ?? n?.locale?.()));
}, { defaultLocale: de, locales: fe } = c, pe = {
	key: "mock-banner",
	content: {
		nodeType: "translation",
		translation: {
			en: { thisPageContainsMockData: "⚠️ This page contains mock data for benchmarking purposes only. It is not related to any real business or service." },
			fr: { thisPageContainsMockData: "⚠️ Cette page contient des données factices à des fins de benchmarking uniquement. Elle n'est liée à aucune entreprise ou service réel." },
			es: { thisPageContainsMockData: "⚠️ Esta página contiene datos de prueba solo para fines de benchmarking. No está relacionada con ninguna empresa o servicio real." },
			de: { thisPageContainsMockData: "⚠️ Diese Seite enthält Mock-Daten nur zu Benchmarking-Zwecken. Sie steht in keinem Zusammenhang mit einem echten Unternehmen oder einer echten Dienstleistung." },
			it: { thisPageContainsMockData: "⚠️ Questa pagina contiene dati fittizi solo a scopo di benchmarking. Non è correlata ad alcuna attività o servizio reale." },
			pt: { thisPageContainsMockData: "⚠️ Esta página contém dados fictícios apenas para fins de benchmarking. Não está relacionada a nenhum negócio ou serviço real." },
			zh: { thisPageContainsMockData: "⚠️ 本页面包含仅用于基准测试目的的模拟数据。它与任何真实的业务 or 服务无关。" },
			ja: { thisPageContainsMockData: "⚠️ このページにはベンチマーク目的のモックデータのみが含まれています。実際のビジネスやサービスとは関係ありません。" },
			ko: { thisPageContainsMockData: "⚠️ 이 페이지에는 벤치마킹 목적의 모의 데이터만 포함되어 있습니다. 실제 비즈니스나 서비스와는 관련이 없습니다." },
			ru: { thisPageContainsMockData: "⚠️ Эта страница содержит мок-данные только для целей бенчмаркинга. Она не связана с каким-либо реальным бизнесом или услугой." }
		}
	},
	localIds: ["mock-banner::local::src/components/mockBanner.content.ts"]
}, me = v("<div class=\"mb-6 rounded-md border border-border bg-muted px-4 py-3 text-center text-sm text-muted-foreground\">");
function he() {
	let e = $(pe);
	return (() => {
		var t = me();
		return g(t, () => e().thisPageContainsMockData), t;
	})();
}
var ge = v("<div class=\"mb-12 text-center\"><h1 class=\"mb-3 text-3xl font-bold text-foreground\"></h1><p class=text-muted-foreground>");
function _e() {
	let e = $(w);
	return [h(he, {}), (() => {
		var t = ge(), n = t.firstChild, r = n.nextSibling;
		return g(n, () => e().simpleTransparentPricing), g(r, () => e().chooseThePlanThatFits), t;
	})()];
}
