...
import { Dynamic as m, className as h, createComponent as g, effect as _, insert as v, memo as y, mergeProps as b, template as x } from "solid-js/web";
import { For as S, Suspense as C, createContext as w, createMemo as T, lazy as E, useContext as D } from "solid-js";
var O = process.env.INTLAYER_EDITOR_ENABLED !== "false" && n?.enabled && typeof window < "u" && window.self !== window.top, k = process.env.INTLAYER_EDITOR_ENABLED === "false", A = (e) => k || !O ? e.children : g(m, {
...
}, I = process.env.INTLAYER_NODE_TYPE_INTLAYER_NODE === "false", L = process.env.INTLAYER_NODE_TYPE_SOLID_NODE === "false", R = process.env.INTLAYER_NODE_TYPE_MARKDOWN === "false", z = process.env.INTLAYER_NODE_TYPE_HTML === "false", B = process.env.INTLAYER_NODE_TYPE_INSERTION === "false", V = process.env.INTLAYER_EDITOR_ENABLED === "false", H = R ? null : E(() => import("./MarkdownRenderer-iWMAOT1d.js").then((e) => ({ default: e.MarkdownMetadataRenderer }))), U = R ? null : E(() => import("./MarkdownRenderer-iWMAOT1d.js").then((e) => ({ default: e.MarkdownRenderer }))), W = z ? null : E(() => import("./HTMLRenderer-_RkJwpIh.js").then((e) => ({ default: e.HTMLRenderer }))), G = I ? c : {
	id: "intlayer-node-plugin",
	canHandle: (e) => typeof e == "bigint" || typeof e == "string" || typeof e == "number",
	transform: (e, { plugins: t, ...r }) => j({
		...r,
		value: r.children,
		children: !V && n.enabled ? g(A, b(r, { get children() {
			return r.children;
		} })) : r.children
	})
}, K = L ? c : {
		...
		let { plugins: i, ...a } = t, o = r(p(e) ?? {}, {
			plugins: [{
				...
				transform: (t, r) => j({
					...r,
					value: t,
					children: !V && n.enabled ? g(A, b(a, { get children() {
						return g(C, {
							fallback: e,
							get children() {
								return g(H, b(a, {
...
var re = process.env.INTLAYER_ROUTING_STORAGE_COOKIES === "false", ie = process.env.INTLAYER_ROUTING_STORAGE_LOCALSTORAGE === "false", ae = process.env.INTLAYER_ROUTING_STORAGE_SESSIONSTORAGE === "false";
process.env.INTLAYER_ROUTING_STORAGE_HEADERS;
var oe = (n = $) => {
	let { locales: r } = e;
	if (n?.isCookieEnabled === !1) return;
	let i = (e) => !!e && r.includes(e);
	if (!re) for (let e = 0; e < (t.storage.cookies ?? []).length; e++) try {
		let r = n?.getCookie?.(t.storage.cookies[e].name);
		if (i(r)) return r;
	} catch {}
	if (!ie) for (let e = 0; e < (t.storage.localStorage ?? []).length; e++) try {
		let r = n?.getLocaleStorage?.(t.storage.localStorage[e].name);
		if (i(r)) return r;
	} catch {}
	if (!ae && t.storage.sessionStorage) for (let e = 0; e < t.storage.sessionStorage.length; e++) try {
...
}, { defaultLocale: ue, locales: de } = e, fe = x("<div class=\"grid gap-6 md:grid-cols-3\">"), pe = x("<div><h3 class=\"text-lg font-semibold text-foreground\"></h3><div class=my-4><span class=\"text-3xl font-bold text-foreground\"></span><span class=\"text-sm text-muted-foreground\"></span></div><ul class=\"mb-6 flex-1 space-y-2\"></ul><button type=button>"), me = x("<li class=\"flex items-center gap-2 text-sm text-muted-foreground\"><span class=text-primary>✓</span> ");
function he() {
	let e = le("pricing-tiers"), t = [
		{
	...
	return (() => {
		...
		return v(n, g(S, {
			each: t,
			children: (t) => (() => {
				var n = pe(), r = n.firstChild, i = r.nextSibling, a = i.firstChild, o = a.nextSibling, s = i.nextSibling, c = s.nextSibling;
				return v(r, () => t.name), v(a, () => t.price), v(o, () => t.period), v(s, g(S, {
					get each() {
						return t.features;
					},
					children: (e) => (() => {
						var t = me();
						return t.firstChild.nextSibling, v(t, e, null), t;
					})()
				})), v(c, (() => {
					var n = y(() => t.name === e().enterprise.value);
					return () => n() ? e().contactSales.value : e().getStarted.value;
				})()), _((r) => {
					var i = `flex flex-col rounded-lg border p-6 ${t.highlighted ? e().borderPrimaryBgPrimary5Ring.value : "border-border bg-card"}`, a = `w-full rounded-md px-4 py-2 text-sm font-medium transition-opacity hover:opacity-90 ${t.highlighted ? "bg-primary text-primary-foreground" : e().borderBorderBorderTextForeground.value}`;
					return i !== r.e && h(n, r.e = i), a !== r.t && h(c, r.t = a), r;
				}, {
					e: void 0,
					t: void 0
				}), n;
