...
function PricingTiers() {
	return (() => {
		...
		insert(_el$, createComponent(For, {
			...
			children: (tier) => {
				const id = tier.id;
				const period = id === "enterprise" ? "" : t(`pricing.tiers.${id}Period`);
				return (() => {
					...
					insert(_el$3, () => t(`pricing.tiers.${id}Name`));
					insert(_el$5, () => t(`pricing.tiers.${id}Price`));
					insert(_el$6, period);
					insert(_el$7, createComponent(For, {
						get each() {
							return [...tier.featureKeys];
						},
						children: (fk) => (() => {
							var _el$9 = _tmpl$3();
							_el$9.firstChild.nextSibling;
							insert(_el$9, () => t(`pricing.tiers.${fk}`), null);
							return _el$9;
						})()
					}));
					insert(_el$8, () => id === "enterprise" ? t("pricing.tiers.contactSales") : t("pricing.tiers.getStarted"));
					effect((_p$) => {
						var _v$ = `flex flex-col rounded-lg border p-6 ${tier.highlighted ? "border-primary bg-primary/5 ring-1 ring-primary" : "border-border bg-card"}`, _v$2 = `w-full rounded-md px-4 py-2 text-sm font-medium transition-opacity hover:opacity-90 ${tier.highlighted ? "bg-primary text-primary-foreground" : "border border-border text-foreground hover:bg-accent"}`;
...
function Wrapped() {
	return createComponent(Wrapper, { get children() {
		return createComponent(PricingTiers, {});
	} });
}
