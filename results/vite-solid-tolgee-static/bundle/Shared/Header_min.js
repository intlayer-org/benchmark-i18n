...
}, M = (e) => Object.fromEntries(ue(e).entries()), N = (e) => {
	let [t, ...n] = e.split(":");
	return {
		language: t,
		namespace: n.join(":") || ""
...
function xe(e, t, n, r, i, a, o, s) {
	...
	function S() {
		return O(s.onError, (e) => new he("Tolgee: Failed to load language", e), () => l.languageStorage?.getLanguage(p()));
	}
	function C() {
	...
	let E = Object.freeze({
		...
		setStoredLanguage(e) {
			return O(s.onError, (e) => new he("Tolgee: Failed to store language", e), () => l.languageStorage?.setLanguage(e, p()));
		},
		getDevBackend() {
			return l.devBackend;
		},
		...
		formatTranslation(n) {
			...
			try {
				...
				console.error(e);
				let n = ae(e) || I, r = t().onFormatError, a = typeof r;
...
var Oe = () => {
	...
	return t;
}, ke = 0, R = 1, Ae = 2, je = class extends Error {
	constructor(e, t, n) {
		let r, i = "";
		e === ke ? r = "Empty parameter" : e === R ? (r = "Unexpected character", i = "Did you forget to use FormatIcu to render ICU message syntax?") : r = "Unexpected end", super(`Tolgee parser: ${r} at ${t} in "${n}"` + (i ? "\n" + i : "")), this.code = e, this.index = t;
	}
};
function Me(e) {
	return /\s/.test(e);
}
var z = 0, B = 1, V = 2, H = 3, U = 4, Ne = /* @__PURE__ */ new Set([
	V,
	B,
	z
]), W = "'", Pe = /* @__PURE__ */ new Set([
	"{",
	"}",
	W
...
function Ie(e) {
	...
	for (s = 0; s < e.length; s++) switch (i = e[s], t) {
		case z:
			...
			break;
		case B:
			Pe.has(i) ? (n = n.slice(0, -1) + i, t = V) : (n += i, t = z);
			break;
		case V:
			i === W ? t = z : (n += i, t = V);
			break;
		case H:
			i === "}" ? (d(), t = z) : Me(i) ? r !== "" && (d(), t = U) : (u(), t = H);
			break;
		case U: i == "}" ? t = z : Me(i) ? t = U : c(R);
	}
...
}, $e = 3e3, et = 200, tt = 0, q = /* @__PURE__ */ new Map(), nt = !1, J, Y;
...
var it = (e) => new K("unavailable", `the Tolgee browser extension did not answer ${e} in time`), at = () => `${Date.now()}-${tt++}-${Math.random()}`;
function ot() {
...
function st(e) {
	return J ||= new Promise((t, n) => {
		let r = () => window.postMessage({ type: Xe }, window.origin), i = () => {
			clearInterval(a), ct(), Y = void 0, n(new K("unavailable", "the Tolgee browser extension did not answer"));
		}, a = setInterval(() => {
			if (Date.now() > e) {
				i();
				return;
...
var X = "__tolgee_", Tt = `${X}apiKey`, Et = `${X}apiUrl`, Dt = `${X}branch`, Ot = `${X}projectId`, kt = `${X}session`, At = "tolgee-in-context-tools.umd.min.js", jt = "@tolgee/in-context-tools", Mt = "InContextTools", Nt = "https://cdn.jsdelivr.net/npm", Pt = null;
function Ft(e) {
...
Z.run();
var [qt, Jt] = p(Z.getLanguage() || "en");
Z.on("language", (e) => {
	Jt(e.value);
});
function Q(...e) {
	return qt(), e.length === 0 ? (...e) => (qt(), Z.t(...e)) : Z.t(...e);
}
var $ = Q, Yt = [
	"en",
	"fr",
...
}, Zt = o("<div class=\"flex items-center gap-2\"><select class=\"h-8 rounded-md border border-border bg-card px-2 text-xs font-medium transition-colors focus:outline-none focus:ring-1 focus:ring-primary\">"), Qt = o("<option>");
function $t() {
	let t = u(), i = l(), a = c(), o = (e) => {
	...
	return (() => {
		var i = Zt(), a = i.firstChild;
		return a.addEventListener("change", (e) => o(e.currentTarget.value)), r(a, e(d, {
			each: Yt,
			children: (e) => (() => {
				var t = Qt();
				return t.value = e, r(t, () => Xt(e)), t;
			})()
		})), n(() => a.value = t.locale ?? "en"), i;
	})();
}
var en = o("<button type=button class=\"rounded-md border border-border bg-accent px-3 py-1.5 text-xs font-medium text-foreground transition-colors hover:bg-accent/80\">");
function tn() {
...
function rn() {
	...
	m(() => {
		...
		if (e() !== "auto") return;
		let t = window.matchMedia("(prefers-color-scheme: dark)"), n = () => nn("auto");
	...
	function i() {
		...
		t(r), nn(r), window.localStorage.setItem("theme", r);
	}
...
t(["click"]);
var an = o("<svg width=14 height=14 viewBox=\"0 0 24 24\"fill=none stroke=currentColor stroke-width=2 stroke-linecap=round stroke-linejoin=round aria-hidden=true><path d=\"m6 9 6 6 6-6\">"), on = o("<header class=\"sticky top-0 z-50 border-b border-border bg-card/80 backdrop-blur-lg\"><nav class=\"container flex h-16 items-center justify-between\"><div class=\"flex items-center gap-8\"><div class=\"hidden items-center gap-6 text-sm font-medium md:flex\"><div class=relative><button type=button class=\"flex cursor-pointer items-center gap-1 border-none bg-transparent nav-link\"></button></div></div></div><div class=\"flex items-center gap-4\"><a href=https://github.com/intlayer-org/benchmark-i18n target=_blank rel=noreferrer class=\"text-muted-foreground transition hover:text-foreground\"><span class=sr-only></span><svg viewBox=\"0 0 16 16\"aria-hidden=true width=20 height=20><path fill=currentColor d=\"M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.012 8.012 0 0 0 16 8c0-4.42-3.58-8-8-8z\">"), sn = o("<div class=\"absolute left-0 top-full w-48 pt-2\"><div class=\"overflow-hidden rounded-md border border-border bg-card py-1 shadow-lg\">");
function cn(e) {
	return (() => {
		var t = an();
...
function ln() {
	...
	return (() => {
		var a = on(), l = a.firstChild.firstChild, u = l.firstChild, f = u.firstChild, p = f.firstChild, m = l.nextSibling, h = m.firstChild.firstChild;
		return r(l, e(s, {
			get href() {
				return `/${o()}`;
			},
		...
		}), f), p.$$click = () => n(!t()), p.addEventListener("mouseleave", () => n(!1)), p.addEventListener("mouseenter", () => n(!0)), r(p, () => Q()("header.mockPages"), null), r(p, e(cn, { get class() {
			return `transition-transform ${t() ? "rotate-180" : ""}`;
		} }), null), r(f, (() => {
			var a = i(() => !!t());
			return () => a() && (() => {
				var t = sn(), i = t.firstChild;
		...
		})(), null), r(h, () => Q()("shared.goToGithub")), r(m, e($t, {}), null), r(m, e(rn, {}), null), a;
	})();
}
t(["click"]);
function un(e) {
