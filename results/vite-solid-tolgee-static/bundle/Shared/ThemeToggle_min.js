...
import { createEffect as s, createSignal as c, onMount as l } from "solid-js";
...
var we = () => {
	let e = {
		...
		init(t) {
			let n = Ce(N(e.options, t));
			return e.plugins.forEach(n.addPlugin), n;
		}
	});
	return t;
}, I = 0, L = 1, Te = 2, Ee = class extends Error {
	constructor(e, t, n) {
		let r, i = "";
...
var Y = "__tolgee_", bt = `${Y}apiKey`, xt = `${Y}apiUrl`, St = `${Y}branch`, Ct = `${Y}projectId`, wt = `${Y}session`, Tt = "tolgee-in-context-tools.umd.min.js", Et = "@tolgee/in-context-tools", Dt = "InContextTools", Ot = "https://cdn.jsdelivr.net/npm", X = null;
function kt(e) {
	return X ||= Pt(At(e)).then(() => window[Et][Dt]), X;
}
function At(e) {
	return jt() || `${Ot}/@tolgee/web@${e}/dist/${Tt}`;
}
function jt() {
	if (Fe()) return;
...
Z.run();
var [Vt, Ht] = c(Z.getLanguage() || "en");
Z.on("language", (e) => {
	Ht(e.value);
});
function Ut(...e) {
	return Vt(), e.length === 0 ? (...e) => (Vt(), Z.t(...e)) : Z.t(...e);
}
var Q = Ut, Wt = o("<button type=button class=\"rounded-md border border-border bg-accent px-3 py-1.5 text-xs font-medium text-foreground transition-colors hover:bg-accent/80\">");
function Gt() {
...
function Kt() {
	...
	let o = () => e() === "auto" ? Q("themeToggle.labelAuto") : Q("themeToggle.labelOther", { mode: e() }), u = () => e() === "auto" ? Q("themeToggle.auto") : e() === "dark" ? Q("themeToggle.dark") : Q("themeToggle.light");
	return (() => {
		var e = Wt();
		return e.$$click = i, r(e, u), n((t) => {
			var n = o(), r = o();
