...
var k = () => ({
	...
	let r = ee(n?.headers);
	return r["x-api-key"] && (r = Object.assign(Object.assign({}, k()), r)), e(t, Object.assign(Object.assign({}, n), { headers: r }));
}, ne = (e) => {
...
}, A = (e) => Object.fromEntries(ne(e).entries()), j = (e) => {
	let [t, ...n] = e.split(":");
	return {
		language: t,
		namespace: n.join(":") || ""
...
function I(e, t) {
	let n = F(de, t?.initialOptions, e);
	return n.apiUrl = E(n.apiUrl), e?.fetch && (n.fetch = te(e.fetch)), {
		initialOptions: n,
...
function fe(e, t, n, r, i, a, o, s) {
	...
	function w() {
		return y(s.onError, (e) => new se("Tolgee: Failed to load language", e), () => l.languageStorage?.getLanguage(p()));
	}
	function T() {
		if (!l.languageDetector) return;
		let e = n();
		return y(s.onError, (e) => new oe("Tolgee: Failed to detect language", e), () => l.languageDetector?.getLanguage(Object.assign({ availableLanguages: e }, p())));
	}
	function E(e) {
		e && l.backends.push(e);
	}
	...
	let k = Object.freeze({
		...
		setStoredLanguage(e) {
			return y(s.onError, (e) => new se("Tolgee: Failed to store language", e), () => l.languageStorage?.setLanguage(e, p()));
		},
		getDevBackend() {
			return l.devBackend;
		},
		...
		formatTranslation(n) {
			...
			try {
				...
				console.error(e);
				let n = D(e) || P, r = t().onFormatError, a = typeof r;
...
function ge({ options: e }) {
	let t = ie(), n = L(!1, () => o.isFetching(), t.onFetchingChange.emit), r = L(!1, () => D.isLoading(), t.onLoadingChange.emit), i = pe(t.onLanguageChange, t.onPendingLanguageChange, t.onRunningChange), a = fe(i.getLanguage, i.getInitialOptions, i.getAvailableLanguages, u, y, x, f, t), o = ce(t, a.getBackendRecord, a.getBackendDevRecord, i.withDefaultNs, i.isInitialLoading, n, r);
	e && p(e);
	let s;
	t.onUpdate.listen(() => {
...
var ve = () => {
	let e = {
		...
		init(t) {
			let n = _e(F(e.options, t));
			return e.plugins.forEach(n.addPlugin), n;
		}
	});
	return t;
}, R = 0, z = 1, ye = 2, be = class extends Error {
	constructor(e, t, n) {
		let r, i = "";
...
var V = 0, H = 1, U = 2, W = 3, G = 4, xe = /* @__PURE__ */ new Set([
	U,
	H,
	V
]), K = "'", Se = /* @__PURE__ */ new Set([
	"{",
	"}",
	K
...
function we(e) {
	...
	for (s = 0; s < e.length; s++) switch (i = e[s], t) {
		case V:
			...
			break;
		case H:
			Se.has(i) ? (n = n.slice(0, -1) + i, t = U) : (n += i, t = V);
			break;
		case U:
			i === K ? t = V : (n += i, t = U);
			break;
		case W:
			i === "}" ? (d(), t = V) : B(i) ? r !== "" && (d(), t = G) : (u(), t = W);
			break;
		case G: i == "}" ? t = V : B(i) ? t = G : c(z);
	}
...
(h = console.assert) == null || h.call(console, je), RegExp(`([${["‌", "‍"].join("")}]{9})+`, "g");
function Me(e, t) {
	return t ? `${t}: ${e}` : e;
}
...
}, Ve = 3e3, He = 200, Ue = 0, Y = /* @__PURE__ */ new Map(), We = !1, X, Z;
async function Ge({ type: e, replyType: t, payload: n, timeoutMs: r = Fe, progressType: i, onProgress: a }) {
	Je();
	let o = Date.now() + r;
	await Ye(Math.min(o, Date.now() + Ve));
	let s = o - Date.now();
	if (s <= 0) throw Ke(e);
	let c = qe();
	return new Promise((r, o) => {
		let l = setTimeout(() => {
			Y.delete(c), Xe(), o(Ke(e));
		}, s);
		Y.set(c, {
			replyType: t,
			progressType: i,
...
var Ke = (e) => new J("unavailable", `the Tolgee browser extension did not answer ${e} in time`), qe = () => `${Date.now()}-${Ue++}-${Math.random()}`;
function Je() {
	We || (We = !0, window.addEventListener("message", (e) => {
		var t;
...
function Ye(e) {
	return X ||= new Promise((t, n) => {
		let r = () => window.postMessage({ type: Re }, window.origin), i = () => {
			clearInterval(a), Xe(), Z = void 0, n(new J("unavailable", "the Tolgee browser extension did not answer"));
		}, a = setInterval(() => {
			if (Date.now() > e) {
				i();
				return;
...
var Q = "__tolgee_", pt = `${Q}apiKey`, mt = `${Q}apiUrl`, ht = `${Q}branch`, gt = `${Q}projectId`, _t = `${Q}session`, vt = "tolgee-in-context-tools.umd.min.js", yt = "@tolgee/in-context-tools", bt = "InContextTools", xt = "https://cdn.jsdelivr.net/npm", St = null;
function Ct(e) {
...
function kt() {
	let e = sessionStorage.getItem(pt) || void 0, t = sessionStorage.getItem(mt) || void 0, n = sessionStorage.getItem(ht) || void 0, r = sessionStorage.getItem(gt) || void 0, i = Be(sessionStorage.getItem(_t));
	if (!t) return;
	let a = p(p({ apiUrl: t }, r === void 0 ? {} : { projectId: r }), n === void 0 ? {} : { branch: n });
	if (e) return m(p({}, a), { apiKey: e });
	if (i && r) return m(p({}, a), { transport: Ze() });
}
function At() {
	let e = [];
...
$.run();
var [Ft, It] = a($.getLanguage() || "en");
$.on("language", (e) => {
	It(e.value);
});
function Lt(...e) {
	return Ft(), e.length === 0 ? (...e) => (Ft(), $.t(...e)) : $.t(...e);
}
var Rt = Lt, zt = r("<div class=\"mx-auto max-w-3xl space-y-4\">"), Bt = r("<details class=\"group rounded-lg border border-border bg-card\"><summary class=\"cursor-pointer px-6 py-4 text-sm font-medium text-foreground transition-colors hover:bg-accent/50\"></summary><p class=\"px-6 pb-4 text-sm text-muted-foreground\">");
function Vt() {
	let n = [
		1,
		2,
	...
	return (() => {
		...
		return t(r, e(i, {
			each: n,
			children: (e) => (() => {
				var n = Bt(), r = n.firstChild, i = r.nextSibling;
				return t(r, () => Rt(`faq.list.q${e}`)), t(i, () => Rt(`faq.list.a${e}`)), n;
			})()
		})), r;
	})();
}
