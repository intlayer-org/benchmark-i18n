...
var O = () => ({
	...
	let r = te(n?.headers);
	return r["x-api-key"] && (r = Object.assign(Object.assign({}, O()), r)), e(t, Object.assign(Object.assign({}, n), { headers: r }));
}, re = (e) => {
...
}, k = (e) => Object.fromEntries(re(e).entries()), A = (e) => {
	let [t, ...n] = e.split(":");
	return {
		language: t,
		namespace: n.join(":") || ""
...
function F(e, t) {
	let n = P(fe, t?.initialOptions, e);
	return n.apiUrl = E(n.apiUrl), e?.fetch && (n.fetch = ne(e.fetch)), {
		initialOptions: n,
...
function pe(e, t, n, r, i, a, o, s) {
	...
	function w() {
		return y(s.onError, (e) => new ce("Tolgee: Failed to load language", e), () => l.languageStorage?.getLanguage(p()));
	}
	function T() {
		if (!l.languageDetector) return;
		let e = n();
		return y(s.onError, (e) => new se("Tolgee: Failed to detect language", e), () => l.languageDetector?.getLanguage(Object.assign({ availableLanguages: e }, p())));
	}
	function E(e) {
		e && l.backends.push(e);
	}
	...
	let O = Object.freeze({
		...
		setStoredLanguage(e) {
			return y(s.onError, (e) => new ce("Tolgee: Failed to store language", e), () => l.languageStorage?.setLanguage(e, p()));
		},
		getDevBackend() {
			return l.devBackend;
		},
		...
		formatTranslation(n) {
			...
			try {
				...
				console.error(e);
				let n = D(e) || N, r = t().onFormatError, a = typeof r;
...
var ye = () => {
	...
	return t;
}, L = 0, R = 1, be = 2, xe = class extends Error {
	constructor(e, t, n) {
		let r, i = "";
...
var B = 0, V = 1, H = 2, U = 3, W = 4, Se = /* @__PURE__ */ new Set([
	H,
	V,
	B
]), G = "'", Ce = /* @__PURE__ */ new Set([
	"{",
	"}",
	G
...
function Te(e) {
	...
	for (s = 0; s < e.length; s++) switch (i = e[s], t) {
		case B:
			...
			break;
		case V:
			Ce.has(i) ? (n = n.slice(0, -1) + i, t = H) : (n += i, t = B);
			break;
		case H:
			i === G ? t = B : (n += i, t = H);
			break;
...
(h = console.assert) == null || h.call(console, Me), RegExp(`([${["‌", "‍"].join("")}]{9})+`, "g");
function Ne(e, t) {
	return t ? `${t}: ${e}` : e;
}
...
}, He = 3e3, Ue = 200, We = 0, J = /* @__PURE__ */ new Map(), Ge = !1, Y, X;
...
var qe = (e) => new q("unavailable", `the Tolgee browser extension did not answer ${e} in time`), Je = () => `${Date.now()}-${We++}-${Math.random()}`;
function Ye() {
...
function Xe(e) {
	return Y ||= new Promise((t, n) => {
		let r = () => window.postMessage({ type: ze }, window.origin), i = () => {
			clearInterval(a), Ze(), X = void 0, n(new q("unavailable", "the Tolgee browser extension did not answer"));
		}, a = setInterval(() => {
			if (Date.now() > e) {
				i();
				return;
...
var Z = "__tolgee_", mt = `${Z}apiKey`, ht = `${Z}apiUrl`, gt = `${Z}branch`, _t = `${Z}projectId`, vt = `${Z}session`, yt = "tolgee-in-context-tools.umd.min.js", bt = "@tolgee/in-context-tools", xt = "InContextTools", St = "https://cdn.jsdelivr.net/npm", Ct = null;
function wt(e) {
	return Ct ||= kt(Tt(e)).then(() => window[bt][xt]), Ct;
}
function Tt(e) {
	return Et() || `${St}/@tolgee/web@${e}/dist/${yt}`;
}
function Et() {
	if (ke()) return;
	let e = window.__TOLGEE_IN_CONTEXT_URL__;
	return Ot(e, window.location) ? e : void 0;
}
var Dt = (e) => e === "localhost" || e === "127.0.0.1" || e === "::1" || e === "[::1]";
function Ot(e, t) {
	if (!e || !Dt(t.hostname)) return !1;
	try {
		let n = new URL(e, t.href);
		return n.origin === t.origin || Dt(n.hostname);
	} catch {
		return !1;
	}
}
...
function At() {
	...
	if (i && r) return m(p({}, a), { transport: Qe() });
}
function jt() {
	let e = [];
...
(() => {
	...
	return Mt(e), e.on("running", ({ value: e }) => {
		e && Nt(() => {
			t.update(n()).catch(jt);
		});
	}), At() && (async () => {
...
function Ht() {
	...
	return [(() => {
		...
		return t(r, e(i, {
			each: n,
			children: (e) => (() => {
				var n = Vt(), r = n.firstChild, i = r.firstChild, a = i.nextSibling, o = a.nextSibling.firstChild, s = o.nextSibling, c = s.nextSibling, l = r.nextSibling;
