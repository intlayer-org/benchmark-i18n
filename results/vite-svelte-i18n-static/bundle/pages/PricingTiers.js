...
function PricingTiers($$anchor, $$props) {
	...
	$.each(div, 5, () => tiers, (t) => t.id, ($$anchor, t) => {
		...
		$.if(node, ($$render) => {
			if ($.get(t).periodKey) $$render(consequent);
		});
		$.reset(div_2);
		...
		$.template_effect(($0, $1, $2) => {
			...
			$.set_text(text, $0);
			$.set_text(text_1, $1);
			$.set_class(button, 1, `w-full rounded-md px-4 py-2 text-sm font-medium transition-opacity hover:opacity-90 ${$.get(t).highlight ? "bg-primary text-primary-foreground" : "border border-border text-foreground hover:bg-accent"}`);
			$.set_text(text_4, $2);
		}, [
			() => $_()(`pricing.tiers.${$.get(t).nameKey}`),
			() => $_()(`pricing.tiers.${$.get(t).priceKey}`),
			() => $_()(`pricing.tiers.${$.get(t).ctaKey}`)
		]);
		$.append($$anchor, div_1);
	});
	$.reset(div);
