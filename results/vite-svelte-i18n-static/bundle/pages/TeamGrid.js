...
function TeamGrid($$anchor, $$props) {
	...
	$.init();
	var div = root();
	$.each(div, 5, () => memberNums, (n) => n, ($$anchor, n) => {
		const name = $.derived_safe_equal(() => $_()(`team.grid.member${$.get(n)}Name`));
		var div_1 = root_1();
		...
		$.template_effect(($0, $1, $2) => {
			$.set_text(text, $0);
			$.set_text(text_1, $.get(name));
			$.set_text(text_2, $1);
			$.set_text(text_3, $2);
		}, [
			() => initialsFromName($.get(name)),
			() => $_()(`team.grid.member${$.get(n)}Role`),
			() => $_()(`team.grid.member${$.get(n)}Bio`)
		]);
		$.append($$anchor, div_1);
	});
	$.reset(div);
