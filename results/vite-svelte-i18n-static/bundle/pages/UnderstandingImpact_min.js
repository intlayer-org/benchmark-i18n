...
function y(e, t) {
	var n = t && t.cache ? t.cache : re, r = t && t.serializer ? t.serializer : T;
	...
	for (var t = {}, n = 0, r = e; n < r.length; n++) {
		...
		switch (i.stem) {
			...
			case "integer-width":
				...
				i.options[0].replace(be, function(e, n, r, i, a, o) {
		...
		if (ve.test(i.stem)) {
			...
			i.stem.replace(ve, function(e, n, r, i, a, o) {
			...
			var a = i.options[0];
			a === "w" ? t = g(g({}, t), { trailingZeroDisplay: "stripIfInteger" }) : a && (t = g(g({}, t), Se(a)));
			continue;
		}
		if (ye.test(i.stem)) {
			t = g(g({}, t), Se(i.stem));
...
var ke = RegExp(`^${fe.source}*`), Ae = RegExp(`${fe.source}*\$`);
function P(e, t) {
	return {
		start: e,
		end: t
...
var je = !!String.prototype.startsWith && "_a".startsWith("a", 1), Me = !!String.fromCodePoint, Ne = !!Object.fromEntries, Pe = !!String.prototype.codePointAt, Fe = !!String.prototype.trimStart, Ie = !!String.prototype.trimEnd, Le = Number.isSafeInteger ? Number.isSafeInteger : function(e) {
	return typeof e == "number" && isFinite(e) && Math.floor(e) === e && Math.abs(e) <= 9007199254740991;
}, Re = !0;
try {
	Re = Ge("([^\\p{White_Space}\\p{Pattern_Syntax}]*)", "yu").exec("a")?.[0] === "a";
} catch {
	Re = !1;
}
var ze = je ? function(e, t, n) {
	return e.startsWith(t, n);
} : function(e, t, n) {
	return e.slice(n, n + t.length) === t;
}, Be = Me ? String.fromCodePoint : function() {
	...
	return t;
}, Ve = Ne ? Object.fromEntries : function(e) {
	for (var t = {}, n = 0, r = e; n < r.length; n++) {
		var i = r[n], a = i[0];
		t[a] = i[1];
	}
...
var Ke;
if (Re) {
	var qe = Ge("([^\\p{White_Space}\\p{Pattern_Syntax}]*)", "yu");
	Ke = function(e, t) {
		return qe.lastIndex = t, qe.exec(e)[1] ?? "";
	};
} else Ke = function(e, t) {
	for (var n = [];;) {
		var r = He(e, t);
		if (r === void 0 || Ze(r) || Qe(r)) break;
...
var Je = function() {
	...
	return e.prototype.parse = function() {
		...
		for (var r = []; !this.isEOF();) {
			...
			} else if (i === 60 && !this.ignoreTag && F(this.peek() || 0)) {
		...
		if (this.bumpIf(">")) {
			...
			if (this.bumpIf("</")) {
				if (this.isEOF() || !F(this.char())) return this.error(D.INVALID_TAG, P(o, this.clonePosition()));
				...
				return r === this.parseTagName() ? (this.bumpSpace(), this.bumpIf(">") ? {
					...
					err: null
				} : this.error(D.INVALID_TAG, P(o, this.clonePosition()))) : this.error(D.UNMATCHED_CLOSING_TAG, P(s, this.clonePosition()));
			}
		...
		var e = this.offset();
		for (this.bump(); !this.isEOF() && Xe(this.char());) this.bump();
		...
		return !this.isEOF() && this.char() === 60 && (this.ignoreTag || !Ye(this.peek() || 0)) ? (this.bump(), "<") : null;
		...
		var n = this.char();
		return n === 60 || n === 123 || n === 35 && (t === "plural" || t === "selectordinal") || n === 125 && e > 0 ? null : (this.bump(), Be(n));
	}, e.prototype.parseArgument = function(e, t) {
		var n = this.clonePosition();
		if (this.bump(), this.bumpSpace(), this.isEOF()) return this.error(D.EXPECT_ARGUMENT_CLOSING_BRACE, P(n, this.clonePosition()));
		if (this.char() === 125) return this.bump(), this.error(D.EMPTY_ARGUMENT, P(n, this.clonePosition()));
		var r = this.parseIdentifierIfPossible().value;
		if (!r) return this.error(D.MALFORMED_ARGUMENT, P(n, this.clonePosition()));
		if (this.bumpSpace(), this.isEOF()) return this.error(D.EXPECT_ARGUMENT_CLOSING_BRACE, P(n, this.clonePosition()));
		switch (this.char()) {
			...
			case 44: return this.bump(), this.bumpSpace(), this.isEOF() ? this.error(D.EXPECT_ARGUMENT_CLOSING_BRACE, P(n, this.clonePosition())) : this.parseArgumentOptions(e, t, r, n);
			default: return this.error(D.MALFORMED_ARGUMENT, P(n, this.clonePosition()));
		}
		...
		switch (a) {
			case "": return this.error(D.EXPECT_ARGUMENT_TYPE, P(i, o));
			case "number":
			case "date":
			case "time":
				this.bumpSpace();
				...
				if (this.bumpIf(",")) {
					...
					if (l.err) return l;
					var u = We(l.val);
					if (u.length === 0) return this.error(D.EXPECT_ARGUMENT_STYLE, P(this.clonePosition(), this.clonePosition()));
					s = {
						style: u,
			...
			case "select":
				...
				if (this.bumpSpace(), !this.bumpIf(",")) return this.error(D.EXPECT_SELECT_ARGUMENT_OPTIONS, P(h, g({}, h)));
				this.bumpSpace();
				var _ = this.parseIdentifierIfPossible(), v = 0;
				if (a !== "select" && _.value === "offset") {
					if (!this.bumpIf(":")) return this.error(D.EXPECT_PLURAL_ARGUMENT_OFFSET_VALUE, P(this.clonePosition(), this.clonePosition()));
					this.bumpSpace();
			...
			default: return this.error(D.INVALID_ARGUMENT_TYPE, P(i, o));
		}
	}, e.prototype.tryParseArgumentClose = function(e) {
		return this.isEOF() || this.char() !== 125 ? this.error(D.EXPECT_ARGUMENT_CLOSING_BRACE, P(e, this.clonePosition())) : (this.bump(), {
			val: !0,
			err: null
		});
		...
		for (var e = 0, t = this.clonePosition(); !this.isEOF();) switch (this.char()) {
			case 39:
				this.bump();
				var n = this.clonePosition();
				if (!this.bumpUntil("'")) return this.error(D.UNCLOSED_QUOTE_IN_ARGUMENT_STYLE, P(n, this.clonePosition()));
				this.bump();
				break;
			case 123:
				e += 1, this.bump();
		...
		return {
			val: {
				...
				tokens: n,
				location: t,
				parsedOptions: this.shouldParseSkeletons ? Ee(n) : {}
			},
			err: null
		};
		...
		for (var i, a = !1, o = [], s = /* @__PURE__ */ new Set(), c = r.value, l = r.location;;) {
			...
			if (!this.bumpIf("{")) return this.error(t === "select" ? D.EXPECT_SELECT_ARGUMENT_SELECTOR_FRAGMENT : D.EXPECT_PLURAL_ARGUMENT_SELECTOR_FRAGMENT, P(this.clonePosition(), this.clonePosition()));
		...
		return o.length === 0 ? this.error(t === "select" ? D.EXPECT_SELECT_ARGUMENT_SELECTOR : D.EXPECT_PLURAL_ARGUMENT_SELECTOR, P(this.clonePosition(), this.clonePosition())) : this.requiresOtherClause && !a ? this.error(D.MISSING_OTHER_CLAUSE, P(this.clonePosition(), this.clonePosition())) : {
			val: o,
			err: null
		};
		...
		if (ze(this.message, e, this.offset())) {
...
function $e(e) {
	e.forEach(function(e) {
		if (delete e.location, j(e) || M(e)) for (var t in e.options) delete e.options[t].location, $e(e.options[t].value);
		else A(e) && ue(e.style) || (oe(e) || se(e)) && de(e.style) ? delete e.style.location : le(e) && $e(e.children);
	});
}
function et(e, t) {
	t === void 0 && (t = {}), t = g({
...
function z(e, t, n, r, i, a, o) {
	...
	for (var s = [], c = 0, l = e; c < l.length; c++) {
		...
		if (oe(u)) {
			var p = typeof u.style == "string" ? r.date[u.style] : de(u.style) ? u.style.parsedOptions : void 0;
			s.push({
				type: R.literal,
		...
		if (se(u)) {
			var p = typeof u.style == "string" ? r.time[u.style] : de(u.style) ? u.style.parsedOptions : r.time.medium;
			s.push({
				type: R.literal,
				value: n.getDateTimeFormat(t, p).format(f)
			});
		...
		if (A(u)) {
			var p = typeof u.style == "string" ? r.number[u.style] : ue(u.style) ? u.style.parsedOptions : void 0;
		...
		if (le(u)) {
			...
			if (!at(g)) throw new nt(h, "function", o);
		...
		if (j(u)) {
			...
			if (!v) throw new tt(u.value, f, Object.keys(u.options), o);
...
var ut = function() {
	function e(t, n, r, i) {
		...
		this.formats = st(e.formats, r), this.formatters = i && i.formatters || lt(this.formatterCache);
	}
...
var B = {}, pt = (e, t, n) => n && (t in B || (B[t] = {}), e in B[t] || (B[t][e] = n), n), mt = (e, t) => {
	if (t == null) return;
...
function bt(e, ...t) {
	delete B[e], V.update((n) => (n[e] = dt.default.all([n[e] || {}, ...t]), n));
}
t([V], ([e]) => Object.keys(e)), V.subscribe((e) => ht = e);
var H = {};
function xt(e, t) {
...
function wt(e) {
	return e != null && K(e).some((e) => St(e)?.size);
}
function Tt(e, t) {
...
var U = {};
function Et(e) {
	if (!wt(e)) return e in U ? U[e] : Promise.resolve();
	let t = Ct(e);
...
var J = Lt(It({}, G), { set: (e) => {
...
}, Xt = Y((e, t = q()) => new ut(e, t, W().formats, { ignoreTag: W().ignoreTag })), Zt = (e, t = {}) => {
	var n;
	let r = t;
	...
	return c;
}, Qt = (e, t) => Yt(t).format(e), $t = (e, t) => Jt(t).format(e), en = (e, t) => qt(t).format(e), tn = (e, t = q()) => mt(e, t), nn = t([J, V], () => Zt);
t([J], () => Qt), t([J], () => $t), t([J], () => en), t([J, V], () => tn);
function rn(e) {
...
function on(t, n) {
	...
	var o = an(), s = e.child(o), c = e.only_child(s, !0), l = e.sibling(s, 2), u = e.child(l), d = e.only_child(u, !0), f = e.sibling(u, 2), p = e.only_child(f, !0), m = e.sibling(f, 2), h = e.child(m), g = e.only_child(h, !0), _ = e.sibling(h, 2), v = e.only_child(_, !0), y = e.sibling(_, 2), b = e.only_child(y, !0);
	e.reset(m), e.reset(l);
	var x = e.sibling(l, 2), S = e.child(x), C = e.only_child(S, !0), w = e.sibling(S, 2), ee = e.only_child(w, !0), te = e.sibling(w, 2), T = e.child(te), ne = e.child(T), re = e.only_child(ne, !0), E = e.sibling(ne);
	e.reset(T);
	var D = e.sibling(T, 2), O = e.child(D), k = e.only_child(O, !0), ie = e.sibling(O);
	e.reset(D);
	var ae = e.sibling(D, 2), A = e.child(ae), oe = e.only_child(A, !0), se = e.sibling(A);
	e.reset(ae), e.reset(te), e.reset(x);
	var j = e.sibling(x, 2), M = e.child(j), ce = e.only_child(M, !0), le = e.sibling(M, 2), ue = e.only_child(le, !0);
	e.reset(j), e.reset(o), e.template_effect((t, n, r, i, a, o, s, l, u, f, m, h, _, y, x, S) => {
		e.set_text(c, t), e.set_text(d, n), e.set_text(p, r), e.set_text(g, i), e.set_text(v, a), e.set_text(b, o), e.set_text(C, s), e.set_text(ee, l), e.set_text(re, u), e.set_text(E, ` ${f ?? ""}`), e.set_text(k, m), e.set_text(ie, ` ${h ?? ""}`), e.set_text(oe, _), e.set_text(se, ` ${y ?? ""}`), e.set_text(ce, x), e.set_text(ue, S);
	}, [
