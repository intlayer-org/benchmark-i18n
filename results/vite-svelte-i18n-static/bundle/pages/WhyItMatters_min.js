...
function Ae(e) {
	for (var t = {}, n = 0, r = e; n < r.length; n++) {
		...
		if (Se.test(i.stem)) {
			...
			var a = i.options[0];
			a === "w" ? t = g(g({}, t), { trailingZeroDisplay: "stripIfInteger" }) : a && (t = g(g({}, t), Ee(a)));
			continue;
		}
		if (Ce.test(i.stem)) {
			t = g(g({}, t), Ee(i.stem));
...
try {
	j = qe("([^\\p{White_Space}\\p{Pattern_Syntax}]*)", "yu").exec("a")?.[0] === "a";
} catch {
	j = !1;
}
...
}, M = Ie ? String.fromCodePoint : function() {
	...
	return e.prototype.parse = function() {
		...
		for (var r = []; !this.isEOF();) {
			...
			if (i === 123) {
				...
				this.bump(), r.push({
					type: E.pound,
					location: A(o, this.clonePosition())
				});
		...
		if (this.bumpIf(">")) {
			...
			if (this.bumpIf("</")) {
				if (this.isEOF() || !P(this.char())) return this.error(T.INVALID_TAG, A(o, this.clonePosition()));
		...
		return !this.isEOF() && this.char() === 60 && (this.ignoreTag || !Xe(this.peek() || 0)) ? (this.bump(), "<") : null;
		...
		var n = this.char();
		return n === 60 || n === 123 || n === 35 && (t === "plural" || t === "selectordinal") || n === 125 && e > 0 ? null : (this.bump(), M(n));
	}, e.prototype.parseArgument = function(e, t) {
		var n = this.clonePosition();
		if (this.bump(), this.bumpSpace(), this.isEOF()) return this.error(T.EXPECT_ARGUMENT_CLOSING_BRACE, A(n, this.clonePosition()));
		if (this.char() === 125) return this.bump(), this.error(T.EMPTY_ARGUMENT, A(n, this.clonePosition()));
		var r = this.parseIdentifierIfPossible().value;
		if (!r) return this.error(T.MALFORMED_ARGUMENT, A(n, this.clonePosition()));
		if (this.bumpSpace(), this.isEOF()) return this.error(T.EXPECT_ARGUMENT_CLOSING_BRACE, A(n, this.clonePosition()));
		switch (this.char()) {
			...
			case 44: return this.bump(), this.bumpSpace(), this.isEOF() ? this.error(T.EXPECT_ARGUMENT_CLOSING_BRACE, A(n, this.clonePosition())) : this.parseArgumentOptions(e, t, r, n);
			default: return this.error(T.MALFORMED_ARGUMENT, A(n, this.clonePosition()));
		}
	}, e.prototype.parseIdentifierIfPossible = function() {
		var e = this.clonePosition(), t = this.offset(), n = N(this.message, t), r = t + n.length;
		...
		switch (a) {
			case "": return this.error(T.EXPECT_ARGUMENT_TYPE, A(i, o));
			case "number":
			case "date":
			case "time":
				this.bumpSpace();
				...
				if (this.bumpIf(",")) {
					...
					if (l.err) return l;
					var u = Ke(l.val);
					if (u.length === 0) return this.error(T.EXPECT_ARGUMENT_STYLE, A(this.clonePosition(), this.clonePosition()));
					s = {
						style: u,
						styleLocation: A(c, this.clonePosition())
					};
				}
				var d = this.tryParseArgumentClose(r);
				if (d.err) return d;
				var f = A(r, this.clonePosition());
				if (s && He(s?.style, "::", 0)) {
			...
			case "select":
				...
				if (this.bumpSpace(), !this.bumpIf(",")) return this.error(T.EXPECT_SELECT_ARGUMENT_OPTIONS, A(h, g({}, h)));
				this.bumpSpace();
			...
			default: return this.error(T.INVALID_ARGUMENT_TYPE, A(i, o));
		}
	}, e.prototype.tryParseArgumentClose = function(e) {
		return this.isEOF() || this.char() !== 125 ? this.error(T.EXPECT_ARGUMENT_CLOSING_BRACE, A(e, this.clonePosition())) : (this.bump(), {
			val: !0,
			err: null
		});
		...
		for (var e = 0, t = this.clonePosition(); !this.isEOF();) switch (this.char()) {
			case 39:
				this.bump();
				var n = this.clonePosition();
				if (!this.bumpUntil("'")) return this.error(T.UNCLOSED_QUOTE_IN_ARGUMENT_STYLE, A(n, this.clonePosition()));
				this.bump();
				break;
			case 123:
				e += 1, this.bump();
		...
		for (var i, a = !1, o = [], s = /* @__PURE__ */ new Set(), c = r.value, l = r.location;;) {
			if (c.length === 0) {
				...
				if (t !== "select" && this.bumpIf("=")) {
					...
					if (d.err) return d;
					l = A(u, this.clonePosition()), c = this.message.slice(u.offset, this.offset());
				} else break;
			}
			...
			if (!this.bumpIf("{")) return this.error(t === "select" ? T.EXPECT_SELECT_ARGUMENT_SELECTOR_FRAGMENT : T.EXPECT_PLURAL_ARGUMENT_SELECTOR_FRAGMENT, A(this.clonePosition(), this.clonePosition()));
		...
		return o.length === 0 ? this.error(t === "select" ? T.EXPECT_SELECT_ARGUMENT_SELECTOR : T.EXPECT_PLURAL_ARGUMENT_SELECTOR, A(this.clonePosition(), this.clonePosition())) : this.requiresOtherClause && !a ? this.error(T.MISSING_OTHER_CLAUSE, A(this.clonePosition(), this.clonePosition())) : {
			val: o,
			err: null
		};
...
function F(e) {
	e.forEach(function(e) {
		if (delete e.location, de(e) || fe(e)) for (var t in e.options) delete e.options[t].location, F(e.options[t].value);
		else ce(e) && he(e.style) || (le(e) || ue(e)) && O(e.style) ? delete e.style.location : me(e) && F(e.children);
	});
}
function et(e, t) {
	t === void 0 && (t = {}), t = g({
...
function z(e, t, n, r, i, a, o) {
	...
	for (var s = [], c = 0, l = e; c < l.length; c++) {
		...
		if (le(u)) {
			var p = typeof u.style == "string" ? r.date[u.style] : O(u.style) ? u.style.parsedOptions : void 0;
			s.push({
				type: R.literal,
		...
		if (ue(u)) {
			var p = typeof u.style == "string" ? r.time[u.style] : O(u.style) ? u.style.parsedOptions : r.time.medium;
			s.push({
				type: R.literal,
				value: n.getDateTimeFormat(t, p).format(f)
			});
...
var lt = function() {
	function e(t, n, r, i) {
		...
		this.formats = st(e.formats, r), this.formatters = i && i.formatters || ct(this.formatterCache);
	}
...
var an = e.from_html("<section class=\"mb-16\"><h2 class=\"mb-6 text-2xl font-bold text-foreground\"> </h2> <div class=\"grid gap-6 md:grid-cols-3\"><div class=\"rounded-lg border border-border bg-card p-6\"><h3 class=\"mb-2 text-lg font-semibold text-foreground\"> </h3> <p class=\"text-sm text-muted-foreground\"> </p></div> <div class=\"rounded-lg border border-border bg-card p-6\"><h3 class=\"mb-2 text-lg font-semibold text-foreground\"> </h3> <p class=\"text-sm text-muted-foreground\"> </p></div> <div class=\"rounded-lg border border-border bg-card p-6\"><h3 class=\"mb-2 text-lg font-semibold text-foreground\"> </h3> <p class=\"text-sm text-muted-foreground\"> </p></div></div></section>");
function on(t, n) {
	e.push(n, !1);
	let r = () => e.store_get(nn, "$_", i), [i, a] = e.setup_stores();
	rn("WhyItMatters"), e.init();
	var o = an(), s = e.child(o), c = e.only_child(s, !0), l = e.sibling(s, 2), u = e.child(l), d = e.child(u), f = e.only_child(d, !0), p = e.sibling(d, 2), m = e.only_child(p, !0);
	e.reset(u);
	var h = e.sibling(u, 2), g = e.child(h), _ = e.only_child(g, !0), v = e.sibling(g, 2), y = e.only_child(v, !0);
	e.reset(h);
	var b = e.sibling(h, 2), x = e.child(b), S = e.only_child(x, !0), C = e.sibling(x, 2), ee = e.only_child(C, !0);
	e.reset(b), e.reset(l), e.reset(o), e.template_effect((t, n, r, i, a, o, s) => {
		e.set_text(c, t), e.set_text(f, n), e.set_text(m, r), e.set_text(_, i), e.set_text(y, a), e.set_text(S, o), e.set_text(ee, s);
	}, [
		() => r()("home.whyItMatters.title"),
		() => r()("home.whyItMatters.bundleSizeTitle"),
		() => r()("home.whyItMatters.bundleSizeDesc"),
		() => r()("home.whyItMatters.renderingTitle"),
		() => r()("home.whyItMatters.renderingDesc"),
		() => r()("home.whyItMatters.dynamicLoadingTitle"),
		() => r()("home.whyItMatters.dynamicLoadingDesc")
