...
function CareersBenefits($$anchor, $$props) {
	...
	const benefits = [
		{
			label: "Remote-first",
			value: get(content$1).workFromAnywhereInThe
		},
		{
			label: get(content$1).competitivePay,
			value: get(content$1).topOfMarketCompensation
		},
		{
			label: get(content$1).openSourceTime,
			value: get(content$1).x20TimeForOssContributions
		}
	];
	$.init();
	var div = root();
