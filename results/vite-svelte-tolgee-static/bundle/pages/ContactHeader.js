...
var GlobalContextPlugin = () => (tolgee) => {
	return tolgee;
};
var tolgee = Tolgee().use(FormatSimple()).use(GlobalContextPlugin()).init({
	language: "en",
	staticData: {
		en: {
			shared: {
...
function createTranslateFn() {
	return (key, options) => {
		if (options?.values) return tolgee.t(key, options.values);
		return tolgee.t(key, options);
	};
}
var tStore = writable(createTranslateFn());
tolgee.on("update", () => {
	tStore.set(createTranslateFn());
});
tolgee.on("language", (lang) => {
	if (typeof document !== "undefined" && lang?.value) document.documentElement.lang = lang.value;
	tStore.set(createTranslateFn());
});
var _ = tStore;
...
function MockBanner($$anchor, $$props) {
	...
	$.template_effect(($0) => $.set_text(text, $0), [() => $_()("mockBanner")]);
	$.append($$anchor, div);
	$.pop();
	$$cleanup();
}
var root = $.from_html(`<!> <h1 class="mb-2 text-3xl font-bold text-foreground"> </h1> <p class="mb-8 text-muted-foreground"> <a href="mailto:contact@intlayer.org" class="text-primary hover:underline"> </a>.</p>`, 1);
function ContactHeader($$anchor, $$props) {
	...
	$.template_effect(($0, $1, $2) => {
		...
		() => $_()("contact.header.title"),
		() => $_()("contact.header.description"),
		() => $_()("shared.contactEmail")
	]);
