...
function oe(e, t, n, r, i, a, o, s) {
	...
	function T() {
		return g(s.onError, (e) => new I("Tolgee: Failed to load language", e), () => l.languageStorage?.getLanguage(p()));
	}
	function E() {
		if (!l.languageDetector) return;
		let e = n();
		return g(s.onError, (e) => new te("Tolgee: Failed to detect language", e), () => l.languageDetector?.getLanguage(Object.assign({ availableLanguages: e }, p())));
	}
	function D(e) {
		e && l.backends.push(e);
	}
	...
	let A = Object.freeze({
		...
		setStoredLanguage(e) {
			return g(s.onError, (e) => new I("Tolgee: Failed to store language", e), () => l.languageStorage?.setLanguage(e, p()));
		},
		getDevBackend() {
			return l.devBackend;
		},
		...
		formatTranslation(n) {
			...
			try {
				...
				console.error(e);
				let n = w(e) || R, r = t().onFormatError, a = typeof r;
...
function de({ options: e }) {
	let t = P(), n = se(!1, () => o.isFetching(), t.onFetchingChange.emit), r = se(!1, () => D.isLoading(), t.onLoadingChange.emit), i = ce(t.onLanguageChange, t.onPendingLanguageChange, t.onRunningChange), a = oe(i.getLanguage, i.getInitialOptions, i.getAvailableLanguages, u, x, C, f, t), o = ne(t, a.getBackendRecord, a.getBackendDevRecord, i.withDefaultNs, i.isInitialLoading, n, r);
	e && p(e);
	let s;
	t.onUpdate.listen(() => {
...
var pe = () => {
	let e = {
		...
		init(t) {
			let n = fe(z(e.options, t));
			return e.plugins.forEach(n.addPlugin), n;
		}
	});
	return t;
}, me = 0, B = 1, he = 2, ge = class extends Error {
	constructor(e, t, n) {
		let r, i = "";
		e === me ? r = "Empty parameter" : e === B ? (r = "Unexpected character", i = "Did you forget to use FormatIcu to render ICU message syntax?") : r = "Unexpected end", super(`Tolgee parser: ${r} at ${t} in "${n}"` + (i ? "\n" + i : "")), this.code = e, this.index = t;
	}
};
function _e(e) {
	return /\s/.test(e);
}
var V = 0, H = 1, U = 2, W = 3, G = 4, ve = /* @__PURE__ */ new Set([
	U,
	H,
	V
]), K = "'", ye = /* @__PURE__ */ new Set([
	"{",
	"}",
	K
]), be = (e) => /[0-9a-zA-Z_]/.test(e);
function xe(e) {
	...
	for (s = 0; s < e.length; s++) switch (i = e[s], t) {
		case V:
			...
			break;
		case H:
			ye.has(i) ? (n = n.slice(0, -1) + i, t = U) : (n += i, t = V);
			break;
		case U:
			i === K ? t = V : (n += i, t = U);
			break;
		case W:
			i === "}" ? (d(), t = V) : _e(i) ? r !== "" && (d(), t = G) : (u(), t = W);
			break;
		case G: i == "}" ? t = V : _e(i) ? t = G : c(B);
	}
	return ve.has(t) || c(he), l(), [a, o];
}
function Se(e, t) {
	let [n, r] = xe(e), i = [n[0]];
...
function Ce() {
	return { format: ({ translation: e, params: t }) => Se(e, t) };
}
var we = () => (e, t) => (t.setFinalFormatter(Ce()), e);
function Te() {
	return globalThis.window?.document?.createElement === void 0;
}
var Ee = typeof globalThis < "u" ? globalThis : typeof window < "u" ? window : typeof global < "u" ? global : typeof self < "u" ? self : {}, De = {};
(function(e) {
	function t(e, t) {
...
(f = console.assert) == null || f.call(console, Oe), RegExp(`([${["‌", "‍"].join("")}]{9})+`, "g");
function ke(e, t) {
	return t ? `${t}: ${e}` : e;
}
...
}, Re = 3e3, ze = 200, Be = 0, Y = /* @__PURE__ */ new Map(), Ve = !1, X, Z;
async function He({ type: e, replyType: t, payload: n, timeoutMs: r = Me, progressType: i, onProgress: a }) {
	Ge();
	let o = Date.now() + r;
	await Ke(Math.min(o, Date.now() + Re));
	let s = o - Date.now();
	if (s <= 0) throw Ue(e);
	let c = We();
	return new Promise((r, o) => {
		let l = setTimeout(() => {
			Y.delete(c), qe(), o(Ue(e));
		}, s);
		Y.set(c, {
			replyType: t,
			progressType: i,
...
var Ue = (e) => new J("unavailable", `the Tolgee browser extension did not answer ${e} in time`), We = () => `${Date.now()}-${Be++}-${Math.random()}`;
function Ge() {
	Ve || (Ve = !0, window.addEventListener("message", (e) => {
		var t;
...
function Ke(e) {
	return X ||= new Promise((t, n) => {
		let r = () => window.postMessage({ type: Fe }, window.origin), i = () => {
			clearInterval(a), qe(), Z = void 0, n(new J("unavailable", "the Tolgee browser extension did not answer"));
		}, a = setInterval(() => {
			if (Date.now() > e) {
				i();
				return;
...
var $e = "ABCDEFGHIJKLMNOPQRSTUVWXYZ234567";
function et(e) {
	let t = $e.indexOf(e);
...
function nt(e) {
	...
	for (let o = 0; o < t; o++) r = r << 5 | et(e[o]), n += 5, n >= 8 && (a[i++] = r >>> n - 8 & 255, n -= 8);
	return tt(a.buffer);
}
function rt(e) {
	if (e) try {
...
function at(e) {
	...
	return r ? {
		...
		hasCredential: !!t,
		projectId: rt(t) ?? n,
		requiresExplicitProject: it(t) === "tgpat"
	};
}
function ot(e) {
...
var Q = "__tolgee_", ut = `${Q}apiKey`, dt = `${Q}apiUrl`, ft = `${Q}branch`, pt = `${Q}projectId`, mt = `${Q}session`, ht = "tolgee-in-context-tools.umd.min.js", gt = "@tolgee/in-context-tools", _t = "InContextTools", vt = "https://cdn.jsdelivr.net/npm", yt = null;
function bt(e) {
	return yt ||= Tt(xt(e)).then(() => window[gt][_t]), yt;
}
function xt(e) {
	return St() || `${vt}/@tolgee/web@${e}/dist/${ht}`;
}
function St() {
	if (Te()) return;
	let e = window.__TOLGEE_IN_CONTEXT_URL__;
	return wt(e, window.location) ? e : void 0;
}
var Ct = (e) => e === "localhost" || e === "127.0.0.1" || e === "::1" || e === "[::1]";
function wt(e, t) {
	if (!e || !Ct(t.hostname)) return !1;
	try {
		let n = new URL(e, t.href);
		return n.origin === t.origin || Ct(n.hostname);
	} catch {
		return !1;
	}
}
...
function Et() {
	let e = sessionStorage.getItem(ut) || void 0, t = sessionStorage.getItem(dt) || void 0, n = sessionStorage.getItem(ft) || void 0, r = sessionStorage.getItem(pt) || void 0, i = Le(sessionStorage.getItem(mt));
	if (!t) return;
	let a = u(u({ apiUrl: t }, r === void 0 ? {} : { projectId: r }), n === void 0 ? {} : { branch: n });
	if (e) return d(u({}, a), { apiKey: e });
	if (i && r) return d(u({}, a), { transport: Je() });
}
function Dt() {
	let e = [];
...
function Ot(e) {
	if (!e.isDev()) return;
	let { requiresExplicitProject: t, projectId: n } = at(e.getInitialOptions());
...
(() => {
	...
	return Ot(e), e.on("running", ({ value: e }) => {
		e && kt(() => {
			t.update(n()).catch(Dt);
		});
	}), Et() && (async () => {
		let e = await bt("7.2.1");
		return (t) => {
			let n = Et();
...
$.on("update", () => {
	...
	typeof document < "u" && e?.value && (document.documentElement.lang = e.value), Nt.set(Mt());
});
var Pt = Nt;
function Ft(e) {
...
var It = e.from_html("<section class=\"mx-auto mb-16 max-w-3xl space-y-6\"><h2 class=\"text-2xl font-bold text-foreground\"> </h2> <div class=\"rounded-lg border border-border bg-card p-6\"><h3 class=\"mb-2 text-lg font-semibold text-foreground\"> </h3> <p class=\"text-sm text-muted-foreground\"> </p> <ul class=\"mt-3 list-disc space-y-2 pl-5 text-sm text-muted-foreground\"><li> </li> <li> </li> <li> </li></ul></div> <div class=\"rounded-lg border border-border bg-card p-6\"><h3 class=\"mb-2 text-lg font-semibold text-foreground\"> </h3> <p class=\"text-sm text-muted-foreground\"> </p> <ul class=\"mt-3 list-disc space-y-2 pl-5 text-sm text-muted-foreground\"><li><strong class=\"text-foreground\"> </strong> </li> <li><strong class=\"text-foreground\"> </strong> </li> <li><strong class=\"text-foreground\"> </strong> </li></ul></div> <div class=\"rounded-lg border border-border bg-card p-6\"><h3 class=\"mb-2 text-lg font-semibold text-foreground\"> </h3> <p class=\"text-sm text-muted-foreground\"> </p></div></section>");
function Lt(t, n) {
	e.push(n, !1);
	let r = () => e.store_get(Pt, "$_", i), [i, a] = e.setup_stores();
	Ft("UnderstandingImpact"), e.init();
	var o = It(), s = e.child(o), c = e.only_child(s, !0), l = e.sibling(s, 2), u = e.child(l), d = e.only_child(u, !0), f = e.sibling(u, 2), p = e.only_child(f, !0), m = e.sibling(f, 2), h = e.child(m), g = e.only_child(h, !0), _ = e.sibling(h, 2), v = e.only_child(_, !0), y = e.sibling(_, 2), b = e.only_child(y, !0);
	e.reset(m), e.reset(l);
	var x = e.sibling(l, 2), S = e.child(x), C = e.only_child(S, !0), w = e.sibling(S, 2), T = e.only_child(w, !0), E = e.sibling(w, 2), D = e.child(E), O = e.child(D), k = e.only_child(O, !0), A = e.sibling(O);
	e.reset(D);
	var j = e.sibling(D, 2), M = e.child(j), N = e.only_child(M, !0), ee = e.sibling(M);
	e.reset(j);
	var P = e.sibling(j, 2), F = e.child(P), te = e.only_child(F, !0), I = e.sibling(F);
	e.reset(P), e.reset(E), e.reset(x);
	var ne = e.sibling(x, 2), L = e.child(ne), re = e.only_child(L, !0), R = e.sibling(L, 2), ie = e.only_child(R, !0);
	e.reset(ne), e.reset(o), e.template_effect((t, n, r, i, a, o, s, l, u, f, m, h, _, y, x, S) => {
		e.set_text(c, t), e.set_text(d, n), e.set_text(p, r), e.set_text(g, i), e.set_text(v, a), e.set_text(b, o), e.set_text(C, s), e.set_text(T, l), e.set_text(k, u), e.set_text(A, ` ${f ?? ""}`), e.set_text(N, m), e.set_text(ee, ` ${h ?? ""}`), e.set_text(te, _), e.set_text(I, ` ${y ?? ""}`), e.set_text(re, x), e.set_text(ie, S);
	}, [
