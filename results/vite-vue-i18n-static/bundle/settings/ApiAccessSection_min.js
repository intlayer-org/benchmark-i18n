...
process.env.NODE_ENV;
var Li = { class: "rounded-lg border border-border bg-card p-6" }, Ri = { class: "mb-4 text-lg font-semibold text-foreground" }, zi = {
	for: "api-key",
	class: "mb-1 block text-sm font-medium text-foreground"
}, Bi = { class: "flex gap-2" }, Vi = {
	type: "button",
	class: "rounded-md border border-border px-4 py-2 text-sm font-medium text-foreground transition-colors hover:bg-accent"
}, Hi = { class: "mt-1 text-xs text-muted-foreground" }, Ui = s({
	__name: "ApiAccessSection",
	setup(e) {
		let { t } = Ti();
		return (e, n) => (_(), i("section", Li, [a("h2", Ri, x(S(t)("settings.apiAccess.title")), 1), a("div", null, [
			a("label", zi, x(S(t)("settings.apiAccess.apiKey")), 1),
			a("div", Bi, [n[0] ||= a("input", {
				id: "api-key",
				readonly: "",
			...
			}, null, -1), a("button", Vi, x(S(t)("settings.apiAccess.copy")), 1)]),
			a("p", Hi, x(S(t)("settings.apiAccess.description")), 1)
		])]));
	}
}), Wi = wi({
	legacy: !1,
	...
	setup(e) {
		...
		return t && !t.config.globalProperties.$i18n && t.use(Wi), (e, t) => y(e.$slots, "default");
	}
}), Ki = { render() {
	return d(Gi, {}, { default: () => d(Ui) });
} };
