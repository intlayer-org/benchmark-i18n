...
}), kt = { class: "rounded-lg border border-border bg-card p-6" }, At = { class: "mb-4 text-lg font-semibold text-foreground" }, jt = {
	for: "api-key",
	class: "mb-1 block text-sm font-medium text-foreground"
}, Mt = { class: "flex gap-2" }, Nt = {
	type: "button",
	class: "rounded-md border border-border px-4 py-2 text-sm font-medium text-foreground transition-colors hover:bg-accent"
}, Pt = { class: "mt-1 text-xs text-muted-foreground" }, Ft = r({
	__name: "ApiAccessSection",
	setup(e) {
		let { t: r } = Ot();
		return (e, i) => (s(), t("section", kt, [n("h2", At, d(f(r)("settings.apiAccess.title")), 1), n("div", null, [
			n("label", jt, d(f(r)("settings.apiAccess.apiKey")), 1),
			n("div", Mt, [i[0] ||= n("input", {
				id: "api-key",
				readonly: "",
			...
			}, null, -1), n("button", Nt, d(f(r)("settings.apiAccess.copy")), 1)]),
			n("p", Pt, d(f(r)("settings.apiAccess.description")), 1)
		])]));
	}
}), It = Dt({
	legacy: !1,
