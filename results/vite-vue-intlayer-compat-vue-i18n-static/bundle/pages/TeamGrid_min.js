...
var p = { class: "grid gap-6 md:grid-cols-2 lg:grid-cols-3" }, m = { class: "mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-accent text-lg font-bold text-accent-foreground" }, ee = { class: "text-base font-semibold text-foreground" }, te = { class: "mb-2 text-xs font-medium text-primary" }, ne = { class: "text-sm text-muted-foreground" }, re = i({
	__name: "TeamGrid",
	setup(t) {
		let i = [
			{
		...
		return (t, o) => (s(), n("div", p, [(s(), n(e, null, u(i, (e) => r("div", {
			key: e.name,
			class: "rounded-lg border border-border bg-card p-6 text-center"
		}, [
			r("div", m, f(a(e.name)), 1),
			r("h3", ee, f(e.name), 1),
			r("p", te, f(e.role), 1),
			r("p", ne, f(e.bio), 1)
		])), 64))]));
