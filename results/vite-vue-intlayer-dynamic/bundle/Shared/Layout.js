...
var v = (v, y$1) => {
	let b$1 = inject(a$1), x$2 = isRef(b$1?.locale) ? b$1.locale : ref(b$1?.locale ?? internationalization.defaultLocale), S = computed(() => {
		return {
			selector: void 0,
...
var Footer_default = defineComponent({
	...
	setup(__props) {
		...
		const { description, resources, contact: contactLabel, github, methodology, contributing, footerText, appName, contactEmail } = v("footer");
...
var ThemeToggle_default = defineComponent({
	__name: "ThemeToggle",
	setup(__props) {
		const { auto, dark, light, ariaLabelAuto, ariaLabelLight, ariaLabelDark } = v("theme-toggle");
		const mode = ref("auto");
...
var Header_default = defineComponent({
	...
	setup(__props) {
		...
		const { home, methodology, mockPages: mockPagesLabel, products, pricing, team, blog, careers, faq, contact, settings, appName, goToGithub } = v("header");
