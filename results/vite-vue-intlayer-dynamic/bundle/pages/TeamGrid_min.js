...
var H = (e) => F, U = F, ve = (e, t = !0) => [
	...
	return f([]);
}, je = { class: "grid gap-6 md:grid-cols-2 lg:grid-cols-3" }, Me = { class: "mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-accent text-lg font-bold text-accent-foreground" }, Ne = { class: "text-base font-semibold text-foreground" }, Pe = { class: "mb-2 text-xs font-medium text-primary" }, Fe = { class: "text-sm text-muted-foreground" }, Ie = i({
	__name: "TeamGrid",
	setup(t) {
		let { members: i } = Ae("team-grid"), a = (e) => e.split(" ").map((e) => e[0]).join("");
		return (t, o) => (l(), n("div", je, [(l(!0), n(e, null, d(te(i), (e) => (l(), n("div", {
			key: e.name,
			class: "rounded-lg border border-border bg-card p-6 text-center"
		}, [
			r("div", Me, f(a(e.name)), 1),
			r("h3", Ne, f(e.name), 1),
			r("p", Pe, f(e.role), 1),
			r("p", Fe, f(e.bio), 1)
		]))), 128))]));
