...
var BlogHeader_default = defineComponent({
	__name: "BlogHeader",
	setup(__props) {
		const content = b(blog_header_default);
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock(Fragment, null, [
				createVNode(MockBanner_default),
				createElementVNode("h1", _hoisted_1, toDisplayString(unref(content).blog), 1),
				createElementVNode("p", _hoisted_2, toDisplayString(unref(content).insightsTutorialsAndAnalysisFrom), 1)
			], 64);
		};
	}
});
