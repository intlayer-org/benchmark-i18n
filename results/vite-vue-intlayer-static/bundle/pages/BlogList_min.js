...
var te = {
	...
	return f([]);
}, Te = { class: "grid gap-6 md:grid-cols-2" }, Ee = { class: "mb-3 flex items-center gap-3" }, De = { class: "rounded-full bg-accent px-3 py-0.5 text-xs font-medium text-accent-foreground" }, Oe = { class: "text-xs text-muted-foreground" }, ke = { class: "mb-2 text-lg font-semibold text-foreground" }, Ae = { class: "mb-4 text-sm text-muted-foreground" }, je = {
	type: "button",
	...
	setup(t) {
		let { b: i, a } = we(te);
		return (t, o) => (u(), n("div", Te, [(u(!0), n(e, null, f(h(a), (e) => (u(), n("article", {
			key: e.title,
			class: "rounded-lg border border-border bg-card p-6"
		}, [
			r("div", Ee, [r("span", De, p(e.category), 1), r("span", Oe, p(e.date), 1)]),
			r("h2", ke, p(e.title), 1),
			r("p", Ae, p(e.excerpt), 1),
			r("button", je, p(h(i)), 1)
		]))), 128))]));
	}
});
