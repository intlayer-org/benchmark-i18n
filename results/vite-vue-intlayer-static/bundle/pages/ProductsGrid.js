...
var ProductsGrid_default = defineComponent({
	__name: "ProductsGrid",
	setup(__props) {
		const { a: learnMore, b: products } = S(products_grid_default);
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", _hoisted_1, [(openBlock(true), createElementBlock(Fragment, null, renderList(unref(products), (p) => {
				return openBlock(), createElementBlock("div", {
					key: p.name,
					class: "flex flex-col justify-between rounded-lg border border-border bg-card p-6"
				}, [createElementVNode("div", null, [createElementVNode("h3", _hoisted_2, toDisplayString(p.name), 1), createElementVNode("p", _hoisted_3, toDisplayString(p.desc), 1)]), createElementVNode("div", _hoisted_4, [createElementVNode("span", _hoisted_5, toDisplayString(p.price), 1), createElementVNode("button", _hoisted_6, toDisplayString(unref(learnMore)), 1)])]);
			}), 128))]);
		};
	}
});
