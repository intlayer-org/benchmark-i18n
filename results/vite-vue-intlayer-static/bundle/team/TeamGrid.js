...
var TeamGrid_default = defineComponent({
	__name: "TeamGrid",
	setup(__props) {
		const { members } = v("team-grid");
		const getInitials = (name) => name.split(" ").map((n) => n[0]).join("");
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", _hoisted_1, [(openBlock(true), createElementBlock(Fragment, null, renderList(unref(members), (m) => {
