import { computed as e, createElementBlock as t, createElementVNode as n, defineComponent as r, getCurrentInstance as i, h as a, inject as o, onBeforeMount as s, onMounted as c, onUnmounted as l, openBlock as u, ref as d, renderSlot as f, toDisplayString as p, unref as m, watch as h } from "vue";
var g = Object.defineProperty, _ = Object.defineProperties, v = Object.getOwnPropertyDescriptors, y = Object.getOwnPropertySymbols, b = Object.prototype.hasOwnProperty, x = Object.prototype.propertyIsEnumerable, S = (e, t, n) => t in e ? g(e, t, {
	enumerable: !0,
	configurable: !0,
	writable: !0,
	value: n
...
}, N = (e) => Object.fromEntries(le(e).entries()), P = (e) => {
	let [t, ...n] = e.split(":");
	return {
		language: t,
		namespace: n.join(":") || ""
...
function ve(e, t) {
	...
	return n.apiUrl = M(n.apiUrl), e?.fetch && (n.fetch = ce(e.fetch)), {
		initialOptions: n,
...
function ye(e, t, n, r, i, a, o, s) {
	...
	function S() {
		return O(s.onError, (e) => new pe("Tolgee: Failed to load language", e), () => l.languageStorage?.getLanguage(p()));
	}
	function C() {
	...
	let E = Object.freeze({
		...
		setStoredLanguage(e) {
			return O(s.onError, (e) => new pe("Tolgee: Failed to store language", e), () => l.languageStorage?.setLanguage(e, p()));
		},
		getDevBackend() {
			return l.devBackend;
		},
...
function xe(e, t, n) {
	let r = ve(), i, a = Object.freeze({
		...
		getFallbackLangs(e) {
			...
			return t ? j([t, ...re(t, r.initialOptions.fallbackLanguage)]) : [];
		},
		getFallbackNs() {
...
var Te = () => {
	...
	return t;
}, Ee = 0, V = 1, De = 2, Oe = class extends Error {
	constructor(e, t, n) {
		let r, i = "";
		e === Ee ? r = "Empty parameter" : e === V ? (r = "Unexpected character", i = "Did you forget to use FormatIcu to render ICU message syntax?") : r = "Unexpected end", super(`Tolgee parser: ${r} at ${t} in "${n}"` + (i ? "\n" + i : "")), this.code = e, this.index = t;
	}
};
function ke(e) {
	return /\s/.test(e);
}
var H = 0, U = 1, W = 2, G = 3, K = 4, Ae = /* @__PURE__ */ new Set([
	W,
	U,
	H
]), q = "'", je = /* @__PURE__ */ new Set([
	"{",
	"}",
	q
...
function Ne(e) {
	...
	for (s = 0; s < e.length; s++) switch (i = e[s], t) {
		case H:
			...
			break;
		case U:
			je.has(i) ? (n = n.slice(0, -1) + i, t = W) : (n += i, t = H);
			break;
		case W:
			i === q ? t = H : (n += i, t = W);
			break;
		case G:
			i === "}" ? (d(), t = H) : ke(i) ? r !== "" && (d(), t = K) : (u(), t = G);
			break;
		case K: i == "}" ? t = H : ke(i) ? t = K : c(V);
	}
...
(T = console.assert) == null || T.call(console, Be), RegExp(`([${["‌", "‍"].join("")}]{9})+`, "g");
function Ve(e, t) {
	return t ? `${t}: ${e}` : e;
}
...
}, Xe = 3e3, Ze = 200, Qe = 0, X = /* @__PURE__ */ new Map(), $e = !1, Z, Q;
...
var tt = (e) => new Y("unavailable", `the Tolgee browser extension did not answer ${e} in time`), nt = () => `${Date.now()}-${Qe++}-${Math.random()}`;
function rt() {
...
function it(e) {
	return Z ||= new Promise((t, n) => {
		let r = () => window.postMessage({ type: qe }, window.origin), i = () => {
			clearInterval(a), at(), Q = void 0, n(new Y("unavailable", "the Tolgee browser extension did not answer"));
		}, a = setInterval(() => {
			if (Date.now() > e) {
				i();
				return;
...
var $ = "__tolgee_", St = `${$}apiKey`, Ct = `${$}apiUrl`, wt = `${$}branch`, Tt = `${$}projectId`, Et = `${$}session`, Dt = "tolgee-in-context-tools.umd.min.js", Ot = "@tolgee/in-context-tools", kt = "InContextTools", At = "https://cdn.jsdelivr.net/npm", jt = null;
function Mt(e) {
...
function Rt() {
	...
	if (e) return w(C({}, a), { apiKey: e });
	if (i && r) return w(C({}, a), { transport: ot() });
}
function zt() {
	let e = [];
	...
	setup(t) {
		...
		if (n.value.isInitialRender && t.ssr) {
			...
			if (r.value.setEmitterActive(!1), r.value.addStaticData(e.staticData), r.value.changeLanguage(e.language), r.value.setEmitterActive(!0), !r.value.isLoaded()) {
				let t = r.value.getRequiredDescriptors(e.language).map((e) => F(e)).filter((t) => !e.staticData?.[t]);
...
var Wt = (t) => {
	let n = ne(t), r = o("tolgeeContext"), i = e(() => r.value.tolgee);
	if (!i.value) throw Error("Tolgee instance not provided");
	let a = d(u()), s = i.value.on("update", () => {
		a.value = u(), c.value = !i.value.isLoaded(n);
	});
	i.value.addActiveNs(n), l(() => {
		s?.unsubscribe(), i.value.removeActiveNs(n);
	});
	let c = d(!i.value.isLoaded(n));
	function u() {
		return (e) => {
			let t = e.ns ?? n?.[0];
...
var Gt = (t) => {
	...
	if (e.provide("tolgeeContext", i), r) {
		...
		i.value.tolgee = t(), h(() => i.value.isInitialRender, (t) => {
			t || (i.value.tolgee = e());
		});
	}
...
var Yt = window.location.pathname.split("/")[1] ?? "", Xt = Ut().use(Ie()).init({
	language: Jt(Yt) ? Yt : "en",
	staticData: {
...
var Qt = { class: "rounded-lg border border-border bg-card p-6" }, $t = { class: "mb-4 text-lg font-semibold text-foreground" }, en = {
	for: "api-key",
	class: "mb-1 block text-sm font-medium text-foreground"
}, tn = { class: "flex gap-2" }, nn = {
	type: "button",
	class: "rounded-md border border-border px-4 py-2 text-sm font-medium text-foreground transition-colors hover:bg-accent"
}, rn = { class: "mt-1 text-xs text-muted-foreground" }, an = r({
	__name: "ApiAccessSection",
	setup(e) {
		let { t: r } = Zt();
		return (e, i) => (u(), t("section", Qt, [n("h2", $t, p(m(r)("settings.apiAccess.title")), 1), n("div", null, [
			n("label", en, p(m(r)("settings.apiAccess.apiKey")), 1),
			n("div", tn, [i[0] ||= n("input", {
				id: "api-key",
				readonly: "",
			...
			}, null, -1), n("button", nn, p(m(r)("settings.apiAccess.copy")), 1)]),
			n("p", rn, p(m(r)("settings.apiAccess.description")), 1)
		])]));
	}
}), on = r({
	__name: "Wrapper",
	setup(e) {
		...
		return t && !t.config.globalProperties.$t && t.use(Kt, { tolgee: Xt }), (e, t) => f(e.$slots, "default");
	}
}), sn = { render() {
	return a(on, {}, { default: () => a(an) });
} };
