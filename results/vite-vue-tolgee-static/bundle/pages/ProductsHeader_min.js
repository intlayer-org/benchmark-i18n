	...
	let E = Object.freeze({
		...
		formatTranslation(n) {
			...
			try {
				...
				console.error(e);
				let n = oe(e) || L, r = t().onFormatError, a = typeof r;
...
var Oe = () => {
	let e = {
		...
		init(t) {
			let n = De(R(e.options, t));
			return e.plugins.forEach(n.addPlugin), n;
		}
	});
	return t;
}, ke = 0, B = 1, Ae = 2, je = class extends Error {
	constructor(e, t, n) {
		let r, i = "";
		e === ke ? r = "Empty parameter" : e === B ? (r = "Unexpected character", i = "Did you forget to use FormatIcu to render ICU message syntax?") : r = "Unexpected end", super(`Tolgee parser: ${r} at ${t} in "${n}"` + (i ? "\n" + i : "")), this.code = e, this.index = t;
	}
};
function Me(e) {
	return /\s/.test(e);
}
var V = 0, H = 1, U = 2, W = 3, G = 4, Ne = /* @__PURE__ */ new Set([
	U,
	H,
	V
]), K = "'", Pe = /* @__PURE__ */ new Set([
	"{",
	"}",
	K
...
function Ie(e) {
	...
	for (s = 0; s < e.length; s++) switch (i = e[s], t) {
		case V:
			...
			break;
		case H:
			Pe.has(i) ? (n = n.slice(0, -1) + i, t = U) : (n += i, t = V);
			break;
		case U:
			i === K ? t = V : (n += i, t = U);
			break;
		case W:
			i === "}" ? (d(), t = V) : Me(i) ? r !== "" && (d(), t = G) : (u(), t = W);
			break;
		case G: i == "}" ? t = V : Me(i) ? t = G : c(B);
	}
...
(E = console.assert) == null || E.call(console, Ue), RegExp(`([${["‌", "‍"].join("")}]{9})+`, "g");
function We(e, t) {
	return t ? `${t}: ${e}` : e;
}
...
}, $e = 3e3, et = 200, tt = 0, Y = /* @__PURE__ */ new Map(), nt = !1, X, Z;
...
var it = (e) => new J("unavailable", `the Tolgee browser extension did not answer ${e} in time`), at = () => `${Date.now()}-${tt++}-${Math.random()}`;
function ot() {
...
function st(e) {
	return X ||= new Promise((t, n) => {
		let r = () => window.postMessage({ type: Xe }, window.origin), i = () => {
			clearInterval(a), ct(), Z = void 0, n(new J("unavailable", "the Tolgee browser extension did not answer"));
		}, a = setInterval(() => {
			if (Date.now() > e) {
				i();
				return;
...
var Q = "__tolgee_", Tt = `${Q}apiKey`, Et = `${Q}apiUrl`, Dt = `${Q}branch`, Ot = `${Q}projectId`, kt = `${Q}session`, At = "tolgee-in-context-tools.umd.min.js", jt = "@tolgee/in-context-tools", Mt = "InContextTools", Nt = "https://cdn.jsdelivr.net/npm", Pt = null;
function Ft(e) {
...
var Zt = { class: "mb-6 rounded-md border border-border bg-muted px-4 py-3 text-center text-sm text-muted-foreground" }, Qt = a({
	__name: "MockBanner",
	setup(e) {
		let { t } = Xt();
		return (e, r) => (f(), n("div", Zt, h(g(t)("mockBanner")), 1));
	}
}), $t = { class: "mb-2 text-3xl font-bold text-foreground" }, en = { class: "mb-10 text-muted-foreground" }, tn = a({
	__name: "ProductsHeader",
	setup(t) {
		let { t: a } = Xt();
		return (t, o) => (f(), n(e, null, [
			i(Qt),
			r("h1", $t, h(g(a)("products.header.title")), 1),
			r("p", en, h(g(a)("products.header.description")), 1)
		], 64));
	}
}), nn = a({
	__name: "Wrapper",
	setup(e) {
		...
		return t && !t.config.globalProperties.$t && t.use(Jt, { tolgee: Yt }), (e, t) => m(e.$slots, "default");
	}
}), rn = { render() {
	return s(nn, {}, { default: () => s(tn) });
} };
