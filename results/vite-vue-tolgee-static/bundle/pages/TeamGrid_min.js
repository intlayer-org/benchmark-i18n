...
var _ = { class: "grid gap-6 md:grid-cols-2 lg:grid-cols-3" }, v = { class: "mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-accent text-lg font-bold text-accent-foreground" }, y = { class: "text-base font-semibold text-foreground" }, b = { class: "mb-2 text-xs font-medium text-primary" }, x = { class: "text-sm text-muted-foreground" }, S = i({
	__name: "TeamGrid",
	setup(t) {
		let i = [
			{
		...
		], a = (e) => e.split(" ").map((e) => e[0]).join("");
		return (t, o) => (d(), n("div", _, [(d(), n(e, null, p(i, (e) => r("div", {
			key: e.name,
			class: "rounded-lg border border-border bg-card p-6 text-center"
		}, [
			r("div", v, h(a(e.name)), 1),
			r("h3", y, h(e.name), 1),
			r("p", b, h(e.role), 1),
			r("p", x, h(e.bio), 1)
		])), 64))]));
...
var Qt = { install(e, t) {
	...
	setup(e) {
		...
		return t && !t.config.globalProperties.$t && t.use(Qt, { tolgee: $t }), (e, t) => m(e.$slots, "default");
	}
}), tn = { render() {
	return o(en, {}, { default: () => o(S) });
} };
