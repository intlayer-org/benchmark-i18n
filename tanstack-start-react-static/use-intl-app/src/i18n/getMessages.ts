...
export async function getMessages(locale: string) {
  const moduleLoader =
    messageModules[`../messages/${locale}.ts`] ??
    messageModules["../messages/en.ts"];

  try {
    const module = (await moduleLoader()) as { default: Record<string, unknown> };
    return module.default;
  } catch (error) {
    console.error(`Failed to load messages for locale: ${locale}`, error);
    const fallback = (await messageModules["../messages/en.ts"]()) as {
